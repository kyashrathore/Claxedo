...
import type { AgentPermissionRow, AgentQuestionRow } from "../../index"
...
export class SdkRuntimeInteractions {
  ...
  listPermissions(directory: string): AgentPermissionRow[] {
    directory = requireWorkspaceDirectory(directory)
    return this.store.listPermissions(directory)
      .filter((row) => this.permissions.has(row.id)) as AgentPermissionRow[]
  }
  ...
  listQuestions(directory: string): AgentQuestionRow[] {
    directory = requireWorkspaceDirectory(directory)
    return this.store.listQuestions(directory)
      .filter((row) => this.questions.has(row.id)) as AgentQuestionRow[]
  }
