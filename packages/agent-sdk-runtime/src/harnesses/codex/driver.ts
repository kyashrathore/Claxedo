...
class CodexAppServerDriver implements SdkRuntimeDriver {
  ...
  private reapIdleProcess() {
    ...
    void retiring.dispose().then((result) => {
      if (!retirementSettled(result)) this.retained.hold(retiring.launchIdentity, result)
    })
  }
  ...
  async dispose() {
    ...
    if (unresolved && running) this.retained.hold(running.launchIdentity, unresolved)
  }
  ...
  private async ensureProcess(directory: string) {
    ...
    const blocker = await this.retained.blocker()
  ...
  private async retireProcess(proc: CodexAppServerProcess) {
    ...
    this.retained.hold(proc.launchIdentity, result)
    throw this.retained.error()!
  }
