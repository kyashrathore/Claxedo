...
function compositeObservation(handles: AgentProcessObserverHandle[]): AgentProcessObserverHandle {
  let exited = false
  return {
    update(event) { handles.forEach((handle) => handle.update(event)) },
    exit(event) {
      if (exited) return
      exited = true
...
export function observeCodexAppServerProcess(input: {
  ...
  return compositeObservation([
    ...
    ...Object.values(input.mcp ?? {}).map((server) => observeAgentProcess(input.observer, {
      ...
      capabilities: { resourceMetrics: server.transport === "stdio" ? "process" as const : "none" as const, ownerActions: false },
...
export class CodexAppServerProcess {
  ...
  private constructor(
    ...
    this.proc.on("error", (cause) => this.onProcessExit(cause instanceof Error ? cause : new Error(String(cause)), "error"))
    this.proc.on("exit", (code, signal) => {
      this.onProcessExit(new Error(`codex app-server exited (${signal ?? code ?? "unknown"})`), "exited", code)
    })
  }
  ...
  onMessage(listener: (message: JsonRecord) => void) { this.listeners.add(listener); return () => this.listeners.delete(listener) }
  onStderr(listener: (message: string) => void) { this.stderrListeners.add(listener); return () => this.stderrListeners.delete(listener) }
  ...
  notify(method: string, params?: unknown) { this.write(params === undefined ? { method } : { method, params }) }
  ...
  private onProcessExit(error: Error, reason: "error" | "exited", exitCode?: number | null) {
    if (this.killTimer) clearTimeout(this.killTimer)
    this.exitObservation({ reason, ...(exitCode !== null && exitCode !== undefined ? { exitCode } : {}) })
    for (const pending of this.pending.values()) pending.reject(error)
    this.pending.clear()
  ...
  private write(message: JsonRecord) { this.proc.stdin?.write(`${JSON.stringify(message)}\n`) }
  ...
  private handleLine(line: string) {
    ...
    if (id !== undefined && ("result" in message || "error" in message)) {
      ...
      if (error) pending.reject(new Error(text(error.message) ?? `codex app-server request ${id} failed`))
