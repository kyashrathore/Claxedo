...
describe("Codex first-party MCP injection", () => {
  test("starts each thread with a per-session claxedo server in the request config and leaves CODEX_HOME untouched", async () => {
    ...
    try {
      ...
      expect(starts.map((row) => row.params.config)).toEqual([
        { features: { default_mode_request_user_input: true }, tools: { update_plan: { enabled: true } }, mcp_servers: { claxedo: { url: "http://127.0.0.1:2593/api/claxedo/mcp?session=session-a", http_headers: { Authorization: `Bearer ${TOKEN}` } } } },
        { features: { default_mode_request_user_input: true }, tools: { update_plan: { enabled: true } }, mcp_servers: { claxedo: { url: "http://127.0.0.1:2593/api/claxedo/mcp?session=session-b", http_headers: { Authorization: `Bearer ${TOKEN}` } } } },
      ])
  ...
  test("a thread the app-server lost is resumed with the same per-session entry, so a restart keeps the tools", async () => {
    ...
    try {
      ...
      expect(resumes[0]?.params).toMatchObject({
        threadId,
        config: { features: { default_mode_request_user_input: true }, tools: { update_plan: { enabled: true } }, mcp_servers: { claxedo: { url: "http://127.0.0.1:2593/api/claxedo/mcp?session=session-a", http_headers: { Authorization: `Bearer ${TOKEN}` } } } },
      })
    } finally {
      await driver.dispose?.()
    }
  ...
  test("enables native questions and task lists without writing config files when no MCP provider is supplied", async () => {
    const fake = await fakeAppServer()
    const driver = driverFor(fake.binary, fake.codexHome)
    try {
      await driver.applyConfig({ mcp: {} })
      await driver.createAgentSession({ directory: fake.dir, model: "gpt-5-codex", sessionId: "session-a" })
      expect((await fake.requests()).map((row) => row.params.config)).toEqual([{ features: { default_mode_request_user_input: true }, tools: { update_plan: { enabled: true } } }])
      expect(await fs.promises.readdir(fake.codexHome)).toEqual([])
    } finally {
      await driver.dispose?.()
    }
