      ...
      message(ws: Bun.ServerWebSocket<RelayWebSocketData>, message: string | Buffer<ArrayBuffer>) {
        if (ws.data.kind === "host-tunnel") {
          ...
          if (parsed?.type === "ws.frame") {
            ...
            if ((channel as { bufferedAmount?: number }).bufferedAmount! > (bunOptions.webSocketBufferedAmountMaxBytes ?? WS_BUFFERED_AMOUNT_MAX_BYTES_DEFAULT)) {
              ws.data.channels.delete(frame.channel_id)
        ...
        if (ws.data.kind === "user-hosted-client") {
          ...
          if ((tunnel as { bufferedAmount?: number }).bufferedAmount! > (bunOptions.webSocketBufferedAmountMaxBytes ?? WS_BUFFERED_AMOUNT_MAX_BYTES_DEFAULT)) {
        ...
        if (ws.data.queue.length >= (bunOptions.upstreamWebSocketPreOpenQueueMaxFrames ?? UPSTREAM_WS_PRE_OPEN_QUEUE_MAX_FRAMES_DEFAULT)) {
