...
async function createStore() {
  const instance = new Miniflare({
    ...
    d1Databases: ["WORKGRAPH_DB"],
  })
  active.push(instance)
  const database = await instance.getD1Database("WORKGRAPH_DB")
  ...
  return { database, store: new D1WorkGraphServiceLifecycleStore(database, identity) }
}
...
describe("WorkGraph D1 lifecycle state", () => {
  test("advances an exact crash-resumable ledger without claiming installation during migration", async () => {
    ...
    await expect(database.prepare("delete from workgraph_service_lifecycle_audit").run()).rejects.toThrow(/append-only/)
  })
  ...
  test("keeps the committed initializer authoritative across a pre-registration crash", async () => {
    ...
    await expect(
      database.prepare(`select count(*) as count from workgraph_service_lifecycle_audit`).first(),
