...
create table workgraph_service_lifecycle (
...
create table workgraph_service_lifecycle_audit (
...
create trigger workgraph_service_lifecycle_audit_no_update before update on workgraph_service_lifecycle_audit
begin select raise(abort, 'WorkGraph lifecycle audit is append-only'); end;

create trigger workgraph_service_lifecycle_audit_no_delete before delete on workgraph_service_lifecycle_audit
begin select raise(abort, 'WorkGraph lifecycle audit is append-only'); end;
