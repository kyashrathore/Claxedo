...
test("renders the report as terminal tables", () => {
  ...
  assert.match(output, /Sessions requiring full machine/)
  assert.match(output, /Median interval before machine needed again/)
  assert.match(output, /p95 interval before machine needed again/)
  assert.match(output, /Median calls after first machine need/)
  assert.match(output, /Median observed work after first machine need/)
  ...
  assert.doesNotMatch(output, /Turns completed without full machine/)
  assert.doesNotMatch(output, /Full-machine turns needing it again/)
  assert.doesNotMatch(output, /Execution-call turn coverage/)
  assert.doesNotMatch(output, /Runtime unresolved/)
  assert.doesNotMatch(output, /Median first-call lead/)
  assert.doesNotMatch(output, /full-machine call duration/)
  assert.doesNotMatch(output, /Gaps longer than/)
...
test("keeps report lines within wide, compact, and narrow terminals", () => {
  ...
  for (const columns of [160, 120, 80, 50, 20]) {
    ...
    if (columns === 50) {
      ...
      assert.match(normalized, /100\.00% of .*classified .*sessions/)
...
test("CLI rejects unknown harnesses and does not share empty reports", () => {
  ...
  assert.doesNotMatch(empty.stdout, /Open your runtime report/)
...
test("CLI defaults to the branded review endpoint", () => {
  ...
  assert.equal(result.status, 0)
  assert.match(result.stdout, /Open your runtime report/)
...
test("share prompt carries only canonical aggregate metrics and becomes clickable in terminals", () => {
  ...
  assert.match(interactive, /\[ Open report \]/)
})
