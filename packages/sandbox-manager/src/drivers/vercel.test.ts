...
import {
  ...
  vercelBrokeredNetworkPolicy,
  type VercelSandboxFactoryLike,
...
describe("VercelSandboxDriver", () => {
  test("brokered secrets are injected via a firewall header-transform network policy, never in env", async () => {
    ...
    // record form spells that "*" so the header transform can ride alongside it.
  ...
  test("brokering MERGES the brokered hosts into the create-time allow-list, never replacing it", async () => {
    ...
    // `npm install` fails mid-run — and would also silently WIDEN egress to a
    // brokered host the create-time policy never approved.
    const created = sandbox()
    const vercel = factory({ create: vi.fn(async () => created) })
    const driver = createVercelSandboxDriver({ ...baseOptions, sandbox: vercel })
    ...
    // The create-time hosts survive with no rules attached; only the brokered
  ...
  test("a deny-all create-time policy contributes nothing to the merge", async () => {
    const created = sandbox()
    const vercel = factory({ create: vi.fn(async () => created) })
    const driver = createVercelSandboxDriver({ ...baseOptions, sandbox: vercel })
  ...
  test("the runtime version is resolved per build, not frozen at module import", async () => {
    ...
    try {
      ...
      const setup = (builder.runCommand as ReturnType<typeof vi.fn>).mock.calls
        .map((call) => JSON.stringify(call))
        .join("\n")
      expect(setup).toContain("@claxedo/workspace-runtime@9.9.9-late")
      expect(setup).toContain("expected 9.9.9-late")
    } finally {
