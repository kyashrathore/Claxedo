...
// Resolved per snapshot build, never at import: `workspaceRuntimeVersion()`
// reads WORKSPACE_RUNTIME_VERSION, and a module-level const would bake in
// whatever the env held when this file was first imported — before a host that
// loads its config after import has had a chance to set it.
function runtimePackage(version: string) {
  return `@claxedo/workspace-runtime@${version}`
}
...
export function vercelBrokeredNetworkPolicy(
  ...
  for (const [host, rules] of baseAllowEntries(base)) allow[host] = [...rules]
  ...
  return { allow, ...subnets } satisfies NetworkPolicy
}

/**
 * A create-time policy's allow-list as (host, rules) pairs, normalizing the two
 * shapes `NetworkPolicy.allow` accepts. `undefined` and `"allow-all"` both mean
 * unrestricted, which the record form spells `"*"` (see the Vercel
 * NetworkPolicy docs' transformer example). Any rules already attached to a
 * host are carried over rather than dropped — the brokered transforms append to
 * them.
 */
function baseAllowEntries(base: NetworkPolicy | undefined): Array<[string, NetworkPolicyRule[]]> {
  if (base === undefined || base === "allow-all") return [["*", []]]
  if (base === "deny-all") return []
  if (Array.isArray(base.allow)) return base.allow.map((host) => [host, []])
  return Object.entries(base.allow ?? {})
}
...
export function createVercelSandboxDriver(options: VercelSandboxDriverOptions): SandboxDriver {
  ...
  async function ensureSnapshot() {
    ...
    const expectedRuntimeVersion = workspaceRuntimeVersion()
    const cacheKey = `${options.teamId}:${options.projectId}:${runtime}:${expectedRuntimeVersion}`
  ...
  async function ensureHost(input: SandboxDriverEnsureInput) {
    ...
    const sandbox = await (await resolveFactory()).create({
      ...
      timeout: timeoutMs,
      env,
      ...(createPolicy ? { networkPolicy: createPolicy } : {}),
    }).catch((err) => {
    ...
    if (input.secrets?.length) {
      ...
      await sandbox.updateNetworkPolicy(vercelBrokeredNetworkPolicy(input.secrets, createPolicy), {
        signal: AbortSignal.timeout(operationTimeoutMs),
      })
    }
