...
test("commercial homepage preserves the product narrative and two conversion classes", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { level: 1, name: "The open-source workspace for coding agents." })).toBeVisible()
  ...
  const events = await page.locator("a[data-analytics-event]").evaluateAll((links) => [...new Set(links.map((link) => link.getAttribute("data-analytics-event")))].sort())
  expect(events).toEqual(["download_app", "explore_framework"])
  ...
  await page.locator('a[data-analytics-event="download_app"][data-analytics-placement="hero"]').click()
  expect(await page.evaluate(() => window.__claxedoAnalyticsEvents?.at(-1))).toEqual({ name: "download_app", route: "/", placement: "hero" })
...
test("download, comparison, and framework journeys resolve", async ({ page, isMobile }, testInfo) => {
  ...
  await expect(page.locator(".comparison-cards a")).toHaveCount(6)
  await page.getByRole("link", { name: /Claxedo vs. Matrix OS/ }).click()
  await expect(page.getByRole("heading", { level: 1, name: "Claxedo vs. Matrix OS" })).toBeVisible()
  ...
  await expect(page.getByRole("link", { name: "Download app" }).first()).toHaveAttribute("href", "/download")
  await assertNoHorizontalOverflow(page)
  await assertNoSeriousAccessibilityViolations(page)
...
test("analytics failures do not block ordinary link navigation", async ({ page }) => {
  const errors: string[] = []
...
test("conversion links work without JavaScript", async ({ browser, isMobile }) => {
...
test("representative public routes have no serious accessibility violations", async ({ page }) => {
  for (const path of ["/pricing", "/download", "/compare/matrix-os", "/framework/guides/install"]) {
...
test("keyboard navigation and mobile menu keep conversions reachable", async ({ page, isMobile }) => {
  ...
  if (isMobile) {
    ...
    await expect(page.locator(".nav-actions").getByRole("link", { name: "Open-source framework", exact: true })).toBeVisible()
    await expect(page.getByRole("link", { name: "Download app" }).first()).toBeVisible()
  }
  await assertNoHorizontalOverflow(page)
})
...
test("machine-readable routes, canonicals, and internal references are valid", async ({ request }, testInfo) => {
  ...
  expect(graph["@graph"].map((item: { "@type": string }) => item["@type"])).toEqual(["Organization", "WebSite", "SoftwareApplication"])
