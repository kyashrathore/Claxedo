...
- **Project consent gate** — repo-shipped `.agent-extensions/` declarations apply only as far as the host-owned trust ledger vouches (scoped, content-bound grants resolved into `projectStateTrusted`); lifecycle commands record grants for the ids they act on.
