...
Be aware of a real gap here rather than trusting the comments: only packages/claxedo-app actually declares wrangler (pinned 4.50.0), so only the Pages deploy in Unit 4 is version-reproducible. packages/claxedo-server and packages/workspace-relay declare no wrangler dependency at all, so "bunx wrangler" run from those directories — which is exactly what Units 2 and 3 and the CI workflow do — silently resolves to whatever the latest release is that day. A comment in .github/workflows/deploy-control-plane.yml claims the Worker deploys get the pinned 4.50.0; that claim is not true today. Before you deploy, run "bunx wrangler --version" in each package directory and tell me what you actually got. If I want reproducible Worker deploys, the fix is to add an explicit wrangler devDependency to packages/claxedo-server and packages/workspace-relay, or to pin inline with "bunx wrangler@<version>". Ask me before changing dependencies.
...
packages/claxedo-server/wrangler.toml — worker name claxedo-control-plane, staging name claxedo-control-plane-staging, main src/worker.ts, compatibility_date "2025-05-01", compatibility_flags ["nodejs_compat", "global_fetch_strictly_public"]. nodejs_compat is required for Worker-compatible npm SDKs and is what populates process.env from text bindings and secrets.
  ...
  Cron trigger "*/15 * * * *", also declared twice — [triggers] and [env.staging.triggers] — because Cron Triggers are not inherited by environments either. It drives sandbox GC and the bounded WorkGraph reconciler.
