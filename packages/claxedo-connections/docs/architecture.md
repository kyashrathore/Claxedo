...
4. `verify` returns a closed result: `{ ok: true, accountLabel? }` or
   `{ ok: false, reason: "unauthorized" | "network" }` — never a raw provider
   error body, because that body can embed the pasted secret.
5. On success, `storeConnection` upserts the row and writes the secret to
...
- **`sweep()`** runs on an unref'd interval (default 30s) and does two
  things: expires pending entries past `expiresAt`, and deletes terminal
  entries past `removeAt`. It never touches an entry that's mid-`consume`
  (`completing: true`) until `settle` moves it to a terminal state.
