import { openWorkspaceFileTab } from "./support/session-switch"
import { syntheticVisibleClick, waitForWorkspaceReviewContent } from "../src/browser/actions/workspace"
// TEMP probe (read-only diagnosis): the fast meter for the
// `session-switch-workspace` experiment loop.
//
// It reproduces the scenario driver's FULL 12-cell matrix (browser/scenarios/session-switch-workspace.ts ->
...
//     bun probes/debug-session-switch-probe.ts
...
import { SESSION_SWITCH_SUBSTANTIAL_FILE_PATH } from "../src/browser/scenarios/session-switch-workspace-contract"
...
try {
  await waitForWorkspaceReviewContent(page, expectedTotal, 20_000)
