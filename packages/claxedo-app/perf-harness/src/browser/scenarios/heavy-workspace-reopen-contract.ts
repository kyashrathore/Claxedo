...
/** The top-level Open Workspace action selects Review and retains the tabs. */
export function heavyWorkspaceReviewReopenFailures(
  before: HeavyWorkspaceSurfaceIdentity,
  after: HeavyWorkspaceSurfaceIdentity,
) {
  const failures: string[] = []
  if (!sameStrings(before.openTabIds, after.openTabIds)) {
    failures.push(`workspace tabs changed across close/reopen: ${JSON.stringify(before.openTabIds)} -> ${JSON.stringify(after.openTabIds)}`)
  }
  if (!before.reviewTabId || after.reviewTabId !== before.reviewTabId || after.activeTabId !== before.reviewTabId) {
    failures.push(`workspace did not reopen on its retained Review tab: ${String(after.activeTabId)}`)
  }
...
/** Exact file proof after explicitly activating its retained public tab. */
export function heavyWorkspaceFileRestorationFailures(
  before: HeavyWorkspaceSurfaceIdentity,
  after: HeavyWorkspaceSurfaceIdentity,
) {
  const failures: string[] = []
...
/**
 * Review is checked when the top-level open or its tab activation makes it
 * the active surface. Its corpus and deep working set must survive both.
 */
