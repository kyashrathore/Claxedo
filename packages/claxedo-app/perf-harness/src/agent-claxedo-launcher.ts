...
async function captureStartupClock(
  runDirectory: string,
  snapshot: {
    timeOrigin: number
    resources: readonly { name: string; startTime: number }[]
...
export async function launchPackagedClaxedo(input: {
  ...
  const [debugPort, serverPort] = await Promise.all([availablePort(), availablePort()])
  ...
  try {
    ...
    try {
      ...
      const snapshot = await connectedPage
        .evaluate(() => ({
          formErrors: [...document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>("input, textarea")]
            ...
            .flatMap((field) =>
              field.value
                .split("\n")
                .slice(0, 12)
                .map((line) => line.replace(/^\s*at\s+/, "").replace(/^.*\/assets\//, "assets/")),
            ),
          url: location.href,
          title: document.title,
          text: document.body?.innerText.slice(0, 1_000) ?? "",
          sessionRows: [...document.querySelectorAll<HTMLElement>('[data-testid="rail-sidebar-session-row"]')]
            .slice(0, 20)
            .map((row) => ({
              id: row.dataset.sessionId,
              text: row.innerText.slice(0, 120),
            })),
          fatal: document.querySelector<HTMLElement>('[data-testid="fatal-error"], [data-fatal-error]')?.innerText,
        }))
        .catch((diagnosticError) => ({
          diagnosticError: diagnosticError instanceof Error ? diagnosticError.message : String(diagnosticError),
        }))
      throw new Error(`Packaged Claxedo semantic readiness timed out: ${JSON.stringify(snapshot)}`, { cause: error })
    }
    ...
    if (exactViewport.width !== requestedViewport.width || exactViewport.height !== requestedViewport.height) {
    ...
    if (!trusted || trusted.state !== "exact")
      throw new Error("Claxedo did not accept the cold-ready trusted input probe")
    ...
    return {
      ...
      async shutdown() {
        ...
        await connectedPage
          .evaluate(() => (window as Window & { api?: { quit?: () => void } }).api?.quit?.())
          .catch(() => undefined)
