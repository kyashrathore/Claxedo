...
type ControlSessionRow = ClaxedoSession & {
  session_id?: string
  project_id?: string
  created_at?: number
  updated_at?: number
...
type AgentRuntimeWorkspaceKind = "local" | "cloud" | "user-hosted"
const USER_HOSTED_WORKSPACE_KIND = "user-hosted"

export type AgentRuntimeOpenCodeClient = {
  session: {
    create?: (input: { directory: AgentRuntimeDirectory }, init?: { headers?: Record<string, string> }) => Promise<{ data?: ClaxedoSession; error?: unknown }>
...
function workspaceKind(input: unknown): AgentRuntimeWorkspaceKind | undefined {
...
export function createAgentRuntimeClient(options: {
  ...
  function shouldUseRuntimeSessionTransport(input: { sessionID?: string; directory: AgentRuntimeDirectory }) {
    ...
    if (options.sessionRef?.host === "central") return true
    if (options.sessionRef?.toolSandbox?.kind === "workspace") return true
    return usesScopedSessionTransport(input.sessionID, input.directory)
  }

  function runtimePlacement(input: {
    directory?: string
    sessionRef?: SessionRef
    workspaceId?: string
    ...
    if (input.sessionRef?.host === "central") {
      return {
        ...
        hosting: "central",
        transport: centralTransportForServer(serverUrl()),
      }
    }
    ...
    if (input.sessionRef) {
      return { hosting: "central", transport: centralTransportForServer(serverUrl()) }
    }
    if (input.workspaceId) {
      return {
        workspaceId: input.workspaceId,
    ...
    if (isLocalPersonalScope({ serverUrl: serverUrl(), directory: input.directory })) {
      return { hosting: "workspace", transport: "loopback" }
    }
    return { hosting: "central", transport: centralTransportForServer(serverUrl()) }
  }
  ...
  function runtimeTransport(input: {
    ...
    return createTransport({
      placement: runtimePlacement(input),
      serverUrl: serverUrl(),
  ...
  return {
    ...
    async createSession(input: { directory: AgentRuntimeDirectory; headers?: Record<string, string> }) {
      ...
      return { data: await readJson<ClaxedoSession>(res) }
    },
