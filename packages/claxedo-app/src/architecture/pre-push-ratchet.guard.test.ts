...
describe("pre-push architecture ratchet", () => {
  test("runs every source-closure policy after typecheck", async () => {
    ...
    expect(manifest.scripts["test:architecture-ratchets"]).toBe(
      "BUN_CONFIG_FILE=./script/architecture-ratchets.bunfig.toml bun test ./script/agent-plugins-retirement.test.ts ./script/upstream-engine-retirement.test.ts && bun ./script/product-boundary/verify.ts --all --source-only",
    )
