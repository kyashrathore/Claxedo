...
const LOCAL_AUTHENTICATED_MODULES: Record<string, string> = {
  ...
  "features/onboarding/ai-step.tsx":
    "The wizard's hosted AI step stores a Pi key through `putProviderAuthEntry`, the plane's own `PUT /auth/:providerID?harness=pi`, which the browser session authenticates; a signed desktop never renders that branch (its server has a filesystem and the machine scan). Not a Hosted Server AccountPort surface.",
