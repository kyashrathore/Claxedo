...
const desktop = (command: string) => /\/(OpenCode|Claxedo)[ .].*\.app\/|\/MacOS\/(OpenCode|Claxedo)\b/i.test(command)
const branded = (command: string) =>
  /(?:^|\s)(?:opencode|claxedo)(?:-cli)?(?:\s|$)/i.test(command) ||
  /\/(?:opencode|claxedo)(?:-cli)?(?:\s|$)/i.test(command) ||
...
function classify(
  ...
  seen.add(row.pid)
  const same = !row.workspace_id || row.workspace_id === refs.directory
...
export function parseDiagnosticPs(
  ...
  const rows = input
    ...
    .flatMap((line) => {
      ...
      const same = !workspaceId || workspaceId === refs.directory
