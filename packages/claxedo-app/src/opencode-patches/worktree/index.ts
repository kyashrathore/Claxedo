...
export namespace Worktree {
  ...
  async function runStartScript(directory: string, cmd: string, kind: StartKind) {
    ...
    const label = kind === "project" ? "Project startup script failed" : "Workspace startup script failed"
    const message = errorText(ran) || label
  ...
  async function runStartScripts(directory: string, input: { projectID: string; extra?: string }) {
    ...
    const err = await runStartScript(directory, startup, "project")
    if (err) return err
  ...
  export async function createFromInfo(info: Info, startCommand?: string) {
    ...
    return () => {
      const start = async () => {
        ...
        const err = await runStartScripts(info.directory, { projectID, extra })
        if (err) {
          fail(err)
          return
        }
