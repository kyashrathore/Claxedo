...
import { useClaxedoEvents } from "../providers/claxedo-events"
...
export function DialogCreateCloudProject(props: WebProjectDialogProps) {
  ...
  const [result, setResult] = createSignal<CreateWorkspaceResult | null>(null)
  ...
  createEffect(() => {
    void api.get<ProviderResponse>(`${baseUrl}/api/workspace/providers`).then((data) => {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      // Subscribe to provision events for this workspace
      const unsub = events.on("provision", (ev) => {
        if (ev.workspaceId !== created.workspaceId) return
        ...
        if (ev.step === "ready") {
          ...
          setTimeout(() => {
            props.onSelect(created.directory!)
            dialog.close()
          }, 1200)
        }
        ...
        <Switch>
          ...
          <Match when={phase() === "provisioning"}>
            <div class="space-y-3">
              ...
              <Show when={error()}>
                <div class="flex items-start gap-2 mt-2 px-3 py-2 rounded bg-red-500/10 border border-red-500/20">
                  <Icon name="warning" size="small" class="text-red-400 mt-0.5 shrink-0" />
                  <span class="text-xs text-red-400 break-words min-w-0">{error()}</span>
                </div>
