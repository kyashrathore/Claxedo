...
export function DialogCreateCloudWorkspace(props: DialogCreateCloudWorkspaceProps) {
  ...
  createEffect(() => {
    void api
      .get<ProviderResponse>(`${baseUrl}/api/workspace/providers`)
      .then((data) => {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (events) {
        ...
        const unsub = events.on("provision", (ev) => {
          if (ev.workspaceId !== result.workspaceId) return

          // On a new acquiring_sandbox after an error, clear the error (server is retrying)
          ...
          if (ev.step === "error") {
            // Don't unsubscribe — server retries automatically with backoff
      ...
      <div class="flex flex-col gap-4 pl-6 pr-2.5 pb-3 min-w-[400px]">
        <Show when={phase() === "form"}>
          <form onSubmit={handleSubmit} class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1.5">
                  <For each={providers()}>
                    {(item) => (
                      <button
                        type="button"
                        class="group flex items-center gap-3 px-3 py-2.5 rounded-md border text-left transition-all duration-150 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent-base"
                        classList={{
                          "border-accent-base/40 bg-accent-base/6": provider() === item.id && item.configured,
                      ...
                      >
                        ...
                        <div
                          ...
                          classList={{
                            "border-accent-base": provider() === item.id && item.configured,
                            "border-border-base": provider() !== item.id || !item.configured,
                          }}
                        >
                          <Show when={provider() === item.id && item.configured}>
                            <div class="size-2 rounded-full bg-accent-base" />
                          </Show>
                        </div>
            ...
            <Show when={hasConfigured()}>
              <div class="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  class="w-full bg-surface-inset-base border border-border-weak-base rounded-md px-3 py-2 text-13-regular text-text-strong placeholder:text-text-weak/40 focus:outline-none focus:ring-1 focus:ring-accent-base focus:border-accent-base/40 transition-colors"
                />
              </div>
            </Show>
            ...
            {/* Error */}
            <Show when={error()}>
              <div class="flex items-start gap-2 px-3 py-2 rounded-md bg-red-500/8 border border-red-500/20">
              ...
              <Show when={isReady()}>
                <div class="flex items-center gap-2">
                  ...
                  <Show when={totalElapsed()}>
                    <span class="text-green-400/60 tabular-nums shrink-0">{totalElapsed()}s</span>
                  </Show>
                </div>
              </Show>
            ...
            <Show when={error()}>
              <div class="flex items-start gap-2 px-3 py-2 rounded-md bg-red-500/8 border border-red-500/20">
                ...
                <div class="flex flex-col gap-0.5">
                  <span class="text-12-regular text-red-400 break-words min-w-0">{error()}</span>
