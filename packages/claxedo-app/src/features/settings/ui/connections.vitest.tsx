...
describe("SourceViewForm", () => {
  test("infers a GitHub cloud target from owner/repository", async () => {
    ...
    render(() => <SourceViewForm connection={connection} provider="github" projects={[]} busy={false} onCancel={() => {}} onSave={onSave} />)
    ...
    await waitFor(() => expect(onSave).toHaveBeenCalledWith(expect.objectContaining({
      ...
      target: {
        environment: { kind: "hosted_workspace", placement: "shared", repositoryUrl: "https://github.com/acme/cloud.git" },
  ...
  test.each(["linear", "jira"] as const)("requires an explicit project mapping for %s", async (provider) => {
    const onSave = vi.fn(async () => {})
    const view = render(() => <SourceViewForm connection={{ ...connection, integrationId: provider }} provider={provider} projects={[]} busy={false} onCancel={() => {}} onSave={onSave} />)
    ...
    render(() => <SourceViewForm connection={{ ...connection, integrationId: provider }} provider={provider} projects={[localProject]} busy={false} onCancel={() => {}} onSave={onSave} />)
    await fireEvent.input(screen.getByLabelText(/Your provider identity/), { target: { value: "user-1" } })
    await fireEvent.change(screen.getByLabelText(/Project/), { target: { value: localProject.id } })
    await fireEvent.click(screen.getByRole("button", { name: "Save source" }))
    await waitFor(() => expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ provider, target: localProject.target })))
  })

  test("keeps an edited mapping attached to its project when the base revision differs", () => {
    render(() => <SourceViewForm
