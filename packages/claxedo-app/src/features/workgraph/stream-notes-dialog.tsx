...
export function StreamNotesDialog(props: { stream?: StreamDto; client: WorkGraphClient; onClose: () => void }) {
  const revision = createAsyncState(async () => {
    const source = (() => props.stream?.notesSource)()
    if (!source) return undefined
    return ((source) => props.client.sourceRevision(source.workSourceId, source.revisionId))(source)
  })
  return (
      ...
      <DialogRoot.Portal>
        <div class="workgraph-dialog-scope">
          ...
          <DialogShell
            fit
            size="normal"
            title={`Notes · ${props.stream?.title ?? "Stream"}`}
            onEscapeKeyDown={props.onClose}
            action={
              <IconButton variant="ghost" size="small" icon="close" aria-label="Close notes" onClick={props.onClose} />
            }
          >
            <Show
              when={!revision.loading() && !revision.error() && revision.data()}
              fallback={
