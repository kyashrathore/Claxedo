...
describe("StreamSettingsDialog", () => {
  test("confirms autonomy explicitly and preserves compiled Stream metadata", async () => {
    const save = vi.fn(async () => ok)
    const source = { workgraphDefaults: vi.fn(async () => defaultsDto), save }
    const compiled = {
      ...streamDto,
      executionDefaults: {
        ...streamDto.executionDefaults,
    ...
    await fireEvent.click(screen.getByRole("button", { name: "Enable autonomy and save" }))
    ...
    expect(save).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        execution: expect.objectContaining({
          autonomy: "autonomous",
          budget: { amount: 25_000, unit: "tokens", window: "stream" },
          maxParallel: 3,
          mayPromote: true,
          agents: compiled.executionDefaults.agents,
          assignments: { execution: "Builder" },
        }),
      }),
    )
