...
describe("WaitingItemDialog — master escalation", () => {
  test("opens Stream settings for a budget-exhausted escalation", async () => {
    ...
    render(() => (
      <WaitingItemDialog
        selection={{
          ...
          reason: "Stream budget exhausted (25000 tokens/stream)",
        }}
        source={baseSource({})}
        onClose={onClose}
        onResolved={() => {}}
    ...
    expect(await screen.findByText("The Stream reached its compiled budget and paused new work.")).toBeInTheDocument()
    await fireEvent.click(screen.getByRole("button", { name: "Open Stream settings" }))
