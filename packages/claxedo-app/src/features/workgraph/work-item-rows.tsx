...
export type TaskStatusLabel = "draft" | "staged" | "waiting" | "ready" | "running" | "needs-you" | "done"
...
export function WorkItemLeaf(props: {
  ...
  const arm = async (event: MouseEvent) => {
    event.stopPropagation()
    if (busy() || props.item.state !== "draft") return
    setBusy(true)
    try {
      await props.mutate(() => props.client.armWorkItem(props.item.id, props.item.version))
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      <Show when={props.item.autoAdmitted}>
        <span class="workgraph-chip">Auto-admitted</span>
      </Show>
      ...
      <Show when={label() === "draft"}>
        <span class="workgraph-chip">Draft</span>
        <button
          type="button"
          class="workgraph-leaf-session workgraph-leaf-arm"
          aria-label={`Arm draft task ${props.item.title}`}
          disabled={busy()}
          onClick={(event) => void arm(event)}
        >
          <span>{busy() ? "Arming…" : "Arm"}</span>
      ...
      <Show when={!hasLiveRun() && label() !== "staged" && label() !== "draft"}>
        <button
          type="button"
          class="workgraph-row-delete"
