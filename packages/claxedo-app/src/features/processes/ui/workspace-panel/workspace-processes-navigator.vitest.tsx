...
vi.mock("@/features/processes/ui/add-process-dialog", () => ({
...
describe("WorkspaceProcessesNavigator", () => {
  test("renders a row per configured process", () => {
    ...
    const view = render(() => (
      <WorkspaceProcessesNavigator processPane={paneState} directory="/repo" onProcessSelect={() => {}} />
    ))
  ...
  test("clicking Stop invokes stop with the config id and stops row selection propagation", () => {
    ...
    paneState = pane
    const view = render(() => (
      <WorkspaceProcessesNavigator processPane={paneState} directory="/repo" onProcessSelect={onProcessSelect} />
    ))
