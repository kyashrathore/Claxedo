...
import { Process } from "@/features/processes/data/process"
import { errorText } from "@/lib/text"
import { formatMB, formatCPU } from "@/features/processes/ui/diagnostics/format"
...
export type ProcessDiagnosticsSurface = {
  id: string
  type: string
  terminalId?: string
  sessionId?: string
...
export function DialogProcessDiagnostics(props: {
  ...
  surfaces?: () => ProcessDiagnosticsSurface[]
  focusSurface?: (paneId: string, surfaceId: string) => void
  openTerminal?: (directory: ProcessDirectory, terminalId: string, title: string) => void
}) {
  ...
  const tabs = createMemo(() => {
    ...
    for (const tab of props.surfaces?.() ?? []) {
  ...
  const focusTab = (paneId: string | undefined, surfaceId: string | undefined) => {
    ...
    props.focusSurface?.(paneId, surfaceId)
    return true
  }
  ...
  const openTerminal = (row: ResourceRow) => {
    ...
    props.openTerminal?.(row.directory || activeDirectory(), row.terminalId, row.title)
  }
  ...
  const openPty = (id: string, title?: string) => {
    ...
    props.openTerminal?.(pty.cwd || activeDirectory(), id, title || tab?.title || pty.title)
  }
