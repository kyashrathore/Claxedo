...
export function AddProcessDialog(props: AddProcessDialogProps) {
  ...
  const buildBody = () => {
    ...
    const preferredPort = store.portPreferred.trim() ? parseInt(store.portPreferred.trim(), 10) : undefined
  ...
  const effectivePortName = () => store.portName.trim() || derivedPortName()

  const hasPort = () => store.usePort || store.portPreferred.trim().length > 0
  ...
  const handleDelete = async () => {
    ...
    setStore(storePath("deleting", true))
    try {
      ...
      setStore(storePath("deleting", false))
    }
  }
  ...
  return (
    <Dialog title={isEdit() ? "Edit Process" : "Add Process"} class="w-full max-w-[480px] mx-auto">
      <form onSubmit={handleSubmit} class="flex flex-col flex-1 min-h-0">
        ...
        <div
          class={[
            "flex flex-col gap-4 px-6 pb-4 flex-1 min-h-0 overflow-y-auto transition-all duration-200",
            {
        ...
        >
          ...
          <TextField
            ...
            value={store.command}
            onChange={(v) => setStore(storePath("command", v))}
            spellcheck={false}
            class="font-mono text-xs"
            required
          />
          ...
          <TextField
            ...
            value={store.cwd}
            onChange={(v) => setStore(storePath("cwd", v))}
            spellcheck={false}
            class="font-mono text-xs"
          />
              ...
              {(entry, index) => (
                <div class="flex gap-2 items-center">
                  <input
                    ...
                    placeholder="KEY"
                    value={entry.key}
                    onInput={(e) => setStore(storePath("envEntries", index(), "key", e.currentTarget.value))}
                  ...
                  <input
                    ...
                    placeholder="value"
                    value={entry.value}
                    onInput={(e) => setStore(storePath("envEntries", index(), "value", e.currentTarget.value))}
                    class="flex-1 min-w-0 bg-surface-inset-base border border-border-base rounded-md px-2 py-1.5 text-12-regular font-mono text-text-strong focus:outline-none focus:ring-1 focus:ring-border-interactive-focus"
                    spellcheck={false}
                  />
                  <button
          ...
          <TextField
            ...
            onChange={(v) => setStore(storePath("dependsOn", v))}
            spellcheck={false}
            class="font-mono text-xs"
          />
          ...
          <div class="flex flex-col gap-3">
            <TextField
              ...
              onChange={(v) => setStore(storePath("portPreferred", v))}
              spellcheck={false}
              class="font-mono text-xs"
            />
                ...
                <RadioGroup
                  ...
                  onSelect={(o) =>
                    setStore(
                      storePath(
                        "portOnConflict",
                        o?.value === "ask" ? undefined : (o?.value as Process.PortConflictStrategy),
                      ),
                    )
                  }
                />
            ...
            <Switch checked={store.usePort} onChange={(v) => setStore(storePath("usePort", v))}>
            ...
            <Show when={store.usePort}>
              ...
              <div class="flex flex-col gap-1.5">
                <TextField
                  ...
                  onChange={(v) => setStore(storePath("portInject", v))}
                  spellcheck={false}
                  class="font-mono text-xs"
                />
              ...
              <div class="flex flex-col gap-1.5">
                <TextField
                  ...
                  onChange={(v) => setStore(storePath("portName", v))}
                  spellcheck={false}
                  class="font-mono text-xs"
                />
        ...
        <div class="flex items-center gap-2 px-6 py-3 border-t border-border-weak-base shrink-0">
          ...
          >
            <Show when={isEdit()}>
              <Button type="button" variant="ghost" size="large" onClick={() => setShowDeleteConfirm(true)}>
                Delete
              </Button>
            </Show>
            <div class="flex-1" />
