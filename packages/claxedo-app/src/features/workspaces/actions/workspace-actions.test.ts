    ...
    expect(opens).toEqual([
      {
        ...
        opts: {
          sessionRef: {
            ...
            // `sessionRefForActionWorkspace` carries the workspace identity for
            // LOCAL workspaces too, not just cloud ones — that is what makes
            // surface routing workspace-id aware. The invariant this test exists
            // for (process meta must not be a reuse target, only session meta)
            // is unaffected by the extra field.
            workspaceId: "ws-local-feature",
