...
export function DialogCreateCloudProject(props: DialogCreateCloudProjectProps) {
  ...
  // The provision stream is subscribed inside the async submit handler below,
  // which runs with no owner — `onCleanup` there is a no-op (Solid 2 warns
  // NO_OWNER_CLEANUP), so a dialog closed mid-provision left the listener
  // attached and still appending to disposed state. The handle lives at
  // component scope now, and the component's own cleanup releases it.
  let provisionUnsub: (() => void) | undefined
  onCleanup(() => provisionUnsub?.())
  ...
  const handleSubmit = async (e: Event) => {
    ...
    provisionUnsub = events.on("provision", (ev) => {
      if (!workspaceId) {
        buffered.push(ev)
      }
    })
    ...
    try {
      ...
      provisionUnsub?.()
      provisionUnsub = events.on("provision", (ev) => appendProvision(ev, created.directory!))
