...
export function HostedBrowserFrame(props: { url?: string }) {
  ...
  const blockedMixedContent = () =>
    typeof location !== "undefined" && location.protocol === "https:" && props.url?.startsWith("http:")
      ...
      fallback={
        <div class="flex h-full w-full flex-col items-center justify-center gap-2 p-6 text-center text-sm text-muted-foreground">
          <div class="font-medium text-text-base">
            {blockedMixedContent() ? "This source requires HTTPS." : "Browser tabs are unavailable."}
          </div>
          <div>
            {blockedMixedContent()
              ? "Hosted Claxedo blocks insecure embedded content."
              : "Open this workspace in the Claxedo desktop app to use the browser."}
          </div>
        </div>
      }
    >
