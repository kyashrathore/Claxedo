...
import { applyVisit, matchRecentPure, RECENT_CAP, PER_TAB_CAP, PERSIST_KEY } from "./browser-history"
...
// file too, so the stub is NAME-SCOPED: only the BrowserHistory context is
...
describe("applyVisit (pure reducer)", () => {
  const empty: BrowserHistoryStore = { recent: [], perTab: {} }
