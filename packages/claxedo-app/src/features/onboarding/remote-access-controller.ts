...
export function useRemoteAccessController(input: {
  ...
  // `enable()` refetches `status`, the very query the conditions read, so this
  // fed itself as one scope. `resumed` stays: the compute only dedupes ADJACENT
  // values, and it is what keeps a later revoke elsewhere (enabled going back to
  // false) from silently re-publishing the machine behind the owner.
  createEffect(
    () =>
      platform.platform === "desktop" &&
      startAtLogin.data() === true &&
      status.data?.hostedSignedIn === true &&
      status.data?.enrolled === true &&
      status.data?.enabled !== true,
    (shouldResume) => {
      if (!shouldResume || resumed) return
      resumed = true
      void enable()
    },
  )
