...
vi.mock("@/features/documents/app-ports", () => ({
  markdownPathFromHref: (href: string) => href,
  useClaxedoState: () => undefined,
  useGlobalSDK: () => ({
    client: { session: { create: h.createSession, prompt: h.api.sessionPrompt } },
  }),
  useSDK: () => ({ client: { permission: { respond: h.permissionRespond } } }),
  useSessionSyncOptional: () => h.sync,
}))
