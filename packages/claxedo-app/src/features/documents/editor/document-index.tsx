...
export function PageIndex(props: PageIndexProps) {
  ...
  // is only legal here — not inside the effect below.
  ...
  // loader runs when something reads the state — and this one's value was
  // discarded, so the index never started its controller and never issued a
  // single query. Nothing here is asynchronous to the caller anyway: the
  // compute phase tracks the scope the index is showing, and the effect phase
  // does the imperative work (retire the old controller, start the new one,
  // fan out the list and status-vocabulary requests).
  createEffect(
