...
describe("subagent presentation", () => {
  ...
  test("covers every status and unknown fallback with deterministic copy", () => {
    expect(
      ["pending", "running", "paused", "interrupted", "completed", "failed", "killed", "unknown"].map((status) =>
        subagentStatusLabel(status as Parameters<typeof subagentStatusLabel>[0]),
      ),
    ).toEqual(["Pending", "Working", "Paused", "Interrupted", "Completed", "Failed", "Killed", "Status unavailable"])
  })
