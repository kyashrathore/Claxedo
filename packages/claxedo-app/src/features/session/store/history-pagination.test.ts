...
describe("history pagination meta state", () => {
  ...
  test("is a persistent update: identical writes do not allocate a new snapshot", () => {
    createRoot((dispose) => {
      const { meta, setValue } = createHistoryMetaState()
      setValue("loading", "k1", true)
      const first = meta()
      setValue("loading", "k1", true)
      expect(meta()).toBe(first)
      setValue("loading", "k1", false)
  ...
  test("historyHasMore requires a cursor that has not failed and is not complete", () => {
    ...
    expect(historyHasMore({ limit: {}, cursor: { k: "c" }, failedCursor: {}, complete: {}, loading: {} }, "k")).toBe(true)
    // The cursor that last failed is dampened (no refetch loop).
    expect(historyHasMore({ limit: {}, cursor: { k: "c" }, failedCursor: { k: "c" }, complete: {}, loading: {} }, "k")).toBe(false)
    // A completed history has no more pages.
    expect(historyHasMore({ limit: {}, cursor: { k: "c" }, failedCursor: {}, complete: { k: true }, loading: {} }, "k")).toBe(false)
  })
  ...
  test("historyIsLoading reflects the loading flag for the key", () => {
    ...
    expect(historyIsLoading({ limit: {}, cursor: {}, failedCursor: {}, complete: {}, loading: { k: true } }, "k")).toBe(true)
  })
})
