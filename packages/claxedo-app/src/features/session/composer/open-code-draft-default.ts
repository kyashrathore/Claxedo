...
export function writeOpenCodeDraftModel(
  input: DraftInput & {
    model?: ModelKey
  ...
  input.controller.rememberDraftModel(
    input.scope,
    input.model,
    {
...
export function writeOpenCodeDraftVariant(
  input: DraftInput & {
    model?: ModelKey
    variant?: string
    labels?: DraftDefaultLabels
...
export function restoreOpenCodeDraftDefault(
  input: DraftInput & {
    ready: boolean
    models: ModelRow[]
...
export function createOpenCodeDraftModelPicker(input: {
  controller?: DraftController
  scope(): string
  ...
  const selected = () => {
    const model = input.model.current()
    if (!model) return undefined
    return {
      providerID: model.provider.id,
      modelID: model.id,
      variant: input.model.variant.current(),
    }
  }
  const picker = createMemo(() =>
    createModelSelectionPicker({
      list: input.model.list,
      current: input.model.current,
      visible: input.model.visible,
      scope: () => ({ key: `prompt:${input.scope()}`, current: selected }),
      write: (model, options) =>
        writeOpenCodeDraftModel({
          ...
          model,
          options,
          labels: openCodeDraftLabels(model, input.model.list()),
          write: input.model.set,
        }),
    }),
  )
  // Compute tracks only what should re-trigger a restore. The controller state
  // the restore consults is also what it writes (resolveDraftDefault), so those
  // reads — and directory/sessionId, which are passed through untouched — stay
  // in the untracked phase. A fresh tuple keeps the per-invalidation cadence.
  createEffect(
    () => [input.scope(), input.newSession(), input.model.ready(), input.model.list()] as const,
    () => {
      restoreOpenCodeDraftDefault({
        ...
        sessionId: input.sessionId(),
        newSession: input.newSession(),
        ready: input.model.ready(),
        models: input.model.list(),
        write: input.model.set,
        writeVariant: input.model.variant.set,
      })
    },
  )
  return { pickerModel: picker, selectedModelKey: selected }
}
import { createEffect, createMemo } from "solid-js"
import type { useLocal } from "@/features/session/providers/session-selection"
