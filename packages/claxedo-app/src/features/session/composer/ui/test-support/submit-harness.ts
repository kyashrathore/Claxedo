 ...
 * `submit.command-dispatch.test.ts`, `submit.new-session.test.ts`,
 * `submit.upstream-contract.test.ts`).
 *
 * Bun's `mock.module` registry is process-wide, so if each carved file
 * registered its own factories closing over its own local state, the
 ...
 * `isEmbedMode`/`normalizeUrl`/… — is inherited so it can't drift).
 *
...
export async function installSubmitMocks(mock: ModuleMocker) {
  ...
  mock.module("@/platform/runtime/session-url", () => ({
    resolveSessionUrl: async (sessionID: string) => (state.runtimeSessionUrl ? `${state.runtimeSessionUrl}/${sessionID}` : null),
  }))
