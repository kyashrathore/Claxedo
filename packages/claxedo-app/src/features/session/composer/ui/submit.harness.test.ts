...
async function captureRealModules() {
  ...
  for (const specifier of mockedSpecifiers) {
    try {
      // `require`, not `await import`. Bun cannot `require()` a module that a
      // plugin loaded and that has already been reached through `import()` — it
      // throws "Requested module is already fetched." The Solid 2 test JSX
      // transform is such a plugin, so importing a `.tsx` specifier here
      // poisoned it for `app-ports-stub.ts`'s `lazy()`, which must resolve
      // synchronously at call time (so `mock.module` wins) and therefore has to
      // use `require`. Loading through `require` first leaves both paths
      // working; the reverse order does not.
      captured.set(specifier, { ...(require(specifier) as Record<string, unknown>) })
    } catch {
