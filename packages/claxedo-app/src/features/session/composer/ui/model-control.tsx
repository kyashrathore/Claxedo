  ...
  return (
    <Show when={!props.harnessMode()}>
      <div data-component="prompt-model-control" class="flex min-w-0 items-center gap-1">
        {/*
          The ONE case that is not the standard picker: the server's legacy
          `big-pickle` placeholder (see signed-workspace-model.ts) names a model
          with no serving path, so the picker has nothing to pick and Connect is
          the only action that resolves it. Every other state — including having
          no provider connected at all — opens the ordinary picker, which lists
          the catalog and carries its own connect affordances.
        */}
        <Show
