...
describe("composer workspace resolver", () => {
  ...
  test("session refs prefer runtime workspace backing, then inventory backing, then local cwd", () => {
    ...
    ).toEqual({ kind: "workspace", workspaceId: "ws_live", hosting: "cloud" })
    ...
    ).toEqual({ kind: "workspace", workspaceId: "ws_cloud_main", hosting: "cloud" })
    ...
    ).toEqual({ kind: "local", cwd: "/repo/main" })
