...
const pendingIdleCallbacks: IdleRequestCallback[] = []
let updatedAt = 1
const seed = (sessionId: string, status: { type: string } = { type: "idle" }) => {
  ...
  queryClient.setQueryData(shellDataKeys.sessionId(sessionId, "todo"), [{ id: `${sessionId}-todo` }], { updatedAt: ++updatedAt })
}
...
afterEach(async () => {
  // Drain maintenance scheduled by the real writers before restoring the clock.
  await vi.runOnlyPendingTimersAsync()
  for (const callback of pendingIdleCallbacks.splice(0)) callback({ didTimeout: true, timeRemaining: () => 0 })
  await vi.runOnlyPendingTimersAsync()
  queryClient.clear()
  clearOpenSessions()
  vi.unstubAllGlobals()
...
describe("session cache ceiling", () => {
  ...
  test("revisiting a session refreshes its recency", () => {
    ...
    // Each write gets an explicit later query timestamp, including this revisit.
    seed("ses_0")
  ...
  test("waits for a 250ms presentation quiet period before requesting idle time", async () => {
    ...
    const idleCallbacks = pendingIdleCallbacks
    ...
    try {
      ...
      // A failed assertion must not leave the module's scheduled generation pending.
      for (const callback of idleCallbacks) callback({ didTimeout: true, timeRemaining: () => 0 })
