...
import {
  ...
  observeElementOffsetReconnectAware as observeOffset,
...
function observeElementOffsetReconnectAware<T extends Element, U extends Element>(
  instance: Virtualizer<T, U>, callback: (offset: number, isScrolling: boolean) => void,
) {
  const cleanup = observeOffset(instance, callback)
...
function createInstance(input: {
  ...
  const parent = input.viewport.parentElement
  elements.push(parent && parent !== document.body ? parent : input.viewport)
...
test("restores the stored offset after reconnect and ignores equal offsets and unrelated mutations", async () => {
  ...
  observeElementOffsetReconnectAware(instance, record)
  await frames(1)
...
// Wait for the observable reconnect outcome with a bounded deadline.
