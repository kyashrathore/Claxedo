...
describe("Claxedo session loaded-empty rendering", () => {
  ...
  test("a cold fold that just reset its reveal latch still schedules the reveal", () => {
    // The caller resets `initialRevealReady` to false and schedules the reveal in
    // the same task. That write stages until the next flush, so the latch still
    // reads `true` at the call — hence `revealed` is an argument. Passing the
    // stale `true` here is what left the timeline permanently hidden.
    expect(
      timelineFirstFoldRevealShouldSchedule({ warmMeasurements: false, revealed: false, rowCount: 80 }),
    ).toBe(true)
    expect(
      timelineFirstFoldRevealShouldSchedule({ warmMeasurements: false, revealed: true, rowCount: 80 }),
    ).toBe(false)

    // Warm measurements reveal synchronously, and an empty fold has nothing to reveal.
    expect(
      timelineFirstFoldRevealShouldSchedule({ warmMeasurements: true, revealed: false, rowCount: 80 }),
    ).toBe(false)
    expect(
      timelineFirstFoldRevealShouldSchedule({ warmMeasurements: false, revealed: false, rowCount: 0 }),
    ).toBe(false)
  })
