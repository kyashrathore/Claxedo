...
describe("harness store state projectors", () => {
  ...
  test("derives the standing health-probe readiness transition (T4)", () => {
    ...
    expect(harnessHealthReadiness({ harness: "codex-app-server", current: "ready", health: "degraded" })).toBe(
      "degraded",
    )
    expect(harnessHealthReadiness({ harness: "codex-app-server", current: "ready", health: "unavailable" })).toBe(
      "degraded",
    )
    ...
    expect(harnessHealthReadiness({ harness: "codex-app-server", current: "degraded", health: "degraded" })).toBe(
      "degraded",
    )
    // The probe never stomps a state owned by hydration / hard failure.
    expect(
      harnessHealthReadiness({ harness: "codex-app-server", current: "error", health: "degraded" }),
    ).toBeUndefined()
    expect(
      harnessHealthReadiness({ harness: "codex-app-server", current: "polling", health: "degraded" }),
    ).toBeUndefined()
