...
const modelsContextInput = {
  ...
  init: () => {
    ...
    function update(model: ModelKey, state: Visibility) {
      // Find and upsert inside ONE draft: a committed `findIndex` cannot see a
      // model added earlier in the same task, and `store.user.length` is still
      // the pre-append length, so two updates in a task duplicated the entry.
      setStore(($state) => {
