...
describe("first-turn recovery", () => {
  ...
  test("reads the server classification and has a legacy-message fallback", () => {
    ...
    expect(sessionRecoveryClass({ name: "UnknownError", data: { message: "harness_switch_not_supported" } })).toBe(
      "harness",
    )
  ...
  test("a status-less failure does not claim the provider was unreachable", () => {
    expect(sessionRecoveryDescription("unknown", { name: "UnknownError", data: {} }, { providerID: "anthropic" })).toBe(
