...
function fontsStub(behaviour: { resolve?: () => void; reject?: () => void; never?: boolean }) {
  ...
  // as-any: FontFaceSet has ~15 members this fake never exercises; waitForFontReady only reads load/ready.
...
describe("waitForFontReady", () => {
  ...
  test("resolves when the environment has no fonts.load", async () => {
    // as-any: deliberately an EMPTY fonts object — the case under test is an environment with no fonts.load.
