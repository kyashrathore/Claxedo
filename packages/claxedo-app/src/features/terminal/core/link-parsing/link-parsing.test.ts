...
describe("TerminalLinkParsing", () => {
  ...
  describe("detectLinks", () => {
    it('foo(1, 2) bar[3, 4] "baz" on line 5', () => {
      expect(detectLinks('foo(1, 2) bar[3, 4] "baz" on line 5', OperatingSystem.Linux)).toEqual([
        {
          path: {
            index: 0,
            text: "foo",
    ...
    it("should extract the link prefix", () => {
      expect(detectLinks('"foo", line 5, col 6', OperatingSystem.Linux)).toEqual([
        {
          path: {
            index: 1,
            text: "foo",
    ...
    it("should be smart about determining the link prefix when multiple prefix characters exist", () => {
      expect(detectLinks("echo '\"foo\", line 5, col 6'", OperatingSystem.Linux)).toEqual([
        {
          path: {
            index: 7,
            text: "foo",
    ...
    describe('"|"', () => {
      it("should exclude pipe characters from link paths", () => {
        expect(detectLinks("|C:\\Github\\microsoft\\vscode|", OperatingSystem.Windows)).toEqual([
          {
            path: {
              index: 1,
              text: "C:\\Github\\microsoft\\vscode",
      ...
      it("should exclude pipe characters from link paths with suffixes", () => {
        expect(detectLinks("|C:\\Github\\microsoft\\vscode:400|", OperatingSystem.Windows)).toEqual([
          {
            path: {
              index: 1,
              text: "C:\\Github\\microsoft\\vscode",
    ...
    describe("query strings", () => {
      for (const os of operatingSystems) {
        ...
        it("should not detect links starting with ? within query strings that contain Windows-style paths (#204195)", () => {
          // ? appended to the cwd will exist since it's just the cwd
          expect(detectLinks("http://foo.com/?bar=a:\\b&baz=c", os).some((e) => e.path.text.startsWith("?"))).toBe(
            false,
          )
        })
      }
    ...
    describe("should detect file names in git diffs", () => {
      ...
      it("diff --git a/foo/bar b/foo/baz", () => {
        expect(detectLinks("diff --git a/foo/bar b/foo/baz", OperatingSystem.Linux)).toEqual([
          {
            path: {
              index: 13,
              text: "foo/bar",
        ...
        it(`\`${line.replaceAll("\u00A0", "<nbsp>")}\``, () => {
          ...
          const detectedLink1: IParsedLink = {
            ...
            path: {
              ...
              text: link1.link.replace(link1Suffix, "").replace(link1.prefix || "", ""),
            },
            suffix: {
          ...
          const detectedLink2: IParsedLink = {
            ...
            path: {
              index:
                ...
                1 +
                (link2.prefix ?? "").length,
              text: link2.link.replace(link2Suffix, "").replace(link2.prefix ?? "", ""),
            },
            suffix: {
          ...
          const detectedLink3: IParsedLink = {
            ...
            path: {
              index:
                ...
                1 +
                (link3.prefix ?? "").length,
              text: link3.link.replace(link3Suffix, "").replace(link3.prefix ?? "", ""),
            },
            suffix: {
          ...
          expect(detectLinks(line, OperatingSystem.Linux)).toEqual([detectedLink1, detectedLink2, detectedLink3])
        })
      }
    })
    ...
    describe("should ignore links with suffixes when the path itself is the empty string", () => {
      it('""",1', () => {
        expect(detectLinks('""",1', OperatingSystem.Linux)).toEqual([] as IParsedLink[])
      })
    })
  })
})
