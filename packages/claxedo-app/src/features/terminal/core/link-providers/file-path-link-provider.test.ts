...
import type { LinkProviderLine, LinkProviderTerminal } from "./multi-line-link-provider"

function createMockLine(text: string, isWrapped = false): LinkProviderLine {
  return {
...
function createMockTerminal(lines: Array<{ text: string; isWrapped?: boolean }>): LinkProviderTerminal {
  const mockLines = lines.map((l) => createMockLine(l.text, l.isWrapped ?? false))
...
function mouseEvent(input: { metaKey?: boolean; ctrlKey?: boolean }) {
...
function getLinks(provider: FilePathLinkProvider, lineNumber: number): Promise<ILink[]> {
...
describe("FilePathLinkProvider", () => {
  describe("basic file path detection", () => {
    it("should detect absolute paths", async () => {
      const terminal = createMockTerminal([{ text: "Error in /path/to/file.ts:10:5" }])
      const onOpen = mock()
    ...
    it("should detect relative paths starting with ../", async () => {
      const terminal = createMockTerminal([{ text: "Import from ../lib/helper.ts" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should detect home directory paths", async () => {
      const terminal = createMockTerminal([{ text: "Config at ~/config/settings.json" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should detect multiple paths on one line", async () => {
      const terminal = createMockTerminal([{ text: "Import ./src/a.ts and ./src/b.ts" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("filtering false positives", () => {
    ...
    it("should skip numeric ratio patterns like 1/2", async () => {
      const terminal = createMockTerminal([{ text: "progress 1/2 complete" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip generic word/word fragments without a file-like basename", async () => {
      const terminal = createMockTerminal([{ text: "this should not linkify foo/bar in text" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip URLs with http://", async () => {
      const terminal = createMockTerminal([{ text: "Visit http://example.com/path" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip URLs with https://", async () => {
      const terminal = createMockTerminal([{ text: "Visit https://example.com/path" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip npm package references", async () => {
      const terminal = createMockTerminal([{ text: "lodash@4.17.21/index.js" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip git hash ranges like 9ebf9967e..a514ef81e", async () => {
      const terminal = createMockTerminal([{ text: "9ebf9967e..a514ef81e 100644" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should skip pure numbers like 123:456", async () => {
      ...
      const terminal = createMockTerminal([{ text: "at position 123:456:789" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("wrapped lines - forward looking (next line)", () => {
    it("should detect path that spans current line and wrapped next line", async () => {
      const terminal = createMockTerminal([{ text: "/path/to/very/long/fi" }, { text: "le/name.ts", isWrapped: true }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("wrapped lines - backward looking (previous line)", () => {
    ...
    it("should handle clicking on wrapped portion of path", async () => {
      ...
      // node_modules paths are intentionally ignored to reduce noise
      expect(links.length).toBe(0)
    })
  })
  ...
  describe("fallback priority over primary detection", () => {
    it("uses the fallback parse for python paths with spaces instead of two bogus primary links", async () => {
      const terminal = createMockTerminal([{ text: '  File "/tmp/my dir/x.py", line 3' }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("URL interiors", () => {
    it("should not linkify the path portion of an ftp URL", async () => {
      const terminal = createMockTerminal([{ text: "get it from ftp://files.example.com/pub/data.tar.gz" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should not linkify the path portion of an https URL", async () => {
      const terminal = createMockTerminal([{ text: "Visit https://example.com/deep/file.ts today" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("npm package references", () => {
    it("still links a path that merely sits near a version string", async () => {
      const terminal = createMockTerminal([{ text: "lodash@4.17.21 needs ./src/patch.ts fixed" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("skips scoped package references with attached versions", async () => {
      const terminal = createMockTerminal([{ text: "installed @scope/package@1.2.3 just now" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("non-wrapped lines", () => {
    it("should not combine lines that are not wrapped", async () => {
      const terminal = createMockTerminal([{ text: "/path/one.ts" }, { text: "/path/two.ts", isWrapped: false }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("handleActivation", () => {
    it("should require metaKey (Cmd) for activation", async () => {
      ...
      const links = await getLinks(provider, 1)
      const mockEvent = mouseEvent({ metaKey: false, ctrlKey: false })
    ...
    it("should activate with metaKey (Cmd)", async () => {
      ...
      const links = await getLinks(provider, 1)
      const mockEvent = mouseEvent({ metaKey: true, ctrlKey: false })
    ...
    it("should activate with ctrlKey", async () => {
      ...
      const links = await getLinks(provider, 1)
      const mockEvent = mouseEvent({ metaKey: false, ctrlKey: true })
  ...
  describe("VSCode-style link formats", () => {
    it("should detect parenthesis format: file.ts(42)", async () => {
      const terminal = createMockTerminal([{ text: "Error in /path/file.ts(42)" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should detect parenthesis format with column: file.ts(42, 10)", async () => {
      const terminal = createMockTerminal([{ text: "Error in /path/file.ts(42, 10)" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it('should detect verbose format: "file.ts", line 42', async () => {
      const terminal = createMockTerminal([{ text: 'Error in "/path/file.ts", line 42' }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it('should detect verbose format with column: "file.ts", line 42, col 10', async () => {
      const terminal = createMockTerminal([{ text: 'Error in "/path/file.ts", line 42, col 10' }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should detect line ranges: file.ts:42-50", async () => {
      const terminal = createMockTerminal([{ text: "See /path/file.ts:42:10-50" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("URL-encoded paths", () => {
    it("should decode URL-encoded path with line number on activation", async () => {
      const terminal = createMockTerminal([{ text: "apps/desktop/src/main/lib/workspace-manager.ts%3A50" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
      ...
      expect(onOpen).toHaveBeenCalled()
      expect(onOpen.mock.calls[0][1]).toBe("apps/desktop/src/main/lib/workspace-manager.ts")
    ...
    it("should decode URL-encoded spaces in path", async () => {
      const terminal = createMockTerminal([{ text: "./path/to%20file/name.ts" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("punctuation handling", () => {
    it("should handle path followed by period at end of sentence", async () => {
      const terminal = createMockTerminal([{ text: "See the file at ./path/something." }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
    ...
    it("should handle path in quotes", async () => {
      const terminal = createMockTerminal([{ text: 'Error in "./path/file.ts"' }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
  ...
  describe("edge cases", () => {
    ...
    it("should handle paths without directories (just relative path)", async () => {
      const terminal = createMockTerminal([{ text: "src/components/Button.tsx" }])
      const onOpen = mock()
      const provider = new FilePathLinkProvider(terminal, onOpen)
