...
function getLinks(provider: UrlLinkProvider, lineNumber: number): Promise<ILink[]> {
...
describe("UrlLinkProvider", () => {
  describe("basic URL detection", () => {
    ...
    it("should detect URLs with query parameters", async () => {
      const terminal = createMockTerminal([{ text: "https://example.com/path?foo=bar&baz=qux" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should detect URLs with fragments", async () => {
      const terminal = createMockTerminal([{ text: "https://example.com/path#section" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should detect multiple URLs on one line", async () => {
      const terminal = createMockTerminal([{ text: "https://a.com and https://b.com" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should detect URLs with port numbers", async () => {
      const terminal = createMockTerminal([{ text: "Server at http://localhost:3000/api" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should handle URLs with parentheses (like Wikipedia)", async () => {
      const terminal = createMockTerminal([{ text: "https://en.wikipedia.org/wiki/URL_(disambiguation)" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
      ...
      expect(links.length).toBe(1)
      expect(links[0].text).toBe("https://en.wikipedia.org/wiki/URL_(disambiguation)")
    })

    it("should strip trailing period from URL", async () => {
      const terminal = createMockTerminal([{ text: "See https://example.com." }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should strip trailing comma from URL", async () => {
      const terminal = createMockTerminal([{ text: "Visit https://example.com, then continue." }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should strip multiple trailing punctuation", async () => {
      const terminal = createMockTerminal([{ text: "Check https://example.com..." }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should strip trailing exclamation and question marks", async () => {
      const terminal = createMockTerminal([{ text: "Is it https://example.com?" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should keep balanced parentheses in URL", async () => {
      const terminal = createMockTerminal([{ text: "https://example.com/path(foo)" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
    ...
    it("should handle URL in parentheses with balanced parens inside", async () => {
      const terminal = createMockTerminal([{ text: "(see https://en.wikipedia.org/wiki/URL_(disambiguation))" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
  ...
  describe("wrapped lines - backward looking (previous line)", () => {
    ...
    it("should handle clicking on wrapped portion of URL", async () => {
      ...
      expect(links.length).toBe(1)
      expect(links[0].text).toBe("https://github.com/anthropics/claude-code/issues")
    })
  })
  ...
  describe("IPv6 URLs", () => {
    it("should detect bracketed IPv6 hosts with port and path", async () => {
      const terminal = createMockTerminal([{ text: "listening on http://[::1]:8080/health now" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
  ...
  describe("non-wrapped lines", () => {
    it("should not combine lines that are not wrapped", async () => {
      const terminal = createMockTerminal([{ text: "https://a.com" }, { text: "https://b.com", isWrapped: false }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
  ...
  describe("edge cases", () => {
    ...
    it("should handle lines without URLs", async () => {
      const terminal = createMockTerminal([{ text: "This is just some text without links" }])
      const onOpen = mock()
      const provider = new UrlLinkProvider(terminal, onOpen)
