...
import { WrappedLineLinkProvider, type LinkProviderTerminal } from "./multi-line-link-provider"
...
export class FilePathLinkProvider extends WrappedLineLinkProvider {
  ...
  private isExplicitPath(pathText: string): boolean {
    ...
    return /^(?:file:\/\/\/|~\/|\.{1,2}\/|\/|\\\\|[a-zA-Z]:[\\/])/.test(pathText)
  }
  ...
  private stripTrailingPunctuation(parsedLink: IParsedLink, combinedText: string): IParsedLink {
    ...
    if (trailingPunctMatch) {
      ...
      if (afterPunct === undefined || /\s/.test(afterPunct) || afterPunct === '"' || afterPunct === "'") {
  ...
  private isUrl(pathText: string, linkStart: number, combinedText: string): boolean {
    if (pathText.startsWith("http://") || pathText.startsWith("https://") || pathText.startsWith("ftp://")) {
      return true
    }
  ...
  private isNpmPackageReference(pathText: string, linkStart: number, combinedText: string): boolean {
