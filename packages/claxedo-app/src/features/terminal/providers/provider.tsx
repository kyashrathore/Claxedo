...
import { createContext, createTrackedEffect, createRoot, onCleanup, useContext, type ParentProps } from "solid-js"
...
export function createTerminalSession(sdk: ReturnType<typeof useSDK>, dir: string, options?: TerminalSessionOptions) {
  ...
  return {
    ...
    next() {
      ...
      setStore(storePath("active", store.all[nextIndex]?.id))
    },
    previous() {
      const index = store.all.findIndex((x) => x.id === store.active)
      if (index === -1) return
      const prevIndex = index === 0 ? store.all.length - 1 : index - 1
      setStore(storePath("active", store.all[prevIndex]?.id))
    },
    async close(id: string) {
      void (() => {
        ...
        if (store.active === id) {
          ...
          setStore(storePath("active", filtered[next]?.id))
        }
        setStore(storePath("all", filtered))
      })()
    ...
    removeStale(ids: Set<string>) {
      ...
      void (() => {
        ...
        if (store.active && ids.has(store.active)) {
          setStore(storePath("active", filtered[0]?.id))
        }
      })()
    },
    trim(id: string) {
...
export function useOptionalTerminal() {
  return useContext(TerminalCtx) ?? undefined
}
