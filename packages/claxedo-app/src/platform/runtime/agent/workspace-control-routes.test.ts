...
describe("workspace control routes", () => {
  test("builds workspace sandbox driver, create, resolve, and delete URLs", () => {
    ...
    expect(workspaceCreateUrl({ baseUrl: "https://control.example.test/" })).toBe(
      "https://control.example.test/api/workspace/create",
    )
    expect(
      workspaceResolveUrl({
        baseUrl: "https://control.example.test/",
  ...
  test("builds control session routes", () => {
    ...
    ).toBe("https://control.example.test/api/control/sessions?workspaceId=ws_1&directory=%2Frepo")
    expect(
      controlSessionUrl({
        baseUrl: "https://control.example.test/",
    ...
    ).toBe("https://control.example.test/api/control/sessions/session%2Fwith%20slash/gateway?workspaceId=ws_1")
    expect(
      controlSessionNavigationListUrl({
        baseUrl: "https://control.example.test/",
        scope: "global",
      ...
      "https://control.example.test/api/control/session-list?scope=global&limit=50&projectId=proj_1&workspaceId=ws_1&directory=%2Frepo&groupBy=workspace&archived=archived&status=running%2Cidle&environment=prod&git=dirty%2Cclean&search=hello+world&cursor=cursor%3A1",
    )
    expect(
      sessionNavigationListUrl({
    ...
    ).toBe("http://127.0.0.1:3001/api/claxedo/session-list?scope=workspace&limit=20&directory=%2Frepo")
    expect(
      sessionNavigationListUrl({
        baseUrl: "https://control.example.test/",
        scope: "workspace",
    ...
    ).toBe("https://control.example.test/api/control/session-list?scope=workspace&limit=20&directory=%2Frepo")
  })
  ...
  test("builds experimental sandbox path", () => {
    expect(experimentalSandboxPath("workspace:ws_cloud")).toBe(
      "/api/experimental/sandbox?directory=workspace%3Aws_cloud",
    )
  })
  ...
  test("workspace create surfaces do not import RuntimeGateway for stable route helpers", async () => {
    ...
    expect(
      await Bun.file(new URL("../../../features/workspaces/actions/project-actions.tsx", import.meta.url)).text(),
