...
export function useClerkConvexToken() {
  ...
  // Nothing reactive to subscribe to: Clerk's own listener is the token source,
  // so the compute is constant and the whole subscription lives in the effect
  // phase, which owns its unsubscribe.
  createEffect(
    () => undefined,
    () => {
      let mounted = true
