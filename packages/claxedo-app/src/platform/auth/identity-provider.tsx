import { createContext, useContext, type Accessor, type JSX } from "solid-js"

/**
 * Who the viewer is — sanitized identity and roles, and nothing else.
 *
 * Deliberately carries no token and no way to mint one: `usePrincipal()` is
 * reachable by anything that can reach a Principal, and every renderer that
 * needs an authenticated call goes through the auth client directly, so a
 * token accessor here would be pure surface area — a handle to bearer
 * material sitting in every component that reads it.
 *
 * On the desktop that distinction becomes structural: Electron main owns the
 * account session, and the renderer receives sanitized state plus named
 * operations — never a token, an OAuth code, or a generic fetch. Keeping a
 * token accessor here would be a second, contradictory path to the same
 * material.
 */
export type Principal =
  | { kind: "anonymous" }
  | { kind: "local"; deviceId: string }
  | { kind: "signed-unresolved" }
  | { kind: "signed"; userId: string }
  | {
      kind: "org-member"
      userId: string
      orgId: string
      memberships: ReadonlyArray<{ workspaceId: string; role: string }>
    }

export type IdentityContextValue = {
  principal: Accessor<Principal>
}

const IdentityContext = createContext<IdentityContextValue>()

export function IdentityProvider(props: {
  principal: Accessor<Principal> | Principal
  children: JSX.Element
}) {
  // Bound to a const so the `typeof` narrowing survives into the accessor;
  // reading `props.principal` inside the closure would widen it back.
  const given = props.principal
  const principal = typeof given === "function" ? given : () => given
  return (
    <IdentityContext.Provider value={{ principal }}>
      {props.children}
    </IdentityContext.Provider>
  )
}

export function useIdentity() {
  const context = useContext(IdentityContext)
  if (!context) throw new Error("useIdentity must be used within IdentityProvider")
  return context
}

export function usePrincipal() {
  return useIdentity().principal
}

export function principalHasSignedAccess(principal: Principal) {
  return principal.kind === "local" || principal.kind === "signed-unresolved" || principal.kind === "signed" || principal.kind === "org-member"
}

/** A durable identity namespace, or null while credentials lack a resolved subject. */
export function principalDataScope(principal: Principal) {
  switch (principal.kind) {
    case "signed-unresolved": return null
    case "anonymous": return "anonymous"
    case "local": return `local:${principal.deviceId}`
    case "signed": return `signed:${principal.userId}`
    case "org-member": return `org-member:${principal.userId}:${principal.orgId}`
    default: {
      const unmapped: never = principal
      throw new Error(`no data scope for principal ${JSON.stringify(unmapped)}`)
    }
  }
}
