...
const languageContextInput = {
  ...
  init: (props: { locale?: Locale; strings?: Record<string, Record<string, string>> } = {}) => {
    ...
    createEffect(locale, (current) => {
      // The `locale()` re-read in the continuation is a staleness check against
      // whatever is current when the load resolves, not a dependency.
    ...
    createEffect(locale, (current) => {
      ...
      document.cookie = cookie(current)
    })
