...
import { resolveRecovery, rememberRecovery } from "../components/multi-pane/pane-terminal-recovery"
...
export const aliasTerminalLogSummary = (oldId: string, newId: string) => {
...
export const loadTerminalLogSummary = (
  ...
  const next = request(`${nextTarget.site}/api/claxedo/process/logs?${query.toString()}`)
    .then((res) => {
      if (!res.ok) {
        if (terminalLogDebug.enabled(2)) {
