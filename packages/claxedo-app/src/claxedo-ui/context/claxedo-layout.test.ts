...
describe("tab isolation between groups", () => {
  ...
  test("adding review to group A does not appear in group B", () => {
    ...
    try {
      ...
      expect(tabs1.items()).toHaveLength(1) // review workspace
      expect(tabs1.items().find((t: any) => t.type === "review-workspace")!.type).toBe("review-workspace")
      expect(tabs2.items()).toHaveLength(0) // empty
    } finally {
      dispose()
    }
  ...
  test("review tabs dedupe by mode and refs", () => {
    ...
    try {
      ...
      expect(a).toBe(b)
      expect(a).not.toBe(c)
      expect(tabs1.items()).toHaveLength(2) // 2 review workspaces
      const modes = tabs1
        .items()
        .filter((tab: any) => tab.type === "review-workspace")
  ...
  test("review focus patch stays scoped to the owning group tab", () => {
    ...
    try {
      ...
      const t2 = tabs2.items().find((tab: any) => tab.type === "review-workspace")
...
describe("page tab integration", () => {
  ...
  test("page tab has correct shape", () => {
    ...
    try {
      ...
      expect(tab.directory).toBeUndefined()
      expect(tab.scope).toBe("global")
  ...
  test("workgraph tab uses a wkg route id", () => {
    const { api, dispose } = createTestLayout()
    try {
      const { tabs1 } = splitInto2(api)
      const id = tabs1.addWorkgraph("/ws")

      expect(id).toMatch(/^wkg-/)
      expect(tabs1.items().find((t: any) => t.id === id)?.type).toBe("workgraph")
    } finally {
      dispose()
    }
  })
