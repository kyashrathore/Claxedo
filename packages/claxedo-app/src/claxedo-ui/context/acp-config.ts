...
export const acpScope = panePreferenceScope
...
export function initialRunner(scope: string, saved?: string, legacy?: string | null): RunnerType {
  return pickRunner(initialPaneRunner(scope, saved, legacy), null) ?? "opencode"
}
...
function init() {
  ...
  const prefs = createPanePreferences(localStorage)
  const runners = prefs.maps.runner
  const models = prefs.maps.model
  const agents = prefs.maps.agent
  ...
  const promote = (from: string, to: string) => {
    ...
    prefs.promote(from, to, ["runner", "model", "agent"])
  }
