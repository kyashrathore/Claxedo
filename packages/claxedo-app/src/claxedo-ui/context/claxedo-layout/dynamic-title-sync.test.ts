...
describe("computeTabTitle", () => {
  ...
  test("page pane uses page title", () => {
    const result = computeTabTitle(
      makeInput({
        ...
        paneContents: [makeContent({ type: "page", directory: "/workspace", pageId: "page-1", title: "Architecture Notes" })],
      }),
    )
  ...
  test("session with page — session takes priority (lower number)", () => {
    const result = computeTabTitle(
      makeInput({
        paneContents: [
          makeContent({ type: "page", directory: "/workspace", pageId: "page-1", title: "My Page" }),
          makeContent({ type: "session", directory: "/workspace", sessionId: "s1" }),
        ],
  ...
  test("page without session uses page title", () => {
    const result = computeTabTitle(
      makeInput({
        tab: makeTab({ type: "page", title: "Page" }),
        paneContents: [makeContent({ type: "page", directory: "/workspace", pageId: "page-1", title: "Design Doc" })],
      }),
    )
