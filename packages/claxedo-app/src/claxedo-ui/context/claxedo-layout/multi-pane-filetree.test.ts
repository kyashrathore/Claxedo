...
describe("toggleFileTree placement", () => {
  ...
  test("toggleFileTree then toggleReviewWorkspace keeps file tree on the far right", () => {
    const { mp, setStore } = setup()
    const tabId = "tab-1"
    const sessionLeaf = "session-leaf"
    ...
    const afterFileTree = mp.activeLayout(tabId)!
    const ftLeaf = findFileTreeLeaf(afterFileTree)!
    expect(leafPosition(afterFileTree.pane, ftLeaf)).toBeGreaterThan(leafPosition(afterFileTree.pane, sessionLeaf))

    mp.toggleReviewWorkspace(tabId, "/ws", "s1", "session")

    const layout = mp.activeLayout(tabId)!
    const reviewLeaf = findReviewLeaf(layout)!
    const ids = paneLeafIds(layout.pane)

    expect(leafPosition(layout.pane, sessionLeaf)).toBe(0)
    expect(leafPosition(layout.pane, reviewLeaf)).toBe(1)
    expect(ids.at(-1)).toBe(ftLeaf)
  })
})
