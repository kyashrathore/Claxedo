...
describe("route intent adapter", () => {
  ...
  test("syncs session route into tab and closes stale new tab", async () => {
    const setActiveCalls: string[] = []
    ...
    const topTabs = baseTopTabs({
      ...
      addSession(directory, sessionId, title) {
        ...
        if (sessionId === "s-1") return "tab-s-1"
    ...
    expect(closeCalls).toEqual(["tab-new"])
    expect(navCalls).toEqual([`/${base64Encode("/ws")}/tab/tab-s-1`])
  })
