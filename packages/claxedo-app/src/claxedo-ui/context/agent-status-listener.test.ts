...
// ---------------------------------------------------------------------------
// Module mocks — must be registered before importing agent-status-listener.ts
// ---------------------------------------------------------------------------
...
mock.module("@/utils/sound", () => ({
  playSound: () => {},
  playSoundById: () => Promise.resolve(undefined),
  soundSrc: () => "",
}))

// Mock claxedo events provider — return mockEmitter so hooks don't early-return
mock.module("@claxedo/providers/claxedo-events", () => ({
...
mock.module("@claxedo/overrides/utils/debug", () => ({
...
mock.module("@claxedo/claxedo-ui/context/claxedo-layout", () => ({
  useClaxedoLayout: () => currentApi,
  // Re-export types that agent-status-listener.ts imports
  get TabItem() {
    return {}
  },
...
describe("usePtyExitCleanup", () => {
  test("clears working status when pty exits", () => {
    ...
    try {
      ...
      // Simulate pty.exited
      mockEmitter.emitOn("pty.exited", { id: "pty-1", exitCode: 0 })
  ...
  test("clears permission status when pty exits", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-2", exitCode: 0 })
  ...
  test("no-op for untracked terminals", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-unknown", exitCode: 0 })
  ...
  test("no-op for terminals already idle", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-idle", exitCode: 0 })
  ...
  test("ignores non-pty.exited events", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.created", { id: "pty-other" })
  ...
  test("handles terminal with no owning tab gracefully", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-orphan", exitCode: 0 })
  ...
  test("multi-terminal tab: one exit keeps other terminal's status", () => {
    ...
    try {
      ...
      // pty-m1 exits
      mockEmitter.emitOn("pty.exited", { id: "pty-m1", exitCode: 0 })
  ...
  test("multi-terminal tab: last exit clears tab loading", () => {
    ...
    try {
      ...
      // Both exit
      mockEmitter.emitOn("pty.exited", { id: "pty-l1", exitCode: 0 })
      mockEmitter.emitOn("pty.exited", { id: "pty-l2", exitCode: 0 })
  ...
  test("idempotent: double pty.exited for same terminal", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-double", exitCode: 0 })
      mockEmitter.emitOn("pty.exited", { id: "pty-double", exitCode: 0 })
...
describe("useAgentLifecycleListener", () => {
  ...
  test("Error event keeps existing permission aggregation behavior", () => {
    const { api, dispose } = createTestHarness(useAgentLifecycleListener)
    try {
      const { tabId } = addTerminal(api, "/ws", "pty-srv-error")

      mockEmitter.emitOn("agent.lifecycle", {
        tabId,
        terminalId: "pty-srv-error",
        eventType: "Error",
      })

      expect(api.terminal.agentStatus("pty-srv-error")).toBe("permission")
      expect(api.terminal.getTabAgentStatus(tabId).attention).toBe(true)
    } finally {
      dispose()
    }
  })

  test("Busy maps to working, Idle maps to idle (server normalizes Start→Busy, Stop→Idle)", () => {
    const { api, dispose } = createTestHarness(useAgentLifecycleListener)
    try {
      const { tabId } = addTerminal(api, "/ws", "pty-srv-legacy")

      // Busy (server-normalized from Start)
      mockEmitter.emitOn("agent.lifecycle", { tabId, terminalId: "pty-srv-legacy", eventType: "Busy" })
      expect(api.terminal.agentStatus("pty-srv-legacy")).toBe("working")

      // Idle (server-normalized from Stop)
      mockEmitter.emitOn("agent.lifecycle", { tabId, terminalId: "pty-srv-legacy", eventType: "Idle" })
...
describe("defense-in-depth: idempotent multi-layer cleanup", () => {
  test("server Idle + pty.exited double-fire: second is no-op", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("agent.lifecycle", { tabId, terminalId: "pty-both", eventType: "Idle"
      })

      expect(api.terminal.agentStatus("pty-both")).toBe("idle")

      // Layer 3: pty.exited also fires — should be no-op
      mockEmitter.emitOn("pty.exited", { id: "pty-both", exitCode: 0 })
  ...
  test("Ctrl+C interrupt + pty.exited: both clear, no crash", () => {
    ...
    try {
      ...
      mockEmitter.emitOn("pty.exited", { id: "pty-ctrlc-exit", exitCode: 0 })
  ...
  test("all three layers for same terminal: no crash, ends idle", () => {
    ...
    try {
      ...
      // Server Idle
      mockEmitter.emitOn("agent.lifecycle", { tabId, terminalId: "pty-triple", eventType: "Idle"
      })

      // PTY exit
      mockEmitter.emitOn("pty.exited", { id: "pty-triple", exitCode: 0 })
...
describe("optimistic interrupt recovery", () => {
  test("Ctrl+C clears status, agent re-sends Busy → tab shows loading again", () => {
    ...
    try {
      ...
      // Agent starts
      mockEmitter.emitOn("agent.lifecycle", { tabId, terminalId: "pty-recover", eventType: "Busy"
      })
