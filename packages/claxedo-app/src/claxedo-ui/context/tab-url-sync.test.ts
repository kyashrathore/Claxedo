...
function syncTabUrl(opts: {
  tab: { id: string; directory?: string; scope?: "directory" | "global" } | undefined
  activeWorkspaceId: () => string | undefined
  params: { tabId?: string; id?: string; pageId?: string }
  navigate: (path: string, opts?: { replace?: boolean }) => void
  suppressAutoTab: (dir: string) => void
  allowAutoTab: (dir: string) => void
}) {
  const { tab, activeWorkspaceId, params, navigate, suppressAutoTab, allowAutoTab } = opts
  if (tab) {
    const dir = tab.directory ?? activeWorkspaceId()
    if (!dir) return
    allowAutoTab(dir)
...
describe("syncTabUrl extracted function", () => {
  ...
  test("close last session tab also resets the route to workspace root", () => {
    const replaceStateCalls: string[] = []
    const origReplaceState = window.history.replaceState.bind(window.history)
    window.history.replaceState = (data: any, title: string, url?: string | URL | null) => {
      if (url) replaceStateCalls.push(String(url))
    ...
    try {
      ...
      expect(suppressed.has("/ws")).toBe(true)
      expect(replaceStateCalls).toEqual([`/${base64Encode("/ws")}`])
    } finally {
      window.history.replaceState = origReplaceState
    }
  })
  ...
  test("global page tab resolves via activeWorkspaceId", () => {
    const navCalls: string[] = []

    syncTabUrl({
      tab: { id: "tab-page", scope: "global" },
