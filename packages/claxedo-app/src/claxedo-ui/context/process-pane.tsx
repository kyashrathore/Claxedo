...
export const { use: useProcessPane, provider: ProcessPaneProvider } = createSimpleContext({
  ...
  init: (props: { surfaceId?: string }) => {
    ...
    const isProcessOpen = () => {
      ...
      return panel.open && panel.mode === "processes" && panel.workspaceDir === sdk.directory
    }
    const requestProcessOpen = () => {
      ...
      state.workspacePanel.open("processes", {
    ...
    const api = {
      ...
      // Pane sizing is retained only for persisted compatibility.
