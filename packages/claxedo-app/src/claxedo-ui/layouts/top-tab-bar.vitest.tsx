...
vi.mock("@/context/global-sync", () => ({
  useGlobalSync: () => ({
    child: () => [{ session_status: {}, permission: {}, session: sessions }],
  }),
}))
...
describe("TopTabBar separators", () => {
  test("prefers the session list title over a static Session tab label", () => {
    const list = [
      {
        id: "tab-session",
        type: "session",
    ...
    const view = render(() => <TopTabBar groupId="g-default" />)

    expect(view.getByText("Sidebar Title")).toBeTruthy()
  })
...
describe("tab closing", () => {
  test("a failing onTabClose callback does not wedge future closes", async () => {
    ...
    const { container } = render(() => (
      <TopTabBar groupId="g-default" onTabClose={close} />
    ))
