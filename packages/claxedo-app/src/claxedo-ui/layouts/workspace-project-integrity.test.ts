...
import { projectCatalog, canAutoOpenProject, type ProjectState } from "@claxedo/context/layout-projects"
...
type WorkspaceMetadata = Record<string, WorkspaceInfo>
type ProjectMetadata = ProjectState & {
...
describe("end-to-end: API response → project display", () => {
  ...
  const valid = (dir: string) => (dir.startsWith("/") && dir.length > 1) || /^workspace:[A-Za-z0-9._-]+$/.test(dir)
  ...
  test("project with local checkout + cloud sandbox shows both workspaces", () => {
    const api: Project[] = [
      fakeApiProject({
        ...
        name: "Claxedo",
        sandboxes: ["workspace:ws_clax_main"],
      }),
    ]
    const sessions = {
    ...
    const projectItem = fakeProjectItem({
      ...
      workspaces: {
        ...
        "workspace:ws_clax_main": { kind: "cloud", workspace_name: "main", provider: "daytona" },
      },
    })
    ...
    expect(ws[1].directory).toBe("workspace:ws_clax_main")
  ...
  test("adding a cloud workspace: appears as sandbox, not new project", () => {
    ...
    const projectItem = fakeProjectItem({
      ...
      sandboxes: ["workspace:ws_cs_dev"],
      workspaces: {
        "workspace:ws_cs_dev": { kind: "cloud", workspace_name: "dev", provider: "daytona" },
      },
    })
    const ws = projectWorkspaces(projectItem, false)
    expect(ws).toHaveLength(2)
    expect(ws[1].name).toBe("dev")
    expect(ws[1].directory).toBe("workspace:ws_cs_dev")
  })
  ...
  test("re-opening a closed project: catalog shows it again with all workspaces", () => {
    const api: Project[] = [
      fakeApiProject({
        ...
        worktree: "/home/user/myapp",
        name: "myapp",
        sandboxes: ["/home/worktrees/proj_reopen/feat", "workspace:ws_reopen_main"],
      }),
    ]
    ...
    const projectItem = fakeProjectItem({
      ...
      sandboxes: ["/home/worktrees/proj_reopen/feat", "workspace:ws_reopen_main"],
      workspaces: {
        "workspace:ws_reopen_main": { kind: "cloud", workspace_name: "main", provider: "daytona" },
      },
    })
    const ws = projectWorkspaces(projectItem, false)
  ...
  test("sandbox name derives from directory basename", () => {
    const project = fakeProjectItem({
      ...
      worktree: "/home/user/Claxedo",
      name: "Claxedo",
      sandboxes: ["workspace:ws_clax_dev"],
      workspaces: {
        "workspace:ws_clax_dev": { kind: "cloud", workspace_name: "dev", provider: "daytona" },
      },
    })

    const ws = projectWorkspaces(project, false)
    const sandbox = ws.find((w) => w.directory === "workspace:ws_clax_dev")
...
describe("data pipeline: workspaces metadata survives API → frontend", () => {
  ...
  // Simulates what projectToProjectItem() does in the app shell.
  function simulateProjectToProjectItem(project: ProjectMetadata): ProjectItem {
    return {
  ...
  function requireProjectMetadata(metadata: ProjectState | undefined): ProjectMetadata {
  ...
  test("step 1: projectCatalog meta preserves workspaces from API response", () => {
    ...
    const apiResponse = [
      fakeApiResponseProject({
        ...
        name: "opencode",
        sandboxes: ["workspace:ws_1"],
        workspaces: {
          "/home/user/opencode": { kind: "local", workspace_name: "main" },
          "workspace:ws_1": { kind: "cloud", workspace_name: "main" },
        },
      }),
    ] as Project[] // cast like the real code does

    const catalog = projectCatalog({ api: apiResponse, current: [], closed: () => false, valid })
    const meta = requireProjectMetadata(catalog.meta.get("/home/user/opencode"))
    // The workspaces field must survive projectCatalog
    expect(meta.workspaces).toBeDefined()
    expect(meta.workspaces!["workspace:ws_1"]).toBeDefined()
    expect(meta.workspaces!["workspace:ws_1"].kind).toBe("cloud")
  })
  ...
  test("step 2: enrich spread preserves workspaces from metadata", () => {
    ...
    expect(enriched.workspaces).toBeDefined()
    expect(enriched.workspaces!["workspace:ws_1"].kind).toBe("cloud")
    // sandboxes must also survive
    expect(enriched.sandboxes).toEqual(["workspace:ws_1"])
  })

  test("step 3: projectToProjectItem extracts workspaces metadata", () => {
    const enriched = {
      id: "proj_1",
      worktree: "/home/user/opencode",
      name: "opencode",
    ...
    expect(item.workspaces!["workspace:ws_1"]).toBeDefined()
    expect(item.workspaces!["workspace:ws_1"].kind).toBe("cloud")
  })
  ...
  test("full pipeline: API response → catalog → enrich → projectItem → display (no duplicates)", () => {
    ...
    const apiResponse = [
      fakeApiResponseProject({
        ...
        workspaces: {
          ...
          "workspace:ws_cloud": {
            id: "ws_cloud", kind: "cloud", workspace_name: "main", directory: "workspace:ws_cloud", provider: "daytona",
          },
          "/Users/user/test/opencode/packages/claxedo-server": {
    ...
    const catalog = projectCatalog({ api: apiResponse, current: [], closed: () => false, valid })
    expect(catalog.list).toHaveLength(1)
    const meta = requireProjectMetadata(catalog.meta.get("/Users/user/test/opencode"))
    expect(meta.workspaces).toBeDefined()

    // Step 3: enrich
    const project = { worktree: "/Users/user/test/opencode", expanded: true }
    const enriched = simulateEnrich(project, meta)
    expect(enriched.workspaces).toBeDefined()
  ...
  test("main worktree isCloud comes from workspace metadata, not global server flag", () => {
    const cloudOnlyProject: ProjectItem = {
      id: "proj_cloud_only",
      worktree: "workspace:ws_cloud_only",
      name: "CloudApp",
      sandboxes: [],
      workspaces: {
        "workspace:ws_cloud_only": { kind: "cloud", workspace_name: "main" },
      },
    }
