...
describe("projectCatalog", () => {
  ...
  // ── BUG: cloud projects appear in local sidebar ──
...
describe("data pipeline: workspaces metadata survives API → frontend", () => {
  ...
  test("full pipeline: without workspaces metadata, cloud sandbox shows as local 'main' (BUG)", () => {
    // This test documents the BROKEN behavior when workspaces metadata is missing.
    // If the API doesn't include workspaces, the fallback is getFilename() which
    // produces duplicate "main" and wrong isCloud=false for cloud sandboxes.
    ...
    const apiResponse = [
      fakeApiResponseProject({
        ...
        // NO workspaces field — simulates broken pipeline
      }),
    ] as Project[]

    const catalog = projectCatalog({ api: apiResponse, current: [], closed: () => false, valid })
    const meta = catalog.meta.get("/Users/user/test/opencode")
    const enriched = simulateEnrich({ worktree: "/Users/user/test/opencode", expanded: true }, meta as Record<string, unknown>)
    const item = simulateProjectToProjectItem(enriched)

    // workspaces is undefined — this is the root cause
    expect(item.workspaces).toBeUndefined()

    const ws = projectWorkspaces(item, false)

    // BUG: duplicate "main" names — cloud sandbox gets getFilename("...main") = "main"
    const names = ws.map((w) => w.name)
    const hasDuplicate = new Set(names).size < names.length
    expect(hasDuplicate).toBe(true) // documents the bug

    // BUG: cloud sandbox shows as local (isCloud=false)
    const cloudSandbox = ws.find((w) => w.directory.includes(".claxedo/cloud"))
    expect(cloudSandbox?.isCloud).toBe(false) // wrong — should be true
  })
  ...
  test("workspace icon: cloud workspace gets isCloud=true even when server is local", () => {
    // BUG: the sidebar uses `isCloud = !server.isLocal()` — a global flag.
    // This means when running locally, ALL workspaces show a laptop icon.
    // The correct behavior: isCloud should be per-workspace from metadata.
    ...
    // Simulate projectWorkspaces with isCloud=false (server.isLocal() = true)
    const ws = projectWorkspaces(item, false)

    // Even though the SERVER is local, the cloud workspace must have isCloud=true
    // because its kind is "cloud" in the workspace metadata.
    const cloudWs = ws.find((w) => w.directory === "/home/cloud/sandbox/main")
    expect(cloudWs).toBeDefined()
    expect(cloudWs!.isCloud).toBe(true) // This is what the test asserts

    // The local workspace must have isCloud=false
  ...
  test("projectWorkspaces never produces duplicate workspace names", () => {
    // Exhaustive: local+cloud with same basename, multiple cloud sandboxes
    const item: ProjectItem = {
      id: "p1",
      worktree: "/a/main",
      sandboxes: ["/b/cloud/main", "/c/cloud/main"],
      workspaces: {
        "/a/main": { kind: "local", workspace_name: "main" },
        "/b/cloud/main": { kind: "cloud", workspace_name: "main" },
        "/c/cloud/main": { kind: "cloud", workspace_name: "main" },
      },
    }
    ...
    // Both cloud sandboxes get "(cloud)" suffix
    expect(ws[1].name).toBe("main (cloud)")
    expect(ws[2].name).toBe("main (cloud)")
    // They're still distinguishable by directory
    expect(ws[1].directory).not.toBe(ws[2].directory)
  })
})
