...
// ── Grouping logic (mirrors createMemo in PageIndex) ──────────────────

function groupPages(pages: Page[], statuses: PageStatus[]) {
  const groups: Array<{ status: PageStatus; pages: Page[] }> = []
  for (const status of statuses) {
    groups.push({
      status,
      pages: pages.filter((p) => p.status === status.id),
    })
  }
  const knownIds = new Set(statuses.map((s) => s.id))
  const orphans = pages.filter((p) => !knownIds.has(p.status))
  if (orphans.length) {
    groups.push({
...
function allowedTransitions(page: Page, statuses: PageStatus[]) {
...
describe("page grouping", () => {
  test("groups pages by their status", () => {
    ...
    const groups = groupPages(pages, STATUSES)
  ...
  test("empty statuses group only produces empty groups", () => {
    const groups = groupPages([], STATUSES)
    expect(groups.length).toBe(5)
  ...
  test("groups preserve status ordering by position", () => {
    ...
    const groups = groupPages([], reversed)
...
describe("allowed transitions", () => {
  test("draft can transition to in_review and in_progress", () => {
    ...
    const allowed = allowedTransitions(page, STATUSES)
  ...
  test("status with no transitions returns empty array", () => {
    ...
    const allowed = allowedTransitions(page, customStatuses)
...
describe("optimistic mutations", () => {
  ...
  test("optimistic transition moves page between groups", () => {
    ...
    const groups = groupPages(after, STATUSES)
    const draft = groups.find((g) => g.status.id === "draft")!
...
describe("reactive grouping (SolidJS signals)", () => {
  test("grouped memo reacts to page signal changes", () => {
    ...
    createRoot((d) => {
      ...
      const grouped = createMemo(() => groupPages(pages(), STATUSES))
  ...
  test("grouped memo reacts to status signal changes", () => {
    ...
    createRoot((d) => {
      dispose = d
      const [statuses, setStatuses] = createSignal<PageStatus[]>(STATUSES)
      const pages: Page[] = [
        makePage("p1", "A", "draft"),
