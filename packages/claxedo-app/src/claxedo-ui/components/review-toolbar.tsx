    ...
    <div class="sticky top-0 shrink-0 px-3 py-1.5 flex items-center gap-2 text-13-medium z-10">
        ...
        <Popover
          ...
          trigger={
            <span class="flex items-center gap-1.5">
              <span>{reviewModeLabel[props.mode]}</span>
              <Show when={props.hasReview}>
                <span class="text-[10px] tabular-nums px-1.5 py-px rounded bg-surface-base text-text-weak">{props.reviewCount}</span>
              </Show>
              <svg class="w-3 h-3 shrink-0 text-text-weak" viewBox="0 0 12 12">
                <path d="M3 5l3 3 3-3" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </span>
          }
          triggerAs="button"
          triggerProps={{
            class: "flex items-center gap-1.5 px-2 py-1 text-12-medium text-text-base hover:bg-surface-base-hover rounded-md transition-[background-color,color,transform] active:scale-[0.96]",
      ...
      <Show when={props.hasReview}>
        <DiffChanges
          ...
          class="flex-shrink-0 tabular-nums opacity-70 [&_[data-slot=diff-changes-additions]]:!text-[11px] [&_[data-slot=diff-changes-deletions]]:!text-[11px] [&_[data-slot=diff-changes-additions]]:!leading-none [&_[data-slot=diff-changes-deletions]]:!leading-none [&]:!gap-1.5"
        />
      </Show>
      <Show when={props.hasReview}>
        <DropdownMenu>
