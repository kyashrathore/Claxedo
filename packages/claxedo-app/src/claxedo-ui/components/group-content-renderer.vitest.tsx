...
describe("GroupContentRenderer", () => {
  test("switching from a draft new tab to a real session tab keeps the draft slot hidden and the real slot visible", async () => {
    const draft = {
      id: "tab-new",
      type: "session",
      title: "New Session",
    ...
    setMultiPaneHasState("tab-new", true)
    setMultiPaneHasState("tab-real", true)

    batch(() => {
      setState("items", [draft, real])
      setState("activeId", draft.id)
    })

    const view = render(() => <GroupContentRenderer groupId="g-default" />)

    await waitFor(() => {
      expect(view.getByTestId("pane-tab-new")).toBeTruthy()
      expect(isSlotHidden(view.getByTestId("pane-tab-new"))).toBe(false)
    })

    setState("activeId", real.id)

    await waitFor(() => {
      expect(view.getByTestId("pane-tab-real")).toBeTruthy()
      expect(isSlotHidden(view.getByTestId("pane-tab-real"))).toBe(false)
      expect(view.getByTestId("pane-tab-new")).toBeTruthy()
      expect(isSlotHidden(view.getByTestId("pane-tab-new"))).toBe(true)
    })
  })
