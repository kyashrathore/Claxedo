...
export type ReviewWorkspaceProps = {
  ...
  tabId?: string
  class?: string
  getLeafView?: (tabId: string) => MultiPaneLeafView[]
}
...
export function ReviewWorkspace(props: ReviewWorkspaceProps) {
  ...
  // VCS diffs go through /api/claxedo/diff/vcs on claxedo-server (no session needed)
  const fetchVcsDiff = async (mode: string, fromRef?: string, toRef?: string, directory = props.directory) => {
  ...
  createEffect(() => {
    ...
    void fetch(`${claxedoServerUrl}/api/claxedo/diff/targets?${targetsParams}`)
      .then((r) => r.json())
      .then((data: any) => {
        if (data?.defaultRef && activeFromRef() === "HEAD~1") setActiveFromRef(data.defaultRef)
      })
      .catch(() => {})
  })
  ...
  const reviewIntent = createReviewIntentAdapter({
    session: {
      diffTargets: async ({ directory }) => {
        const params = new URLSearchParams({ directory })
        const res = await fetch(`${claxedoServerUrl}/api/claxedo/diff/targets?${params}`)
        const data = res.ok ? await res.json() : undefined
        return { data }
      },
      diff: async ({ directory, mode, fromRef, toRef }) => {
        const data = await fetchVcsDiff(mode, fromRef, toRef, directory)
        return { data }
      },
    },
  ...
  const contextTabSessionId = createMemo(() => {
    ...
    const leaves = props.getLeafView?.(props.tabId) ?? []
  ...
  // --- VCS diff fetching with cancellation/deduplication ---
  // Run counter prevents stale responses from overwriting newer data.
  let vcsRun = 0
  let vcsTask: Promise<void> | undefined

  const isVcsMode = (mode: string) =>
    mode !== "session" && mode !== "session-turn"

  const loadVcsDiffs = (force = false) => {
    const mode = untrack(activeMode)
    if (!isVcsMode(mode)) return

    if (!force && !store.loading && store.remoteDiffs.length > 0) {
      // Already loaded and not forcing refresh
      return
    }

    const from = untrack(activeFromRef).trim() || undefined
    const to = untrack(activeToRef).trim() || undefined

    // Bump run counter to invalidate any in-flight request
    const run = ++vcsRun

    setStore("loading", true)
    const task = fetchVcsDiff(mode, from, to)
      .then((diffs) => {
        if (vcsRun !== run) return // stale
  ...
  // Fetch VCS diffs reactively when mode/refs change
  createEffect(() => {
    const currentMode = activeMode()
    if (!isVcsMode(currentMode)) return
    // Track refs so changes trigger re-fetch
    activeFromRef()
    activeToRef()
    loadVcsDiffs(true)
  })

  // Auto-refresh on file changes (ported from upstream session.tsx)
  ...
  // Auto-refresh when session goes idle (agent finished working → files likely changed)
  createEffect(
    on(
      () => sync.data.session_status[props.sessionId]?.type,
      (next, prev) => {
        if (!isVcsMode(untrack(activeMode))) return
        ...
        <Tabs value={store.activeTab} onChange={openFileTab}>
          <div class="sticky top-0 shrink-0 flex">
            <Tabs.List>
              ...
              <div class="sticky right-0 shrink-0">
                ...
                <Show when={hasReview()}>
                  <div class="flex items-center gap-1 flex-shrink-0">
                    <div class="flex items-center gap-0.5 p-0.5 bg-surface-base rounded-md">
                      <Tooltip value={language.t("ui.sessionReview.diffStyle.unified")}>
                        <div class="flex items-center">
                          <IconButton
                            ...
                            aria-label={language.t("ui.sessionReview.diffStyle.unified")}
                          />
                        </div>
                      </Tooltip>
                      <Tooltip value={language.t("ui.sessionReview.diffStyle.split")}>
                        <div class="flex items-center">
                          <IconButton
                            icon="layout-right"
                            size="small"
                            ...
                            aria-label={language.t("ui.sessionReview.diffStyle.split")}
                          />
                        </div>
                      </Tooltip>
                    </div>
                    <Tooltip value={store.openDiffs.length > 0 ? language.t("ui.sessionReview.collapseAll") : language.t("ui.sessionReview.expandAll")}>
                      <IconButton
                        ...
                        aria-label={store.openDiffs.length > 0 ? language.t("ui.sessionReview.collapseAll") : language.t("ui.sessionReview.expandAll")}
                      />
                    </Tooltip>
                  </div>
                </Show>
