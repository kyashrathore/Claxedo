...
type StatusEditorDialogProps = {
  statuses: PageStatus[]
  onSave: (statuses: PageStatus[]) => Promise<void> | void
}

export function updateStatusItem(items: PageStatus[], index: number, patch: Partial<PageStatus>) {
  return items.map((item, i) => (i === index ? { ...item, ...patch } : item))
}

export function addStatusItem(items: PageStatus[], id = `status_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`) {
  return [
    ...items,
    {
      id,
      name: "New Status",
      color: STATUS_PRESET_COLORS[items.length % STATUS_PRESET_COLORS.length],
...
export function removeStatusItem(items: PageStatus[], index: number) {
  const removedId = items[index]?.id
  const next = items.filter((_, i) => i !== index)
  return next.map((item, i) => ({
    ...item,
    position: i,
    transitions: removedId ? item.transitions.filter((id) => id !== removedId) : item.transitions,
  }))
}

export function moveStatusItemUp(items: PageStatus[], index: number) {
...
export function moveStatusItemDown(items: PageStatus[], index: number) {
...
export function toggleStatusTransition(items: PageStatus[], index: number, targetId: string) {
  return items.map((item, i) => {
    ...
    return {
      ...item,
      transitions: has
        ? item.transitions.filter((id) => id !== targetId)
...
export function StatusEditorDialog(props: StatusEditorDialogProps) {
  ...
  const updateItem = (index: number, patch: Partial<PageStatus>) => {
    setItems((prev) => updateStatusItem(prev, index, patch))
  }

  const addStatus = () => {
    setItems((prev) => addStatusItem(prev))
  }

  const removeStatus = (index: number) => {
    setItems((prev) => removeStatusItem(prev, index))
  }

  const moveUp = (index: number) => {
    setItems((prev) => moveStatusItemUp(prev, index))
  }

  const moveDown = (index: number) => {
    setItems((prev) => moveStatusItemDown(prev, index))
  }

  const toggleTransition = (index: number, targetId: string) => {
    setItems((prev) => toggleStatusTransition(prev, index, targetId))
  }
    ...
    <Dialog>
      <div class="flex flex-col gap-4 p-4 max-h-[80vh] overflow-auto min-w-[420px]">
            ...
            {(item, index) => (
              <div class="flex flex-col gap-2 p-3 rounded-md border border-border-weak-base bg-surface-base">
                ...
                <div class="flex items-center gap-2">
                  <input
                    type="text"
                    class="flex-1 min-w-0 px-2 py-1 text-12-regular rounded border border-border-weak-base bg-background-base text-text-base focus:outline-none focus:ring-1 focus:ring-border-interactive-focus"
                    value={item.name}
                  ...
                  <button
                    type="button"
                    class="flex items-center justify-center w-6 h-6 rounded transition-colors text-text-weaker hover:text-text-on-critical-base hover:bg-surface-base-hover"
                  ...
                  <For each={items().filter((s) => s.id !== item.id)}>
                    {(other) => (
                      <label class="flex items-center gap-1 cursor-pointer">
                        <input
                          type="checkbox"
                          class="accent-[var(--surface-interactive-base)]"
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-border-weak-base">
          ...
          <button
            type="button"
            class="px-3 py-1.5 text-12-medium rounded-md bg-surface-interactive-base text-text-on-interactive-base hover:bg-surface-interactive-base/90 transition-colors disabled:opacity-50"
            disabled={saving()}
            onClick={handleSave}
          >
            {saving() ? "Saving..." : "Save"}
