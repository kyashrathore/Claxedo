  ...
  return (
    <Dialog title="Process Diagnostics" size="full">
      <div class="flex h-full min-h-0 flex-col">
        ...
        <Show when={error()}>
          {(err) => (
            <div class="mx-5 mb-3 rounded-md border border-red-500/20 bg-red-500/8 px-3 py-2 text-[12px] text-red-500">
              {err()}
            </div>
          )}
        </Show>
...
function MetricCell(props: {
  ...
  const accent = () => {
    if (props.tone === "ok") return "text-green-600"
    if (props.tone === "warn") return "text-amber-500"
    if (props.tone === "danger") return "text-red-500"
    ...
    <div class="group/row">
      ...
      <div class="flex items-center gap-2 px-4 py-2.5 transition-colors hover:bg-surface-base-hover/30">
        ...
        <Show when={props.group.problem_children > 0}>
          <span class="shrink-0 text-[10px] font-medium text-amber-500">{props.group.problem_children} issues</span>
        </Show>
  ...
  return (
    <div class="flex items-center gap-2 px-3 py-1.5 text-[11px]">
      ...
      <Show when={props.row.mcp_name}>
        {(name) => (
          <span class="shrink-0 rounded bg-cyan-500/10 px-1.5 py-0.5 text-[10px] font-medium text-cyan-600">
            MCP {name()}
          </span>
        )}
      </Show>
