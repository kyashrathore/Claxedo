...
function getLatestAssistantText(store: typeof MOCK_STORE, sessionId: string): string {
  const msgs = (store.message as Record<string, typeof store.message[keyof typeof store.message]>)[sessionId] ?? []
  const msg = msgs.findLast(
    (m: (typeof msgs)[number]) => m.role === "assistant" && typeof (m as any).time?.completed === "number",
  )
  if (!msg) return ""
