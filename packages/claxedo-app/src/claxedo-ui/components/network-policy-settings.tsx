    ...
    >
      ...
      >
        <div class="flex items-center justify-between px-5 py-4 border-b border-border-weak-base shrink-0">
          ...
          <button
            class="text-icon-weak hover:text-icon-base transition-colors p-1 rounded"
            ...
            {([name, hosts]) => (
              <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-wrap gap-1">
                  <For each={hosts}>
                    {(host) => (
                      <span class="text-[11px] font-mono text-text-weaker bg-surface-inset px-1.5 py-0.5 rounded">
                        {host}
                      </span>
                    )}
                  </For>
...
export function NetworkPolicySettings(props: { workspaceId?: string }) {
  ...
  return (
    <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-0.5">
          <For each={policies()}>
            {(entry) => {
              ...
              return (
                <div class="flex items-center justify-between px-2 py-1 rounded bg-surface-inset">
                  <div class="flex items-center gap-2">
                  ...
                  <Show when={!entry.constraints.auto}>
                    <button
                      class="text-icon-weak hover:text-icon-danger transition-colors p-1"
      ...
      <div class="flex items-center gap-2">
        <select
          class="text-12-regular bg-surface-inset border border-border-base rounded px-2 py-1.5 text-text-strong"
          value={newKind()}
        ...
        <Show when={newKind() === "group"}>
          <select
            class="flex-1 text-12-regular bg-surface-inset border border-border-base rounded px-2 py-1.5 text-text-strong"
            value={newTarget()}
