...
export function GenericFlatPaneRenderer(props: { tabId: string; groupId: string; active?: boolean; scrollable?: boolean; minPaneWidth?: number }) {
  ...
  const visibleLeafCount = createMemo(() => leaves().filter((leaf) => !leaf.hidden).length)
  ...
  return (
    <div
      class="relative size-full max-md:flex max-md:flex-col"
      data-scrollable-pane={props.scrollable ? "" : undefined}
      style={props.scrollable ? { "overflow-x": "auto" } : undefined}
    >
      ...
      <div
        class="relative size-full max-md:flex-1 max-md:min-h-0"
        style={props.scrollable ? { width: `${visibleLeafCount() * (props.minPaneWidth ?? 400)}px` } : undefined}
      >
        <For each={leafIds()}>
          {(leafId) => {
            const leaf = createMemo(() => leafMap().get(leafId))
