    ...
    return ({ node, getPos, editor }) => {
      ...
      async function renderDiagram() {
        ...
        try {
          ...
          svgContainer.innerHTML = ""
          errorContainer.style.display = ""
          errorContainer.textContent = e?.message || "Invalid mermaid syntax"
          lastRenderedSource = ""
        }
      }
      ...
      function openFullscreen() {
        ...
        renderMermaid(source)
          ...
          .catch((e) => {
            fsContent.textContent = e?.message || "Render failed"
      ...
      return {
        ...
        stopEvent(event) {
          ...
          if (event.type === "wheel" && target?.closest?.(".mermaid-preview") && ((event as WheelEvent).ctrlKey || (event as WheelEvent).metaKey)) return true
