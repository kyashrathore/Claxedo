 ...
 * - Add mode: POST /api/claxedo/process to create a new config
 * - Edit mode: PUT /api/claxedo/process/:id to update, with Delete button
 *
...
export function AddProcessDialog(props: AddProcessDialogProps) {
  ...
  const processUrl = (path = "") => `${claxedoServerUrl}/api/claxedo/process${path}`
  ...
  const handleSubmit = async (e: SubmitEvent) => {
    ...
    try {
      ...
      if (isEdit() && props.config) {
        const res = await fetchFn(processUrl(`/${props.config.id}`), {
          method: "PUT",
          headers: headers(),
        ...
        const res = await fetchFn(processUrl(), {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(body),
        })
