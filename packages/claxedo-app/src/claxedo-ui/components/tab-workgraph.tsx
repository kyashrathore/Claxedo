...
import { useClaxedoState, realDirectory } from "../state"
...
import {
  ...
  type WorkgraphEvent,
  type WorkgraphItem,
  type WorkgraphItemArtifacts,
  type WorkgraphPreview,
  type WorkgraphRelations,
...
type PickerDeps = {
  prompt: string
  parse: (text: string) => Spec | null
}

let pickerp: Promise<PickerDeps> | undefined

const loadPicker = () =>
  (pickerp ??= Promise.all([
    import("zod"),
    import("@json-render/solid"),
    import("@json-render/core"),
  ]).then(([zod, solid, core]) => {
    const z = zod.z
    const catalog = core.defineCatalog(solid.schema, {
      components: {
    ...
    return {
      prompt: catalog.prompt({ mode: "inline" }),
      parse(text: string) {
        const compiler = core.createSpecStreamCompiler<Spec>()
        let patches = false
        for (const line of text.split("\n")) {
          const next = line.trim()
          if (!next.startsWith("{")) continue
          if (compiler.push(next + "\n").newPatches.length) patches = true
        }
...
function IntakeDock(props: {
  ...
  const question = session.questionRequest
  const permission = session.permissionRequest
...
export function TabWorkgraph(_props: TabWorkgraphProps) {
  ...
  const [picker] = createResource(loadPicker)
  ...
  const [detail, setDetail] = createSignal<WorkgraphRelations>()
  ...
  const [intakeDir, setIntakeDir] = createSignal<string>()
  ...
  const [itemArtifactsData, setItemArtifactsData] = createSignal<WorkgraphItemArtifacts>()
  const [graph, setGraph] = createSignal<Record<string, WorkgraphRelations>>({})
  ...
  const panel = createMemo((): WorkgraphDetail | undefined => {
    const cur = current()
    const rel = detail()
    if (!showSlice() && cur && rel) {
      const arts = itemArtifactsData()
      return {
        item: normalizeItem(cur),
        ...rel,
        artifacts: arts?.artifacts ?? [],
        descendant_artifacts: arts?.descendant_artifacts ?? [],
        synthesis_artifact: arts?.synthesis_artifact ?? null,
      }
    }
    return undefined
  })
  ...
  const dir = createMemo(() => {
    ...
    const tab = focusedContentId ? state.meta.get(focusedContentId) : undefined
    const tabDir = realDirectory(tab?.directory)
    if (tabDir) return tabDir
    const paneId = state.wb.state.focusedPaneId
    const paneWorktree = paneId ? state.workspace.paneWorktree(paneId) : undefined
    const worktree = paneWorktree?.pinned ?? paneWorktree?.default
  ...
  const intakeSystem = createMemo(() => {
    const next = picker()
    if (!next) return INTAKE_SYSTEM_BASE
    return [INTAKE_SYSTEM_BASE, "", next.prompt].join("\n")
  })
  ...
  const tab = (run: { session_id: string | null; directory: string | null }) => {
    ...
    return state.meta.find(
      (meta) => meta.type === "session" && meta.directory === run.directory && meta.sessionId === run.session_id,
    )
  }
  ...
  const attention = (item: WorkgraphItem) => {
    ...
    // attention is now a derived selector; without a store/session we treat it as false.
    return false
  }
  ...
  const refresh = async () => {
    const [page, nextSlices, nextRepos] = await Promise.all([
    ...
    setRepos(nextRepos)
    const id = selected()
    if (id && !page.items.some((item) => item.item_id === id)) {
      setSelected(undefined)
      setDetail(undefined)
      setItemArtifactsData(undefined)
    }
  }
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true)
    try {
      const page = await workgraphApi.items({ ...query(), cursor })
  ...
  const loadDetail = async (id: string) => {
    try {
      const [rel, arts] = await Promise.all([
        workgraphApi.item(id),
        workgraphApi.itemArtifacts(id),
      ])
      batch(() => {
        setDetail(rel)
        setItemArtifactsData(arts)
      })
      setErr(undefined)
    } catch (e) {
      setErr((e as Error).message)
  ...
  createEffect(() => {
    ...
    const id = selected()
    if (!rel || !id) return
    setGraph((prev) => ({ ...prev, [id]: rel }))
  })

  // Removed: eager per-item detail fetch for all visible items.
  // graph() is now populated only via loadDetail (on selection / 4s poll for selected item).
  // note() and depth() already have fallbacks using WorkgraphItem counts when detail is absent.
  ...
  const openLive = async (
    ...
    if (!dir) return
    if (run.session_id) {
      state.layout.openSession(dir, run.session_id, "WorkGraph Session")
      return
    }
    if (run.pty_id) {
      state.layout.openTerminal(dir, run.pty_id, "WorkGraph Terminal")
      return
    }
    ...
    if (next) {
      state.layout.openSession(dir, next.id, next.title || "WorkGraph Session")
      return
    }
    showToast({
  ...
  const tag = async (id: string) => {
    await authFetch(`${getClaxedoServerUrl()}/api/claxedo/session/${id}/meta`, {
      method: "PUT",
  ...
  const spawn = async () => {
    ...
    if (!dir) throw new Error("Failed to resolve intake session directory")
    await tag(id)
  ...
  const handoff = async (text?: string) => {
    ...
    try {
      const dir = dockDir()
      if (!dir) throw new Error("No workspace available to continue from WorkGraph")
      ...
      state.layout.openSession(dir, id, "Session")
    ...
    <div class="relative flex size-full min-h-0 flex-col overflow-hidden bg-background-base text-text-base [&_button]:cursor-pointer">
            ...
            <div style="margin-top: 40px; margin-bottom: 16px;">
              <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex shrink-0 items-center gap-2 pt-1">
                  <select
                    value={repoRef()}
                    class="h-7 max-w-56 rounded-md border border-border-weak-base bg-surface-base/50 px-2.5 text-12 text-text-base outline-none focus:border-border-strong-base focus:bg-surface-base"
                    onChange={(e) => {
                  ...
                  <input
                    ...
                    class="h-7 w-40 rounded-md border border-border-weak-base bg-surface-base/50 px-2.5 text-12 text-text-base outline-none placeholder:text-text-weaker focus:border-border-strong-base focus:bg-surface-base"
                  />
                </div>
              </div>
                  ...
                  <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <Show when={item().bindings.length > 1}>
                      <select
                        ...
                        class="h-7 max-w-64 rounded-md border border-border-weak-base bg-background-base px-2.5 text-12 text-text-base outline-none focus:border-border-strong-base"
                        onChange={(e) =>
                          setRepoDir((prev) => ({
                            ...prev,
              ...
              <div>
                ...
                <Show when={nextCursor()}>
                  <div class="mt-4 flex justify-center pb-4">
                    <button
                      type="button"
                      class="rounded-md border border-border-weak-base px-4 py-1.5 text-12 text-text-weaker transition-colors hover:bg-surface-base hover:text-text-base disabled:opacity-50"
                  ...
                  <div class="mb-1 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <div class="mb-1.5 flex items-center gap-2">
                        <div class="h-2 w-2 shrink-0 rounded-full bg-surface-info-strong" />
                            ...
                            <div class="flex flex-col gap-4 p-4 pb-10">
                              ...
                              <Show when={!isMission() && (info().item.session_id || info().item.pty_id || info().item.worktree_path || info().item.directory)}>
                                <div class="flex items-center justify-between gap-3 rounded-lg border border-border-weak-base bg-background-base/50 px-3 py-2.5">
                                  <div class="flex items-center gap-2">
                                    <div class="h-1.5 w-1.5 rounded-full bg-surface-success-strong" />
      ...
      >
        <div class="pointer-events-auto flex w-full max-w-3xl flex-col items-center gap-3">
            ...
            fallback={
              <div class="w-full rounded-[14px] border border-border-base bg-background-stronger px-4 py-5 text-center text-12 text-text-weaker shadow-lg shadow-black/15">
                <div class="flex flex-col items-center gap-3">
                  <div>{err() ? "WorkGraph intake could not start." : "Preparing WorkGraph intake..."}</div>
                  <Show when={err()}>
                    <Button
                      size="small"
                      variant="secondary"
          ...
          >
            <DirectoryScope directory={intakeDir()!}>
              <IntakeDock
                id={intakeId()!}
                system={intakeSystem()}
