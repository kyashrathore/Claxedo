...
export function PageArenaDock(props: PageArenaDockProps) {
  ...
  const [agents, setAgents] = createSignal<DraftAgent[]>([...defaultAgents])
  ...
  const connect = () => {
    stream?.close()
    const url = pagesApi.arenaEventsUrl(props.pageId, props.directory)
    stream = new EventSource(url)
    stream.onmessage = async (event) => {
      try {
        const payload = JSON.parse(event.data) as { type?: string; state?: ArenaState; message?: ArenaMessage }
        if (payload.state) {
        ...
        if (payload.type && payload.type.startsWith("arena.")) {
          await refresh(false)
        }
      } catch {
