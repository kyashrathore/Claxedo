...
describe("D. split.resize", () => {
  test("resize updates the split node size at the given path", () => {
    ...
    expect((h.state().split.root as any).size).toBe(0.7)
  })
  ...
  test("resize clamps to 0..1", () => {
    ...
    expect((h.state().split.root as any).size).toBeLessThanOrEqual(1)
    h.api.split.resize([], -0.2)
    expect((h.state().split.root as any).size).toBeGreaterThanOrEqual(0)
  })
