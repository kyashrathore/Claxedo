...
function dispatchDragEvent(
  ...
  if (init.clientX !== undefined) ev.clientX = init.clientX
  if (init.clientY !== undefined) ev.clientY = init.clientY
...
describe("H. drag & drop", () => {
  ...
  test("drop with right-edge content adds a horizontal split", () => {
    ...
    const targetPane = h.utils.queryByTestId(`pane-${targetId}`)! as HTMLElement
  ...
  test("drop over rendered content body routes to its pane", () => {
    ...
    h.api().contents.add("b")
    h.api().navigation.show("a")
    const slot = h.utils.container.querySelector('[data-workbench-content="a"]')! as HTMLElement
  ...
  test("drop on pane middle chooses the nearest split edge", () => {
    ...
    const p1El = h.utils.queryByTestId(`pane-${p1}`)! as HTMLElement
