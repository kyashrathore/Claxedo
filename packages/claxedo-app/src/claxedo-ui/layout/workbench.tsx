...
export function Workbench(props: WorkbenchProps): JSX.Element {
  ...
  const isVisibleContent = (contentId: string) => {
    const s = ctx.getState()
    return s.panes.some((p) => p.contentId === contentId)
  }
  const paneOfContent = (contentId: string) => {
    const s = ctx.getState()
    return s.panes.find((p) => p.contentId === contentId)?.id ?? null
  }

  const aliveForRender = () => {
    const ids = ctx.getState().contentIds
    if (mountPolicy() === "always") return ids
  ...
  return (
    ...
    >
      <Show
        ...
        fallback={<div data-testid="empty">{props.renderEmpty?.()}</div>}
      >
        {/* Pane chrome layers — rendered absolutely positioned */}
        <For each={ctx.getState().panes}>
          {(pane) => {
            const ctxIsFocused = () => ctx.getState().focusedPaneId === pane.id
            const renderEmptyForPane = () => (
              <div data-testid="empty">{props.renderEmpty?.()}</div>
            )
            return (
              <div
        ...
        <For each={aliveForRender()}>
          {(contentId) => {
            ...
            const slotStyle = (): JSX.CSSProperties => {
              const pid = paneId()
              if (!pid) {
                // Stashed — keep mounted but hidden.
                return { display: "none" }
              }
        ...
        {/* Floating pane controls; no reserved header chrome. */}
        <For each={ctx.getState().panes}>
          {(pane) => (
            <Show when={pane.contentId}>
              {(cid) => (
                ...
                >
                  <div
                    class="group/pane-close absolute right-0 top-0 flex size-12 items-start justify-end p-2"
                    style={{ "pointer-events": "auto", "user-select": "none" }}
                  >
                    <button
                      type="button"
                      class="flex size-6 items-center justify-center bg-background-base/80 text-text-base/85 opacity-0 shadow-[0_2px_8px_rgba(0,0,0,0.24)] transition-[opacity,background-color,color] duration-100 hover:bg-surface-base-hover hover:text-text-base group-hover/pane-close:opacity-100 focus:opacity-100"
