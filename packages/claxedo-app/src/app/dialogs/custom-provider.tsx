...
export function DialogCustomProvider(props: Props) {
  ...
  // Custom providers are OpenCode catalog entries, so the id-collision check
  ...
  const saveMutation = useMutation(() => ({
    mutationFn: async (draft: CustomProviderDraft) => {
      // The credential first: a config the picker can reach but nothing can
      // authenticate against is the worse half to leave behind.
      if (draft.key) {
        await claxedoCredentialRequest(undefined, {
          method: "PUT",
          body: JSON.stringify({
            provider_id: draft.config.providerID,
            kind: "api_key",
            source: "managed",
            label: draft.config.name,
            secret: draft.key,
          }),
        })
      }
      await saveCustomProviderConfig({ config: draft.config })
      return draft
    },
    onSuccess: (draft) => {
      // Every machine that cached an OpenCode catalog must re-ask: the entry is
      // stored centrally, not on whichever host answered last.
      void queryClient.invalidateQueries({
        predicate: (query) => query.queryKey[2] === "providers" && query.queryKey[4] === "opencode",
      })
      dialog.close()
      showToast({
        variant: "success",
        icon: "circle-check",
        title: language.t("provider.connect.toast.connected.title", { provider: draft.config.name }),
        description: language.t("provider.connect.toast.connected.description", { provider: draft.config.name }),
      })
    },
    onError: (err) => {
