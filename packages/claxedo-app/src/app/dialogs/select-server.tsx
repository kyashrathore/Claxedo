...
import { useMutation, useQueries } from "@tanstack/solid-query"
import { showToast } from "@opencode-ai/ui/toast"
import { useNavigate } from "@solidjs/router"
import { createEffect, createMemo, createResource, Show } from "solid-js"
import { createStore } from "solid-js/store"
import { ServerHealthIndicator, ServerRow } from "@/app/connection/server-row"
import { useLanguage } from "@/platform/i18n/provider"
import { usePlatform } from "@/platform/runtime/platform-provider"
import { normalizeServerUrl, ServerConnection, useServer } from "@/app/connection/server"
import { type ServerHealth, serverHealthQueryOptions, useCheckServerHealth } from "@/app/connection/server-health"
...
export function DialogSelectServer() {
  ...
  const healthStatus = createMemo(() => {
    const results: Record<ServerConnection.Key, ServerHealth | undefined> = {}
    items().forEach((conn, index) => {
      results[ServerConnection.key(conn)] = healthQueries[index]?.data
    })
    return results
  })
  ...
  const sortedItems = createMemo(() => {
    ...
    return list.slice().sort((a, b) => {
      ...
      const diff = rank(healthStatus()[ServerConnection.key(a)]) - rank(healthStatus()[ServerConnection.key(b)])
  ...
  async function select(conn: ServerConnection.Any, persist?: boolean) {
    if (!persist && healthStatus()[ServerConnection.key(conn)]?.healthy === false) return
    dialog.close()
  ...
  const startEdit = (conn: ServerConnection.Http) => {
    ...
    setStore("editServer", {
      ...
      error: "",
      status: healthStatus()[ServerConnection.key(conn)]?.healthy,
    })
  }
              ...
              return (
                <div class="flex items-center gap-3 min-w-0 flex-1 w-full group/item">
                  <div class="flex flex-col h-full items-start w-5">
                    <ServerHealthIndicator health={healthStatus()[key]} />
                  </div>
                  <ServerRow
                    conn={i}
                    dimmed={healthStatus()[key]?.healthy === false}
