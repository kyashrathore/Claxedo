/**
 * Streaming connection lifecycle for each event stream target
 * (`app/integrations/claxedo-events.tsx`). The alphabet matches the real
 * flow: a fetch attempt (`connecting`), an open stream with a heartbeat armed
 * (`live`), a scheduled retry after timeout/error (`reconnect-scheduled`),
 * and teardown (`stopped`).
 *
 ...
 * reconciles its timers to the machine state cannot hold both.
 */
