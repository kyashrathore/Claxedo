...
export function OnboardingEmptyState(props: {
  ...
  // Funnel emission is telemetry, never a source: `funnel()` and `surface()`
  // are read in the untracked effect phase so they cannot re-fire the events
  // they are used to send.
  createEffect(
    () => ({ steps: setup().steps, visible: visible() }),
    ({ steps, visible: isVisible }) => {
      if (isVisible && !emitted.has("setup_form_shown")) {
