...
export function useRailHeaderSurfaces(input: {
  ...
  // O(1) keyed-selection replacement (see packages/ui/src/hooks).
  const isHeaderContentActive = createKeySelector<string>(() => input.state.wb.selectors.focusedContent() ?? undefined)
  ...
  const switcherItems = createMemo(
    mapArray(headerContentIds, (contentId): SwitcherItem => {
      const meta = () => input.state.meta.get(contentId)
      const sessionId = () => {
        const current = meta()
      ...
      createEffect(
        ...
        (id) => {
          if (!id) return
          return subscribeSessionActivity(id, () => setActivityRevision((value) => value + 1))
        },
      )
      const sessionStatus = () => {
        activityRevision()
      ...
      // Edge-detector: unseen-done latches on an active -> inactive transition and
      // clears once the surface is focused. Solid 2 has no `createComputed`, so
      // this is the two-phase split: the compute tracks both inputs and returns a
      // fresh tuple (so the apply phase runs on every invalidation, as the old
      // computed did), and the apply phase's `previous` carries what the manual
      // `previousSessionActive` used to hold — undefined on the first run.
      createEffect(
        () => [sessionActive(), isHeaderContentActive(contentId)] as const,
        ([active, focused], previous) => {
          setSessionUnseenDone((current) =>
            nextUnseenDone({
              active,
              previousActive: previous?.[0],
              focused,
              current,
            }),
          )
      ...
      const status = (): SwitcherStatus => {
        ...
        return surfaceStatusForMeta({
          meta: current,
          terminalAgentStatus: current?.terminalId ? input.state.terminal.agentStatus(current.terminalId) : undefined,
