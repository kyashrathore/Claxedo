          ...
          {(_) => {
            ...
            const tabsEnriched = iife(() => {
              const base = mapArray(
                () => tabsStore,
                (tab) => {
                  const sessionsQuery = useQuery(() => directorySessionCacheQuery(tab.dir))
                  return () => {
                    const session = sessionsQuery.data?.session.find((session) => session.id === tab.sessionId)
                    return session ? { ...tab, info: session } : null
                  }
                },
              )

              return () => base().flatMap((session) => {
                const value = session()
                return value ? [value] : []
              })
            })
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Icon size="small" name={creating() ? "new-session-active" : "new-session"} />
                          </Button>
                        </TooltipKeybind>
                      </div>
                    </div>
