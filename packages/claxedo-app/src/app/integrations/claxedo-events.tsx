...
import { createContext, createTrackedEffect, onCleanup, useContext, type ParentProps } from "solid-js"
...
export function useClaxedoEventsOptional() {
  return useContext(ClaxedoEventsContext) ?? undefined
}
...
export function ClaxedoEventsProvider(
  ...
  // `createTrackedEffect` owns the initial connection as well as subsequent
  // route/server changes. Calling `reconcileTargets()` synchronously here used
  // to start the stream a little earlier, but it also let `connectTarget()`
  // publish stream lifecycle state while the provider's component scope was
  // still being created. Solid 2 correctly rejects that owned-scope write.
  // The tracked effect runs after render, where the stream is an imperative
  // side effect and its lifecycle projection may be updated safely.
  createTrackedEffect(reconcileTargets)
