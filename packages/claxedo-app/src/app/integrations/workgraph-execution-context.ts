...
export function workGraphExecutionContext(
  ...
  const hosted = new Set(["cloud", "user-hosted"]).has(workspace?.kind ?? "") || current.startsWith("workspace:")
  const repositoryUrl = workspace?.git_remote ?? workspace?.repo_url ?? project?.git?.remote ?? undefined
  if (hosted) return repositoryUrl ? { kind: "hosted_workspace", repositoryUrl } : undefined
  return current.startsWith("/") ? { kind: "local_worktree", directory: current } : undefined
}
