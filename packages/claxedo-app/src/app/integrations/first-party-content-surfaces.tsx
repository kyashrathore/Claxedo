...
import { optionalFeatureFlags, type OptionalFeatureFlags } from "./optional-feature-flags"
...
const PageContent = lazy(() => import("./documents-content-surfaces").then((m) => ({ default: m.PageContent })))
...
const PagesIndexContent = lazy(() =>
  import("./documents-content-surfaces").then((m) => ({ default: m.PagesIndexContent })),
)
const MarketplaceContent = lazy(() =>
  import("@/features/extensions/marketplace").then((m) => ({ default: m.MarketplaceContent })),
)
const WorkGraphSurface = lazy(() =>
  import("./workgraph-content-surfaces").then((m) => ({ default: m.WorkGraphSurface })),
)
const WorkspaceWorkGraphSurface = lazy(() =>
  import("./workgraph-content-surfaces").then((m) => ({ default: m.WorkspaceWorkGraphSurface })),
)
const TaskComposerSurface = lazy(() =>
  import("./workgraph-content-surfaces").then((m) => ({ default: m.TaskComposerSurface })),
)
...
const availableFirstPartyContentSurfaces: ContentSurfaceContribution[] = [
  {
  ...
  },
  {
    ...
    renderer: (context) => (
      <Suspense fallback={<SurfaceFallback />}>
        <WorkspaceWorkGraphSurface context={context} />
      </Suspense>
    ),
  },
  {
    ...
    renderer: (context) => (
      <Suspense fallback={<SurfaceFallback />}>
        <TaskComposerSurface context={context} />
      </Suspense>
    ),
  },
]

export function createFirstPartyContentSurfaces(flags: OptionalFeatureFlags = optionalFeatureFlags()) {
  return availableFirstPartyContentSurfaces.filter((surface) => {
    if (surface.surface === "page" || surface.surface === "pages-index") return flags.documents
    if (
      surface.surface === "workgraph" ||
      surface.surface === "workspace-workgraph" ||
      surface.surface === "task-composer"
    )
...
export const firstPartyContentSurfaces = createFirstPartyContentSurfaces()
