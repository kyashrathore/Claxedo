...
describe("shouldStoreOpenedProject (persisting the open-project intent)", () => {
  ...
  test("an entry stored before the catalog confirms it still does not RENDER", () => {
    // Storing the intent must not create a phantom row: projectCatalog remains
    // the render gate, so an unconfirmed directory stays stored and unlisted.
    const catalog = projectCatalog({
      api: [],
    ...
    expect(healed.list).toEqual([{ worktree: "/Users/me/formlink", expanded: true }])
  })
})
