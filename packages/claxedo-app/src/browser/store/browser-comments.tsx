...
export const { use: useBrowserComments, provider: BrowserCommentsProvider } = createSimpleContext({
  ...
  init: (): BrowserCommentsState => {
    ...
    const list: BrowserCommentsState["list"] = (tabId) =>
      store.items.filter((e) => e.tabId === tabId).sort((a, b) => a.createdAt - b.createdAt)

    const pending: BrowserCommentsState["pending"] = (sessionId) =>
      store.items.filter((e) => e.sessionId === sessionId).sort((a, b) => a.createdAt - b.createdAt)
