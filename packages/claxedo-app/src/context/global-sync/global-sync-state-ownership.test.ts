...
describe("global sync state ownership", () => {
  test("keeps request dedupe and session load metadata in Query", async () => {
    ...
    expect(source).not.toContain("@claxedo/shell/data/session-load")
    expect(bootstrapSource).toContain("shell/data/session-load")
    expect(source).not.toContain("@/context/global-sync/session-load")
    expect(source).toContain("@claxedo/shell/data/global-sync-types")
    expect(source).not.toContain("@/context/global-sync/types")
