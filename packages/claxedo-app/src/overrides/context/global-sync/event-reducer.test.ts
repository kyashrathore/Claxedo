...
const recentMessage = (id: string, sessionId: string) =>
  ({
    ...msg(id, sessionId),
...
describe("claxedo cleanupDroppedSessionCaches", () => {
  ...
  test("keeps a non-idle session even before open-session registration catches up", () => {
    const live = {
      message: recentMessage("msg-ses_busy", "ses_busy"),
      part: part("part-ses_busy", "ses_busy", "msg-ses_busy"),
    }
    const [store, setStore] = state({
      message: { ses_busy: [live.message] },
      part: { [live.message.id]: [live.part] },
      session_status: { ses_busy: { type: "busy" } as State["session_status"][string] },
    })

    cleanupDroppedSessionCaches(store, setStore, [root("ses_keep")], undefined, "/tmp/ws")

    expect(store.message.ses_busy).toEqual([live.message])
    expect(store.part[live.message.id]).toEqual([live.part])
...
describe("claxedo applyDirectoryEvent", () => {
  test("session.error releases optimistic busy status", () => {
    const [store, setStore] = state({
      session_status: { ses_error: { type: "busy" } as State["session_status"][string] },
    })
    ...
    expect(store.session_status.ses_error).toEqual({ type: "idle" })
  })

  test("session.idle releases optimistic busy status", () => {
    const [store, setStore] = state({
      session_status: { ses_idle: { type: "busy" } as State["session_status"][string] },
    })
    ...
    expect(store.session_status.ses_idle).toEqual({ type: "idle" })
  })
