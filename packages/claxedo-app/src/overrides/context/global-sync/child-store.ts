...
import { canDisposeDirectory, pickDirectoriesToEvict } from "@/context/global-sync/eviction"
  ...
  function ensureChild(directory: string) {
    ...
    if (!children[directory]) {
      ...
      const session = runWithOwner(input.owner, () =>
        persisted(
          Persist.workspace(directory, "session-cache", ["session-cache.v1"]),
          createStore({
            value: {
              at: 0,
              limit: 0,
              total: 0,
              session: [] as SessionCacheValue["session"],
            },
          }),
        ),
      )
      ...
      const init = () =>
        createRoot((dispose) => {
          ...
          createEffect(() => {
            const cached = session[0].value
            if (!session[3]()) return
            if (!cached.session.length) return
            child[1]("session", cached.session)
            child[1]("sessionTotal", cached.total)
          })
        })
