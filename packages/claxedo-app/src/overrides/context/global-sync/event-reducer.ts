...
function recentlyActive(store: Store<State>, sessionId: string) {
  const messages = store.message[sessionId]
  const lastCreated = messages?.at(-1)?.time?.created
  if (!lastCreated) return false
  return Date.now() - lastCreated < SESSION_ACTIVITY_GRACE_MS
}
...
export function cleanupDroppedSessionCaches(
  ...
  const stale = all.filter((sessionId) => {
    ...
    if (hasOpenSession(directory, sessionId)) return false
    const status = store.session_status[sessionId]
    if (status && status.type !== "idle" && recentlyActive(store, sessionId)) return false
    return true
  })
...
export function applyDirectoryEvent(input: {
  ...
  switch (input.event.type) {
    ...
    case "session.idle": {
      ...
      if (props.sessionID) input.setStore("session_status", props.sessionID, { type: "idle" })
      break
    }
    case "session.error": {
      const props = input.event.properties as { sessionID?: string }
