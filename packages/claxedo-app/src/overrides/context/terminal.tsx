...
export function createTerminalSession(sdk: ReturnType<typeof useSDK>, dir: string, options?: TerminalSessionOptions) {
  ...
  tlog("createTerminalSession", { dir, sdkUrl: sdk.url, scopedUrl: url, serverScoped: SERVER_SCOPED_PERSIST })
  ...
  createEffect(() => {
    tlog("store snapshot", {
      ...
      ready: ready(),
      active: store.active,
      ids: store.all.map((item) => item.id),
      pties: store.all.map((item) => ({
        id: item.id,
        title: item.title,
        cwd: item.cwd,
  ...
  return {
    ...
    new(initialCommand?: string, title?: string, previousPtyId?: string): Promise<string | undefined> {
      ...
      return fetch(`${claxedoBase}/api/claxedo/pty`, {
        ...
        .then((pty) => {
          ...
          debug.log("created", { id, title: terminalTitle, cwd: pty.cwd, initialCommand })
          setStore("all", (all) => {
    ...
    async close(id: string) {
      ...
      await fetch(`${claxedoBase}/api/claxedo/pty/${id}`, { method: "DELETE" }).catch((e) => {
    ...
    trim(id: string) {
      ...
      // the keyed <Show> in terminal-panel to remount <Terminal> on every connect,
  ...
  init: () => {
    ...
    const load = (dir: string) => {
      ...
      tlog("workspace load", {
        dir,
        sdkUrl: sdk.url,
        scopedUrl: scope(sdk.url),
        key,
        cacheHit: cache.has(key),
      })
      const existing = cache.get(key)
      if (existing) {
        tvlog("workspace load existing", { dir, key, ids: existing.value.all().map((item) => item.id) })
        return existing.value
      }
      const entry = acquireTerminalSession(key, sdk, dir, terminalOptions)
      cache.set(key, entry)
      tlog("workspace load new", { dir, key, ids: entry.value.all().map((item) => item.id) })
      return entry.value
    }
    ...
    createEffect(() => {
      ...
      workspace = next
      lastWorkspace = next
      tlog("workspace effect", {
        sdkDir: sdk.directory,
        key: SERVER_SCOPED_PERSIST ? `${scope(sdk.url)}:${base64Encode(sdk.directory)}:${WORKSPACE_KEY}` : `${base64Encode(sdk.directory)}:${WORKSPACE_KEY}`,
        ids: next.all().map((item) => item.id),
        active: next.active(),
      })
    })
