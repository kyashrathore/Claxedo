...
function createGlobalSync() {
  ...
  async function reloadWorkspaceGroups(filter?: SessionFilter) {
    try {
      ...
      console.error("Failed to reload workspace groups", err)
    }
  }
  ...
  async function loadMoreForWorkspace(directory: string, filter?: SessionFilter) {
    ...
    try {
      ...
      console.error("Failed to load more sessions for workspace", err)
    } finally {
  ...
  async function reloadProjects() {
    const result = await globalSDK.client.project.list()
  ...
  async function ensureProject(directory: string) {
    ...
    if (!dir) return
    const url = new URL("/api/workspace/resolve", globalSDK.url)
    url.searchParams.set("directory", dir)
    url.searchParams.set("create", "true")
    const res = await (platform.fetch ?? globalThis.fetch)(url, {
      headers: { Accept: "application/json" },
    })
    if (!res.ok) throw new Error(await res.text().catch(() => `Failed to ensure workspace: ${res.status}`))
