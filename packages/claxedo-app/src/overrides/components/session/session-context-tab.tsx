/**
 * Override of SessionContextTab that resolves session params from
 * SessionParamsProvider (multi-pane) with fallback to useParams() (route).
 */
...
export function SessionContextTab() {
  ...
  const sessionKey = createMemo(() => {
    const dir = directory() ?? routeParams.dir ?? ""
    const id = sessionId()
    return `${dir}${id ? "/" + id : ""}`
  })
