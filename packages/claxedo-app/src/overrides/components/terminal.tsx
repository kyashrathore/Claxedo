...
export const Terminal = (props: TerminalProps) => {
  ...
  const debug = createDebugLogger("terminal.ui", "terminal:ui", {
    legacyKey: "opencode.debug.terminal",
  })
  const isDebug = () => debug.enabled(1)
  const isVerbose = () => debug.enabled(2)
  ...
  const trace = (...args: unknown[]) => {
    if (!isDebug()) return
    tlog("trace", { mountId, ptyId: safePty()?.id ?? ptySnapshot?.id ?? "unknown" }, ...args)
  }
  const traceVerbose = (...args: unknown[]) => {
    if (!isVerbose()) return
    tvlog("trace", { mountId, ptyId: safePty()?.id ?? ptySnapshot?.id ?? "unknown" }, ...args)
  }
