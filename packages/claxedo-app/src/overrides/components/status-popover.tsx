...
export function StatusPopover(props: StatusPopoverProps) {
  ...
  // Helper to update child sync store
  const setSync = (key: string, value: unknown) => {
    const dir = directory()
    if (!dir) return
    const [_, setStore] = globalSync.child(dir)
    ;(setStore as any)(key, value)
  }
  ...
  const toggleMcp = async (name: string) => {
    const dir = directory()
    if (store.loading || !dir) return
    setStore("loading", name)

    try {
      // Create directory-specific client
      ...
      const status = syncStore()?.mcp[name]
