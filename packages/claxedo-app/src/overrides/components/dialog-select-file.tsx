  ...
  const open = (path: string) => {
    if (claxedo) {
      ...
      if (groupId) {
        ...
        const groupWorktree = claxedo.groupWorktree(groupId)
        const dir = groupTabs.active()?.directory ?? groupWorktree.default() ?? projectDirectory()
        if (dir) {
          const tabId = groupTabs.addFile(dir, path, getFilename(path))
          if (tabId) groupTabs.setActive(tabId)
