...
import { createEffect, on, Component, Show, onCleanup, Switch, Match, createMemo, createResource, createSignal } from "solid-js"
...
import { createTextFragment, getCursorPosition, setCursorPosition, setRangeEdge } from "@/components/prompt-input/editor-dom"
...
import { useAcpConfig } from "@claxedo/claxedo-ui/context/acp-config"
import { AgentRunnerSelector } from "@claxedo/claxedo-ui/components/agent-runner-selector"
import { resolveSessionId } from "./prompt-input/resolve-session-id"
import { createPanePreferences, panePreferenceScope } from "../../pane/store/pane-preferences"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  let _sessionParams: ReturnType<typeof useSessionParams> | undefined
  try {
    _sessionParams = useSessionParams()
  } catch {
    /* not in split mode */
  }
  ...
  const resolvedSessionId = () => resolveSessionId(props.sessionID, _sessionParams?.sessionId(), params.id)
  const resolvedSessionDirectory = createMemo(() => props.sessionDirectory ?? _sessionParams?.directory?.())
  const resolvedDraftId = createMemo(() => props.draftId ?? _sessionParams?.surfaceId?.())
  const scope = createMemo(() => panePreferenceScope({
    directory: resolvedSessionDirectory() ?? sdk.directory,
    sessionId: resolvedSessionId(),
    surfaceId: _sessionParams?.surfaceId?.(),
    draftId: resolvedDraftId(),
  }))
  const acpPending = createMemo(() => acpConfig.isAcpMode(scope()) && acpConfig.readiness(scope()) === "polling")
  const fade = createMemo(() => acpPending() ? 0.45 : 1)
  const panePreferences = createMemo(() => createPanePreferences(localStorage))
  const sessionKey = createMemo(() => {
    const sessionID = resolvedSessionId()
    if ((!sessionID || sessionID === "new") && resolvedDraftId()) {
      return `draft:${resolvedDraftId()!}`
    }
    const dir = resolvedSessionDirectory() ? base64Encode(resolvedSessionDirectory()!) : params.dir
    return `${dir ?? ""}${sessionID ? "/" + sessionID : ""}`
  })
  const tabs = createMemo(() => layout.tabs(sessionKey))
  const view = createMemo(() => layout.view(sessionKey))
  const commandDirectory = createMemo(() => resolvedSessionDirectory() ?? sdk.directory)
  const [customCommands] = createResource(
    commandDirectory,
    async (directory) =>
  ...
  const working = createMemo(() => {
    ...
    if (!id) return status()?.type === "busy" || status()?.type === "retry"
    return isSessionTurnActive({
      status: status(),
      permissions: sync.data.permission[id],
      questions: sync.data.question[id],
    })
  })
  const [boot, setBoot] = createSignal<{ runner: string; sessionID?: string }>()
  const booting = createMemo(() => !!boot())
  const busy = createMemo(() => booting() || working())
  const bootText = createMemo(() => {
    const item = boot()
    if (!item) return ""
    return `Booting ${item.runner}...`
  })
  ...
  // Cursor-like collapsed/expanded: single-line when unfocused, expands on focus
  const [inputExpanded, setInputExpanded] = createSignal(false)
  let promptRootRef: HTMLDivElement | undefined

  const handleRootFocusIn = () => setInputExpanded(true)

  // Collapse only when clicking outside the prompt root AND outside portal overlays
  createEffect(() => {
    const handler = (e: MouseEvent) => {
      if (!inputExpanded()) return
      const target = e.target as HTMLElement
      if (!target) return
      if (promptRootRef?.contains(target)) return
      // Don't collapse when interacting with portals (dropdowns, dialogs, popovers, menus)
      if (target.closest('[role="dialog"], [role="listbox"], [role="menu"], [data-dismissable-layer], [data-kb-popper-content-wrapper]')) return
  ...
  const collapsed = () => !!props.collapsible && !inputExpanded() && !busy()
  ...
  createEffect(
    on(
      () => acpConfig.isAcpMode(scope()),
      (next, prev) => {
        if (next || !prev) return
        const current = local.agent.current()?.name
        if (!current || agentNames().includes(current)) return
        local.agent.set(agentNames()[0])
      },
      { defer: true },
    ),
  )
  ...
  return (
    <div ref={(el) => (promptRootRef = el)} class="relative size-full _max-h-[320px] flex flex-col gap-0" onFocusIn={handleRootFocusIn}>
      <PromptPopover
      ...
      >
        ...
        >
          <div
            classList={{
              "relative no-scrollbar": true,
              "max-h-[240px] overflow-y-auto": !collapsed(),
              "max-h-11 overflow-hidden": collapsed(),
            }}
            ref={(el) => (scrollRef = el)}
          >
            <div
              ...
              aria-disabled={acpPending()}
              contenteditable={acpPending() ? "false" : "true"}
              autocapitalize="off"
              autocorrect="off"
              spellcheck={false}
              onInput={handleInput}
              ...
              classList={{
                "select-text w-full text-14-regular text-text-strong focus:outline-none": true,
                "pl-3 pr-2 pt-2 pb-11 whitespace-pre-wrap": !collapsed(),
                "pl-3 pr-20 py-2.5 truncate": collapsed(),
            ...
            <Show when={!prompt.dirty()}>
              <div
                classList={{
                  ...
                  "pl-3 pr-2 pt-2 pb-11": !collapsed(),
                  "pl-3 pr-20 py-2.5": collapsed(),
                  "font-mono!": store.mode === "shell",
                }}
              >
                {placeholder()}
          ...
          <div
            classList={{
              "pointer-events-none absolute flex items-center gap-2": true,
              "bottom-2 right-2": !collapsed(),
              "right-2 top-1/2 -translate-y-1/2": collapsed(),
            }}
          >
            <input
              ref={fileInputRef}
            ...
            >
              ...
              >
                <Button
                  ...
                  onClick={pick}
                  disabled={store.mode !== "normal" || acpPending()}
                  tabIndex={store.mode === "normal" && !acpPending() ? undefined : -1}
              ...
              >
                <div class="pointer-events-auto flex items-center gap-2">
                  ...
                  <IconButton
                    ...
                    type="submit"
                    disabled={store.mode !== "normal" || acpPending() || booting() || (!prompt.dirty() && !working() && commentCount() === 0)}
                    tabIndex={store.mode === "normal" && !acpPending() ? undefined : -1}
                    icon={working() ? "stop" : booting() ? "dot-grid" : "arrow-up"}
                    variant="primary"
                    class="size-8"
                    style={{
                      opacity: buttonsSpring(),
                    ...
                    aria-label={booting() ? bootText() : working() ? language.t("prompt.action.stop") : language.t("prompt.action.send")}
                  />
                </div>
              </Tooltip>
            </div>
      ...
      <Show when={!collapsed() && (store.mode === "normal" || store.mode === "shell")}>
        <DockTray attach="top">
            ...
            <div class="flex items-center gap-1.5 min-w-0 flex-1 relative">
              ...
              <div class="flex items-center gap-1.5 min-w-0 flex-1">
                <AgentRunnerSelector
                  ...
                  sessionLocked={resolvedSessionId() !== undefined && resolvedSessionId() !== "new"}
                />
                <Show when={!acpConfig.isAcpMode(scope())}>
                  <TooltipKeybind
                    placement="top"
                    gutter={4}
                  ...
                  >
                    <Select
                      ...
                      classList={{
                        "capitalize max-w-[160px]": true,
                        "text-text-base": !acpPending(),
                        "text-text-weak": acpPending(),
                      }}
                      valueClass={acpPending() ? "truncate text-13-regular text-text-weak" : "truncate text-13-regular text-text-base"}
                      triggerStyle={{
                        height: "28px",
                ...
                <Show when={!acpConfig.isAcpMode(scope())}>
                  ...
                  >
                    <Select
                      ...
                      onSelect={(x) => setScopedVariant(x === "default" ? undefined : x)}
