...
import { fetchSessionMessagesByTransport } from "../../session/store/session-transport"
import {
  displayName,
  errorMessage,
...
export default function Layout(props: ParentProps) {
  ...
  onMount(() => {
    ...
    const unsub = globalSDK.event.listen((e) => {
      ...
      const session = store.session.find((s: Session) => s.id === props.sessionID)
    ...
    createEffect(() => {
      ...
      const childSessions = store.session.filter((s: Session) => s.parentID === currentSession)
  ...
  async function archiveSession(session: Session) {
    ...
    const index = sessions.findIndex((s: Session) => s.id === session.id)
  ...
  async function chooseProject() {
    ...
    // Use extension-provided web project dialog if available (cloud mode)
    if (platform.platform === "web" && ext.app.webProjectDialog) {
                  ...
                  fallback={
                    <>
                      ...
                      <div class="flex-1 min-h-0">
                        <LocalWorkspace ctx={workspaceSidebarCtx} project={p()} mobile={panelProps.mobile} sortNow={() => Date.now()} />
                      </div>
                    </>
                  }
                >
        ...
        >
          <div class="@container w-full h-full contain-strict">
            <SidebarContent
              ...
              renderProject={(project) => <SortableProject ctx={projectSidebarCtx} project={project} sortNow={() => Date.now()} />}
        ...
        <div class="xl:hidden">
          ...
          >
            <SidebarContent
              ...
              aimMove={aim.move}
              projects={() => layout.projects.list()}
              renderProject={(project) => <SortableProject ctx={projectSidebarCtx} project={project} mobile sortNow={() => Date.now()} />}
              handleDragStart={handleDragStart}
              handleDragEnd={handleDragEnd}
              handleDragOver={handleDragOver}
              openProjectLabel={language.t("command.project.open")}
