/**
 * Override: use-session-commands.tsx
 *
 * Claxedo override that changes the fileTree.toggle keybind from mod+\ to
 * mod+shift+e. In Claxedo, mod+\ is used for split view toggle instead.
 *
 * This is a copy of the upstream file with a single keybind change on
 * fileTree.toggle (search for "CLAXEDO OVERRIDE" comment).
 */
  ...
  const permissionCommands = createMemo(() => [
    permissionsCommand({
      ...
      onSelect: () => {
        ...
        const nowAutoAccepting = permission.isAutoAccepting(sessionID, sdk.directory)
