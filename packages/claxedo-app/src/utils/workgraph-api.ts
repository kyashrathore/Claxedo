...
function detail(input: WorkgraphDetail): WorkgraphDetail {
  return {
    ...input,
    item: {
      ...input.item,
      labels: input.item?.labels ?? [],
      slices: input.item?.slices ?? [],
      parent_id: input.item?.parent_id ?? null,
      repo_ref: input.item?.repo_ref ?? null,
      repo_label: input.item?.repo_label ?? null,
      node_type: input.item?.node_type ?? "task",
    },
...
function htmlErr() {
  return "WorkGraph API resolved to app HTML. Set VITE_OPENCODE_BACKEND_URL=http://localhost:4096."
}
...
export const workgraphApi = {
  ...
  items(input: WorkgraphItemsQuery = {}) {
    return request<WorkgraphItem[]>(`${base()}/graph/items${query(input)}`)
  },
