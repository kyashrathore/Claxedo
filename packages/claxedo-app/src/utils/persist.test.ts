...
// Shadow `./api` with the real `isDemoMode`
// behavior (read from window.location.pathname). Several other test
// files in this suite mock the same module with `isDemoMode: () => false`,
// and bun's module cache lets that leak in here — silently breaking
// the demo-mode persisted-storage assertion. Defining our own mock
// LAST guarantees the demo-mode-aware impl is what persist.ts sees.
mock.module("./api", () => ({
  isDemoMode: () => {
  ...
  isEmbedMode: () => false,
  getConfiguredClaxedoServerUrl: () => undefined,
