...
const ptyWs = ws.link(/wss?:\/\/[^/]+\/pty\/([^/]+)\/connect/)
...
export async function createHandlers() {
  ...
  return [
    ...
    http.get(`${DEMO_BASE}/pty`, () => {
      return HttpResponse.json(state.ptys)
    }),
    http.get(`${DEMO_BASE}/pty/:id`, ({ params }) => {
    ...
    http.post(`${DEMO_BASE}/pty`, async ({ request }) => {
      const body = (await request.json().catch(() => ({}))) as Record<string, unknown>
    ...
    http.patch(`${DEMO_BASE}/pty/:id`, async ({ params, request }) => {
      const pty = state.ptys.find((item) => item.id === params.id)
      if (!pty) return HttpResponse.json(null, { status: 404 })
      Object.assign(pty, (await request.json().catch(() => ({}))) as Record<string, unknown>)
      return HttpResponse.json(pty)
    }),
    http.delete(`${DEMO_BASE}/pty/:id`, ({ params }) => {
      const id = params.id as string
    ...
    http.get(`${DEMO_BASE}/hook/terminal-session`, ({ request }) => {
    ...
    http.get(`${DEMO_BASE}/hook/tab-context`, ({ request }) => {
    ...
    http.post(`${DEMO_BASE}/hook/tab-context`, async ({ request }) => {
      const body = (await request.json().catch(() => ({}))) as Record<string, unknown>
    ...
    http.get(`${DEMO_BASE}/api/pages`, () => {
      return HttpResponse.json(state.pages)
    }),
    http.post(`${DEMO_BASE}/api/pages`, async ({ request }) => {
      const body = (await request.json().catch(() => ({}))) as Record<string, unknown>
      const page: DemoPage = {
        id: pageID(),
    ...
    http.delete(`${DEMO_BASE}/api/pages/:id`, ({ params }) => {
    ...
    http.post(`${DEMO_BASE}/api/pages/:id/arena/start`, () => {
      return HttpResponse.json(state.arenaState)
    }),
    http.post(`${DEMO_BASE}/api/pages/:id/arena/message`, () => {
      return HttpResponse.json({ ok: true, wave_id: "wave_demo", state: state.arenaState })
    }),
    http.post(`${DEMO_BASE}/api/pages/:id/arena/control`, () => {
      return HttpResponse.json({ ok: true, state: state.arenaState })
    }),
    http.get(`${DEMO_BASE}/api/pages/:id/arena/events`, () => {
      return sse(
        heartbeat({
