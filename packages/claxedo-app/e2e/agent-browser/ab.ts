...
export async function scroll(direction: "up" | "down" | "left" | "right", px = 500): Promise<string> {
  return ab("scroll", direction, String(px))
}
...
/**
 * Open the workspace "more menu" (chevron-down dropdown) and select a menu item
 * using keyboard navigation (ArrowDown + Enter).
 *
 * Raw `.click()` via evaluate doesn't trigger Kobalte's onSelect handler,
 * so keyboard events are used instead — they also auto-close the menu.
 *
 * Menu order: New Terminal, New Page, [custom commands...], Configure...
 *
 * @param itemText  Visible text of the menu item (e.g. "New Terminal")
 * @param scopeSelector  Optional CSS scope to target a specific panel's trigger
 */
export async function clickMoreMenuItem(
  itemText: string,
  scopeSelector?: string,
): Promise<void> {
  const trigger = scopeSelector
    ? `${scopeSelector} [data-component="workspace-more-menu"]`
    : `[data-component="workspace-more-menu"]`
  await click(trigger)
  await settle(300)

  // Known menu item order (before custom commands)
  const menuOrder = ["New Terminal", "New Page"]
  const idx = menuOrder.indexOf(itemText)
  // ArrowDown from trigger focuses first item, then navigate to target
  const presses = idx >= 0 ? idx + 1 : 1
