...
// (`claxedo-server/src/server.ts:550`) and defined in
// `claxedo-server/src/routes/agent-config.ts:18` → `agentConfigHarnessRoutes(options)`.
// The handlers live in `claxedo-server/src/routes/agent-config-harness-routes.ts`:
//
...
//       (`claxedo-server/src/routes/agent-config-harness.ts:53`), which is the function
...
import type { harnessFromRequest } from "../../../../claxedo-server/src/routes/agent-config-harness"
  ...
  constructor(method: "POST" | "GET", url: string, problems: string[]) {
    super(
      `${method} ${url} violated the real server's harness-config contract `
        + `(claxedo-server/src/routes/agent-config-harness-routes.ts):\n  - ${problems.join("\n  - ")}\n`
 ...
 * (`claxedo-server/src/routes/http.ts:3-11`). The two failure codes this route can
 * produce:
 */
