...
test.describe("unified usage dashboard @core @surface-web", () => {
  test("opens from the account menu, reconciles cards, queries controls, and restores focus", async ({ page }) => {
    ...
    expect(requests.every((request) => request.searchParams.get("view") === "quota")).toBe(true)
    ...
    expect(requests.some((request) => request.searchParams.get("view") === "total")).toBe(true)
  ...
  test("uses the full viewport on a narrow mobile surface", async ({ page }) => {
    ...
    await expect.poll(async () => (await dialog.boundingBox())?.width ?? 0).toBeGreaterThanOrEqual(380)
    await expect.poll(async () => (await dialog.boundingBox())?.height ?? 0).toBeGreaterThanOrEqual(830)
    await expect(dialog.getByRole("button", { name: "Usage through Claxedo" })).toBeVisible()
