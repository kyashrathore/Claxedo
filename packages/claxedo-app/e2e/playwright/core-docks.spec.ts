...
async function installDockMutationRoutes(page: Page, mock: MockRuntimeHandles): Promise<DockRequestCounters> {
  ...
  await page.route("**/question/*/reply", async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true }) })
    // Model the real server (and mirror the permissions route above): emit the
    // settling SSE event. The dock clears optimistically on mutation success, but
    // the mock replays its full log on every SSE reconnect — without a logged
    // question.replied the replayed question.asked revives the dock a beat later.
