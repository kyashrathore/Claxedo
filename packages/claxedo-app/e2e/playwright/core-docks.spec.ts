...
test.describe("core docks — permission @core", () => {
  test("an approved Codex permission stays absent after switching sessions and replaying runtime events", async ({ page }, testInfo) => {
    test.fixme(true, "an approved Codex permission reappears when the runtime stream replays after a session switch")
...
test.describe("core docks — question wizard @core", () => {
  test("Codex questions answered while away stay absent on rail return", async ({ page }, testInfo) => {
    test.fixme(true, "a resolved Codex question repaints from cached state and runtime replay when returning through the rail")
...
test.describe("core docks — todo tray @core", () => {
  test("todo dock preserves all five completed steps after the turn ends and reloads", async ({
    ...
    test.fixme(true, "the completed todo list disappears when the turn ends and stays absent after reload")
    const completedBatches: string[] = []
    page.on("response", async response => {
      if (!response.headers()["content-type"]?.includes("text/event-stream")) return
      const body = await response.text().catch(() => "")
      if (body.includes('"content":"Finish task","status":"completed"')) completedBatches.push(body)
    })
    const { mock } = await establishSession(page)
    ...
    await expect.poll(() => completedBatches.length, { timeout: 20_000 }).toBeGreaterThan(0)
    await writeFile(testInfo.outputPath("completed-todo-events.txt"), completedBatches.join("\n"))
    await page.reload()
    await expectAssistantReplyVisible(page, "ack 1: core docks establishing turn")
    ...
    await expect(list.locator('[data-component="checkbox"]')).toHaveText([
