...
/** URL-safe base64 encode (matches @opencode-ai/core/util/encode) */
...
const PROCESS_TERMINAL_PROMPT = "~/e2e-process-test $ "
...
async function seedApp(page: Page) {
  await page.addInitScript(
    (args: { directory: string; slug: string; prompt: string }) => {
      localStorage.clear()
      ...
      const FakeWebSocket = new Proxy(OriginalWebSocket, {
        construct(_target, wsArgs: [string | URL, (string | string[])?]) {
          ...
          if (!url.includes("/api/wr/pty/") && !url.includes("/api/claxedo/pty/")) {
          ...
          setTimeout(() => {
            ...
            const message = new MessageEvent("message", { data: `\x1b[35m${args.prompt}\x1b[0m` })
    ...
    { directory: TEST_DIR, slug: TEST_DIR_SLUG, prompt: PROCESS_TERMINAL_PROMPT },
  )
}
...
async function showProcessTab(page: Page) {
  const add = addProcessButton(page)
  if (await add.isVisible().catch(() => false)) return
  if (await page.locator('[data-testid="process-pane-panel"]').isVisible().catch(() => false)) return
  ...
  if (!await page.locator('[data-testid="workspace-panel-shell"][data-open="true"]').isVisible().catch(() => false)) {
  ...
  if (await page.locator('[data-testid="process-pane-panel"]').isVisible().catch(() => false)) return
  await clickVisibleButton(page, "Open Processes")
...
async function showSessionTab(page: Page) {
  ...
  await expect(addProcessButton(page)).toBeHidden({ timeout: 10_000 })
}

function addProcessButton(page: Page) {
  return page.getByRole("button", { name: "Add process" }).first()
}

async function openProcessPanel(page: Page, name: string) {
  const panel = page.locator(`[data-testid="process-pane-panel"][data-process-name="${name}"]`)
  if (await panel.isVisible().catch(() => false)) return

  if (!await page.locator('[data-testid="workspace-navigator-overlay"][data-navigator="processes"][data-open="true"]').isVisible().catch(() => false)) {
    await clickVisibleButton(page, "Open Processes")
  }
  const overlay = page.locator('[data-testid="workspace-navigator-overlay"][data-navigator="processes"][data-open="true"]')
  await expect(overlay).toBeVisible({ timeout: 10_000 })
  await overlay.getByRole("button", { name: new RegExp(name) }).click()
  await expect(page.locator(`[data-testid="process-pane-panel"][data-process-name="${name}"]`)).toBeVisible({ timeout: 5_000 })
}
...
async function terminalPaintSummary(page: Page) {
  ...
  return {
    visible: box.width > 320 && box.height > 80,
    width: box.width,
    height: box.height,
    chromaPixels,
  }
...
type MockProcessConfigInput = {
  name: string
  command: string
  cwd?: string
  env?: Record<string, string>
  autoStart?: boolean
  restartPolicy?: MockProcessConfig["restartPolicy"]
  maxRestarts?: number
  color?: string
}

let mockConfigs: MockProcessConfig[] = []
let mockProcesses: MockManagedProcess[] = []
...
function createMockConfig(body: MockProcessConfigInput) {
  const config = {
    id: `cfg-${nextConfigId++}`,
    name: body.name,
...
async function setupAPIMocks(page: Page) {
  ...
  // /experimental/session is the new sessions-list endpoint (renamed from /session)
  await page.route(/\/experimental\/session(?:\?.*)?$/, async (route: Route) => {
    if (!isAPICall(route)) { await route.continue(); return }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  // Bootstrap endpoint that the global-sync provider hits on app load
  await page.route("**/api/claxedo/bootstrap", async (route: Route) => {
    if (!isAPICall(route)) { await route.continue(); return }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        healthy: true,
        version: "1.0.0-test",
        path: { state: "", config: "", worktree: TEST_DIR, directory: TEST_DIR, home: "" },
        project: [{ id: "proj_process_e2e", name: "process-e2e", worktree: TEST_DIR, sandboxes: [] }],
        provider: {
          all: [{ id: "opencode", name: "OpenCode", models: { "test-model": { id: "test-model", name: "Test Model", providerID: "opencode" } } }],
          default: { opencode: "test-model" },
          connected: [],
        },
        provider_auth: {},
        config: { provider: { id: "opencode", model: "test-model" }, agent: { id: "build" } },
      }),
    })
  })
  ...
  await page.route("**/project", async (route: Route) => {
    ...
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify([{ id: "proj_process_e2e", name: "process-e2e", worktree: TEST_DIR, sandboxes: [] }]),
    })
  })

  await page.route("**/project/current", async (route: Route) => {
    if (!isAPICall(route)) { await route.continue(); return }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  // Workspace resolve endpoint — returns the workspaceId for a directory
  await page.route("**/api/workspace/resolve**", async (route: Route) => {
    if (!isAPICall(route)) { await route.continue(); return }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        workspaceId: "ws-e2e-mock",
        projectId: "proj_process_e2e",
        directory: TEST_DIR,
        kind: "local",
        access: "local",
        status: "ready",
  ...
  await page.route(/\/process\/?(?:\?.*)?$/, async (route: Route) => {
    ...
    if (route.request().method() === "GET") {
      ...
      const body = route.request().postDataJSON() as MockProcessConfigInput
  ...
  await page.route(/\/api\/(?:wr|claxedo)\/pty\/[^/]+(?:\?.*)?$/, async (route: Route) => {
    if (!isAPICall(route)) { await route.continue(); return }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  await page.route(/\/process\/[^/?]+(?:\?.*)?$/, async (route: Route) => {
    ...
    const match = new URL(url).pathname.match(/\/process\/([^/]+)$/)
    const configId = match?.[1]
    ...
    if (method === "PUT") {
      const body = route.request().postDataJSON() as Partial<MockProcessConfigInput>
      const idx = mockConfigs.findIndex((c) => c.id === configId)
      if (idx === -1) {
        await route.fulfill({ status: 404, contentType: "application/json", body: JSON.stringify({ error: "Not found" }) })
        return
      }
...
test.describe.skip("Process Management", () => {
  ...
  test("toggle pane shows empty state and hides on re-toggle", async ({ page }) => {
    await expect(addProcessButton(page)).toBeHidden()

    await showProcessTab(page)
    await expect(page.getByText("No processes configured")).toBeVisible()
    await expect(addProcessButton(page)).toBeVisible()
  ...
  test("add process via dialog", async ({ page }) => {
    await showProcessTab(page)
    await addProcessButton(page).click()
  ...
  test("start and stop all buttons visible when configs exist", async ({ page }) => {
    ...
    await showProcessTab(page)
    await expect(page.getByRole("button", { name: "Start all processes" })).toBeVisible()
    await page.getByRole("button", { name: "Start all processes" }).click()
    await expect(page.getByRole("button", { name: "Stop all processes" })).toBeVisible({ timeout: 5_000 })
  })

  test("full happy flow: create, start, close, reopen", async ({ page }) => {
    await showProcessTab(page)
    await expect(page.getByText("No processes configured")).toBeVisible({ timeout: 10_000 })
    ...
    await openProcessPanel(page, "dev-server")
    ...
    await expect(page.locator('[data-testid="process-pane-panel"][data-process-name="dev-server"] .xterm')).toBeVisible({
      timeout: 15_000,
    })
    await expect.poll(async () => (await terminalPaintSummary(page)).visible, { timeout: 15_000 }).toBe(true)
    await expect.poll(async () => (await terminalPaintSummary(page)).chromaPixels, { timeout: 15_000 }).toBeGreaterThan(25)
  ...
  test("edit process dialog shows pre-filled values", async ({ page }) => {
    ...
    await showProcessTab(page)
    await openProcessPanel(page, "build-watcher")
  ...
  test("delete process with confirmation", async ({ page }) => {
    ...
    await showProcessTab(page)
    await openProcessPanel(page, "to-delete")
    await page.getByRole("button", { name: "Edit process" }).first().click()
    const dialog = page.getByRole("dialog")
    await expect(dialog).toBeVisible()
