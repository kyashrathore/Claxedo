...
test.describe("live real-harness smoke @live", () => {
  ...
  for (const harness of ["claude", "codex"] as const) {
    test(`${harness} live unsupported fork leaves history unchanged`, async ({ page }) => {
      const dir = await makeWorkspace(`unsupported-fork-${harness}`)
      await seedOneProject(page, dir)
      const input = await openDraftPrompt(page, dir)
      await switchDraftHarness(page, harness === "claude" ? /^Claude$/ : /^Codex$/, 0)
      await waitForHarnessReady(page)
  ...
  for (const harness of ["claude", "codex"] as const) {
    test(`${harness} live session rename preserves history and harness across server restart`, async ({ page }) => {
      const dir = await makeWorkspace(`session-rename-${harness}`)
      await seedOneProject(page, dir)
      const input = await openDraftPrompt(page, dir)
      await switchDraftHarness(page, harness === "claude" ? /^Claude$/ : /^Codex$/, 0)
      await waitForHarnessReady(page)
      ...
      await expectSessionRenamePersistence(page, {
        ...
        restartServer: async () => { await stopServer(); await startServer(dataDir) },
      })
      await expectAssistantReplyVisible(page, marker)
  ...
  test("live source control stages, unstages and commits only the selected workspace file", async ({ page }) => {
    ...
    const git = async (args: string[], cwd = dir) => (await execFileAsync("git", args, { cwd })).stdout.trim()
    await git(["config", "user.name", "Claxedo E2E"])
    await git(["config", "user.email", "e2e@test.com"])
    await git(["config", "commit.gpgsign", "false"])
    const contents = "Source-control live audit: selected file\n"
    await fs.writeFile(path.join(dir, "README.md"), contents)
    await fs.writeFile(path.join(dir, "leave-untracked.txt"), "Must remain outside the commit\n")
    await fs.writeFile(path.join(neighbor, "README.md"), "Neighbor workspace must remain untouched\n")
    const neighborHead = await git(["rev-parse", "HEAD"], neighbor)
    const neighborDiff = await git(["diff"], neighbor)
    await seedOneProject(page, dir)
    await openDraftPrompt(page, dir)
    await page.locator('[data-testid="workbench-shell-header"] [data-testid="workspace-panel-toggle"]').click()
    const panel = page.getByTestId("workspace-panel-shell")
    await expect(panel).toHaveAttribute("data-open", "true")
    await panel.getByRole("button", { name: "Open Changes", exact: true }).click()
    const view = panel.getByTestId("source-control-view")
    await expect(view).toBeVisible()
    const row = view.locator('[data-testid="source-control-row"][data-path="README.md"]')
    await expect(row).toHaveAttribute("data-group", "changes")
    const act = async (action: "stage" | "unstage") => {
      await row.hover()
      await row.locator(`[data-action="${action}"]`).click()
    }
    await act("stage")
    await expect(row).toHaveAttribute("data-group", "staged")
    await expect.poll(() => git(["diff", "--cached", "--name-only"])).toBe("README.md")
    await act("unstage")
    await expect(row).toHaveAttribute("data-group", "changes")
    await expect.poll(() => git(["diff", "--cached", "--name-only"])).toBe("")
    expect(await fs.readFile(path.join(dir, "README.md"), "utf8")).toBe(contents)
    await act("stage")
    await expect(row).toHaveAttribute("data-group", "staged")
    const message = view.getByTestId("source-control-message")
    const commit = view.getByTestId("source-control-commit")
    await expect(commit).toBeDisabled()
    const subject = "test: commit one file through the live app"
    const fullMessage = `${subject}\n\nPreserve the untracked file and neighboring workspace.`
    await message.fill(fullMessage)
    await commit.click()
    await expect.poll(() => git(["log", "-1", "--format=%s"])).toBe(subject)
    expect(await git(["log", "-1", "--format=%B"])).toBe(fullMessage)
    expect(await git(["show", "HEAD:README.md"])).toBe(contents.trim())
    expect(await git(["diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD"])).toBe("README.md")
    expect(await git(["status", "--porcelain", "--", "README.md"])).toBe("")
    expect(await git(["status", "--porcelain", "--", "leave-untracked.txt"])).toBe("?? leave-untracked.txt")
    expect(await git(["rev-parse", "HEAD"], neighbor)).toBe(neighborHead)
    expect(await git(["diff"], neighbor)).toBe(neighborDiff)
    await expect(message).toHaveValue("")
    await expect(view.getByTestId("source-control-group-staged")).toHaveAttribute("data-count", "0")
    await page.reload()
    await expect(page.getByTestId("source-control-view")).toBeVisible()
    await expect(page.locator('[data-testid="source-control-row"][data-path="README.md"]')).toHaveCount(0)
    await expect(page.locator('[data-testid="source-control-row"][data-path="leave-untracked.txt"]')).toHaveAttribute("data-group", "changes")
    const graph = page.getByTestId("source-control-group-graph")
    if (await graph.getAttribute("data-collapsed") === "true") await graph.getByRole("button", { expanded: false }).click()
    const head = page.locator('[data-testid="source-control-graph"] [data-testid="source-control-commit-row"]').first()
    await expect(head).toHaveAttribute("data-hash", await git(["rev-parse", "HEAD"]))
    await expect(head).toContainText(subject)
    await page.screenshot({ path: test.info().outputPath("source-control-after-commit-reload.png") })
    await test.info().attach("git-readback", {
      body: JSON.stringify({ commit: await git(["rev-parse", "HEAD"]), message: await git(["log", "-1", "--format=%B"]), status: await git(["status", "--porcelain"]), neighborHead }),
