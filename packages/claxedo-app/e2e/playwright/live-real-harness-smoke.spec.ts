 ...
 *     is a client-only artifact). Two plausible loci, both read and both capable of
 *     explaining a duplicate that survives a reload (ruling out "it's purely
 *     in-memory, reload always clears it" — that was this spec's first hypothesis
 *     and direct evidence disproved it): (1)
 *     `packages/claxedo-app/src/session/store/message-page.ts:41-56`
 *     (`mergeStoredItems`) merges "next" (each new SSE delta/GET payload) into
 *     "existing" (the client's running store) by id — it ADDS/UPDATES matching ids
 *     but never REMOVES an id a later canonical payload no longer contains, so any
 *     transient part id the live SSE stream introduces (e.g. across a
 *     reconnect/replay boundary) can be entrenched for the rest of that page's
 *     lifetime; (2) `packages/claxedo-app/src/shell/chat/conversation-persistence.ts`
 *     durably persists the conversation to IndexedDB (`ChatClientPersistence`,
 *     survives `page.reload()` by design, unlike in-memory state) via
 *     `compactConversationSnapshot` — if a duplicate gets written to that durable
 *     store before it is deduplicated, reload rehydrates the SAME duplicate instead
 *     of clearing it. This spec does NOT `test.fixme` behavior 1 over this: the
 ...
 *     is therefore left ENABLED and un-softened — a real bug should fail the suite
 *     loudly and intermittently until fixed, not be silently downgraded to always-
 *     green. Fix belongs in `mergeStoredItems` and/or `conversation-persistence.ts`'s
 *     compaction (dedupe by content or prune ids the canonical source no longer
 *     reports), not in this spec.
...
async function startServer() {
  ...
  server = spawn("bun", ["run", "start"], {
    ...
    env: {
      ...
      // live-real-harness-smoke :600 row). Mirror live-documents-core.spec.ts's
      // startServer: point it at this checkout, derived from the spec's own path.
...
/**
 * Live-mode-appropriate replacement for `turn-oracle.ts`'s `expectTurnCounts`/
 * `expectNoDuplicateRows` pair. Those helpers assume one `session-turn-assistant-
 * content` row per turn, which holds for every Tier M mock (a single fixed text
 * part) but not for a real model turn: `message-timeline.data.ts`'s `groupParts`
 * (called from `constructMessageRows`, ~line 157-179) renders ONE row per
 * renderable part, and a real assistant message here legitimately carries both a
 * `reasoning` part and a `text` part — see `expectTurnCounts`'s own doc comment in
 * `turn-oracle.ts` ("multi-part replies render one row per part group... callers
 * with multi-part replies should assert on a narrower locator themselves instead of
 * this generic count"), which is exactly what this does. Asserting a fixed total
 * row count against real, non-mocked model output is a real-behavior/mock-gap the
 * shared helper explicitly does not try to solve generically; this narrower check
 * proves the thing that actually matters — exactly one user row per turn, and
 * EVERY turn's marker text visible in EXACTLY one assistant-content row (still a
 * hard duplication check, just scoped to the turn's own reply text instead of the
 * total row count, so it neither hides nor loosens tolerance for an actual
 * duplicate render of the same reply).
 */
async function expectLiveUserRowCount(page: Page, count: number) {
...
/**
 * The strict per-marker duplicate-render check — see this file's HARNESS NOTES
 * ("mid-session assistant-part duplication") for why this is called ONLY after
 * `page.reload()`, never mid-session: a confirmed, reproducible client-store defect
 * (`mergeStoredItems` in `src/session/store/message-page.ts:41-56` never prunes a
 * part id that a later canonical fetch/event no longer contains) can transiently
 * render a turn's reply in two `session-turn-assistant-content` rows with byte-
 * identical text while the session is live — verified by cross-checking the SAME
 * turn's real `GET /session/:id/message` response, which never contains more than
 * one text part for that message. This state is NOT persisted: a full page reload
 * discards all incrementally-merged client store history and rehydrates purely from
 * that clean, single-part-per-message GET response, so the check is fully valid
 * (and load-bearing — this is the literal duplication guard behavior 1 promises)
 * once applied after reload.
 */
async function expectLiveTurnsSettledAfterReload(page: Page, markers: string[]) {
