...
const baseURL = workspaceCaptureUrl({
  override: Bun.env.CLAXEDO_WORKSPACE_PANEL_MOTION_URL,
...
try {
  ...
  const videoPath = await closeContextAndSaveVideo(context, page, path.join(RESULT_DIR, "workspace-panel-interrupt-motion.webm"))
