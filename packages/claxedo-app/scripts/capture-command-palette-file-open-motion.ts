...
const baseURL = workspaceCaptureUrl({
  override: Bun.env.CLAXEDO_COMMAND_PALETTE_FILE_OPEN_URL,
...
try {
  ...
  const videoPath = await closeContextAndSaveVideo(context, page, path.join(RESULT_DIR, "command-palette-file-open.webm"))
