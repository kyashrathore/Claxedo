...
// Wrap entire bundle in async IIFE (CJS, no top-level await)
// and prepend Bun shim before bundle (bundler hoists `import "bun"` above our code)
const bunShim = `
// --- Bun shim for browser runtime ---
;(function() {
  var fs = require("fs");
...
// Replace bun: prefixed modules with plain names for browser runtime compatibility
bundle = bundle.replace(/require\("bun:(\w+)"\)/g, 'require("bun_$1")')

// Replace import.meta.require with require (browser runtime doesn't have import.meta.require)
...
// Polyfill AsyncLocalStorage inline (browser runtime stubs async_hooks as no-op)
