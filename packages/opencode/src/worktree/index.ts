...
export interface Interface {
  ...
  readonly createFromInfo: (
    info: Info,
    options?: { startCommand?: string; baseRef?: string },
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createFromInfo = Effect.fn("Worktree.createFromInfo")(function* (
      ...
      yield* setup(info, options?.baseRef)
      yield* boot(info, options?.startCommand).pipe(
    ...
    const create = Effect.fn("Worktree.create")(function* (input?: CreateInput) {
      ...
      yield* createFromInfo(info, { startCommand: input?.startCommand, baseRef: input?.baseRef })
      return info
    })
