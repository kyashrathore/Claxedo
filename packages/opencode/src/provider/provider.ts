...
/** Anthropic's public API, and the default this provider pins itself to. */
export const ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com/v1"

/**
 * Opt-in: point THIS engine's anthropic traffic somewhere else (a proxy, a
 * gateway). Deliberately its own name rather than `ANTHROPIC_BASE_URL` — see
 * `anthropicBaseURL`.
 */
export const ANTHROPIC_BASE_URL_OPT_IN_ENV = "OPENCODE_ANTHROPIC_BASE_URL"

/**
 * Where anthropic requests go.
 *
 * The catalog carries no `api` url for anthropic, so `options.baseURL` was left
 * undefined — and `@ai-sdk/anthropic` fills an absent `baseURL` from the
 * ambient `ANTHROPIC_BASE_URL` env var. Any shell that exports it for a CLI
 * (pointing `claude` at a local proxy is a normal thing to do) silently
 * rerouted the engine too. Observed: a user's engine request arriving at their
 * proxy on 127.0.0.1, which rejected the engine's credential and surfaced as
 * "Couldn't reach Anthropic — Not Found".
 *
 * So the engine pins its own default instead of inheriting one. Precedence:
 *   1. explicit config (`provider.anthropic.options.baseURL`) — a user
 *      configuring THIS provider means it;
 *   2. `OPENCODE_ANTHROPIC_BASE_URL` — opt in by naming the engine, so
 *      redirecting it is a decision rather than a side effect;
 *   3. Anthropic's public API.
 *
 * `ANTHROPIC_BASE_URL` is deliberately NOT consulted. Nothing scrubs it from
 * `process.env`: spawned harnesses (the claude CLI) inherit it through
 * `harnessSpawnEnv` on purpose, and that passthrough is the behaviour users
 * want. This narrows only what the in-process provider reads.
 */
function anthropicBaseURL(configured: unknown, envs: Record<string, string | undefined>) {
  if (typeof configured === "string" && configured !== "") return configured
  const optIn = envs[ANTHROPIC_BASE_URL_OPT_IN_ENV]?.trim()
  return optIn || ANTHROPIC_DEFAULT_BASE_URL
}

/**
 * Whether Anthropic must be given this credential as OAuth rather than an API key.
 *
 * Decided by the SECRET's shape, not by where it was stored. A
 * `claude setup-token` value is OAuth material (`sk-ant-oat01-…`) but is pasted
 * through the API-key path and lands in auth.json as `{type:"api", key}` — so
 * the stored kind says "api" while the wire needs a bearer token. Sent as
 * `x-api-key` it is rejected outright, which is the second half of the
 * "Couldn't reach Anthropic" failure. Verified against the real API with a
 * user's token: `x-api-key` → 401 `invalid x-api-key`; `Authorization: Bearer`
 * → past auth (429). The `anthropic-beta: oauth-2025-04-20` header is NOT
 * required — tested both with and without, both authenticate.
 *
 * This mirrors `claxedo-server`'s `credentials/verify.ts` `anthropicOAuth`,
 * which reads the prefix for the same reason, and `claudeAuthEnv` at spawn.
 *
 * Matched on the TRIMMED value: a token pasted out of a terminal usually
 * carries a trailing newline and is stored verbatim, so an untrimmed compare
 * would miss the prefix and take the API-key branch — precisely the failure
 * this exists to prevent.
 */
function anthropicOAuthCredential(key: unknown) {
  return typeof key === "string" && /^sk-ant-o/i.test(key.trim())
}

/**
 * The two anthropic credential-path decisions, exposed for unit tests.
 *
 * They are pure functions of their inputs, but they run inside `resolveSDK`,
 * which needs a compiled provider layer and a real model — so testing them
 * through it would assert on assembly plumbing rather than on the rules
 * themselves. These are the rules.
 */
export const __anthropicCredentialPathForTest = {
  baseURL: anthropicBaseURL,
  isOAuth: anthropicOAuthCredential,
}
  ...
  return {
    anthropic: Effect.fnUntraced(function* () {
      ...
      return {
        autoload: false,
        options: {
          // Pinned rather than left to the SDK's own env fallback. See
          // `anthropicBaseURL`. Config options are merged OVER a custom
          // loader's (`mergeDeep(existing, provider.options)` at the config
          // merge), so an explicit `provider.anthropic.options.baseURL` still
          // wins — which is the precedence we want.
          baseURL: anthropicBaseURL(undefined, env),
          headers: {
  ...
  Effect.gen(function* () {
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        // OAuth material must ride as a bearer token, not as x-api-key. The SDK
        // treats `apiKey` and `authToken` as mutually exclusive, so the key is
        // moved rather than duplicated. See `anthropicOAuthCredential`.
        if (model.providerID === "anthropic" && anthropicOAuthCredential(options["apiKey"])) {
          options["authToken"] = String(options["apiKey"]).trim()
