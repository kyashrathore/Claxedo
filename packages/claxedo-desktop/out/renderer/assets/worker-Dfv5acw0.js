...
function iterateOverDiff({ diff, diffStyle, startingLine = 0, totalLines = Infinity, expandedHunks, collapsedContextThreshold = DEFAULT_COLLAPSED_CONTEXT_THRESHOLD, callback }) {
  const state = {
    ...
    emit(props, silent = false) {
      if (!silent) state.incrementCounts(1, 1);
...
function getChangeIterationRanges(state, content, diffStyle) {
  ...
  function pushRange(range2, kind) {
    ...
    const [start, end] = mapRangeToIteration(range2);
    if (end > start) iterationRanges.push([start, end]);
  }
  {
    pushRange(getVisibleRange(state.unifiedCount, content.deletions));
    pushRange(getVisibleRange(state.unifiedCount + content.deletions, content.additions));
  }
  {
    pushRange(getVisibleRange(state.splitCount, content.deletions));
    pushRange(getVisibleRange(state.splitCount, content.additions));
  }
  if (iterationRanges.length === 0) return iterationRanges;
  iterationRanges.sort((a, b2) => a[0] - b2[0]);
...
function getHighlighter(preferredHighlighter = "shiki-js") {
  highlighter ??= createHighlighterCore({
    themes: [],
    langs: [],
    engine: preferredHighlighter === "shiki-wasm" ? createOnigurumaEngine(import("./wasm-DDgzZJey.js")) : createJavaScriptRegexEngine()
  });
  return highlighter;
}
