 ...
 * module it compiles, which saves measured startup time on the server bundle's
 * static closure — but only
 ...
 *     entry name is RECOMPUTED here, at runtime, from the artifact's real path.
 *
 ...
 *     argv — see scripts/build-compile-cache.ts, which imports the
 ...
 *     the shipped Electron binary, the shipped artifact bytes and the flags.
 *
...
/** Directory name for the server bundle's shipped static-closure cache. */
...
export const CLAXEDO_COMPILE_CACHE_MANIFEST_NAME = "manifest.json"

/** Subdirectory of the server data dir used as the writable compile cache base. */
export const CLAXEDO_COMPILE_CACHE_BASE_DIR_NAME = "claxedo-compile-cache"
 ...
 *
 * Multiple artifacts may provide separate manifests, but all are seeded into
 * one runtime directory because Node has exactly one compile cache.
 */
...
export type CompileCacheOutcome = {
  /**
...
export function seedShippedCompileCaches(input: {
...
}): Omit<CompileCacheOutcome, "enabled"> {
  ...
  for (const { shippedDir, rootDir } of sources) {
    const manifest = parseCompileCacheManifest(
      readFileSync(join(shippedDir, CLAXEDO_COMPILE_CACHE_MANIFEST_NAME), "utf8"),
    )
...
export function enableShippedCompileCaches(input: {
  ...
  try {
    ...
    // caching at all".
    const baseDir = join(dataDir, CLAXEDO_COMPILE_CACHE_BASE_DIR_NAME)
