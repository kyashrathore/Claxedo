 ...
 * "open this exact location", the whole privilege it grants: a relative
...
import { isOpenInAppName, OPEN_IN_APP_NAMES } from "./open-in-apps"
...
export async function openInVerdict(
  ...
  return { allowed: false, reason: `app "${request.app}" is not on the open-path allowlist` }
}
