...
export type WillAttachHandlerOptions = {
  ...
  /**
   * Trusted guest preload path to install on the guest `webPreferences`
   * after stripping any embedder-supplied preload. Typically points to the
   * built `browser-preload.mjs` (a `file://` URL). When omitted, no preload
   * is attached — the guest runs with only the site's own scripts.
   *
   * The guest preload is what brings react-grab into the page so the
   * element picker has a visible overlay (Chromium's CDP-driven picker
   * does not render inside `<webview>` guests). It is strictly controlled
   * by the main process — the renderer never supplies the path.
   */
...
export function createWillAttachWebviewHandler(opts: WillAttachHandlerOptions = {}) {
  ...
  return function willAttachWebview(
    ...
    // After stripping, install the main-process-controlled guest preload —
    // this brings react-grab into the page so the picker has a visible
    // overlay. The renderer never gets to pick the path; it comes from the
    // trusted main config (`opts.guestPreloadUrl`).
