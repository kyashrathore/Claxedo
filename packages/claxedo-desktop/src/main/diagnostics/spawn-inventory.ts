...
export const SPAWN_INVENTORY: readonly SpawnInventoryRow[] = [
  ...
  product({
    id: "desktop-sidecar",
    family: "OpenCode sidecar and bootstrap helper",
    classification: "registered-root",
    owner: "desktop",
    linkage: "app",
    observation: "host-tree",
    stop: "supported",
    kill: "supported",
    source: { file: "packages/claxedo-desktop/src/main/cli.ts", callee: "spawn", calls: 2 },
  }),
  product({
    id: "desktop-sidecar-probes",
    family: "Sidecar version and identity probes",
    owner: "desktop",
    linkage: "app",
    observation: "lifecycle-only",
    stop: "unsupported",
    kill: "unsupported",
    source: { file: "packages/claxedo-desktop/src/main/cli.ts", callee: "execFileSync", calls: 2 },
  }),
  product({
