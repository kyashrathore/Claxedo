...
export const SPAWN_INVENTORY: readonly SpawnInventoryRow[] = [
  ...
  product({
    id: "local-shell-git",
    family: "Local shell Git and shell-command operations",
    owner: "server",
    linkage: "workspace",
    observation: "lifecycle-only",
    stop: "owner-dependent",
  ...
  product({
    id: "local-project-clone-git",
    family: "Project creation Git clone",
    owner: "server",
    linkage: "workspace",
    observation: "lifecycle-only",
    stop: "owner-dependent",
    kill: "unsupported",
    source: {
      file: "packages/claxedo-local-server/src/workspace/routes/projects-route.ts",
      callee: "execFileAsync",
      calls: 1,
    },
  }),
