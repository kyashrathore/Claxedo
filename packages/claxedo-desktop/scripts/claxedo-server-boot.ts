...
// A cache enabled inside the product entry had no entries for the server
// bundle. The measured cost was 41.0 ms of pure V8 compile on every launch.
//
...
if (compileCache.status === "failed") {
  console.error(`[claxedo-server-compile-cache] not seeded: ${compileCache.reason ?? "unknown"}`)
}
