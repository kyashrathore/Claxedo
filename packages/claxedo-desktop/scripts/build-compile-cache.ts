 ...
 * `src/shared/compile-cache.ts` for the five ways this can silently
 * produce a cache the runtime ignores.
 *
 * Only entries whose source file lives inside the bundle root are shipped.
 * Everything else the generator compiles has no stable root-relative name, so
 * its runtime key cannot be derived and it is dropped rather than guessed at.
 *
 * HOW THE SERVER BUNDLE IS CACHED WITHOUT STARTING A SERVER. Evaluating the
 * bundle binds a port and opens a database, which a build must never do. It does
...
export async function buildCompileCache(input: {
   ...
   * directory. The server bundle's entry lives in `chunks/` while its root is
   * the bundle directory above it.
   */
  rootDir?: string
  outputDir: string
  electronPath: string
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(input.outputDir, CLAXEDO_COMPILE_CACHE_MANIFEST_NAME),
