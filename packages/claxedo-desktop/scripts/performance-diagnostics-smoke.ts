...
async function measureDiagnosticsTreeCpu(enabled: boolean) {
  ...
  try {
    ...
    const initial = await pidtree(child.pid, { root: true })
    await packageUsage(initial, {
      maxage: 0,
      ...(process.platform === "darwin" ? { usePs: true } : {}),
    })
    await Bun.sleep(5_000)
    const measured = await pidtree(child.pid, { root: true })
    const usage = await packageUsage(measured, {
      maxage: 0,
      ...(process.platform === "darwin" ? { usePs: true } : {}),
    })
    return Math.min(
