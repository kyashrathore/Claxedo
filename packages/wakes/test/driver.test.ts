...
describe("push driver", () => {
  ...
  it("a burst of same-lane nudges coalesces but drains every due wake", async () => {
    ...
    await driver.idle()
    // Lanes guarantee one-at-a-time; a coalesced run only claims one wake, so
    // remaining due wakes are picked up by the next nudge or the sweep. All 5
    // must eventually fire here because each schedule() nudged after insert.
