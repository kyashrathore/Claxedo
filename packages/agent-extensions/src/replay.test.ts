...
describe("runtime Agent Extensions replay", () => {
  ...
  test("writes desired, lock, and materialized replay files without generated paths", async () => {
    ...
    await expect(fs.readFile(path.join(root, ".agent-extensions", "lock.json"), "utf8").then(JSON.parse)).resolves.toEqual({
      version: 1,
      packages: {
        review: {
          resolved_sha: "abcdef1234567890",
    ...
    await expect(fs.readFile(path.join(root, ".agent-extensions", "materialized.json"), "utf8").then(JSON.parse)).resolves.toMatchObject({
      version: 1,
      packages: {
        review: {
          status: "applied",
  ...
  test("removes previously materialized owned artifacts when runtime replay disables an install", async () => {
    ...
    await expect(fs.stat(path.join(root, ".cursor", "skills", "review"))).rejects.toThrow()
    await expect(fs.readFile(path.join(root, ".agent-extensions", "materialized.json"), "utf8").then(JSON.parse)).resolves.toEqual({
      version: 1,
      packages: {
        review: {
          package_name: "review",
