...
import { launchdPlist, serviceKind, serviceUnitPath, startService, systemdUnit, uninstallService, writeServiceUnit, type ServiceDeps } from "./service"
...
describe("service units", () => {
  test("Linux gets a systemd --user unit that never restarts into a decision", async () => {
    ...
    expect(lines.some((line) => line.includes("loginctl enable-linger"))).toBe(true)
