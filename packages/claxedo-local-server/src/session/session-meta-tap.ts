 ...
 *
 * A delete only drops the projection row: the runtime's own `DELETE
 * /session/:id` publishes `session.deleted` on the workspace's `wr/events`,
 * which every reader of that workspace holds open.
 */
