...
describe("serving", () => {
  ...
  test("rejects malformed percent-encoding without surfacing a server error", async () => {
    writeExtension("hello-clock", manifest("hello-clock"), { "main.js": "export {}" })

    const response = await app().request("http://localhost/hello-clock/%E0%A4%A.js")

    expect(response.status).toBe(404)
    expect(await response.json()).toMatchObject({ error: { code: "user_extension_file_not_found" } })
  })
