...
// The central lifecycle stream combines flat `claxedoBus` control-plane facts
// with explicitly published `globalBus` envelopes. Workspace conversation
// output is intentionally absent; it has one producer and one transport on
...
describe("streamGlobalEvents", () => {
  ...
  test("forwards explicit globalBus control-plane envelopes in {directory,payload} shape", async () => {
    const app = mountStreamGlobalEvents()
    const ac = new AbortController()
    const res = await app.request("http://127.0.0.1/global/event", { signal: ac.signal })
    const reader = res.body!.getReader()
