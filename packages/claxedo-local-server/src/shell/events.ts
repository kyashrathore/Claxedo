 ...
 * frames stay wrapped in `{directory, payload}` and `claxedoBus` frames stay
 * flat, because `ClaxedoEventsProvider`'s
 ...
 * non-claxedo/harness (ACP) session's `POST /session` emits (see
...
export function createGlobalEventsHandler(
  ...
  // than the workspace one: embedded runtimes can bridge token-level
  // `message.part.updated` frames onto `globalBus`. Growing the main ring is
 ...
 * soon as the shell routes are importable — retention that only began at the
