...
describe("credential secret scope policy", () => {
  test("shared runtime snapshots require explicit shared scope", () => {
    expect(credentialSecretInScope({ source: "managed" }, "shared")).toBe(false)
    expect(credentialSecretInScope({ source: "managed", scope: "local" }, "shared")).toBe(false)
    expect(credentialSecretInScope({ source: "managed", scope: "shared" }, "shared")).toBe(true)
