...
describe("hosted shell marketplace routes", () => {
  ...
  test("connections refuses an anonymous caller", async () => {
    const res = await app.fetch(new Request("http://cp.test/api/claxedo/agent-config/connections"))
  ...
  test("connections reports unsupported once signed (operator local configuration)", async () => {
    ...
    const res = await signed.fetch(
      new Request("http://cp.test/api/claxedo/agent-config/connections", {
        headers: { authorization: "Bearer hosted-token" },
      }),
    )
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toEqual({ status: "unsupported", reason: "operator_local_configuration" })
  })
