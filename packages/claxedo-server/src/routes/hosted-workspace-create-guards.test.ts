...
import { HostedWorkspaceRoutes, type HostedWorkspaceRouteOptions } from "./hosted-workspace"
 ...
 * in `hosted-workspace.ts` with no rate limiter, and it is the only one that
...
const routeSource = fs.readFileSync(path.join(import.meta.dirname, "hosted-workspace.ts"), "utf8")
...
function leaseDb(rows: Array<Record<string, unknown>>) {
  return { query: () => ({ collect: async () => rows }) }
}
