...
import { applySessionMeta, GLOBAL_SHOW_TAG, GLOBAL_TAG, listSessionMetas, taggedSessionMetas } from "../session-meta"
...
async function buildAcpProviderResponse() {
  ...
  return {
    ...
    connected: [
      ...
      ...(process.env.OPENAI_API_KEY || auth["codex-acp"] || auth.openai || getCredentialByProvider("codex-acp") || getCredentialByProvider("openai") ? ["codex-acp"] : []),
...
export function OpenCodeCompatRoutes() {
  return new Hono()
    ...
    .get("/file/all", async (c) => {
      ...
      const paths = fromGit ?? (await walkAll(root))
      return c.json({ paths })
    })
    ...
    .get("/experimental/session", async (c) => {
      ...
      await Promise.allSettled(locals.map(async (ws) => {
        if (ws.kind === "cloud") {
          const rows = await listSessionMetas({
            workspaceID: ws.id,
