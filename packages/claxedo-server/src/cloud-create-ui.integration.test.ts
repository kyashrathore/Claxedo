...
vi.doMock("./cloud/sandbox", () => ({
  sandboxProvider: (input: string | undefined) =>
    input === "daytona" || input === "modal" || input === "vercel" || input === "cloudflare" ? input : undefined,
  ...
  poolEnabled: async () => false,
  initPool: async () => {},
  startPoolMonitor: () => {},
  shutdown: () => {},
}))

vi.doMock("./workspace-supervisor", async () => {
  const bus = await vi.importActual<typeof import("./bus")>("./bus")
  const store = await vi.importActual<typeof import("./workspace-store")>("./workspace-store")
  ...
  return {
    ...
    stopRuntime: async () => {},
    discardWorkspaceRuntime: async () => {},
    ensureWorkspaceRuntime: async (workspaceId: string) => {
      ...
      const next = await store.updateWorkspace(workspaceId, {
        status: "ready",
        sandbox_id: `sb-${workspaceId}`,
        sandbox_url: `http://runtime/${workspaceId}`,
      })
      ...
      return {
        workspaceId,
        url: next?.sandbox_url ?? `http://runtime/${workspaceId}`,
        status: "ready",
      }
    },
  }
...
const [serverMod, store, agent, hooks] = await Promise.all([
  import("./server"),
...
describe("cloud create UI integration", () => {
  ...
  test("UI cloud create flow provisions a workspace and exposes it via bootstrap", async () => {
    ...
    const boot = await wait(async () => {
      ...
      return await res.json() as {
        ...
        project: Array<{
          id: string
          workspaces: Record<string, { kind: string; status?: string; workspace_name?: string }>
        }>
      }
    })
    ...
    expect(hit?.workspaces[created.directory]).toMatchObject({
      kind: "cloud",
      status: "ready",
  ...
  test("bootstrap stays healthy when upstream opencode is unavailable", async () => {
    ...
    expect(body.provider_auth).toEqual({})
