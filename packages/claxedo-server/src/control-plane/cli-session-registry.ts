...
/**
 * A registry that refuses everything, with a reason.
 *
 * For the composition that KNOWS it cannot reach durable storage — a missing
 * credential, say. Semantically identical to installing nothing (the port fails
 * closed either way), but it carries the actionable reason all the way to the
 * caller instead of the generic "no registry is configured", and it keeps the
 * "every composition names a registry" invariant intact so the absence is a
 * deliberate, visible choice rather than an omission.
 */
export function unavailableCliSessionTokenRegistry(reason: string): CliSessionTokenRegistry {
  const refuse = (): never => {
    throw new CliSessionRegistryUnavailableError(reason)
  }
  return {
    recordMint: refuse,
    rotate: refuse,
    active: refuse,
    revoke: refuse,
    revokeSession: refuse,
    revokeForUser: refuse,
  }
}
  ...
  return {
    ...
    async active(args) {
      ...
      if (
        ...
        return inactive(
          "cli_session_token_mismatch",
          "CLI session token registry returned an incomplete record",
        )
      }
      return {
        active: true,
...
/** The configured registry, or `undefined` when none has been installed. */
export function cliSessionTokenRegistry() {
  return configured
}
