...
const ALLOWED_FUNCTION_BUILDERS = new Set([
  ...
  "webhookMutation",
  // W6.3: the cron handler that needs `fetch` (the Clerk reconciliation sweep;
  // queries and mutations cannot make network calls). Wraps
  // `internalActionGeneric` — same "not callable by any client" guarantee as
  // the two above, and the guarantee carries MORE weight here: this is the only
  // function in convex/ holding an outbound third-party credential
  // (`CLERK_SECRET_KEY`), so a publicly callable version would be an open proxy
  // to the Clerk Backend API under this deployment's own secret key.
  "cronAction",
  // The read half of the same pair: what a `cronAction` calls via
  // `ctx.runQuery` (an action has no `db`). `internalQueryGeneric`, so same
  // visibility guarantee, and no write capability at all.
  "cronQuery",
])
...
describe("Convex authz builder guard (D8)", () => {
  ...
  test("no convex module outside the builder module value-imports from convex/server builder entrypoints", () => {
    ...
    const allowedValueImports = new Map<string, Set<string>>([
      ...
      // internalActionGeneric/internalQueryGeneric back cronAction/cronQuery (W6.3).
      [BUILDER_MODULE, new Set([
        "queryGeneric",
        "mutationGeneric",
