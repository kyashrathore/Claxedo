...
describe("control-plane services", () => {
  ...
  test("hosted createApp does not expose unauthenticated local execution routes", async () => {
    const { createApp } = await import("../server")
    const built = createApp(createHostedControlPlaneServices(fakePorts(), hostedOptions()))

    const health = await built.app.request("/api/claxedo/health")
    await expect(health.json()).resolves.toMatchObject({ localExecution: false })
  ...
  test("cloud workspace creation uses normalized provider, branch, and credential inputs", () => {
    const file = path.resolve(import.meta.dirname, "../routes/workspace.ts")
    const text = fs.readFileSync(file, "utf8")
