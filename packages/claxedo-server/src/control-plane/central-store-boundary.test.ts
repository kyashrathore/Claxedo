...
// Import-boundary pin (plan provenance: Unit 6 / R4): control-plane CENTRAL persistence is separate
// from workspace-runtime / agent-sdk-runtime EXECUTION persistence. The
...
describe("control-plane central-store boundary", () => {
  test("central-store files exist", () => {
    for (const file of centralStoreFiles) {
      expect(fs.existsSync(path.resolve(import.meta.dirname, file)), `${file} should exist`).toBe(true)
    }
  })
