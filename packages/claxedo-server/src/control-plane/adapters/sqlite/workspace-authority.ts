...
export function createSqliteWorkspaceAuthority(options: SqliteWorkspaceAuthorityOptions = {}): WorkspaceAuthority {
  ...
  return {
    ...
    async registerLocalHostLink(auth: SignedControlPlaneAuth, args) {
      ...
      db.prepare(`UPDATE host_attestation_challenges SET used_at = ? WHERE challenge_id = ?`).run(now, args.challengeId)
      // Host proof passed: a never-registered workspace is created HERE —
      // never before signature verification — owned by the proving caller.
      const workspace = existingWorkspace ?? (() => {
        db.prepare(`
          INSERT INTO workspaces (workspace_id, owner_token_identifier, backing, access, display_name, created_at, updated_at)
          VALUES (?, ?, 'local-worktree', 'user-hosted', ?, ?, ?)
        `).run(args.workspaceId, who.token_identifier, args.displayName ?? args.workspaceId, now, now)
        return workspaceByPublicId(db, args.workspaceId)!
      })()
