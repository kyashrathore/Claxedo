/**
 * CLI session tokens are the credential `claxedo login` leaves on a developer's
 * disk. Before this suite they were signature-only: no registry, no revocation
 * path, and an accepted TTL upper bound of ten years. A leaked credentials file
 * was therefore valid until it expired, and nothing could pull it.
 *
...
describe("refresh rotation", () => {
  ...
  test("the login chain has a bounded absolute lifetime", async () => {
    ...
    // 90 days, not the 10 years the signer used to accept.
...
describe("TTL bounds", () => {
  test("bounds replace the old 90-day default and 10-year ceiling", () => {
