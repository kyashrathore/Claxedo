...
describe("the hosted composition serves CLI login", () => {
  ...
  test("a hosted composition always names a registry, and says why when it cannot reach storage", () => {
    // Hosted composition tolerates a missing service token elsewhere, so this
    // path cannot hard-fail boot without overturning that. What it must not do
    // is compose NOTHING — which is how the hole existed. The plane always
    // carries a registry; when storage is unreachable it is one that refuses
    // with the variable to set, so the failure names its own fix instead of
    // surfacing as a bare "not configured".
    ...
    expect(plane.cliSessionTokenRegistry).toBeDefined()
    expect(() => plane.cliSessionTokenRegistry.revokeForUser({ tokenIdentifier: OWNER }))
