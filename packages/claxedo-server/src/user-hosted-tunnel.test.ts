...
describe("user-hosted Workspace Relay tunnel manager", () => {
  ...
  test("forwards the sandbox lease's own region, not the configured default", async () => {
    // For a workspace that already exists, the LEASE is authoritative: its region
    // may predate the current CLAXEDO_HOME_REGION. Reading the config instead
    // would hand the relay a region the workspace is not actually in, and the
    // resulting Durable Object placement is permanent.
    mocks.getWorkspace.mockResolvedValueOnce({
      id: "ws_cloud",
      directory: "/workspace",
      kind: "cloud",
    ...
    mocks.sandboxManagerEnsure.mockResolvedValueOnce({
      status: "ready",
      workspaceId: "ws_cloud",
      sandboxId: "sandbox_cloud",
      hostId: "host_cloud",
