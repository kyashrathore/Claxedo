...
async function startRemoteRuntime(s: State): Promise<State> {
  ...
  log.info("Remote runtime bootstrap start", {
    workspaceId: s.ws.id,
    provider,
    ...
    remoteDirectory: s.ws.remote_directory || WORKSPACE_DIR,
    hasRepoUrl: !!s.ws.repo_url,
  })
  ...
  try {
    ...
    const net = provider === "daytona"
      ...
      : undefined
    log.info("Remote runtime network policy resolved", {
      workspaceId: s.ws.id,
      provider,
      action: action.action,
      policyCount: rows.length,
      netMode: net?.mode ?? (provider === "daytona" ? "skipped-daytona" : "allow-all"),
    })
    ...
    if (!sandbox) {
      log.info("Remote runtime acquiring fresh sandbox", {
        workspaceId: s.ws.id,
        provider,
        projectId: s.ws.project_id,
      })
    ...
    s.remote = true
    log.info("Remote runtime sandbox ready", {
      workspaceId: s.ws.id,
      provider,
    ...
    // Record sandbox assignment on authority before the runtime starts so the
    // workspace-runtime can report the current fenced identity back to control plane.
    const assignedLease = lifecycle.recordSandboxAssigned(s.ws.id, sandbox.id, null, sandbox.id)
    ...
    const { url } = await sandboxRuntime.deployAndStart(sandbox, s.ws.id, {
      ...
      envVars: {
        ...
        CLAXEDO_CONTROL_PLANE_URL: needOpts().server_url,
        ...(assignedLease.ok ? {
          CLAXEDO_WR_LEASE_ID: assignedLease.lease.lease_id,
          CLAXEDO_WR_EPOCH: String(assignedLease.lease.epoch),
          CLAXEDO_WR_SANDBOX_ID: sandbox.id,
        } : {}),
      },
      onStep: async (step) => {
        if (step === "ready" || step === "error") return
        log.info("Remote runtime provision step", {
          workspaceId: s.ws.id,
          provider,
          sandboxId: sandbox.id,
          step,
    ...
    s.url = url
    log.info("Remote runtime deploy completed", {
      workspaceId: s.ws.id,
      provider,
      sandboxId: sandbox.id,
      url,
