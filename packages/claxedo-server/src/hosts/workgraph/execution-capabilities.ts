...
export function createExecutionCapabilitiesPort(
  ...
  return {
    async read(context, request) {
      ...
      // A repository-required environment must expose revisions for the repository
      // the caller NAMED. An unscoped read has no repository to speak for, so an
      // empty list there is the honest answer rather than a catalog failure.
      if (input.environment.repositoryRequired && clean(request.directory) && baseRevisions.length === 0) {
