...
import type { WorkGraphSessionGateway } from "./execution"
...
async function runTurn(
  ...
  const claimed = input.database.transaction(() => {
    ...
    if (wake.status === "pending") {
      ...
      // master turn never starts while a Run is live in the workspace —
  ...
  try {
    ...
    await input.sessions.admit({
      ...
      prompt: buildMasterPrompt({
        ...
        agents: ExecutionProfileDefaultsSchema.parse(JSON.parse(claimed.stream.execution_defaults_json)).agents ?? [],
