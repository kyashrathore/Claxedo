...
import { createLocalWorkGraphMasterRuntime } from "./master-runtime"
  ...
  test("admits one stable lease-less master Session and drains its mailbox", async () => {
    ...
    database.prepare("UPDATE wg_v2_streams SET execution_defaults_json = ? WHERE id = ?").run(JSON.stringify({
      ...
      agents: [
        {
          name: "implementation",
          brief: "Own code changes and focused verification.",
          generation: {
            harness: "opencode",
            agent: "build",
            model: { providerId: "openai", modelId: "gpt-5" },
        ...
        {
          name: "review",
          brief: "Inspect landed candidates for correctness risks.",
          generation: {
            harness: "opencode",
            agent: "review",
            model: { providerId: "openai", modelId: "gpt-5" },
    ...
    expect(admit).toHaveBeenCalledWith(expect.objectContaining({
      ...
      prompt: expect.stringMatching(
        /Land task one\.[\s\S]*Your agents:\n- implementation: Own code changes and focused verification\.\n- review: Inspect landed candidates for correctness risks\.[\s\S]*Trusted Connection handles:\n- connection-github/,
      ),
