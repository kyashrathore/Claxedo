...
import { createLocalWorkGraphAgentTools, localSessionContext } from "./agent-tools"
...
describe("embedded WorkGraph agent tools", () => {
  test("invoke the process-owned service with trusted owner context and no HTTP transport", async () => {
    ...
    const source = fs.readFileSync(path.resolve(import.meta.dirname, "agent-tools.ts"), "utf8")
  ...
  test("routes a mechanical landing conflict into durable master triage", async () => {
    const execute = vi.fn(async (_context, request) => ({
      ok: true as const,
      operationId: request.operationId,
      cursor: "cursor-1",
      value: { streamId: "stream-1", workItemId: "item-1" },
    }))
    const base = embedded(execute)
    const tools = await createLocalWorkGraphAgentTools({
      ...base,
      execution: {
        ...terminalExecution(),
        land: async () => {
          throw Object.assign(new Error("Candidate conflicts with the moving Stream head"), {
    ...
    await expect(tools.workgraph_land_candidate!.execute(
      {
        work_item_id: "item-1",
    ...
    expect(execute).toHaveBeenCalledWith(
      expect.objectContaining({ actor: { type: "agent", id: "ses_master_stream-1" } }),
      {
        operationId: "landing_conflict_land-1",
        command: {
          version: 1,
          type: "record_landing_conflict",
          streamId: "stream-1",
          workItemId: "item-1",
          reason: "Candidate conflicts with the moving Stream head",
        },
      },
    )
  })
  ...
  test("infers a new Stream's local execution target from the invoking Session", async () => {
    ...
    const tools = await createLocalWorkGraphAgentTools(embedded(execute) as never, {
      ...
      sessionExecution: async (sessionId) => ({
        environment: { kind: "local_worktree", placement: "shared", directory: `/projects/${sessionId}` },
    ...
    expect(execute).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
      command: expect.objectContaining({
        execution: {
          environment: { kind: "local_worktree", placement: "shared", directory: "/projects/session-1" },
          repository: { baseRevision: "HEAD" },
        },
      }),
    }))
  ...
  test("derives trusted Session, project, and execution identity for ledger tools", async () => {
    ...
    expect(context).toEqual({
      ...
      projectId: "project-1",
      resolvedExecution: {
        environment: { kind: "local_worktree", placement: "shared", directory: "/projects/repo" },
        repository: { baseRevision: "HEAD" },
  ...
  test("keeps one real Session across Task checkpoints, completion, and the next ready Task", async () => {
    ...
    try {
      ...
      const session = {
        ...
        resolvedExecution: {
          environment: { kind: "local_worktree" as const, placement: "shared" as const, directory: "/projects/repo" },
          repository: { baseRevision: "HEAD" },
          harness: "codex",
          agent: "build",
