...
describe("embedded local WorkGraph v2", () => {
  ...
  it("exports and restores an owner archive through the embedded HTTP boundary", async () => {
    ...
    source.database.prepare("UPDATE wg_v2_streams SET spend_json = ? WHERE id = ?").run(JSON.stringify({
      totalTokens: 1_234,
      totalUsd: 0.5,
      asOf: 42,
  ...
  it("reaps the deleted owner's personal Connections and spares team + other owners", async () => {
    // Personal rows are owner-matched, so once the subject is gone no principal
    // can match them again — they would leak forever with the third-party token
    // still live at the provider. The cascade rides on owner deletion because
    // that is the moment the subject stops existing.
    ...
    const connections = createConnectionsService({
      ...
      newId: () => `connection-${++next}`,
    })
    // The owner string must be the one connect-time stores; for the unsigned
    // local composition that subject is "local".
    for (const owner of ["local", "other-owner"]) {
      expect(await connections.connect({ integrationId: "github", owner, fields: {}, secret: "live" }))
        .toEqual({ ok: true })
    }
    // Team row: owner absent, so a string owner filter must never match it.
    expect(await connections.connect({ integrationId: "github", fields: {}, secret: "live" }))
      .toEqual({ ok: true })
    // `list` is a PARTITION query, not "every row": no owner means the team
    // partition alone. Assert each partition explicitly.
    const personal = (owner: string) => connections.list({ owner, scope: "personal" })
    const team = () => connections.list({ scope: "team" })
    expect((await personal("local")).map((row) => row.id)).toEqual(["connection-1"])
    expect((await personal("other-owner")).map((row) => row.id)).toEqual(["connection-2"])
    expect((await team()).map((row) => row.id)).toEqual(["connection-3"])
    ...
    const app = new Hono()
    mountEmbeddedWorkGraph(app, embedded)
    await command(app, "cascade_stream", { version: 1, type: "create_stream", title: "Owner with connections" })

    const request = {
      method: "DELETE",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ operationId: "owner_delete_with_connections" }),
    }
    expect((await app.request("/api/workgraph/owner", request)).status).toBe(200)

    expect(await personal("local")).toEqual([])
    // Neither the other owner's personal row nor the shared team row is touched.
    expect((await personal("other-owner")).map((row) => row.id)).toEqual(["connection-2"])
    expect((await team()).map((row) => row.id)).toEqual(["connection-3"])
    // The deleted owner's backing credential goes with the row, not just the row.
    expect(await connections.getToken("connection-1", "code-host")).toMatchObject({ ok: false })
  })
...
const profile = {
  environment: { kind: "local_worktree" as const, placement: "shared" as const, directory: "/repo" }, repository: { baseRevision: "HEAD" }, harness: "claxedo-v2", agent: "build",
