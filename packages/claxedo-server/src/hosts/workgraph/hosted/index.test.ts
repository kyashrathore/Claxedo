  ...
  return createHostedWorkGraph({
    ...
    executor: {
      ...
      query: async (_fn, args) => {
        ...
        if (args.query && options?.queryError) throw options.queryError
...
describe("hosted WorkGraph composition", () => {
  test("caches successful authority identity for 60 seconds per bearer token", async () => {
    const calls: Record<string, unknown>[] = []
    const usersMe = vi.fn(async () => ({ user_id: "internal_identity_cache_user" }))
    const resolveOrgId = vi.fn(async () => "org_internal_a")
    let now = 1_000
    const workgraph = composition(calls, undefined, undefined, undefined, undefined, () => now, {
      authority: { usersMe, resolveOrgId } as unknown as WorkspaceAuthority,
    })

    const request = () => workgraph.router.request("/snapshot?limit=10", {
      headers: { authorization: "Bearer identity_cache_user" },
    })
    expect((await Promise.all([request(), request()])).map((response) => response.status)).toEqual([200, 200])
    expect(usersMe).toHaveBeenCalledTimes(1)
    expect(resolveOrgId).toHaveBeenCalledTimes(1)

    now += 60_001
    expect((await request()).status).toBe(200)
    expect(usersMe).toHaveBeenCalledTimes(2)
    expect(resolveOrgId).toHaveBeenCalledTimes(2)
  })

  test("evicts failed authority identity lookups so the same token can retry", async () => {
    const usersMe = vi.fn()
      .mockRejectedValueOnce(new ControlPlaneAuthError(403, "workspace_authorization_denied", "Identity denied"))
      .mockResolvedValue({ user_id: "internal_identity_retry_user" })
    const resolveOrgId = vi.fn(async () => "org_internal_a")
    const workgraph = composition([], undefined, undefined, undefined, undefined, undefined, {
      authority: { usersMe, resolveOrgId } as unknown as WorkspaceAuthority,
    })
    const request = () => workgraph.router.request("/snapshot?limit=10", {
      headers: { authorization: "Bearer identity_retry_user" },
    })
  ...
  test("evicts a cached identity after a downstream forbidden response", async () => {
    const usersMe = vi.fn(async () => ({ user_id: "internal_identity_forbidden_user" }))
    const resolveOrgId = vi.fn(async () => "org_internal_a")
    const workgraph = composition([], undefined, undefined, undefined, undefined, undefined, {
      authority: { usersMe, resolveOrgId } as unknown as WorkspaceAuthority,
      queryError: new ControlPlaneAuthError(403, "workspace_authorization_denied", "Snapshot denied"),
    })
    const request = () => workgraph.router.request("/snapshot?limit=10", {
      headers: { authorization: "Bearer identity_forbidden_user" },
    })
  ...
  test("returns the authority 503 when the real Convex executor times out", async () => {
    vi.stubEnv("CLAXEDO_CONVEX_MUTATION_TIMEOUT_MS", "25")
    const mutation = vi.spyOn(ConvexHttpClient.prototype, "mutation")
      .mockReturnValue(new Promise(() => undefined))
    const workgraph = composition([], undefined, undefined, undefined, undefined, undefined, {
      authority: createConvexAuthority({
        url: "https://authority-timeout.convex.test",
        serviceToken: "service-secret",
      }),
    })

    try {
      const response = await workgraph.router.request("/snapshot?limit=10", {
        headers: { authorization: "Bearer identity_timeout_user" },
      })
    ...
    expect(nudges).toEqual([
      {
        ...
        event: { type: "workgraph.changed", ownerUserId: "user_a", cursor: "1", ts: expect.any(Number) },
      },
    ])
  })
