...
import { createHostedConnectionsSetup, createHostedRepositoryAccess } from "./connections-setup"
...
describe("hosted Connections setup", () => {
  ...
  describe("personal accounts (no Clerk org claim)", () => {
    function createPersonalRig() {
      const secrets = new Map<string, { secret: string; status: "available" | "error" }>()
      const metadata: Array<Record<string, unknown>> = []
      const mutations: Array<Record<string, unknown>> = []
      ...
      const input = {
        ...
        serviceToken: "service-token",
        authConfig: { enabled: true as const, issuer: "https://issuer.example", jwksUrl: "https://issuer.example/jwks" },
        verifier: async () => ({
          mode: "signed" as const,
          user: { subject: "clerk-solo", issuer: "https://issuer.example", tokenIdentifier: "token" },
        }),
        credentials: () => credentials,
        requireEntitlement: async (clerkOrgId: string) => {
          entitlementChecks.push(clerkOrgId)
          return undefined
        },
        integrations: [githubIntegration({
          fetchImpl: (async (input: RequestInfo | URL) => {
            ...
            if (url.includes("/user/repos")) {
              return Response.json([{ id: 1, name: "repo", full_name: "octocat/repo", clone_url: "https://github.com/octocat/repo.git", private: false, permissions: { pull: true, push: true } }])
            }
        ...
        executor: {
          ...
          async mutation(_fn: unknown, args: Record<string, unknown>) {
            mutations.push(args)
            if ("clerk_subject" in args && !("connectionId" in args)) {
              return { member: true, org_id: "personal-org-1", user_id: "user-solo", role: "owner" }
            }
            if (!args.integrationId) {
              const deleted = metadata.some((row) => row.id === args.connectionId)
              metadata.splice(0, metadata.length, ...metadata.filter((row) => row.id !== args.connectionId))
      ...
      return { input, secrets, metadata, mutations, queries, entitlementChecks }
    }

    test("connect + list scope to the resolved personal org, not a Clerk org", async () => {
      const rig = createPersonalRig()
      const app = createHostedConnectionsSetup(rig.input)
      ...
      // Personal-org resolution went through the mutation, never the Clerk-org query.
      expect(rig.mutations[0]).toMatchObject({ service_token: "service-token", clerk_subject: "clerk-solo" })
      expect(rig.queries.some((args) => "clerk_org_id" in args)).toBe(false)
      expect(rig.entitlementChecks).toEqual([])

      const upsert = rig.mutations.find((args) => args.integrationId === "github")
      expect(upsert).toMatchObject({ ownerUserId: "user-solo", orgId: "personal-org-1", status: "connected" })
    ...
    test("missing bearer token answers 401, not an unhandled 500", async () => {
      const app = createHostedConnectionsSetup(createPersonalRig().input)
      const response = await app.request("/")
      expect(response.status).toBe(401)
      expect((await response.json()).error.code).toBe("missing_bearer_token")
    })

    test("repository access resolves the personal org for connected repos", async () => {
      const rig = createPersonalRig()
      const app = createHostedConnectionsSetup(rig.input)
      await app.request("/github/connect", {
        method: "POST",
        headers: { authorization: "Bearer token", "content-type": "application/json" },
        body: JSON.stringify({ scope: "team", method: "key", secret: "github-secret" }),
      })
      const connectionId = rig.metadata[0]!.id as string

      const repositoryForAuth = createHostedRepositoryAccess(rig.input)
      const access = await repositoryForAuth({ user: { subject: "clerk-solo" } }, connectionId, "octocat/repo")
      expect(access).toMatchObject({
        ok: true,
        repository: { fullName: "octocat/repo", permissions: { read: true, write: true } },
        token: "github-secret",
      })

      const missing = await repositoryForAuth({ user: { subject: "clerk-solo" } }, "conn-unknown", "octocat/repo")
      expect(missing).toMatchObject({ ok: false, status: 404, code: "connection_not_found" })
    })
  })
})
