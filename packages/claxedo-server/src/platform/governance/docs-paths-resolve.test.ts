...
/**
 * A backticked file path (`foo/bar.ts`) or directory path (`foo/bar/`). Bare
 * words are excluded: a path has to carry a slash or a known source extension
 * to be checked at all.
 */
const CODE_SPAN = /`([^`\s]+)`/g
const LOOKS_LIKE_PATH = /^(?:[\w.@-]+\/)+[\w.*-]+\/?$|^[\w-]+\.(?:ts|tsx|mjs|json|sql|md)$/
