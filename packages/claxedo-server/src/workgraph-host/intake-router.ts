...
// NOTE: the `SourceIssue*` error CLASSES are deliberately not imported here.
// They would arrive from a different bundled entry point than the connectors
// that throw them, so `instanceof` is unreliable — see `sourceIssueFailure`.
import {
...
function intakeFailure(error: unknown): Readonly<{
  ...
  // Source-issue failures are classified by the error's `name`, NOT `instanceof`.
  // The connector factories are imported from `@claxedo/workgraph` (dist/index
  // .mjs) while these classes come from `@claxedo/workgraph/connectors`
  // (dist/connectors/index.mjs). Both entries are bundled separately with
  // `--packages=external` (packages/workgraph/scripts/build.ts), so each embeds
  // its OWN copy of these classes: every `instanceof` here was false against a
  // real connector's throw, and every provider failure fell through to
  // `router.onError` as an unclassified 500 — making the documented
  // "503 unavailable, never 401" rail unreachable in production. `name` is set
  // in each constructor and survives the bundle split.
  const sourceIssue = sourceIssueFailure(error)
...
/**
 * Classify a connector throw without relying on class identity. See the note in
 * `intakeFailure`: the connectors reach this router from a different bundled
 * entry point than the error classes, so `instanceof` cannot be trusted here.
 */
function sourceIssueFailure(error: unknown): Readonly<{
  status: ContentfulStatusCode
  error: Readonly<{ code: string; message: string; retryable: boolean }>
}> | undefined {
  if (!(error instanceof Error)) return
  const detail = error as Error & { code?: unknown; status?: unknown; retryable?: unknown }
  const code = typeof detail.code === "string" ? detail.code : undefined
  switch (error.name) {
    case "SourceIssueConfigurationError":
      return {
        status: 409,
        error: {
          code: code ?? "source_issue_configuration_required",
          message: "Source issue provider configuration is required",
          retryable: false,
        },
      }
    case "SourceIssueUnauthorizedError":
      return {
        status: 401,
        error: {
          code: code ?? "source_issue_provider_unauthorized",
          message: "Source issue provider authorization failed",
          retryable: false,
        },
      }
    case "SourceIssueResponseError":
      return {
        status: 502,
        error: {
    ...
    case "SourceIssueProviderError":
      return {
        status: (typeof detail.status === "number" ? detail.status : 502) as ContentfulStatusCode,
        error: {
          code: code ?? "source_issue_provider_error",
          message: "Source issue provider request failed",
          retryable: detail.retryable === true,
        },
      }
    case "SourceIssueTransportError":
      return {
        status: 503,
        error: {
          code: code ?? "source_issue_transport_unavailable",
