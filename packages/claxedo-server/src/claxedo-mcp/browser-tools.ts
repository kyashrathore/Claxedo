...
export async function handleBrowserGetConsoleLogs(
  ...
  const paneId = args.pane_id.trim()
  if (!paneId) return errorResult("pane_id is required.")
  const url = new URL(`/browser/${encodeURIComponent(paneId)}/console`, "http://placeholder")
  if (typeof args.since === "number") url.searchParams.set("since", String(args.since))
...
export function registerBrowserTools(server: McpServerLike): void {
  server.registerTool(
    "browser_list_tabs",
    {
      description:
