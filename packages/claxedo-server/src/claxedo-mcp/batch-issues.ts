...
async function createWorktrees(
  ...
  for (const issue of issues) {
    try {
      const body = rec(await http(
        "/experimental/worktree",
        { method: "POST", body: JSON.stringify({ name: issue.title }) },
        "json",
        directory,
      ))
      const worktreeDir = str(body?.directory)
      if (!worktreeDir) throw new Error("Worktree creation response missing directory")
      created.push({ issue, worktreeDir })
    } catch (err) {
      failed.push({
        title: issue.title,
        success: false,
...
async function startOpencodeAgent(
  ...
  try {
    ...
    if (!sessionId) throw new Error("Session creation response missing id")
  } catch (err) {
    return {
      title: issue.title,
      success: false,
...
async function startCliAgent(
  ...
  try {
    ...
    const ptyId = str(body?.id)
    if (!ptyId) throw new Error("PTY creation response missing id")
    return { title: issue.title, success: true, ptyId, worktreeDir }
  } catch (err) {
    return {
      title: issue.title,
      success: false,
