...
For a signed runtime (on a provisioned cloud VM or on an enrolled machine), provisioning checks Connections readiness but never resolves the upstream OAuth token. It creates a short-lived, narrowly scoped gateway capability for `(user, organization, project, workspace, harness, plugin, server, integration)`. The existing sandbox driver's secret broker attaches that capability only to its unique gateway hostname. It is absent from VM environment variables, plugin bytes, harness config, and runtime snapshots. A driver with `secretBrokering: none` marks only that protected MCP server unavailable; skills, stdio MCP, public MCP, and other plugin content remain available.
