...
function sessionDirectory(input: unknown) {
...
export async function createLocalSession(ws: Workspace, runner: SessionRunner, title?: string) {
  ...
  return {
    ...session,
    directory: sessionDirectory((session as { directory?: unknown }).directory) ?? ws.directory,
  }
}
  ...
  await Promise.allSettled(
    [...uniq.values()].map(async (runner) => {
      ...
      for (const row of rows) {
        ...
        const normalized = {
          ...row,
          directory: sessionDirectory((row as { directory?: unknown }).directory) ?? ws.directory,
        }
        const prev = merged.get(id)
        if (prev && stamp(prev as Record<string, unknown>) > stamp(normalized as Record<string, unknown>)) continue
