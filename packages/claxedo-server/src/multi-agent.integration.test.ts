 ...
 *
 * Regression targets:
 *  - Runner type validation (only 4 valid types)
...
describe("multi-agent integration", () => {
          ...
          return new Response(
            new ReadableStream<Uint8Array>({
              start(next) {
                ...
                next.enqueue(enc.encode(`data: ${JSON.stringify({ directory: "global", payload: { type: "server.connected", properties: {} } })}\n\n`))
              },
              cancel() { if (ctrl) upstreamSubs.delete(ctrl) },
            }),
  ...
  test("POST /runner accepts all four valid runner types", async () => {
    for (const type of ["claude-acp", "codex-acp", "cursor-acp", "opencode"]) {
      const res = await fetch(`${base()}/api/claxedo/agent-config/runner`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  test("POST /runner persists chosen runner to disk", async () => {
    await fetch(`${base()}/api/claxedo/agent-config/runner`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  test("POST /runner preserves model when switching to same type", async () => {
    ...
    const cfg = await savedConfig()
    expect(cfg.runner?.type).toBe("claude-acp")
    expect(cfg.runner?.model).toBe("claude-opus-4-6")
  })
  ...
  test("POST /runner drops model when switching to different type", async () => {
    ...
    const cfg = await savedConfig()
    expect(cfg.runner?.type).toBe("codex-acp")
    expect(cfg.runner?.model).toBeUndefined()
  })
  ...
  test("GET /runner returns configured runner state without workspace", async () => {
    const res = await fetch(`${base()}/api/claxedo/agent-config/runner`)
    expect(res.status).toBe(200)
    const body = await res.json() as { type: string; activeType: string; status: string }
    expect(body.type).toBe("opencode")
  ...
  test("POST /runner/model rejects missing model", async () => {
    const res = await fetch(`${base()}/api/claxedo/agent-config/runner/model`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    })
  ...
  test("PUT /auth stores API key and reflects in provider connected list", async () => {
    ...
    const body = await provider.json() as { connected: string[] }
    expect(body.connected).toContain("claude-acp")
  })
  ...
  test("DELETE /auth removes API key and updates connected list", async () => {
    ...
    const provider = await fetch(`${base()}/provider`)
    const body = await provider.json() as { connected: string[] }
    expect(body.connected).not.toContain("claude-acp")
  })
  ...
  test("provider endpoint returns ACP providers in ACP mode", async () => {
    await agent.saveUserConfig({
      mcp: {},
      auth: {},
      runner: { type: "claude-acp" },
    ...
    // Claude ACP should have known models
    const claude = body.all.find((p) => p.id === "claude-acp")!
    expect(Object.keys(claude.models)).toContain("claude-sonnet-4-6")
    expect(Object.keys(claude.models)).toContain("claude-opus-4-6")
    expect(Object.keys(claude.models)).toContain("claude-haiku-4-5")
  })
  ...
  test("bootstrap response includes ACP providers when in ACP mode", async () => {
    await agent.saveUserConfig({
      mcp: {},
    ...
    const body = await res.json() as {
      healthy: boolean
      provider: { all: Array<{ id: string }>; connected: string[] }
    ...
    // Provider should be ACP, not upstream
    const ids = body.provider.all.map((p) => p.id)
    expect(ids).toContain("claude-acp")
    expect(ids).not.toContain("upstream")

    // Connected should include claude-acp (we stored an auth key)
    expect(body.provider.connected).toContain("claude-acp")

    // Auth should be ACP-style
    expect(body.provider_auth["claude-acp"]).toBeDefined()

    // Config model should be prefixed with runner type
    expect(body.config.model).toContain("claude-acp/")
  })
  ...
  test("GET /config returns ACP-style config body in ACP mode", async () => {
    ...
    expect(body.model).toBe("claude-acp/claude-opus-4-6")
  ...
  test("GET /runner/options requires directory or workspaceId", async () => {
    const res = await fetch(`${base()}/api/claxedo/agent-config/runner/options`)
    expect(res.status).toBe(400)
  })

  test("GET /runner/options returns structured empty state for opencode runner", async () => {
    const ws = await workspace("options-opencode")
    const res = await fetch(
      `${base()}/api/claxedo/agent-config/runner/options?directory=${encodeURIComponent(ws.directory)}&type=opencode`,
    )
    expect(res.status).toBe(200)
  ...
  test("GET /runner/options returns Claude static fallback before live ACP options exist", async () => {
    const ws = await workspace("options-claude")
    const res = await fetch(
      `${base()}/api/claxedo/agent-config/runner/options?directory=${encodeURIComponent(ws.directory)}&type=claude-acp`,
    )
    expect(res.status).toBe(200)
    const body = await res.json() as {
      options: Array<{ category?: string; type: string; selectOptions?: Array<{ id: string }> }>
      source: string
      stale: boolean
    }
    expect(body.source === "static" || body.source === "live" || body.source === "cache").toBe(true)
    if (body.source === "static") expect(body.stale).toBe(true)
    const model = body.options.find((item) => item.category === "model" && item.type === "select")
    expect(model?.selectOptions?.map((item) => item.id)).toContain("claude-sonnet-4-6")
  })
