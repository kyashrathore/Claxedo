...
export async function syncSessionMetas(ws: Workspace | undefined, input: unknown[]) {
  await upsertRows(input.map((item) => row(item, ws)))
}
...
export async function sessionMetas(input: string[]) {
  ...
  const tags = ClaxedoDB.use((db) => db.select().from(ClaxedoSessionTagTable).where(inArray(ClaxedoSessionTagTable.session_id, hit)).all())
  const attachments = ClaxedoDB.use((db) => db.select().from(ClaxedoSessionAttachmentTable).where(inArray(ClaxedoSessionAttachmentTable.session_id, hit)).all())
