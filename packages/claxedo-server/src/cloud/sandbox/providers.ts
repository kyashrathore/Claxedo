...
function fingerprint(input: string | undefined) {
  if (!input) return
...
export class DaytonaProvider extends SandboxProvider<DaytonaAuth> {
  ...
  private async daytona(cfg?: SandboxConfig) {
    const syncAuth = this.sync(cfg)
    const managedAuth = await this.managed().catch(() => undefined)
    const auth = syncAuth ?? managedAuth
    if (!auth?.api_key) {
      log.warn("Daytona auth missing", {
        syncSource: clean(cfg?.auth?.daytona?.api_key) ? "config" : "none",
        hasManaged: !!managedAuth?.api_key,
      })
      throw new Error("Missing Daytona API key")
    }
    if (this.client && this.key === auth.api_key) {
      log.info("Daytona client reuse", {
        chosenSource: syncAuth ? "sync" : "managed",
        chosenKey: fingerprint(auth.api_key),
      })
      return this.client
    }
    log.info("Daytona client initializing", {
      syncSource: clean(cfg?.auth?.daytona?.api_key) ? "config" : "none",
      chosenSource: syncAuth ? "sync" : "managed",
      syncKey: fingerprint(syncAuth?.api_key),
      managedKey: fingerprint(managedAuth?.api_key),
      sameKey:
        !!syncAuth?.api_key &&
        !!managedAuth?.api_key &&
        syncAuth.api_key === managedAuth.api_key,
      chosenKey: fingerprint(auth.api_key),
      keyLength: auth.api_key.length,
    })
