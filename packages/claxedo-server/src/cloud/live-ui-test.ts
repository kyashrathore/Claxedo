 ...
 *
 * Usage:
 *   node --import tsx src/cloud/live-ui-test.ts
 *
 * Optional:
 *   CLAXEDO_SERVER_URL=http://127.0.0.1:3001 node --import tsx src/cloud/live-ui-test.ts
 *   LIVE_REPO_URL=https://github.com/kyashrathore/formlink.git node --import tsx src/cloud/live-ui-test.ts
 */
...
async function main() {
  ...
  try {
    ...
    const runnerRes = await fetch(`${base}/api/claxedo/agent-config/runner`)
    ...
    const switchRes = await fetch(`${base}/api/claxedo/agent-config/runner`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const live = await wait(async () => {
      const res = await fetch(`${base}/api/claxedo/agent-config/runner?directory=${encodeURIComponent(item.directory)}`)
      if (!res.ok) return
