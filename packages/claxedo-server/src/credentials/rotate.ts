 ...
 *   scheme drift fails loudly instead of silently duplicating a secret into a
 *   new slot and orphaning the old one.
...
export async function rotateEnvelopeKeys(input: {
  ...
  for await (const item of input.items as AsyncIterable<EnvelopeRotationItem>) {
    ...
    try {
      ...
      record({ ref: item.ref, orgId, outcome: "failed", error: describe(error) })
    }
  }
