...
function fakeDb(seed: Row[] = []) {
  ...
  return {
    rows,
    db: {
      // W5: index names are resolved against the real convex/schema.ts, so a
      // query naming an index that does not exist — or comparing fields out of
      // index order — throws instead of degrading to an unfiltered scan.
