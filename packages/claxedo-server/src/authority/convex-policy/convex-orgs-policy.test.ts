...
function db(seed: Record<string, Row[]>) {
  ...
  return {
    ...
    // W6.3: the applier now WRITES on paths these tests exercise — a webhook
    // liveness stamp (`clerk_sync_state`), a revocation tombstone, and an audit
    // event — so a double without `insert` no longer models it. Auto-creating
    // the table keeps each test's seed limited to the rows it actually asserts
    // on rather than every table the applier may touch.
