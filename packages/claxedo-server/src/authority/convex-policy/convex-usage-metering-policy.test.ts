...
/**
 * Multi-table fake. Index names are RESOLVED against the real
 * convex/schema.ts — see the note in the query builder below. They used to be
 * ignored so that a pure index rename could not break these arithmetic tests,
 * but the same blindness meant a query could name an index that does not exist
 * and still pass, which is the one thing the W5 conversions needed checked.
 * A rename is a schema edit; this suite failing on one is correct.
 */
function fakeDb(seed: Record<string, Row[]> = {}) {
  ...
  // W5: the rollup now reads its pending queue as a `by_rolled_up_at` range
  // (`.eq(…, undefined)` = "not yet consumed") and the reader picks
  // `by_org_date`/`by_date`, so this double resolves index names against the
  // real convex/schema.ts and enforces index-order stepping
  // (`convex-index-harness.ts`). Its comparisons also follow Convex's value
  // ordering, where a MISSING field sorts below every value — the previous
  // hand-rolled `>=`/`<=` on `undefined` returned false for every row, which
  // would make the pending-queue range silently read nothing.

  const selection = (table: string, checks: Array<(row: Row) => boolean>, index?: string) => {
    const rows = () => {
      const found = [...rowsOf(table).values()].filter((row) => checks.every((check) => check(row)))
      return index ? orderByIndex(found, table, index) : found
    }
    const chain = {
      ...
      filter: (build: (query: {
        ...
        and: (...predicates: Array<(row: Row) => boolean>) => (row: Row) => boolean
      }) => (row: Row) => boolean) => {
        checks.push(build({
          field: (field) => field,
    ...
    db: {
      query: (table: string) => ({
        withIndex: (name: string, build: (q: unknown) => unknown) => {
          ...
          build(builder)
          return selection(table, checks, name)
        },
        ...selection(table, []),
      }),
...
describe("user_activated is an idempotent check-and-set", () => {
  ...
  test("an error turn then an ok turn fires exactly once; a third turn does not re-fire", async () => {
    ...
    const run = (input: Record<string, unknown>) =>
      handler(recordLlmTurn)({ db } as never, {
        ...SERVICE,
        ...TURN,
  ...
  test("an exact Session message retry records usage once", async () => {
    ...
    const first = await handler(recordLlmTurn)({ db } as never, input as never)
    const retry = await handler(recordLlmTurn)({ db } as never, input as never)
...
describe("hosted WorkGraph usage attribution", () => {
  test("derives Run ownership from the deterministic Session ID", async () => {
    ...
    await expect(handler(resolveWorkGraphAttribution)({ db } as never, {
      ...SERVICE,
      org_id: "org_1",
      user_id: "user_sub_1",
  ...
  test("rejects a Run owned by another organization", async () => {
    const { db } = fakeDb({
      orgs: [{ _id: "org_internal", clerk_org_id: "org_1" }],
      users: [{ _id: "user_internal", clerk_subject: "user_sub_1" }],
      org_memberships: [{ _id: "membership_1", org_id: "org_internal", user_id: "user_internal" }],
      workgraph_runs: [{
        _id: "run_row",
