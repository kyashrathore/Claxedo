...
function db(seed: Record<string, Row[]>) {
  ...
  return {
    rows,
    // W5: the billing sweeps now range `by_polar_customer_id`,
    // `by_billing_reconcile_flagged_at` and `by_polar_subscription_id` instead
    // of scanning `orgs`, so index names resolve against the real
    // convex/schema.ts and ranges must step fields in index order
    // (`convex-index-harness.ts`). The `.gt(field, undefined)` ranges those
    // conversions rely on — "rows that have this optional field at all" — are
    // modelled with Convex's value ordering, not JS comparison.
    query(table: string) {
      ...
      const query = {
        ...
        async collect() {
          return (rows[table] ?? []).filter((row) => checks.every((check) => check(row)))
        },
