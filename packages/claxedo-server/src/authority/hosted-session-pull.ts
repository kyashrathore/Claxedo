...
export async function pullHostedControlSessionMessages(
  ...
  // Registration can precede the runtime's asynchronous auto-title. The
  // completed-turn checkpoint therefore refreshes metadata and transcript as
  // one authoritative pull, so a reload cannot regress a settled title to the
  // raw session id stored by the earlier register request.
...
async function syncHostedSessionMetadata(
  ...
  target: Awaited<ReturnType<typeof hostedWorkspaceForPull>> & Awaited<ReturnType<typeof resolveWorkspaceRuntimeTarget>>,
  sessionId: string,
  session: unknown,
) {
