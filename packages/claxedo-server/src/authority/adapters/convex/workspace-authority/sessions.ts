...
export function sessionAuthority(input: ConvexAuthorityInput): PrivateSessionAuthority {
  ...
  return {
    ...
    async listSessions(auth, value) {
      return requireExecutor(input, auth).query(convexApi.privateSessions.list, { workspace_id: value.workspaceId })
    },
  ...
  return {
    ...
    async listSessions(
      ...
      return requireExecutor(input, auth).query(convexApi.sessions.list, {
        workspace_id: args.workspaceId,
      }) as Promise<AuthoritySessionInventoryRow[]>
    },
