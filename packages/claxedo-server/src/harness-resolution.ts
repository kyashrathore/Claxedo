import { harnessHostForHarness, type HarnessHost } from "./architecture"
import { defaultHarness, loadUserConfig } from "./agent-config"
import { sessionMeta } from "./session-meta"
import { getSessionConfig, normalize, type SessionHarness } from "./session-harness"
...
export async function resolveHarnessHostForRequest(input: Input = {}): Promise<HarnessHost> {
  return harnessHostForHarness(await resolveHarnessForRequest(input))
}
