...
import { createControlPlaneServices, type ControlPlaneServices } from "./control-plane/services"
import { ControlPlaneSessionRoutes } from "./routes/control-plane-session"
import { controlPlaneTrpcHandler } from "./control-plane/trpc"
...
export function createApp(services: ControlPlaneServices) {
  const app = new Hono()
  ...
  mount(app, "/", AgentSessionRoutes(services))
  ...
  app.all("/api/control/trpc/*", async (c) => controlPlaneTrpcHandler(services)(c.req.raw))
  ...
  // Agent hook routes (terminal-session, agent-lifecycle, setup)
...
export function startServer(port = 3001, opencodeUrl = "http://127.0.0.1:4096", opencodePassword?: string | null) {
  ...
  const services = createControlPlaneServices({
    sync: createSyncDB({ mode: getSessionWriteMode }),
  })
  configureHarnessHost(createPiHost(services.projectionStore))
  initPostHog()
  mirrorProcessEvents()
  ...
  try {
    ...
    built.app.route("/api/workgraph", workgraphApp)
  } catch (err) {
