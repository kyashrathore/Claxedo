...
describe("workspace store integrity", () => {
  ...
  describe("local + cloud project grouping", () => {
    test("local checkout is not demoted to sandbox when cloud workspace has workspace_name=main", async () => {
      ...
      // The local checkout directory must be the project worktree, not a sandbox.
      // A cloud workspace with workspace_name="main" must not steal the root.
      expect(proj!.worktree).toBe(local.directory)
      expect(proj!.workspaces[local.id]?.directory).toBe(local.directory)
      expect(proj!.sandboxes).not.toContain(local.id)
    })
    ...
    test("both local and cloud workspaces appear under one project", async () => {
      ...
      const cloudRef = cloudRuntimeDirectory("ws_dual_cloud")
      await mod.ensureWorkspace({
      ...
      // The combined set of worktree + sandboxes must cover the local directory
      // and cloud workspace identity.
      const allWorkspaceRefs = [proj!.worktree, ...proj!.sandboxes]
      expect(allWorkspaceRefs).toContain(git.dir)
      expect(allWorkspaceRefs).toContain("ws_dual_cloud")
      expect(proj!.workspaces.proj_dual?.directory).toBe(git.dir)
    })
  ...
  describe("main() root selection", () => {
    test("self-root workspace (id===project_id) takes priority over workspace_name=main", async () => {
      ...
      await mod.ensureWorkspace({
        ...
        project_id: "proj_priority",
        workspace_name: "main",
        directory: cloudRuntimeDirectory("ws_priority_cloud"),
        kind: "cloud",
      })
      ...
      expect(proj!.workspaces.proj_priority?.directory).toBe(git.dir)
    })

    test("when only cloud workspaces exist, workspace_name=main is fine as root", async () => {
      const cloudMain = cloudRuntimeDirectory("ws_oc_main")
      const cloudFeat = cloudRuntimeDirectory("ws_oc_feat")
      ...
      expect(proj).toBeDefined()
      expect(proj!.worktree).toBe("ws_oc_main")
      expect(proj!.sandboxes).toContain("ws_oc_feat")
    })
  })
  ...
  describe("real-world scenario: local dev + cloud sandbox", () => {
    test("reproduces the Claxedo bug: local checkout must not become 'opencode' sandbox", async () => {
      ...
      const cloudRef = cloudRuntimeDirectory("ws_clax_cloud")
      await mod.ensureWorkspace({
      ...
      expect(proj.worktree).toBe(git.dir)
      expect(proj.workspaces.proj_clax?.directory).toBe(git.dir)

      // The cloud workspace id is a sandbox, not the worktree
      expect(proj.sandboxes).toContain("ws_clax_cloud")

      // The local checkout identity must NOT appear in sandboxes
      expect(proj.sandboxes).not.toContain("proj_clax")
    ...
    test("reproduces the /workspace orphan bug", async () => {
      ...
      expect(good!.workspaces.proj_real?.directory).toBe(git.dir)
    })
  ...
  describe("adding a project", () => {
    test("first workspace for a repo creates a project with unique id", async () => {
      ...
      const projects = await mod.listProjects()
      const proj = projects.find((p) => p.worktree === ws.directory)
      expect(proj).toBeDefined()
  ...
  describe("adding a local workspace (worktree)", () => {
    ...
    test("local worktree appears as sandbox under the same project, not as separate project", async () => {
      ...
      expect(proj.sandboxes).toContain(wt.id)
    })
    ...
    test("multiple local worktrees all group under one project", async () => {
      ...
      const wt1 = defined(await mod.ensureWorkspace({
        project_id: main.project_id,
        workspace_name: "wt1",
        directory: sb1,
      }))
      const wt2 = defined(await mod.ensureWorkspace({
        project_id: main.project_id,
        workspace_name: "wt2",
        directory: sb2,
      }))
      ...
      expect(proj.sandboxes).toContain(wt2.id)
  ...
  describe("adding a cloud workspace under a project", () => {
    test("cloud workspace with explicit project_id groups under that project", async () => {
      ...
      const cloudId = `ws_${Date.now().toString(36)}`
    ...
    test("cloud workspace appears as sandbox, not as project worktree", async () => {
      ...
      const cloudId = `ws_${Date.now().toString(36)}`
      const cloudRef = cloudRuntimeDirectory(cloudId)
      const cloud = defined(await mod.ensureWorkspace({
        workspaceId: cloudId,
        project_id: projectId,
        workspace_name: "main",
      ...
      // Local checkout directory is the project worktree
      expect(proj!.worktree).toBe(main.directory)
      expect(proj!.workspaces[main.id]?.directory).toBe(main.directory)
      // Cloud workspace is a sandbox
      expect(proj!.sandboxes).toContain(cloud.id)
      // Not the other way around
      expect(proj!.sandboxes).not.toContain(main.id)
    })

    test("cloud workspace has kind=cloud and driver set", async () => {
    ...
    test("cloud workspace remote_directory does not become a separate workspace", async () => {
      ...
      expect(all.map((w) => w.directory).sort()).toEqual(["/workspace", git.dir].sort())
    })
    ...
    test("multiple cloud workspaces under one project", async () => {
      ...
      const cloudMain = `ws_${Date.now().toString(36)}a`
      const cloudDev = `ws_${Date.now().toString(36)}b`

      await mod.ensureWorkspace({
        workspaceId: cloudMain,
        project_id: projectId,
        workspace_name: "main",
    ...
    test("cloud-first project: cloud workspace created before local checkout", async () => {
      ...
      const cloudRef = cloudRuntimeDirectory(projectId)
      ...
      expect(proj).toBeDefined()
      expect(proj!.worktree).toBe(projectId)
      ...
      proj = matching[0]!
      // Both workspace refs are in the project.
      const allWorkspaceRefs = [proj.worktree, ...proj.sandboxes]
      expect(allWorkspaceRefs).toContain(local.id)
      expect(allWorkspaceRefs).toContain(projectId)
      expect(proj.workspaces[local.id]?.directory).toBe(local.directory)
    })
  })
  ...
  describe("re-adding a closed project", () => {
    ...
    test("listProjects recovers all workspaces regardless of creation order", async () => {
      ...
      const worktree = defined(await mod.ensureWorkspace({
        project_id: projectId,
      ...
      // All 3 workspace refs must be accounted for (1 local worktree + 2 sandboxes)
      const allWorkspaceRefs = [proj!.worktree, ...proj!.sandboxes]
      expect(allWorkspaceRefs).toHaveLength(3)
      expect(allWorkspaceRefs).toContain(local.id)
      expect(allWorkspaceRefs).toContain(worktree.id)
      expect(allWorkspaceRefs).toContain(cloudId)
      expect(proj!.workspaces[local.id]?.directory).toBe(local.directory)
    })
