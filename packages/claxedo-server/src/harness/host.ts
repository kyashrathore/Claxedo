...
function row(input: Awaited<ReturnType<SyncDB["session_meta"]>>) {
...
class PlaceholderAdapter implements AgentAdapter {
  ...
  async listSessions(directory: string) {
    return (await this.sync.list_session_metas({
  ...
  async getSession(id: string) {
    return row(await this.sync.session_meta(id))
  }

  async createSession(directory: string, title?: string) {
    const id = randomUUID()
    await this.sync.put_session_meta(id, {
      ws: this.ws,
      directory,
      title: title ?? null,
    })
  ...
  async updateSession(id: string, input: { title?: string; time?: { archived?: number } }) {
    const hit = await this.sync.session_meta(id)
    if (!hit) return null
    await this.sync.put_session_meta(id, {
      ws: this.ws,
  ...
  async deleteSession(id: string) {
    await this.sync.delete_session_meta(id)
  ...
  async getMessages(id: string) {
    return this.sync.read_session_messages(id)
  }
  ...
  async forkSession(id: string) {
    ...
    await this.sync.put_session_meta(next, {
      ws: this.ws,
      directory: hit.directory,
...
export function getHarnessHost(sync: SyncDB): HarnessHost {
  if (state.host) return state.host
  return {
    id: "placeholder",
    createAdapter: async (ws) => new PlaceholderAdapter(ws, sync),
  }
}
