...
async function sse(url: string, match: (event: any) => boolean, ms = 8_000) {
...
async function setAcpRunner(type: "claude-acp" | "codex-acp" | "cursor-acp") {
  await agent.saveUserConfig({
    ...
    runner: { type, binary: fakeBinaryPath },
  })
...
describe("agent lifecycle integration", () => {
  ...
  describe("cursor-acp runner lifecycle", () => {
    it("full session CRUD and message send through fake ACP binary (with acp arg)", async () => {
      ...
      // Create session (ACPAdapter.spawnArgs() adds ["acp"] for cursor — fake binary ignores it)
      const createRes = await fetch(`${base()}/session?${q(ws)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  describe("cross-runner session isolation", () => {
    ...
    it("binds discovered sessions to the runner that listed them so all session APIs keep working", async () => {
      ...
      const plainGetRes = await fetch(`${base()}/session/${encodeURIComponent("ses_discovered")}`)
      expect(plainGetRes.status).toBe(200)
      const plainGot = await plainGetRes.json() as { id: string; title: string | null }
      expect(plainGot.id).toBe("ses_discovered")
      expect(plainGot.title).toBe("Discovered Upstream Session")
    ...
    it("keeps the binding aligned with the freshest discovered runner when duplicate session ids appear across runners", async () => {
      ...
      const runner = await import("./session-runner.js")
      runner.setSessionRunner(ws.id, "ses_duplicate", {
        type: "claude-acp",
    ...
    it("revalidates workspace runner status after ACP failures recover", async () => {
      ...
      await agent.saveUserConfig({
        ...
        runner: {
          type: "claude-acp",
          binary: path.join(root, "missing-acp"),
        },
      })
      await hooks.rebuildOpencodeJsonc()

      const fail = await fetch(`${base()}/api/claxedo/agent-config/runner?${q(ws)}`)
      ...
      const pass = await fetch(`${base()}/api/claxedo/agent-config/runner?${q(ws)}`)
  ...
  describe("lifecycle events fire through SSE", () => {
    it("agent.lifecycle Start event is normalized to Busy (terminal hook path)", async () => {
      ...
      await fetch(`${base()}/api/claxedo/process?${q(ws)}`)
    ...
    it("ACP message send emits Busy and Idle lifecycle events on claxedo bus", async () => {
      ...
      const collecting = (async () => {
        const res = await fetch(`${base()}/api/claxedo/events`, {
          headers: { Accept: "text/event-stream" },
          signal: ctrl.signal,
        })
      ...
      // Give the async bridge time to publish
