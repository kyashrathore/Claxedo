...
import { onSessionStopped, openSqliteEventStore, openSqliteExecutionStore, type ExecutionAdapter } from "@claxedo/workgraph"
...
function shared(kind: string) {
  return ["research", "docs", "design", "review", "triage", "captain"].includes(kind)
}
...
async function createIsolatedWorktree(
  ...
  try {
    const res = await opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/experimental/worktree`, {
      method: "POST",
      headers: {
...
export function createWorkGraphExecution(
  ...
  const store = openSqliteExecutionStore(db, openSqliteEventStore(db))
  const execution: ExecutionAdapter = {
    async launch(input) {
      ...
      if (isolated) {
        sessionDir = await createIsolatedWorktree(dir, input.title, opencodeRequest, opencodeEvents)
        worktreePath = sessionDir
      }

      // POST /session { directory, title } — x-opencode-directory sets Instance context
      const sessionRes = await opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/session`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      void opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/session/${session.id}/message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-opencode-directory": sessionDir,
      ...
      if (input.hidden) {
        void opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/session/${session.id}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            "x-opencode-directory": sessionDir,
          ...
          console.warn("[workgraph] failed to archive hidden session", err)
        })
      }
      ...
      const stop = async (message: string) => {
        ...
        if (!input.node_id) {
          // Planner sessions no longer exist (flat model, plan 2026-07-06-004);
          // sessions without a node are not attempt-backed — nothing to settle.
          return
        }
    ...
    async cleanup(input) {
      ...
      if (input.session_id) {
        await opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/session/${input.session_id}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            "x-opencode-directory": dir,
      ...
      if (input.pty_id) {
        await opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/pty/${input.pty_id}`, {
          method: "DELETE",
      ...
      await opencodeRequest(new Request(`${OPENCODE_INTERNAL_BASE}/experimental/worktree`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "x-opencode-directory": dir,
