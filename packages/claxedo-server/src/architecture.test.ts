...
describe("harness-scoped resolution", () => {
  ...
  test("keeps launch architecture workspace-only", () => {
    expect(getHarnessMode()).toBe("workspace")
    expect(getWorkspaceProfile()).toBe("workspace")
  ...
  test("keeps sandbox drivers from becoming sandbox remote-control APIs", () => {
    ...
    // Matched at a word START, not as a bare substring: `pty` is a real word and
    // a bare `includes` flags the `pty` inside "empty" in ordinary prose. Still
    // catches every way a driver could grow the capability (`pty`, `ptyProcess`,
    // `.pty(`), because an identifier always begins on a word boundary.
    const forbiddenTermPattern = (term: string) => new RegExp(`\\b${term}`)
    ...
    const offenders = walk(driversSrc)
      ...
      .flatMap((file) => {
        ...
        return [
          ...forbiddenTerms
            .filter((term) => forbiddenTermPattern(term).test(text))
  ...
  test("keeps server route modules mounted or explicitly accounted for", () => {
    ...
    expect(routeFiles).toEqual([
      ...
      "event-visibility.ts",
      "events.ts",
      // Pure parsing/sanitization behind project-remote.ts; no Hono app of its own.
      "git-remote-derivation.ts",
      // Shared harness/provider catalog constants; no Hono app of its own. Mirrored
      // by claxedo-app's test-support copy, whose provider-id arrays
      // harness-provider-contract.test.ts asserts stay in sync with this one.
      ...
      "opencode-compat.ts",
      // Local-only: derives the cloud clone source from a folder on this
      // machine, so server.ts mounts it and hosted-app.ts does not.
      "project-remote.ts",
      "provider-auth.ts",
      "remote-access.ts",
      "repository-clone.ts",
