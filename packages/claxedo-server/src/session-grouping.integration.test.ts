...
async function createSession(ws: { id: string; directory: string; kind?: string }, title: string) {
  ...
  if (ws.kind === "cloud") {
    await syncHostedSessions(ws, [...upstreamSessions.values()].filter((item) => item.directory === ws.directory))
  }
  return session
}
