...
export type PolarProductConfig = {
  /**
   * Our per-seat product ids (CLAXEDO_POLAR_PRODUCT_MONTHLY/_YEARLY).
   * Subscriptions for every other product are ignored.
   */
  knownProductIds: ReadonlySet<string>
  /**
   * Sandbox deployments may intentionally omit product ids while testing
   * webhook and reconciliation flows. Production keeps this false so an
   * empty allowlist cannot grant or revoke entitlements.
   */
  allowAllProductsWhenUnconfigured: boolean
}

function relevantProduct(subscription: Rec, config: PolarProductConfig): boolean {
  if (config.knownProductIds.size === 0) return config.allowAllProductsWhenUnconfigured
...
export function customerStateToApplyArgs(
  ...
  // A full-state write downgrades mirrored orgs absent from org_states. Reject
