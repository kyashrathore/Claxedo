...
describe("Agent Config Agent Extensions routes", () => {
  ...
  test("workspace scope stores desired installs through the Control Plane boundary without depending on telemetry", async () => {
    const convex = {
      ...
      authorizeSessionWrite: async () => {},
      grantSessionParticipant: async () => ({ participant_id: "participant_1" }),
      revokeSessionParticipant: async () => ({ removed: true }),
      ...
      bindChannelIdentity: async () => ({ bindingId: "chn_1", created: true, userId: "user_1", actorId: "user_1", actorKind: "human" as const }),
      revokeChannelIdentity: async () => ({ revoked: true }),
      openWorkspace: async () => ({ allowed: true }),
      listWorkspaces: async () => [],
      registerLocalForSharing: async () => ({}),
      createHostEnrollmentRequest: async () => ({ request_id: "req_1", nonce: "nonce_1", expires_at: 1 }),
      enrollHost: async () => ({
        enrollment_id: "enr_1",
        host_id: "host_1",
        expires_at: 1,
      ...
      heartbeatHostEnrollment: async () => ({ expires_at: 1, last_seen_at: 1, assigned_workspace_ids: [] }),
      pauseHostEnrollment: async () => ({ paused: true }),
      activeHostEnrollment: async () => ({ active: false as const, reason: "not-enrolled" as const }),
      assignWorkspaceHost: async () => ({ assigned: true as const, workspace_id: "ws_1", host_id: "host_1" }),
      unassignWorkspaceHost: async () => ({ unassigned: true }),
      activeWorkspaceHost: async () => ({ active: false as const }),
      listHostAssignments: async () => [],
      markSecondDeviceOpen: async () => ({ recorded: false, second_device_open_at: 1 }),
