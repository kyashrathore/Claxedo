...
export async function syncClerkWorkGraphFixtures(env: SmokeEnvironment = process.env, request: typeof fetch = fetch) {
  ...
  const [userRecords, organizationRecords, membershipPages] = await Promise.all([
    ...
    Promise.all(
      organizations.map((id, index) => clerkMemberships(request, clerkSecret, id, index === 0 ? users : [users[0]!])),
    ),
  ])
  ...
  for (let offset = 0; ; offset += limit) {
    const page = record(
      await clerkGet(
        request,
        secret,
        `/v1/organizations/${encodeURIComponent(organizationId)}/memberships?limit=${limit}&offset=${offset}`,
      ),
    )
    const data = array(page?.data)
    memberships.push(...data)
    if (
      requiredUserIds.every((userId) =>
        memberships.some(
          (membership) =>
            membershipOrganizationId(membership) === organizationId && membershipUserId(membership) === userId,
        ),
      )
    ) {
      return memberships
    }
    if (!data.length || offset + data.length >= (typeof page?.total_count === "number" ? page.total_count : 0))
      return memberships
  }
}
