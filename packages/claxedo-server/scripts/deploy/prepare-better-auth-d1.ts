...
export function betterAuthD1PreparationTransition(env: NodeJS.ProcessEnv) {
  ...
  if (previousPhase !== "locked" && previousPhase !== "open") {
    throw new Error("CLAXEDO_PREVIOUS_PHASE must be locked or open")
  }
  return {
...
async function run(command: PreparationCommand) {
  ...
  const child = spawn(executable, command.args, {
    cwd: serverRoot,
    env: process.env,
    stdio: command.verify ? ["ignore", "pipe", "inherit"] : "inherit",
...
async function main() {
  const modes = ["register-candidate", "activate-candidate", "rollback-candidate"] as const
