...
const checkedGuideEnvironment = Object.freeze({
  ...
  CLAXEDO_PRODUCTION_WORKSPACE_RELAY_URL: "https://relay.example.com",
  CLAXEDO_STAGING_WORKSPACE_RELAY_URL: "https://relay-staging.example.com",
...
bun run scripts/deploy/release-better-auth-d1.ts --cutover${stagingFlag}
...
bun run scripts/deploy/provision-user-deployed-owner-claim.ts --provision${stagingFlag}
...
export CLAXEDO_CUTOVER_RECEIPT_ID='<unique immutable receipt ID>'
export CLAXEDO_CUTOVER_OPERATION_ID='<unique idempotent operation ID>'
bun run scripts/deploy/cutover-better-auth-d1.ts --begin-canary${stagingFlag}
\`\`\`
...
# multiplayer_identity receipts. Use a different ${providerNames} account in each window.
