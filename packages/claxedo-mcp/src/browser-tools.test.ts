...
describe("browser-tools: input schemas", () => {
  ...
  test("browser_navigate accepts only http and https schemes", () => {
    const shape = z.object(browserNavigateSchema as Record<string, z.ZodTypeAny>)
    for (const url of [
      "file:///etc/passwd",
      "javascript:alert(1)",
      ...
      expect(shape.safeParse({ pane_id: "p1", url }).success, url).toBe(false)
    }
    for (const url of ["http://example.com", "https://example.com/a?b=1#c"]) {
      expect(shape.safeParse({ pane_id: "p1", url }).success, url).toBe(true)
    }
  })
})
