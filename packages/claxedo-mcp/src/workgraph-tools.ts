...
export type ToolResult = Readonly<{ content: readonly Readonly<{ type: "text"; text: string }>[]; isError?: boolean }>
type Register = (
  name: string,
  config: Readonly<{ description: string; inputSchema: Record<string, z.ZodTypeAny> }>,
  handler: (input: Record<string, unknown>) => Promise<ToolResult>,
) => void
// `Promise<unknown>` rather than `<Result>(…) => Promise<Result>`: the generic
// form let a caller name any return type and get it unchecked, which is exactly
// the assumption every response here disproves — each one is parsed by a
// contract schema before use.
...
export const WORKGRAPH_CAPABILITY_MAP = [
  ...
  { uiAction: "Cancel a Run", tool: "workgraph_cancel", mutating: true },
...
export async function callWorkGraph(
  ...
  rejectUndeclaredKeys(tool, input)
...
// Names that must never appear as caller input on any tool, matched on a
// punctuation-stripped lowercase form so `owner_id`, `ownerId` and `owner-id`
// all land. `subject` is deliberately absent: it is a declared key on the three
// evidence tools, and rejectUndeclaredKeys already refuses it everywhere else.
const TENANT_SELECTOR_NAMES = [
  "owner",
  "ownerid",
  "owneruserid",
  "ownersubject",
...
function rejectTenantSelectors(input: Record<string, unknown>) {
  const selector = Object.keys(input).find((key) =>
    TENANT_SELECTOR_NAMES.includes(key.replace(/[^a-z0-9]/gi, "").toLowerCase()))
...
/**
 * The property the denylist was reaching for: a caller may only send keys the
 * tool actually declares. `toCommandRequest` already picks fields explicitly,
 * so an undeclared key could not reach a command — but that made the guard
 * depend on every future assembler staying exhaustive. Asserting the subset
 * here fails the request outright instead, and covers impersonation-shaped
 * names nobody thought to enumerate.
 */
function rejectUndeclaredKeys(tool: keyof typeof WORKGRAPH_TOOL_SCHEMAS, input: Record<string, unknown>) {
  const declared = WORKGRAPH_TOOL_SCHEMAS[tool] as Record<string, z.ZodType>
  const undeclared = Object.keys(input).filter((key) => !Object.hasOwn(declared, key))
  if (undeclared.length) {
    throw new Error(`WorkGraph tool ${tool} does not accept ${undeclared.map((key) => `'${key}'`).join(", ")}; allowed keys: ${Object.keys(declared).sort().join(", ") || "(none)"}`)
  }
}
