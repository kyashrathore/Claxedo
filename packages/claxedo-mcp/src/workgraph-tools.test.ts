...
describe("WorkGraph MCP parity", () => {
  ...
  it("rejects impersonation-shaped selectors the original denylist missed", async () => {
    const request = vi.fn(async () => ({}))
    for (const selector of [
      "sub",
      "account",
  ...
  it("rejects any key the tool does not declare, before transport", async () => {
    const request = vi.fn(async () => ({}))
    await expect(callWorkGraph(request, "workgraph_create_stream", {
      operation_id: "operation-1",
      title: "Launch",
      unexpected_field: "value",
    })).rejects.toThrow("workgraph_create_stream does not accept 'unexpected_field'")
    expect(request).not.toHaveBeenCalled()
  })

  it("accepts 'subject' only on the tools that declare it", async () => {
    const request = vi.fn(async () => ({}))
    // Declared on the evidence tools, so the denylist must not blanket-ban it.
    expect(Object.keys(WORKGRAPH_TOOL_SCHEMAS.workgraph_record_finding)).toContain("subject")
    await expect(callWorkGraph(request, "workgraph_create_stream", {
      operation_id: "operation-1",
      title: "Launch",
      subject: { kind: "work_item", workItemId: "work-item-1" },
    })).rejects.toThrow("workgraph_create_stream does not accept 'subject'")
    expect(request).not.toHaveBeenCalled()
  })
    ...
    expect(request).toHaveBeenCalledWith("/api/workgraph/commands", expect.objectContaining({
      body: JSON.stringify({
        ...
        command: {
          ...
          title: "Cloud project",
          execution: {
            // placement is materialized by WorkGraphCommandRequestSchema's .default("shared")
  ...
  it("uses the exact create Stream command payload and result cursor as the UI HTTP boundary", async () => {
    ...
    // The double takes the command it is handed so both boundaries call it the
    // same way; the assertion below proves the two payloads are identical.
    const execute = vi.fn(async (_command: unknown) => ({ ok: true as const, operationId: "operation-1", cursor: "7", value: { streamId: "stream-1" } }))
