...
- Runtime-neutral contracts cover Streams, Tasks (`Work Item` in the service contract), optional Outcomes, Attempts, Decisions, Recaps, Work Sources, execution profiles, completion evidence, events, changes, and lifecycle commands.
...
- Eight-hour incremental Recaps run from verified local durable state. The hosted path follows the same exact tool-less Session V2 publication contract and remains part of final server/Convex verification.
...
- Targeted proposal, candidate, Task/Attempt, Decision, Recap, and Evidence reads are implemented locally and hosted. Background source planning and Recaps require explicit configured profiles and publish only valid Session output.
