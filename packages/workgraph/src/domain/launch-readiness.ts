...
import { resolvedPlacement } from "../contracts/execution"
...
export interface WorkItemLaunchabilityInput {
  ...
  readonly stream: Readonly<{
    ...
    readonly placement?: "shared" | "worktree" | "sandbox"
...
export function evaluateWorkItemLaunchability(input: WorkItemLaunchabilityInput): WorkItemLaunchability {
  if (input.state === "draft") return unlaunchable("draft")
  ...
  if (resolvedPlacement(input.stream.placement) === "shared" && input.stream.hasRunningRun)
    return unlaunchable("workspace_busy")
  if (
    resolvedPlacement(input.stream.placement) !== "shared" &&
    (input.stream.liveRunCount ?? 0) >= (input.stream.maxParallel ?? 1)
  )
    return unlaunchable("width_exhausted")
