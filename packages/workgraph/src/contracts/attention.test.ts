...
describe("Attention contracts", () => {
  it("round-trips an owner-bound deterministic cursor", () => {
    ...
    const cursor = createAttentionCursor("owner one", position)
    expect(readAttentionCursor(cursor, "owner one")).toEqual(position)
    expect(() => readAttentionCursor(cursor, "owner two")).toThrow(AttentionCursorError)
    expect(() => readAttentionCursor("broken", "owner one")).toThrow(AttentionCursorError)
  })
  ...
  it("orders newest items first with stable kind and id tie-breakers", () => {
    ...
    ].sort(compareAttentionPosition)).toEqual([
