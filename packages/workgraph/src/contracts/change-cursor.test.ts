...
describe("ChangeCursor", () => {
  it("round-trips an organization, owner, and filter-bound position", () => {
    ...
    expect(readChangeCursor(createChangeCursor({
      organizationId: "org:one",
      ownerUserId: "user@example.com",
      position: 42.125,
    }), "org:one", "user@example.com")).toBe(42.125)
  })
  ...
  it("rejects cross-organization same-user, cross-owner, cross-filter, and malformed reuse", () => {
    ...
    expect(() => readChangeCursor("wgc1:org_a:same_user:all:*:2", "org_a", "same_user"))
      .toThrow(expect.objectContaining<Partial<ChangeCursorError>>({ reason: "invalid" }))
