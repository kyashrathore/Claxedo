 ...
 */
export function initTriggersTable(db: SqliteDb): void {
  db.exec(`
...
export function createTrigger(db: SqliteDb, input: CreateTriggerInput): RecurringTrigger {
...
export function getTrigger(db: SqliteDb, id: string): RecurringTrigger | undefined {
  const row = db.query("SELECT * FROM triggers WHERE id = ?").get(id);
  return row ? rowToTrigger(row) : undefined;
}

export function listTriggers(db: SqliteDb): RecurringTrigger[] {
  const rows = db.query("SELECT * FROM triggers ORDER BY created_at ASC").all();
...
export function deleteTrigger(db: SqliteDb, id: string): void {
  const existing = getTrigger(db, id);
  if (!existing) throw new Error(`Trigger '${id}' not found`);
...
export function recordTriggerFired(db: SqliteDb, id: string): void {
  const existing = getTrigger(db, id);
  if (!existing) return;
...
export function listRunsByTrigger(db: SqliteDb, triggerId: string): any[] {
  return db.query("SELECT * FROM runs_current WHERE trigger_id = ? ORDER BY rowid DESC").all(triggerId);
}

/** Returns all enabled triggers whose next_run_at <= now. */
export function getDueTriggers(db: SqliteDb, now: Date = new Date()): RecurringTrigger[] {
  const rows = db.query("SELECT * FROM triggers WHERE enabled = 1 AND next_run_at IS NOT NULL AND next_run_at <= ?").all(now.toISOString());
  return rows.map(rowToTrigger);
}
