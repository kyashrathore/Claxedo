...
/**
 * Record in-subtree dependencies as blocker rows so held nodes are released
 * as their in-run dependencies complete. Complements writeBlockers (which
 * only records outside-the-run blockers). INSERT OR IGNORE keeps rows unique
 * when an in-subtree dependency was already recorded (e.g. a busy item).
 */
export function writeInternalBlockers(
  db: SqliteDb,
  runId: string,
  items: WorkItem[],
  hold: Set<string>,
  ...
  const ids = new Set(items.map((item) => item.id))
  // nodeItemMap is nodeId → itemId; we need itemId → nodeId
  const nodeFor = new Map(Array.from(nodeItemMap.entries()).map(([nodeId, itemId]) => [itemId, nodeId] as const))
  for (const itemId of hold) {
    const nodeId = nodeFor.get(itemId)
    if (!nodeId) continue
    for (const dep of wg.getBlockedBy(itemId)) {
      if (!ids.has(dep.id) || dep.status === "done") continue
      db.run(
        "INSERT OR IGNORE INTO run_blockers_current (run_id, work_item_id, target_node_id, title) VALUES (?, ?, ?, ?)",
        [runId, dep.id, nodeId, dep.title],
      )
    }
  }
}
