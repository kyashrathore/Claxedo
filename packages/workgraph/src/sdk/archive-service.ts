import { cancelAttempt, reconcileAttempts } from "./attempts";
...
import type { SpawnAgentFn } from "./attempts";
...
export async function archiveWorkItem(
  ...
  for (const row of rows) {
    cancelAttempt(executionStore, row.run_id, row.node_id, "archived");
    runs.add(row.run_id);
  ...
  for (const runId of runs) await reconcileAttempts(executionStore, runId, spawnFn);
