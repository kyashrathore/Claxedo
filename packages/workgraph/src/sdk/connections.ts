 ...
 * (packages/claxedo-connections): one connections layer, no parallel auths.
 * Kept only because the current app UI still drives /graph/providers/*;
 * dies in the app-plugin lane. Do not extend.
 *
...
/** Create an IConnectionStore backed by the given SQLite database. */
export function openSqliteConnectionStore(db: SqliteInput): IConnectionStore {
  return new SqliteConnectionStore(sqlite(db))
}
