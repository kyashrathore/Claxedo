 ...
 * TODO: replace with real chain hash (payload + previous hash).
 *
 * Two samples are concatenated so the result is never empty — Math.random()
 * can legitimately return 0, which makes toString(36).substring(2) produce "".
 */
export const generateHash = () => {
  const a = Math.random().toString(36).substring(2);
  const b = Math.random().toString(36).substring(2);
  return (a + b).substring(0, 16) || "0000000000000000";
};
 ...
 * Returns 1 for the first event on a run (1-indexed).
 */
