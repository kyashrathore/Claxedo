import type { NotificationID, NotificationPageCursor, WorkGraphContext, WorkGraphNotification, WorkGraphNotificationPage } from "../contracts"

export type NotificationStore = Readonly<{
  list(context: WorkGraphContext, input: Readonly<{ after?: NotificationPageCursor; limit: number; state?: "unread" | "read" }>): Promise<WorkGraphNotificationPage>
...
export function createNotificationService(store: NotificationStore) {
  return {
    list(context: WorkGraphContext, input: Readonly<{ after?: NotificationPageCursor; limit?: number; state?: "unread" | "read" }> = {}) {
