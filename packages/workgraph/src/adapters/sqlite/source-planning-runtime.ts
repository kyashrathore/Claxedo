...
function markPlanningQueued(database: BetterSqlite3.Database, context: WorkGraphContext, proposalId: string, now: number) {
  ...
  const failed = JSON.parse(row.proposed_work_json) as {
    ...
    diffSummary?: string
    planningEvidence?: PlanningEvidence
    generation?: { tryNumber?: number }
  }
  const generation = { method: "planning" as const, tryNumber: failed.generation?.tryNumber ?? 0, queuedAt: now }
  const changed = database.prepare(`
...
function markPlanningStarted(
  ...
  const planning = JSON.parse(row.proposed_work_json) as {
    ...
    diffSummary?: string
    planningEvidence?: PlanningEvidence
    generation?: { tryNumber?: number; queuedAt?: number }
  }
  const generation = {
    method: "planning" as const,
    tryNumber: (planning.generation?.tryNumber ?? 0) + 1,
  ...
  return database.transaction(() => {
    ...
    if (row) {
      ...
      const generation = {
        ...
        tryNumber: planning.generation?.tryNumber ?? 0,
        reason,
        failedAt: now,
        retryable,
      }
...
function appendProposalPublication(
  ...
  input: Readonly<{
    ...
    version: number
    generation: Readonly<
      | { method: "planning"; tryNumber: number }
      | { method: "planning_failed"; tryNumber: number; reason: string; retryable: boolean }
