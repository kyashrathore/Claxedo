    ...
    restore: async (context, input) => {
      ...
      return database.transaction(() => {
        ...
        records.filter(kind("stream")).forEach((record) => database.prepare(`
          INSERT INTO wg_v2_streams
            (organization_id, owner_user_id, id, workgraph_id, title, purpose, lifecycle, visibility, pinned,
             ...
             spend_json, activity_granularity, replacement_reset_json, row_version, schema_version, created_at, updated_at, closed_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          context.organizationId, context.ownerUserId,
          record.id,
          ...
          record.value.spend === undefined ? null : JSON.stringify(record.value.spend),
        ...
        records.filter(kind("work_item")).forEach((record) => database.prepare(`
          INSERT INTO wg_v2_work_items
            (organization_id, owner_user_id, id, stream_id, outcome_id, title, description, lifecycle, priority,
             ...
             origin_run_id, auto_admitted, abandoned_reason, abandoned_at, row_version, schema_version, created_at,
        ...
        `).run(
          ...
          record.value.autoAdmitted === undefined ? null : Number(record.value.autoAdmitted),
        ...
        records.filter(kind("run")).forEach((record) => database.prepare(`
          INSERT INTO wg_v2_runs
            (organization_id, owner_user_id, id, stream_id, work_item_id, run_number, generation, lifecycle,
             ...
             terminal_result_json, parked_reason, resume_attempts, completion_retry_json, row_version, schema_version,
        ...
        `).run(
          ...
          record.value.resumeAttempts,
          record.value.completionRetry === undefined ? null : JSON.stringify(record.value.completionRetry),
          record.value.version,
          record.value.schemaVersion,
...
function assertCanonicalCoverage(database: Database, context: WorkGraphContext) {
  ...
  const sequenceRows = rows(database, "wg_v2_stream_sequences", context)
  const eventScopes = database.prepare(`
    SELECT DISTINCT COALESCE(stream_id, ?) AS stream_id FROM wg_v2_events
    WHERE organization_id = ? AND owner_user_id = ?
  `).all(ownerEventScopeId, context.organizationId, context.ownerUserId) as Array<{ stream_id: string }>
  if (sequenceRows.length !== eventScopes.length) {
    throw new WorkGraphArchiveRestoreError("target_incompatible")
  }
