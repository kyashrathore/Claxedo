  ...
  return {
    async begin(signal) {
      return database.transaction(() => {
        const existing = database.prepare(`
          ...
          WHERE organization_id = ? AND connection_id = ? AND provider = ? AND delivery_id = ?
        `).get(signal.organizationId, signal.connectionId, signal.provider, signal.deliveryId) as { status: string; claim_expires_at: number | null } | undefined
        ...
        if (existing) {
          database.prepare(`
            ...
            WHERE organization_id = ? AND connection_id = ? AND provider = ? AND delivery_id = ?
          `).run(leaseId, now + 60_000, now, signal.organizationId, signal.connectionId, signal.provider, signal.deliveryId)
          return { state: "acquired" as const, leaseId }
        }
        database.prepare(`
          INSERT INTO wg_v2_webhook_deliveries
            (organization_id, connection_id, provider, delivery_id, event_type, status, claimed_by, claim_expires_at, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)
        `).run(signal.organizationId, signal.connectionId, signal.provider, signal.deliveryId, signal.event, leaseId, now + 60_000, now, now)
        return { state: "acquired" as const, leaseId }
      })()
    },
    async listSourceViews(input) {
      return (database.prepare(`
        ...
        WHERE organization_id = ? AND team_connection_id = ? AND provider = ? AND status = 'active'
        ORDER BY updated_at DESC, owner_user_id, id
        LIMIT ?
      `).all(input.organizationId, input.connectionId, input.provider, input.limit) as SourceViewRow[]).map(sourceView)
    },
    async complete(signal, leaseId) {
      return database.prepare(`
        UPDATE wg_v2_webhook_deliveries
        SET status = 'completed', claimed_by = NULL, claim_expires_at = NULL, updated_at = ?
        WHERE organization_id = ? AND connection_id = ? AND provider = ? AND delivery_id = ? AND status = 'pending' AND claimed_by = ?
      `).run(Date.now(), signal.organizationId, signal.connectionId, signal.provider, signal.deliveryId, leaseId).changes === 1
    },
