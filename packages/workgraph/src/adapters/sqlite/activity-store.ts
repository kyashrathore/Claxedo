...
function appendSqliteOwnedChange(
  ...
  const cursorPosition = allocateActivityCursor(database, context, input.occurredAt)
...
function allocateActivityCursor(database: Database, context: WorkGraphContext, occurredAt: number) {
  database.prepare(`
  ...
  const row = database.prepare(`
    SELECT next_cursor FROM wg_v2_change_cursors WHERE organization_id = ? AND owner_user_id = ?
