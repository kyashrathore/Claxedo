...
import { z } from "zod"
import { getWorkGraph, getReadyWork } from "../model/registry"
...
export function workRouter() {
  ...
  router.get("/work", async (c) => {
    ...
    items = items.filter((item) => {
      ...
      if (lifecycle === "archived") return !!item.archivedAt && !item.deletedAt
      if (lifecycle === "deleted") return !!item.deletedAt
  ...
  router.get("/work/:id", async (c) => {
    ...
    const life = c.req.query("lifecycle")
    if (!item || (item.deletedAt && life !== "deleted" && life !== "all")) {
  ...
  router.delete("/work/:id", async (c) => {
    ...
    try {
      wg.remove(id)
      return c.json({ deleted: true, item: wg.get(id) })
    } catch (err: any) {
      return c.json({ error: err.message }, 404)
    }
  })

  router.post("/work/:id/archive", async (c) => {
    const id = c.req.param("id")
    const wg = getWorkGraph()
    try {
      return c.json(wg.archive(id))
    } catch (err: any) {
      return c.json({ error: err.message }, 404)
    }
  })
