...
export class WorkGraph {
  ...
  constructor(dbPath?: string) {
    ...
    const items = getAllItems(this.db).filter((item) => !item.deletedAt && this.exec(item))
    const ids = new Set(items.map((item) => item.id))
    const edges = getAllEdges(this.db).filter((edge) => ids.has(edge.source) && ids.has(edge.target))
    ...
    for (const item of getAllItems(this.db)) {
  ...
  create(input: {
    ...
    const parent = input.parentId ? dbGetItem(this.db, input.parentId) : null
  ...
  update(id: string, changes: Partial<Omit<WorkItem, "id" | "createdAt">>): WorkItem {
    ...
    const parent = parentId ? dbGetItem(this.db, parentId) : null
  ...
  getBySlice(sliceId: string): WorkItem[] {
    const ids = new Set(getSliceItems(this.db, sliceId))
    return getAllItems(this.db).filter((item) => ids.has(item.id))
  }

  getSlices(id: string): string[] {
    return getItemSlices(this.db, id)
  }

  linkSlice(id: string, sliceId: string): void {
    if (!dbGetItem(this.db, id)) throw new Error(`WorkItem '${id}' not found`)
    linkItemSlice(this.db, id, sliceId)
  }
  ...
  addDep(blocking: string, blocked: string): void {
    const source = dbGetItem(this.db, blocking)
    const target = dbGetItem(this.db, blocked)
  ...
  getChildren(id: string): WorkItem[] {
    return getAllItems(this.db).filter((item) => item.parentId === id && !item.deletedAt)
  }
  ...
  async pushStatus(id: string, connector: ConnectorInterface): Promise<void> {
    ...
    const all = getAllItems(this.db).filter((node) => this.binding(node, connector.provider)?.key === binding.key)
  ...
  private syncMissions() {
    const items = getAllItems(this.db).filter((item) => !item.deletedAt)
    ...
    const walk = (item: WorkItem): WorkItem["status"] => {
      ...
      dbUpdateItem(this.db, item.id, { status: next, updatedAt: new Date().toISOString() })
      item.status = next
