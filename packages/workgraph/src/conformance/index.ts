...
export const WORKGRAPH_ADAPTER_CONFORMANCE_VERSION = 8 as const
...
export const WORKGRAPH_ADAPTER_CONFORMANCE_SCOPE = {
  ...
  remaining: ["migration_export_restore_roundtrip", "workspace_cleanup_and_permanent_deletion"],
} as const
...
export type ConformanceQueries = Readonly<{
  ...
  streams: Readonly<{
    read: (context: WorkGraphContext, input: Readonly<{ streamId: StreamID }>) => Promise<ConformanceStreamView | undefined>
  }>
  sources: Readonly<{
    readRevision: (
  ...
  changes: Readonly<{
    list: (context: WorkGraphContext, input: Readonly<{ after?: ChangeCursor; deliveredEventIds?: readonly string[] }>) => Promise<readonly ConformanceChangeView[]>
    listStream: (context: WorkGraphContext, input: Readonly<{ streamId: StreamID; after?: ChangeCursor }>) => Promise<readonly ConformanceChangeView[]>
  }>
  workItems: Readonly<{
    read: (context: WorkGraphContext, input: Readonly<{ workItemId: string }>) => Promise<ConformanceWorkItemView | undefined>
  }>
}>
...
export type StoreConformanceFactory = (
  ...
  Readonly<{
     ...
     */
    admit: (owner: WorkGraphContext, input: Readonly<{ workItemId: WorkItemID }>) => Promise<CommandResult>
  }>
>
...
export function workGraphAdapterConformance(factory: StoreConformanceFactory): readonly WorkGraphAdapterConformanceCase[] {
  return [
    ...
    testCase("keeps owner and per-stream cursors monotonic and supports resume-after", async () => {
      ...
      const streamId = valueId(created, "streamId") as StreamID
      const updated = await fixture.service.execute(fixture.owners.first, {
        operationId: operation("operation_2"),
        command: { version: 1, type: "update_stream", streamId, expectedVersion: 1, title: "Updated" },
      })
      assert(created.ok && updated.ok, "Cursor fixture writes failed")
      const all = await fixture.service.query(fixture.owners.first, "changes", "list", {})
      const stream = (await fixture.service.query(fixture.owners.first, "changes", "listStream", {
        streamId,
      })) as unknown as readonly ConformanceChangeView[]
      assertDeepEqual(
        all.map((change) => change.cursor),
        [created.cursor, updated.cursor],
        "Owner change cursors do not exactly match committed command cursors",
      )
      assertDeepEqual(
        stream.map((change) =>
          readChangeCursor(change.cursor, fixture.owners.first.organizationId, fixture.owners.first.ownerUserId, {
            type: "stream",
            streamId,
          }),
        ),
        [1, 2],
        "Per-stream cursors are not monotonic",
      )
      assertDeepEqual(
        all.map((change) => change.event.occurredAt),
        [1_000, 1_001],
        "Adapter ignored the supplied clock ordering",
      )
      assertDeepEqual(await fixture.service.query(fixture.owners.first, "changes", "list", { after: all[0]!.cursor }), [all[1]], "Resume-after returned the wrong change window")
    }),
    testCase("paginates a stable ordered snapshot through returned resume cursors", async () => {
      ...
      assert(
        firstPass.pages.every((page) => page.snapshotCursor === firstPass.pages[0]!.snapshotCursor),
        "Snapshot cursor changed while resuming an unchanged snapshot",
      )
      assert(
        firstPass.pages.every((page) => page.capturedAt === firstPass.pages[0]!.capturedAt),
        "Snapshot capture time changed while resuming an unchanged snapshot",
      )
      ...
      assertDeepEqual(
        firstPass.identity.map((entry) => entry.sequence),
        firstPass.identity.map((_, index) => index + 1),
        "Snapshot references were not globally ordered across pages",
      )
      assert(
        firstPass.identity.every((entry) => entry.ownerUserId === fixture.owners.first.ownerUserId),
        "Snapshot pagination leaked another owner's record",
      )
    }),
    testCase("rejects malformed, cross-owner, and mutation-invalidated snapshot resume cursors", async () => {
      ...
      await assertSnapshotCursorError(() => fixture.service.query(fixture.owners.second, "snapshot", "page", { after: page.nextCursor!, limit: 1 }), "owner_mismatch")
      await assertSnapshotCursorError(
        () =>
          fixture.service.query(fixture.owners.first, "snapshot", "page", {
            after: branded<SnapshotResumeCursor>("malformed"),
            limit: 1,
          }),
        "invalid",
      )
      ...
      await assertSnapshotCursorError(
        () =>
          fixture.service.query(fixture.owners.first, "snapshot", "page", {
            after: branded<SnapshotResumeCursor>("wgsp2:%ZZ:3:1:1000"),
            limit: 1,
          }),
        "invalid",
      )
      ...
      await assertSnapshotCursorError(() => fixture.service.query(fixture.owners.first, "snapshot", "page", { after: page.nextCursor!, limit: 1 }), "invalidated")
    }),
    testCase("delivers changes after a captured snapshot cursor in order exactly once", async () => {
      ...
      const deliveredBeforeSnapshot = await fixture.service.query(fixture.owners.first, "changes", "list", {})
      ...
      assert(created.ok && updated.ok, "Snapshot convergence fixture writes failed")
      await createStream(fixture, fixture.owners.second, "operation_1", "Other owner")

      const changes = await fixture.service.query(fixture.owners.first, "changes", "list", {
        after: snapshot.snapshotCursor,
      })
      assertDeepEqual(
        changes.map((change) => change.cursor),
        [created.cursor, updated.cursor],
        "Snapshot convergence returned missing, duplicate, unordered, or cursor-mismatched changes",
      )
      assertDeepEqual(
        changes.map((change) => change.event.type),
        ["stream_created", "stream_updated"],
        "Snapshot convergence returned the wrong owner change sequence",
      )
      ...
      assertDeepEqual(
        (
          await fixture.service.query(fixture.owners.first, "changes", "list", {
            after: changes.at(-1)!.cursor,
            deliveredEventIds: [...deliveredBeforeSnapshot, ...changes].map((change) => change.event.id),
          })
        ).map((change) => change.event.id),
        [],
        "A supplemental delivered-event window replayed an already applied change",
      )
      assertDeepEqual(
    ...
    testCase("preserves immutable Work Source revisions and reads the exact requested revision", async () => {
      ...
      const first = await fixture.service.query(fixture.owners.first, "sources", "readRevision", {
        workSourceId,
        revisionId: firstRevisionId,
      })
      const second = await fixture.service.query(fixture.owners.first, "sources", "readRevision", {
        workSourceId,
      ...
      assert(
        (await fixture.service.query(fixture.owners.second, "sources", "readRevision", {
          workSourceId,
          revisionId: firstRevisionId,
        })) === undefined,
        "Source revision leaked across owners",
      )
    }),
    testCase("surfaces semantic completion only after matching positive evidence", async () => {
      ...
      const workItem = await fixture.service.execute(fixture.owners.first, {
        ...
        command: {
          ...
          completionContract: {
            version: 1,
            mode: "all",
            requirements: [{ id: branded("requirement_1"), kind: "test", description: "Tests pass" }],
          },
        },
      })
      ...
      assert(
        (await fixture.service.query(fixture.owners.first, "workItems", "read", { workItemId }))?.completionSatisfied === true,
        "Matching positive evidence did not satisfy completion",
      )
    }),
      ...
      const workItem = await fixture.service.execute(fixture.owners.first, {
        ...
        command: {
          ...
          completionContract: {
            version: 1,
            mode: "all",
            requirements: [{ id: branded("requirement_1"), kind: "integration", description: "Merged" }],
          },
        },
      })
      const workItemId = valueId(workItem, "workItemId")
    ...
    testCase("fences Run leases across acquisition, renewal, expiry, and runtime restart", async () => {
      ...
      assert(
        (await fixture.runRuntime.renewLease(fixture.owners.second, {
          runId,
          expectedLeaseEpoch: leaseEpoch,
          occurredAt: 2_000,
          durationMs: 300_000,
        })) === undefined,
        "Another owner renewed the Run lease",
      )
      assert(
        (await fixture.runRuntime.renewLease(fixture.owners.first, {
          runId,
          expectedLeaseEpoch: leaseEpoch + 1,
          occurredAt: 2_000,
          durationMs: 300_000,
        })) === undefined,
        "A non-current epoch renewed the Run lease",
      )
      ...
      assert(
        (await restarted.runRuntime.renewLease(fixture.owners.first, {
          runId,
          expectedLeaseEpoch: leaseEpoch,
          occurredAt: 1_000_001,
          durationMs: 300_000,
        })) === undefined,
        "Recovered lease accepted its stale epoch",
      )
      assert(
        !(
          await restarted.runRuntime.requestCompletionRetry(fixture.owners.second, {
            runId,
        ...
        "Another owner requested the completion-only continuation",
      )
      assert(
        !(
          await restarted.runRuntime.requestCompletionRetry(fixture.owners.first, {
            runId,
            leaseEpoch,
            terminalSeq: 7,
            occurredAt: 1_000_001,
        ...
        "A stale lease epoch requested the completion-only continuation",
      )
      const completionRetry = await restarted.runRuntime.requestCompletionRetry(fixture.owners.first, {
        runId,
        leaseEpoch: recovered.leaseEpoch,
        terminalSeq: 7,
        occurredAt: 1_000_001,
      ...
      assertDeepEqual(
        completionRetry,
        {
          accepted: true,
          terminalSeq: 7,
          requestedAt: 1_000_001,
        },
        "Current lease holder could not persist the completion-only continuation",
      )
      assertDeepEqual(
        await restarted.runRuntime.requestCompletionRetry(fixture.owners.first, {
          runId,
        ...
        completionRetry,
        "Completion-only continuation was not durably idempotent",
      )
      assert(
        !(await restarted.runRuntime.recordResult(fixture.owners.first, {
          runId,
          workItemId,
          leaseEpoch,
          state: "result",
...
async function assertSnapshotCursorError(run: () => Promise<unknown>, reason: SnapshotResumeCursorError["reason"]) {
  try {
    await run()
  } catch (error) {
...
function isCanonicalSnapshotResumeCursorError(error: unknown): error is Error &
  Readonly<{
  ...
  return (
    ...
    "reason" in error &&
    (error.reason === "invalid" || error.reason === "owner_mismatch" || error.reason === "invalidated")
  )
}

async function snapshotPages(fixture: Awaited<ReturnType<typeof setup>>, owner: WorkGraphContext, limit: number) {
...
function createStream(fixture: Awaited<ReturnType<typeof setup>>, owner: WorkGraphContext, operationId: string, title: string) {
