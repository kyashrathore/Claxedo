...
describe("WorkGraph northbound HTTP router", () => {
  it("preserves an explicit workspace-authority outage as a retryable 503", async () => {
    const fixture = createFixture({
      resolveContext: () => {
        throw Object.assign(new Error("Workspace authority request timed out"), {
          status: 503,
    ...
    const response = await fixture.app.request("/snapshot?limit=1", fixture.auth("owner_a"))

    expect(response.status).toBe(503)
    expect(await response.json()).toEqual({
      error: {
        code: "workspace_authority_unavailable",
        message: "Workspace authority request timed out",
        retryable: true,
      },
    })
  })

  it("keeps a missing bearer token on the generic unauthorized boundary", async () => {
    const fixture = createFixture()

    const response = await fixture.app.request("/snapshot?limit=1")

    expect(response.status).toBe(401)
    expect(await response.json()).toEqual({
      error: { code: "unauthorized", message: "Trusted WorkGraph context is required", retryable: false },
    })
  })
  ...
  it("accepts a SQLite service with additional query capabilities without an adapter cast", async () => {
    ...
    try {
      ...
      expect(firstPage.nextCursor).toMatch(/^wgsp2:/)
...
function createFixture(options?: Readonly<{
  ...
  resolveContext?: (request: Request) => WorkGraphContext | undefined
}>) {
  ...
  const commands = Object.fromEntries([
    ...
    "set_stream_visibility", "approve_work_item", "reject_work_item", "approve_work_items", "arm_work_item", "arm_work_items", "cancel_run", "restart_run", "retry_work_item",
  ...
  const app = createWorkGraphHttpRouter({
    ...
    resolveContext: options?.resolveContext ?? ((request) => {
