...
export function createWorkGraphHttpRouter<Queries extends WorkGraphHttpQueries>(input: Readonly<{
  ...
  router.use("*", async (context, next) => {
    const candidate = await Promise.resolve().then(() => input.resolveContext(context.req.raw)).catch((error) => {
      if (isExplicitHttpError(error)) return error
      return undefined
    })
    if (isExplicitHttpError(candidate)) {
  ...
  router.onError((error, context) => {
    ...
    if (isExplicitHttpError(error)) {
      return errorResponse(context, error.status, error.code, error.message, error.status === 503)
    }
...
function isExplicitHttpError(error: unknown): error is Readonly<{
  status: 400 | 401 | 403 | 503
  code: WorkGraphHttpErrorCode
  message: string
}> {
  if (typeof error !== "object" || error === null) return false
  if (!("status" in error) || ![400, 401, 403, 503].includes(Number(error.status))) return false
  if (!("code" in error) || !WorkGraphHttpErrorCodeSchema.safeParse(error.code).success) return false
  return "message" in error && typeof error.message === "string" && error.message.trim().length > 0
}
