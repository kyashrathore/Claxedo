...
describe("WorkGraph", () => {
  ...
  describe("CRUD", () => {
    ...
    test("remove deletes item", () => {
      ...
      wg.remove(item.id)
      expect(wg.get(item.id)).toBeUndefined()
      expect(wg.getAll()).toHaveLength(0)
    })
  ...
  describe("complete()", () => {
    ...
    test("complete with connector pushes to downstream", async () => {
      ...
      expect(comments[0]).toContain("Blocker")
    })
  })
  ...
  describe("Events", () => {
    test("mutations emit events", () => {
      ...
      expect(events[2].type).toBe("item_removed")
    })
