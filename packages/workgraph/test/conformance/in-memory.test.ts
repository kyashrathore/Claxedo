...
const memoryFactory: StoreConformanceFactory = async (input) => {
  ...
  const queries: ConformanceQueries = {
    ...
    changes: {
      list: async (context, query) => {
        ...
        const delivered = new Set(query.deliveredEventIds ?? [])
  ...
  const runRuntime: RunRuntimePort = {
    ...
    park: async (context, request) => {
      ...
      runs.set(request.runId, { ...run, state: "parked" })
      return { state: "parked" }
    },
    requestCompletionRetry: async (context, request) => {
      const run = runs.get(request.runId)
      const lease = run ? leases.get(run.workItemId) : undefined
      if (
        !run ||
        ...
        lease.runId !== request.runId ||
        lease.epoch !== request.leaseEpoch ||
        lease.expiresAt <= request.occurredAt
      ) return { accepted: false }
      if (run.completionRetry) return { accepted: true, ...run.completionRetry }
      const completionRetry = { terminalSeq: request.terminalSeq, requestedAt: request.occurredAt }
      runs.set(request.runId, { ...run, completionRetry })
      return { accepted: true, ...completionRetry }
    },
