...
describe("ExecutionProfileCapabilityPolicy", () => {
  ...
  it.each([
    ["environment", { environment: { kind: "hosted_workspace", placement: "shared" } }, localOnly(), "unsupported_environment"],
    ["preset", { environment: { kind: "local_worktree", placement: "shared", presetId: "fast" } }, capabilities, "preset_unavailable"],
  ...
  it("returns stable diagnostics that can be attached to validation_error details", () => {
    const result = validateResolved(ResolvedExecutionProfileSchema.parse({
      ...profile,
      environment: { kind: "hosted_workspace", placement: "shared", repositoryUrl: "https://example.com/repo.git", presetId: "unknown" },
