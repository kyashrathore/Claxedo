 ...
 *  17. HTTP API e2e (POST /orchestrate → GET status → complete via MCP)
 *  18. ACP endpoint e2e (create run → status → scratchpad → complete)
 *  19. Complex real-world graph (architect → parallel devs → reviewer → QA)
 *  20. Artifact creation flow
 */
...
function createAgentTracker() {
  ...
  const spawnFn = async (prompt: string, runId: string, nodeId: string) => {
    ...
    return { id: `agent_${spawned.length}`, status: "running" } as any;
  };
...
async function registerRun(
  ...
  return startOrchestration(db, runId, "test", spawnFn, () => undefined, async () => ({} as any));
}
...
describe("E2E MCP Orchestration", () => {
  ...
  describe("16. WorkGraph item integration", () => {
    it("should link run to WorkGraph item and track lifecycle", async () => {
      ...
      await startOrchestration(
        ...
        spawnFn, () => undefined, async () => ({} as any),
        parentItem.id,
      );
  ...
  describe("18. ACP endpoint integration", () => {
    it("should create run via ACP and query status with scratchpad data", async () => {
      const db = new Database(":memory:");
      initializeDb(db);
      db.run(`
        CREATE TABLE IF NOT EXISTS scratchpad_entries (
      ...
      const createRes = await app.request("/acp/agents/orchestrator/runs", {
        ...
        body: JSON.stringify({
          messages: [{
            role: "user",
            parts: [{ content_type: "text/plain", content: "Build a REST API" }],
          }],
        }),
      });
      expect(createRes.status).toBe(201);
      const createBody = await createRes.json();
      const acpRunId = createBody.id;

      // Add scratchpad entries for the run
      db.run(
        "INSERT INTO scratchpad_entries (id, run_id, node_id, content, created_at, expires_at, size_bytes) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ["sp_acp", acpRunId, "planner", "Plan: 3 tasks identified", new Date().toISOString(), new Date(Date.now() + 86400000).toISOString(), 30],
      );

      // Query run status via ACP
      const statusRes = await app.request(`/acp/agents/orchestrator/runs/${acpRunId}`);
      expect(statusRes.status).toBe(200);
      const statusBody = await statusRes.json();

      expect(statusBody.id).toBe(acpRunId);
      expect(statusBody.messages.length).toBeGreaterThanOrEqual(1);
      // First message is the user's goal
      expect(statusBody.messages[0].parts[0].content).toBe("Build a REST API");
      // Scratchpad entry should appear as agent message
      expect(statusBody.messages.some((m: any) =>
        m.role === "agent" && m.parts[0].content.includes("3 tasks identified")
      )).toBe(true);
    });

    it("should list agents and get descriptors", async () => {
      const app = createApp();

      const listRes = await app.request("/acp/agents");
      const agents = await listRes.json();
      expect(agents).toHaveLength(2);

      const orchRes = await app.request("/acp/agents/orchestrator");
      const orch = await orchRes.json();
      expect(orch.name).toBe("orchestrator");
      expect(orch.capabilities.streaming).toBe(true);

      const directRes = await app.request("/acp/agents/direct");
  ...
  describe("21. MCP tool definitions", () => {
    it("should expose all 11 tools with valid schemas", () => {
