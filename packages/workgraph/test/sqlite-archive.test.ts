...
describe("SQLite canonical WorkGraph archive", () => {
  ...
  it("round-trips terminal execution, review, provenance, runtime, migration, and idempotency records", async () => {
    ...
    expect(archive.records.find((record) => record.kind === "run")?.value).toMatchObject({
      completionRetry: { terminalSeq: 7, requestedAt: 6 },
      executionIdentity: {
  ...
  database.exec(`
    ...
    INSERT INTO wg_v2_runs
      (organization_id, owner_user_id, id, stream_id, work_item_id, run_number, lifecycle,
       ...
       terminal_result_json, completion_retry_json, row_version, created_at, updated_at, started_at, finished_at)
    VALUES ('organization', 'owner', 'run_1', 'stream_1', 'work_item_1', 1, 'result',
      ...
      '{"summary":"Implemented","artifactRefs":["pr:1"],"finishedAt":8}',
      '{"terminalSeq":7,"requestedAt":6}', 2, 4, 8, 5, 8);
