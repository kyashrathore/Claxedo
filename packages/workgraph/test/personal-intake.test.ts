...
describe("Connections-backed personal intake", () => {
  ...
  it("rejects unsupported and credential-shaped filters before the store sees them", async () => {
    ...
    await expect(sourceViews.create(context("alice"), {
      ...
      target: {
        environment: { kind: "hosted_workspace", placement: "shared", repositoryUrl: "https://secret-token@github.com/acme/cloud.git" },
  ...
  it("releases a failed external effect for retry and allows only one concurrent winner", async () => {
    ...
    // Wait for the winner's POST to actually REACH the provider (it is held
    // open there) before firing the concurrent call. A fixed sleep raced on
    // starved runners: if the concurrent call arrived first, IT became run 2
    // and was held while the test awaited it — releaseFirst only runs after
    // the await, so the test deadlocked to its timeout.
    await expect.poll(() => releaseFirst !== undefined, { timeout: 10_000 }).toBe(true)
