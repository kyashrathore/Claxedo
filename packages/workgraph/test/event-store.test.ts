...
import { openSqliteEventStore, type IEventStore } from "../src/substrate/event-store-sqlite";
...
describe("IEventStore (openSqliteEventStore)", () => {
  ...
  it("replayEvents produces deterministic state", async () => {
    ...
    type Replay = { types: string[]; goal: string | null };
    const reducer = (state: Replay, evt: { type: string; payload_json: string }): Replay => ({
      types: [...state.types, evt.type],
      goal: evt.type === "run_created" ? JSON.parse(evt.payload_json).goal : state.goal,
    });
    const result = await store.replayEvents("run_1", reducer, { types: [], goal: null });
    expect(result.types).toEqual(["run_created"]);
  ...
  it("replayEvents is deterministic on re-replay", async () => {
    ...
    const reducer = (state: string[], evt: { type: string }) => [...state, evt.type];
    const s1 = await store.replayEvents("run_1", reducer, [] as string[]);
    const s2 = await store.replayEvents("run_1", reducer, [] as string[]);
  ...
  it("accepts a Node-style sqlite database directly", async () => {
    const db = nodeDb();
    db.exec(`
      CREATE TABLE events (
        id TEXT PRIMARY KEY,
    ...
    const evt = await store.append(partial({ id: "evt_node", op_id: "op_node" }));

    expect(evt.stream_seq).toBe(1);
    await expect(store.getEvents("run_1")).resolves.toHaveLength(1);
    db.close();
  });
});
