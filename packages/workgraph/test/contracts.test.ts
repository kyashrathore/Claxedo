...
describe("WorkGraph contracts", () => {
  ...
  it("distinguishes inheritable execution defaults from a complete immutable Run profile", () => {
    ...
    expect(ResolvedExecutionProfileSchema.parse({
      ...profile,
      autonomy: "autonomous",
      budget: { amount: 10, unit: "usd", window: "stream" },
      maxParallel: 4,
      mayPromote: true,
      agents: [],
      assignments: {},
  ...
  it("validates completion requirements and provenance-bearing evidence without treating a Run result as completion", () => {
