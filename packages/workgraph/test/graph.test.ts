...
import { onAttemptStatusUpdate, onSessionStopped, resetAttemptState } from "../src/sdk/attempts"
...
function executionStore(db: InstanceType<typeof Database>) {
  return openSqliteExecutionStore(db, openSqliteEventStore(db))
}
...
describe("Graph routes", () => {
  ...
  it("plan decomposition is removed — /plan returns 410 and creates nothing", async () => {
    const res = await app.request(
      "/graph/slices",
      json({
        title: "Ops brief",
    ...
    expect(plan.status).toBe(410)
    expect(await plan.json()).toEqual(expect.objectContaining({ error: "planning_removed" }))

    const runs = db.query("SELECT COUNT(*) count FROM runs_current").get() as { count: number }
    expect(runs.count).toBe(0)
    expect((await (await app.request(`/graph/slices`)).json())[0]).toEqual(
      expect.objectContaining({ slice_id: slice.slice_id, status: "draft" }),
    )
  })
  ...
  it("creates an execution run from only the ready subset and stores execution metadata", async () => {
    ...
    expect(links).toEqual([{ work_item_id: a.id }])
  ...
  it("skips mission items when creating an execution run", async () => {
    ...
    // Flat model: only ready work enters the run. The synthesis item is
    // dep-gated on its siblings, so it is reported as skipped, not queued.
    expect(rows).toEqual([
      expect.objectContaining({ title: "Review API", node_type: "task" }),
    ])
    expect(rows.some((row) => row.title === mission.title)).toBe(false)
    expect(body.skipped).toEqual([expect.objectContaining({ item_id: synth!.id })])
  })
  ...
  it("releases in-subtree dependencies: completing A unblocks and spawns B, completing B completes the run", async () => {
    const wg = getWorkGraph()
    const a = make(wg, { title: "Task A" })
    const b = make(wg, { title: "Task B" })
    wg.addDep(a.id, b.id) // B depends on A; both in the subtree, no outside deps
    ...
    // Only A spawns; B is held as blocked on its in-run dependency.
    const nodes = () =>
      db
        .query("SELECT title, status FROM nodes_current WHERE run_id = ? ORDER BY title ASC")
        .all(body.run_id) as Array<{ title: string; status: string }>
    ...
    const spawnFn = async (_prompt: string, _runId: string, nodeId: string) => ({
      id: `sess_${nodeId}`,
      runtime_type: "workspace",
      session_id: `sess_${nodeId}`,
      directory: `/repo/${nodeId}`,
    ...
    const nodeFor = (itemId: string) =>
      (db
        .query("SELECT node_id FROM run_node_items_current WHERE run_id = ? AND work_item_id = ?")
        .get(body.run_id, itemId) as { node_id: string }).node_id

    await onAttemptStatusUpdate(executionStore(db), body.run_id, nodeFor(a.id), "completed", spawnFn)

    // A done → B released and spawned; the run keeps executing.
    expect(nodes()).toEqual([
      { title: "Task A", status: "completed" },
      { title: "Task B", status: "active" },
    ])
    expect(
      (db.query("SELECT status FROM runs_current WHERE run_id = ?").get(body.run_id) as { status: string }).status,
    ).toBe("executing")

    await onAttemptStatusUpdate(executionStore(db), body.run_id, nodeFor(b.id), "completed", spawnFn)
    expect(
      (db.query("SELECT status FROM runs_current WHERE run_id = ?").get(body.run_id) as { status: string }).status,
    ).toBe("completed")
  })
  ...
  it("marks a session-backed node as failed when its live session dies", async () => {
    ...
    const item = page.items.find((row: { item_id: string }) => row.item_id === a.id)
    const run = db
      .query("SELECT status FROM runs_current WHERE run_id = ?")
      .get(body.run_id) as { status: string }
  ...
  it("completing a card via MCP unblocks its dependent for the next run", async () => {
    const wg = getWorkGraph()
    const a = make(wg, { title: "Task A" })
    const b = make(wg, { title: "Task B" })
    wg.addDep(a.id, b.id)
    ...
    expect(res.status).toBe(202)
    const body = await res.json()
    // Flat model: B is dep-gated, so only A runs; B is skipped.
    expect(body.skipped).toEqual([expect.objectContaining({ item_id: b.id })])
    ...
    expect(getWorkGraph().get(a.id)?.status).toBe("done")

    // A is done → B is now ready; a second run picks it up and spawns it.
    const res2 = await app.request("/graph/runs", json({ item_ids: [b.id] }, { "x-opencode-directory": "/repo/main" }))
    expect(res2.status).toBe(202)
    const body2 = await res2.json()
    expect(body2.ready_ids).toEqual([b.id])

    const bNode = db
      .query("SELECT node_id FROM run_node_items_current WHERE run_id = ? AND work_item_id = ?")
      .get(body2.run_id, b.id) as { node_id: string }
    const next = db
      ...
      .get(body2.run_id, bNode.node_id) as { status: string; session_id: string | null; worktree_path: string | null }
  ...
  it("returns node-scoped artifacts for item detail", async () => {
    ...
    const detail = await app.request(`/graph/items/${a.id}/artifacts`)
    expect(detail.status).toBe(200)
    const body2 = await detail.json()
  ...
  it("archives a running item and cancels its live attempt", async () => {
    ...
    const list = await app.request("/graph/items")
    const page = await list.json()
    expect(page.items.some((item: { item_id: string }) => item.item_id === a.id)).toBe(false)
  })
