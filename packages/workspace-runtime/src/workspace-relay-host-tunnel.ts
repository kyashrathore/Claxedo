...
const DEFAULT_WS_PRE_OPEN_QUEUE_LIMIT = 64
...
function forwardFrame(
  ...
  if (channel.queue.length >= (input.wsPreOpenQueueLimit ?? DEFAULT_WS_PRE_OPEN_QUEUE_LIMIT)) {
    closeChannel(tunnel, channels, message.channel_id, channel, 1013, "Host upstream WebSocket queue overflow")
    return
  }
