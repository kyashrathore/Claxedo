...
import { ACPAdapter, setAcpAuth } from "./acp"
...
describe.skipIf(!API_KEY)("ACP title generation (live)", () => {
  ...
  beforeAll(() => {
    ...
    adapter = new ACPAdapter({ binary: BINARY, storeRoot: storeDir })
    setAcpAuth({ anthropic: API_KEY })
  })
  ...
  it("creates a session and generates a title after first message", async () => {
    ...
    const aiTitle = await new Promise<string | null>((resolve) => {
      ...
      const unsub = subscribeGlobalEvents((envelope) => {
