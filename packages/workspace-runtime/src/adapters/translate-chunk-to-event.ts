...
function acpMetadataInput(metadata: Record<string, unknown>) {
  ...
  for (const key of ["intent", "kind", "mode", "summary", "description", "command", "filePath", "path", "pattern", "query", "url", "sourcePath", "targetPath"]) {
    copyScalar(input, acp, key)
  }
  copyScalar(input, acp, "title", "summary")
  copyScalar(input, acp, "rawToolName", "toolName")

  for (const key of ["command", "path", "pattern", "query", "url", "offset", "limit", "old_string", "new_string", "replace_all"]) {
...
function locationsFromMetadata(metadata: Record<string, unknown>) {
  const acp = object(metadata.acp)
  if (!acp || !Array.isArray(acp.locations)) return []
  return acp.locations.flatMap((item) => {
    const row = object(item)
...
function hydrateToolInput(
  ...
  const input = mergeInput(acpMetadataInput(metadata), current)
  return normalizeLocationInput(tool, input, locationsFromMetadata(metadata))
}
...
export function translateChunkToEvent(chunk: UIMessageChunk, ctx: ChunkToEventContext): CompatEnvelope[] {
  ...
  switch (chunk.type) {
    ...
    case "tool-start": {
      ...
      const input = hydrateToolInput(tool, ctx.toolInputsByCallId[chunk.toolCallId], metadata)
    ...
    case "tool-input": {
      ...
      const metadata = mergeMetadata(ctx.toolMetadataByCallId[chunk.toolCallId], chunk.metadata)
      const input = hydrateToolInput(tool, mergeInput(ctx.toolInputsByCallId[chunk.toolCallId], next), metadata)
