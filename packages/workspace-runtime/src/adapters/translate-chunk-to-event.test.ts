...
describe("tool-input", () => {
  ...
  it("hydrates sparse Claude input from ACP metadata", () => {
    const ctx = makeCtx({ toolNamesByCallId: { tc1: "read" } })
    const chunk: UIMessageChunk = {
      type: "tool-input",
      toolCallId: "tc1",
      input: { summary: "Read File", intent: "read" },
      metadata: {
        acp: {
          title: "Read src/foo.ts",
          filePath: "/workspace/src/foo.ts",
          rawInput: { file_path: "/workspace/src/foo.ts", offset: 10, limit: 20 },
        },
      },
    }
    const events = translateChunkToEvent(chunk, ctx)
...
describe("tool-output", () => {
  ...
  it("promotes command from ACP metadata for completed shell cards", () => {
    const ctx = makeCtx({ toolNamesByCallId: { tc1: "bash" } })
    const chunk: UIMessageChunk = {
      type: "tool-output",
      toolCallId: "tc1",
      output: { stdout: "ok\n", stderr: "" },
      metadata: { acp: { intent: "shell", command: "bun test", rawInput: { command: "bun test" } } },
    }
    const events = translateChunkToEvent(chunk, ctx)
    const state = ((events[0].payload.properties as { part: Record<string, unknown> }).part.state) as Record<string, unknown>
    expect(state.input).toMatchObject({ intent: "shell", command: "bun test" })
    expect(ctx.toolInputsByCallId.tc1).toMatchObject({ command: "bun test" })
    expect(state.output).toBe("ok\n")
  })

  it("promotes grep pattern and path from ACP metadata for completed search cards", () => {
    const ctx = makeCtx({ toolNamesByCallId: { tc1: "grep" } })
    const chunk: UIMessageChunk = {
      type: "tool-output",
      toolCallId: "tc1",
      output: { totalFiles: 2 },
      metadata: {
        acp: {
          intent: "search",
          mode: "grep",
          rawInput: { pattern: "\\.tabs\\.", path: "/workspace/src" },
        },
      },
    }
    const events = translateChunkToEvent(chunk, ctx)
    const state = ((events[0].payload.properties as { part: Record<string, unknown> }).part.state) as Record<string, unknown>
    expect(state.input).toMatchObject({ intent: "search", mode: "grep", pattern: "\\.tabs\\.", path: "/workspace/src" })
  })

  it("promotes read file path from ACP metadata even without a prior input chunk", () => {
    const ctx = makeCtx({ toolNamesByCallId: { tc1: "read" } })
    const chunk: UIMessageChunk = {
      type: "tool-output",
      toolCallId: "tc1",
      ...
      metadata: {
        acp: {
          intent: "read",
          locations: [{ path: "/workspace/src/foo.ts", line: 4 }],
          rawInput: { file_path: "/workspace/src/foo.ts" },
        },
      },
    }
    const events = translateChunkToEvent(chunk, ctx)
    const state = ((events[0].payload.properties as { part: Record<string, unknown> }).part.state) as Record<string, unknown>
    expect(state.input).toMatchObject({ intent: "read", filePath: "/workspace/src/foo.ts", file_path: "/workspace/src/foo.ts" })
  })
...
describe("tool-error", () => {
  ...
  it("promotes edit path from ACP metadata for failed edit cards", () => {
    const ctx = makeCtx({ toolNamesByCallId: { tc1: "edit" } })
    const chunk: UIMessageChunk = {
      type: "tool-error",
      toolCallId: "tc1",
