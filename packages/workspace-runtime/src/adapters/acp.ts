...
class ACPProcess {
  ...
  async cancel(agentSessionId: string): Promise<void> {
    ...
    await this.conn.cancel({ sessionId: agentSessionId }).catch((err) => {
      log.info("ACP cancel: error (ignored)", { agentSessionId, err })
    })
  }
...
export class ACPAdapter implements AgentAdapter {
  ...
  async abort(id: string, directory: string): Promise<void> {
