...
function subagentType(raw: unknown) {
  const item = object(raw)
  ...
  if (value !== undefined) {
    diagnoseTranslation("acp.nested_session_unrepresented", {
...
function todoMode(raw: unknown) {
  const item = object(raw)
  if (typeof item?.merge !== "boolean") return
  if (item.merge) {
    diagnoseTranslation("acp.todo_merge_unrepresented", {
...
function pick(state: ToolState) {
  ...
  const childSubagentType = subagentType(raw)
...
export function reduceTool(
  ...
  if (
    ...
    diagnoseTranslation("acp.impossible_state_transition", {
  ...
  for (const item of content ?? []) {
    ...
    if (item.type !== "terminal") {
      diagnoseTranslation("acp.dropped_content", {
        ...
        name: state.name,
        kind: state.kind,
        intent: viewTool(state).metadata.acp && typeof viewTool(state).metadata.acp === "object"
          ? (viewTool(state).metadata.acp as Record<string, unknown>).intent as string | undefined
          : undefined,
        reason: item.type === "content" ? "tool_content_preserved_in_metadata" : "unknown_tool_content_preserved_in_metadata",
        shape: shape(item),
      })
      continue
    }
