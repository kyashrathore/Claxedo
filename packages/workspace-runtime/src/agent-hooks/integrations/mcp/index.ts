...
import { asRecord, readJSON, writeIfChanged } from "../../core/utils"
import { upsertClaudeSettings } from "../../core/hooks"
import { generateOpencodeJsonc, opencodePaths } from "../opencode/config"
...
export async function applyManagedMcpSetup(input: {
...
export async function toggleAgentMcp(server: string, agent: string, enabled: boolean): Promise<void> {
  ...
  switch (agent) {
    case "claude": {
      const notifyPath = path.join(CLAXEDO_DIR, "hooks", NOTIFY_SCRIPT)
    ...
    case "cursor":
      ...
      break
    case "opencode": {
      const dir = opencodePaths(CLAXEDO_DIR)
