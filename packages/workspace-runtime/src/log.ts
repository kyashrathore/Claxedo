...
function formatMsg(
  ...
  if (extra) {
    for (const [k, v] of Object.entries(extra)) {
      ...
      parts.push(
        `${k}=${v instanceof Error ? v.message : typeof v === "object" ? JSON.stringify(v) : v}`,
      )
    }
  }
...
function makeLogger(tags: Record<string, unknown>) {
  const logger = {
    debug(message?: unknown, extra?: LogExtra) {
      if (process.env.CLAXEDO_DEBUG === "1") {
        process.stderr.write(formatMsg("DEBUG", tags, message, extra))
      }
    },
