...
test("a failed opener can retry the same SDK after its cause is corrected", async () => {
  const root = tempRoot()
  const database = path.join(root, "opencode.db")
  fs.mkdirSync(database)
  const host = hostAt(root)
  await expect(host.client()).rejects.toBeInstanceOf(OpenCodeUnavailableError)
  expect(host.status().lifecycle).toBe("unavailable")
...
test("shutdown during first use drains the opener and releases the database", async () => {
  const root = tempRoot()
  const host = hostAt(root)
  const opening = host.client()
  const closing = host.close()
  await expect(host.client()).rejects.toBeInstanceOf(OpenCodeUnavailableError)
  await Promise.all([opening, closing])
  expect(host.status().lifecycle).toBe("closed")
  await expect(host.client()).rejects.toBeInstanceOf(OpenCodeUnavailableError)
  await hostAt(root).client()
})
