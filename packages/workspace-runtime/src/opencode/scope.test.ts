...
import { assertLocationInScope, WorkspaceScope, sameScope, WorkspaceScopeError } from "./scope"
...
test("requires a canonical workspace id", () => {
  expect(() => WorkspaceScope.authorize({ workspaceID: "  ", directory: tempDir() })).toThrow(WorkspaceScopeError)
})

test("requires an absolute directory", () => {
  expect(() => WorkspaceScope.authorize({ workspaceID: "w", directory: "./relative" })).toThrow(/absolute/)
})

test("rejects a directory that does not exist", () => {
  expect(() => WorkspaceScope.authorize({ workspaceID: "w", directory: "/nope/does/not/exist" })).toThrow(WorkspaceScopeError)
})
...
test("rejects a path that is not a directory", () => {
  ...
  expect(() => WorkspaceScope.authorize({ workspaceID: "w", directory: file })).toThrow(/not a directory/)
})
...
test("resolves symlinks so a later retarget cannot widen access", () => {
  ...
  const scope = WorkspaceScope.authorize({ workspaceID: "w", directory: link })
...
test("refuses to attribute a record with no location", () => {
  const scope = WorkspaceScope.authorize({ workspaceID: "w", directory: tempDir() })
  expect(() => assertLocationInScope(scope, undefined)).toThrow(/no location/)
})

test("sameScope compares identity and resolved path", () => {
  const dir = tempDir()
  const a = WorkspaceScope.authorize({ workspaceID: "w", directory: dir })
  const b = WorkspaceScope.authorize({ workspaceID: "w", directory: dir })
  const c = WorkspaceScope.authorize({ workspaceID: "other", directory: dir })
