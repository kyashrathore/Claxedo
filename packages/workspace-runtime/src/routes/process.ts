...
export const ProcessRoutes = lazy(() =>
  new Hono()
    ...
    .post("/:id/start", async (c) => {
      ...
      // Validate via the schema so an invalid `portConflict`/`routeConflict` value
      // produces a 400 rather than silently falling through to the default branch.
      const parsed = raw === undefined ? { success: true as const, data: {} } : Process.LaunchRequest.safeParse(raw)
      if (!parsed.success) {
        return c.json({ error: "Invalid launch request body", issues: parsed.error.issues }, 400)
      }
      const result = await ProcessManager.start(directory, id, {
        portConflict: parsed.data.portConflict,
        routeConflict: parsed.data.routeConflict,
      })
