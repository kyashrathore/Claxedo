/**
 * Workspace Contract — schema and loader for `.workspace-runtime/workspace` and `.workspace-runtime/processes`.
 *
 * `.workspace-runtime/workspace` is authoritative for:
 ...
 *
 * `.workspace-runtime/processes` is authoritative for:
 * - service definitions, commands, ports, per-service readiness
 * (This extends the existing process config with contract-level authority)
 *
...
const CONTRACT_DIR = ".workspace-runtime"
const LEGACY_CONTRACT_DIR = ".claxedo"
...
export async function loadWorkspaceContract(directory: string): Promise<WorkspaceContract | undefined> {
  return await readJsonc(path.join(directory, CONTRACT_DIR, WORKSPACE_FILE), WorkspaceContract)
    ?? await readJsonc(path.join(directory, LEGACY_CONTRACT_DIR, WORKSPACE_FILE), WorkspaceContract)
}

export async function loadProcessesContract(directory: string): Promise<ProcessesContract | undefined> {
  return await readJsonc(path.join(directory, CONTRACT_DIR, PROCESSES_FILE), ProcessesContract)
    ?? await readJsonc(path.join(directory, LEGACY_CONTRACT_DIR, PROCESSES_FILE), ProcessesContract)
}
