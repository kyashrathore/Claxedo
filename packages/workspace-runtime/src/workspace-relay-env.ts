...
export function hostTunnelFromEnv(env: Env = process.env, port = 3002): WorkspaceRelayHostTunnelOptions | undefined {
  ...
  return {
    ...
    ...hostTunnelPreOpenQueueFromEnv(env),
  }
}

/**
 * Bounds on frames held while a channel's upstream socket is still connecting.
 *
 * Env-tunable because the shipped default was itself the bug once — a
 * frame-count bound small enough that an ordinary startup burst tripped it. An
 * operator who hits a workload the defaults do not fit should be able to widen
 * them without waiting for a release. Omitted keys keep the defaults in
 * `workspace-relay-host-tunnel.ts`, which mirror the relay's own bounds.
 */
export function hostTunnelPreOpenQueueFromEnv(env: Env = process.env) {
  const maxFrames = numberEnv(env, "WORKSPACE_RUNTIME_RELAY_PRE_OPEN_QUEUE_MAX_FRAMES")
  const maxBytes = numberEnv(env, "WORKSPACE_RUNTIME_RELAY_PRE_OPEN_QUEUE_MAX_BYTES")
  return {
    ...(maxFrames ? { wsPreOpenQueueMaxFrames: maxFrames } : {}),
    ...(maxBytes ? { wsPreOpenQueueMaxBytes: maxBytes } : {}),
  }
}
