 ...
 *
 * A path is therefore treated as absent rather than passed through: callers
 * in this codebase have a standing habit of handing directories where an
 * identity is expected (the directory-shape routing debt), so an explicit
 * path-shaped value is far more likely to be that mistake than a deliberate
 * identity.
 */
