 ...
 * that shape rather than to `typeof fetch` — the full interface drags in
 * statics like `preconnect` that a caller supplying a plain function has no
 * reason to provide. The global `fetch` still satisfies this.
 */
...
function link(baseUrl: string, input: Pick<Input, "directory" | "workspaceId">, path: string) {
  ...
  return `${baseUrl}/process${path}${path.includes("?") ? "&" : "?"}${search.toString()}`
}
...
export function createProcessClient(input: Input) {
  ...
  async function req(path: string, init?: RequestInit) {
    ...
    headers.set("x-claxedo-directory", input.directory)
