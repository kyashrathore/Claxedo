...
function workspace(directory: string): string {
  return workspaceMap.get(real(directory)) ?? real(directory)
}

export function bindWorkspace(directory: string, workspaceId?: string) {
  if (!workspaceId) return
  workspaceMap.set(real(directory), workspaceId)
}
