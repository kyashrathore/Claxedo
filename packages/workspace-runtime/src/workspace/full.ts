...
function codexAuthInput(auth: Record<string, string>) {
  return auth["codex-acp"] ?? auth["openai"]
}
      ...
      try {
        ...
        if (adapter && acp(next.runner.type) && "setAuth" in adapter) {
          ;(adapter as ACPAdapter).setAuth({
            anthropic: runtimeAuthKey(next.auth["claude-acp"]),
            openai: runtimeAuthKey(codexAuthInput(next.auth)),
            cursor: runtimeAuthKey(next.auth["cursor-acp"]),
          })
        }
