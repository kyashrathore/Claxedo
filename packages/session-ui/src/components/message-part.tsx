    ...
    <Show when={props.diagnostics.items.length > 0}>
      <div data-component="diagnostics" class="ui-diagnostics">
        <For each={props.diagnostics.items}>
          {(diagnostic) => (
            <div data-slot="diagnostic">
              <span
                data-slot="diagnostic-icon"
                class="ui-diagnostic-icon"
                aria-label={i18n.t("ui.messagePart.diagnostic.error")}
              >
              ...
              <span data-slot="diagnostic-message" class="ui-diagnostic-message">
                {diagnostic.message}
              </span>
            </div>
          )}
        ...
        <Show when={overflow() > 0}>
          <div data-slot="diagnostic-overflow" class="ui-diagnostic-overflow">
            {i18n.t("ui.messagePart.diagnostic.more", { count: overflow() })}
          </div>
        </Show>
      </div>
    </Show>
...
function createPacedValue(getValue: () => string, live?: () => boolean) {
  ...
  // A fresh object per run keeps the "react to every invalidation" semantics the
  // pacer needs: what it does is decided by the mutable `shown`/`timeout` pair, not
  // by the value. `live` stays in the compute so the end of a stream still flushes.
  createEffect(
    () => ({ text: getValue(), live: !!live?.() }),
    ({ text, live: streaming }) => {
      if (
        !streaming ||
        !text.startsWith(shown) ||
        text.length < shown.length ||
        text.length - shown.length <= TEXT_RENDER_IMMEDIATE
      ) {
        clear()
        sync(text)
        return
                          ...
                          <div data-slot="basic-tool-tool-info-main">
                            ...
                            <Show when={!running() && trigger().args?.length}>
                              <For each={trigger().args}>
                                {(arg) => (
                                  <span data-slot="basic-tool-tool-arg" class="ui-basic-tool-tool-arg">
                                    {arg}
                                  </span>
                                )}
                              </For>
                    ...
                    <Show
                      ...
                      fallback={
                        <div data-slot="user-message-attachment-file">
                          ...
                          <span data-slot="user-message-attachment-name" class="ui-user-message-attachment-name">
                            {name}
                          </span>
                        </div>
                      }
      ...
      <div data-component="text-part" class="ui-text-part" data-timeline-part-id={part().id}>
        ...
        <Show when={showCopy()}>
          <div
            ...
            class="ui-text-part-copy-wrapper"
          ...
          <div data-slot="basic-tool-tool-info-main">
            ...
            >
              <span
                ...
                class="ui-task-tool-spinner"
                style={{ color: props.tone ?? "var(--icon-interactive-base)" }}
              >
            ...
            <span data-component="task-tool-title" class="ui-task-tool-title">
              {props.subagent.agentLabel || props.subagent.label}
            </span>
      ...
      <>
        ...
        <Show when={localUrl()}>
          <a data-component="local-preview-row" href={localUrl()!} target="_blank" rel="noopener noreferrer">
            ...
            <span data-slot="local-preview-verb" class="ui-local-preview-verb">
              Local preview
            </span>
            <span data-slot="local-preview-url" class="ui-local-preview-url">
              {localLabel()}
            </span>
          </a>
        </Show>
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    // `seeded` stays: `files()` yields a fresh array as patch data streams in, so the
    // compute cannot dedupe the reseed away on its own.
    createEffect(files, (list) => {
      if (list.length === 0 || seeded) return
      seeded = true
                ...
                >
                  <For each={files()}>
                    {(file) => {
                      ...
                      createEffect(active, (open) => {
                        if (!open) {
                          setVisible(false)
                          return
                        }
                        // Cancelling the frame replaces the re-check inside it, and stops a
                        // pending frame from writing after disposal.
              ...
              {(q, i) => {
                ...
                return (
                  <div data-slot="question-answer-item">
                    <div data-slot="question-text" class="ui-question-text">
                      {q.question}
                    </div>
                    <div data-slot="answer-text" class="ui-answer-text">
