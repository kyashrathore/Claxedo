...
render(() => {
  ...
  window.api.awaitInitialization((next) => setStep(next as InitStep)).then(() => {
    // Server is ready — mark as done (the "done" init step may arrive after
    // the IPC listener is removed, so set it explicitly here)
    setStep({ phase: "done" })
