...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.claxedo.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `claxedo.settings.dat` is special: it maps to the `claxedo.settings` store
...
function migrateFile(datPath: string, filename: string) {
  ...
  // claxedo.settings.dat → the electron settings store ("claxedo.settings").
  ...
  const storeName = filename === "claxedo.settings.dat" ? "claxedo.settings" : filename
