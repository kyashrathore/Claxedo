...
describe("group mention gating wired into the core", () => {
  function groupCore(input: { engagement?: "mention" | "unprompted" } = {}) {
    const rt = runtime()
    const denials: ChannelDenialReason[] = []
    const core = createChannelCore({
      runtime: rt,
      dedup: createMemoryDedupStore({ initializedAt: 0 }),
      sessions: createMemorySessionResolver(rt),
      access: createChannelAccess({
        dmPolicy: "open",
        groupPolicy: "open",
        ...(input.engagement ? { groupEngagement: input.engagement } : {}),
        store: createMemoryChannelAccessStore(),
      }),
      onDenial: (_e, reason) => { denials.push(reason as ChannelDenialReason) },
    })
    return { rt, core, denials }
  }

  test("an unaddressed group message never reaches the runtime", async () => {
    // The whole point of gating in the ACCESS step: no session, no LLM turn, no
    // transcript entry for conversation that was not directed at the bot.
    const { rt, core, denials } = groupCore()

    await core.handleInbound(
      envelope({ chatType: "group", externalUserId: "teammember", text: "chatting with a colleague" }),
      { reply: () => {} },
    )

    expect(rt.created).toEqual([])
    expect(rt.sent).toEqual([])
    expect(denials).toEqual(["group_not_addressed"])
  })

  test("an unaddressed group message draws no reply at all", async () => {
    // Silence, not an explanation: replying would make the bot a noise source
    // in a room it was not invited into.
    const { core } = groupCore()
    const chunks: OutboundChunk[] = []

    await core.handleInbound(
      envelope({ chatType: "group", externalUserId: "teammember", text: "chatting" }),
      { reply: (c) => chunks.push(c) },
    )
  ...
  test("an addressed group message runs a turn", async () => {
    const { rt, core, denials } = groupCore()

    await core.handleInbound(
      envelope({ chatType: "group", externalUserId: "teammember", text: "@claxedo fix the build", mentions: ["@claxedo"] }),
      { reply: () => {} },
    )

    expect(rt.sent).toEqual(["ses_1:@claxedo fix the build"])
    expect(denials).toEqual([])
  })

  test("unprompted mode runs a turn for an unaddressed group message", async () => {
    const { rt, core } = groupCore({ engagement: "unprompted" })

    await core.handleInbound(
      envelope({ chatType: "group", externalUserId: "teammember", text: "just do it" }),
      { reply: () => {} },
    )

    expect(rt.sent).toEqual(["ses_1:just do it"])
  })
})
