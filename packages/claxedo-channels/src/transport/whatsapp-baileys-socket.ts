...
import type { WhatsAppBaileysInboundMessage, WhatsAppBaileysSocket } from "./whatsapp-baileys"
...
export function createBaileysWhatsAppSocket(options: BaileysWhatsAppSocketOptions = {}): WhatsAppBaileysSocket {
  const state = {
    socket: undefined as WASocket | undefined,
    auth: undefined as StoredAuthState | undefined,
    messageHandlers: [] as ((message: WhatsAppBaileysInboundMessage) => Promise<void> | void)[],
    authHandlers: [] as ((authState: unknown) => void)[],
    off: [] as (() => void)[],
  }
