...
8. **Approval-reply / cancel intents** — resolved against the `approvals`
   bridge or `runtime.abortSession` using the session found in step 4; these
   do not go through session *resolution* (step 9), only session *lookup*.
...
`parseChannelCommand` (`src/core/approval-token.ts`) to populate
`envelope.intent` (message / approval reply / cancel / new_session /
list_sessions / status / whoami / pairing_approve / pairing_list) before the
envelope ever reaches `ChannelCore`.
