...
import type {
  DocumentsServiceLifecycle,
  DocumentsServiceLifecycleReader,
  DocumentsServiceLifecycleWriter,
} from "./service"

type D1Result = Readonly<{ success: boolean; meta: Readonly<{ changes: number }> }>

export interface DocumentsLifecycleStatement {
  bind(...values: unknown[]): DocumentsLifecycleStatement
  first(): Promise<unknown>
  run(): Promise<D1Result>
}

export interface DocumentsLifecycleD1 {
  prepare(sql: string): DocumentsLifecycleStatement
  batch(statements: DocumentsLifecycleStatement[]): Promise<unknown[]>
}

type DocumentsLifecycleIdentity = Pick<DocumentsServiceLifecycle, "environmentId" | "deploymentId" | "serviceBuildId">
...
export class D1DocumentsServiceLifecycleStore
  implements DocumentsServiceLifecycleReader, DocumentsServiceLifecycleWriter
{
  constructor(
    private readonly database: DocumentsLifecycleD1,
    private readonly identity: DocumentsLifecycleIdentity,
  ) {}

  async read(): Promise<DocumentsServiceLifecycle | undefined> {
  ...
  async apply(request: ServiceLifecycleMutationRequest): Promise<ServiceLifecycleMutationResponse> {
    ...
    if (request.action === "initialize_disabled") {
      await this.database.batch([
        this.database
          .prepare(
            `
            insert or ignore into documents_service_lifecycle (
              singleton, initializer_operation_id, state, revision, updated_at
            )
            select 1, ?, 'installed_disabled', 1, ?
            where not exists (select 1 from documents_service_lifecycle_audit where operation_id = ?)
          `,
          )
        ...
        this.database
          .prepare(
            `
            insert or ignore into documents_service_lifecycle_audit (
            ...
            from documents_service_lifecycle
    ...
    if (request.action === "commit_enable") {
      await this.database.batch([
        ...
        this.database
          .prepare(
            `
            update documents_service_lifecycle set state = 'enabled', updated_at = ?
            where singleton = 1 and state = 'enabling' and revision = ?
              and exists (
                select 1 from documents_service_lifecycle_audit
    ...
    if (request.action === "uninstall") {
      await this.database.batch([
        ...
        this.database
          .prepare(
            `
            delete from documents_service_lifecycle
    ...
    await this.database.batch([
      this.database
        .prepare(
          ...
          where singleton = 1 and revision = ? and state in (${allowedStates})
            and not exists (select 1 from documents_service_lifecycle_audit where operation_id = ?)
        `,
        )
        .bind(
          request.identity.operationId,
      ...
      this.database
        .prepare(
          `
          update documents_service_lifecycle
  ...
  private audit(operationId: string): Promise<AuditRow | null> {
    return this.database
      .prepare(
        ...
        from documents_service_lifecycle_audit where operation_id = ?
      `,
      )
      .bind(operationId)
  ...
  private auditInsert(
    ...
    return this.database
      .prepare(
        ...
        select ?, ?, ?, ?, ?, ?, ?, ?
        where exists (
          select 1 from documents_service_lifecycle where singleton = 1 and state = ? and revision = ?
        )
      `,
      )
      .bind(
