...
async function attachSessionTask(ctx: any, context: ActivityContext, input: AttachSessionTaskInput) {
  ...
  await ctx.db.insert("workgraph_runs", {
    ...
    // Attached Runs are created already running under one local Session binding;
    // they never pass through managed claim/recovery, so their only valid fence
    // generation is the initial epoch.
    generation: 1,
    state: "running",
...
async function withOperation(
  ...
  run: () => Promise<Readonly<{
    ...
    cursor?: Readonly<{ sequence: number; watermark: number }>
  }>>,
  ...
  const result = {
    ...
    cursor: createChangeCursor({
      ...
      position: completed.cursor.watermark,
    }),
  ...
  await ctx.db.insert("workgraph_operation_results", {
    ...
    result_status: 200,
    result,
    change_cursor: completed.cursor.sequence,
    schema_version: 1,
    created_at: context.now,
  })
