...
import {
  applyWorkGraphCommand,
  appendSystemWorkGraphChange,
  continueStream,
  enqueueMasterWake,
  reconcileReadyStreams,
} from "./workgraphCommands"
import { assertWorkGraphOwnerWritable } from "./workgraphModel"

type RuntimeMutationCtx = GenericMutationCtx<DataModel>

async function foldAndGateStreamBudget(
  ctx: RuntimeMutationCtx,
  run: Pick<Doc<"workgraph_runs">, "organization_id" | "owner_user_id" | "stream_id">,
  now: number,
) {
  if (!run.organization_id) return false
  const stream = await ctx.db
    .query("workgraph_streams")
    .withIndex("by_tenant_id", (query) =>
      query.eq("organization_id", run.organization_id).eq("owner_user_id", run.owner_user_id).eq("id", run.stream_id),
    )
    .unique()
  if (!stream) return false
  const budget = stream.execution_defaults?.budget as
    | { amount: number; unit: "usd" | "tokens"; window: "day" | "stream" }
    | undefined
  if (!budget) {
    if (stream.master_status?.state !== "attention" || stream.master_status.escalation !== "budget_exhausted") {
      return false
    }
    const masterStatus = stream.master_status.budgetPriorStatus ?? {
      state: "hibernating" as const,
      sessionId: masterSessionId(run.stream_id),
    ...
    await ctx.db.patch(stream._id, {
      master_status: released.master_status,
      row_version: released.row_version,
      updated_at: now,
    })
    await syncAttentionRecord(ctx, "workgraph_streams", released)
    return false
  }
  const previous = stream.spend ?? { total_tokens: 0, as_of: 0 }
  const dayStart = utcDayStart(now)
  // `llm_usage_events.org_id` is the W5 metering id space — the Clerk org id
  // string (`orgs.ts` ORG_CLERK_TABLES), not the Convex document id this run
  // row carries. Personal orgs have no Clerk id; their usage rows are keyed on
  // the document id string, so that is the fallback rather than a miss.
  const organization = await ctx.db.get(run.organization_id)
  const meteringOrgId = organization?.clerk_org_id ?? String(run.organization_id)
  const rows = await ctx.db
    .query("llm_usage_events")
    .withIndex("by_org_stream_created", (query) =>
      query
        .eq("org_id", meteringOrgId)
        .eq("stream_id", run.stream_id)
        .gte("created_at", previous.as_of),
    )
    .filter((query) =>
      query.or(
        query.gt(query.field("created_at"), previous.as_of),
        query.and(
          query.eq(query.field("created_at"), previous.as_of),
          query.gt(query.field("message_id"), previous.as_of_message_id ?? ""),
        ),
      ),
    )
    .take(5_000)
  // This bounded fold fails closed: if the page saturates, the persisted
  // watermark remains behind and the next budget check continues folding
  // before admitting more spend.
  const profileByRun = new Map<string, unknown>(
    await Promise.all(
      Array.from(new Set(rows.flatMap((row) => row.run_id ? [row.run_id] : []))).map(async (runId) => {
        const usageRun = await ctx.db
          ...
          .unique()
        return [runId, usageRun?.resolved_execution] as const
      }),
    ),
  )
  const totals = convexUsageSpend(rows, previous)
  const daily = convexUsageSpend(
    rows.filter((row) => row.created_at >= dayStart),
    previous.day_start === dayStart
      ? {
          total_tokens: previous.day_tokens ?? 0,
          ...(previous.day_usd === undefined ? {} : { total_usd: previous.day_usd }),
          as_of: previous.as_of,
        }
      : { total_tokens: 0, total_usd: 0, as_of: 0 },
  )
  const last = rows.at(-1)
  const spend = {
    ...totals,
    by_profile: convexUsageByProfile(
      rows,
      stream.execution_defaults?.agents ?? [],
      profileByRun,
      previous.by_profile ?? [],
    ),
    ...(last ? { as_of: last.created_at, as_of_message_id: last.message_id } : {}),
    day_start: dayStart,
    ...(daily.total_usd === undefined ? {} : { day_usd: daily.total_usd }),
    day_tokens: daily.total_tokens,
  }
  const measured =
    budget.window === "day"
      ? { total_tokens: spend.day_tokens, total_usd: spend.day_usd }
      : spend
  if (rows.length === 5_000) {
    await ctx.db.patch(stream._id, { spend, updated_at: now })
    return true
  }
  const exhausted =
    (budget.unit === "tokens" ? measured.total_tokens : measured.total_usd ?? 0) >= budget.amount
  if (exhausted) {
    if (
      stream.master_status?.state === "attention" &&
      stream.master_status.escalation === "budget_exhausted"
    ) {
      if (rows.length > 0) await ctx.db.patch(stream._id, { spend, updated_at: now })
      return true
    }
    const masterStatus = {
      state: "attention",
      ...
      receiptRefs: [],
      ...(stream.master_status ? { budgetPriorStatus: stream.master_status } : {}),
      updatedAt: now,
    }
    const held = {
      ...stream,
    ...
    await syncAttentionRecord(ctx, "workgraph_streams", held)
    return true
  }
  if (stream.master_status?.state === "attention" && stream.master_status.escalation === "budget_exhausted") {
    const masterStatus = stream.master_status.budgetPriorStatus ?? {
...
function convexUsageSpend(
  rows: readonly Doc<"llm_usage_events">[],
  ...
  return rows.reduce((current, row) => {
    ...
    return {
      ...
      ...(current.total_usd !== undefined || cost !== undefined
        ? { total_usd: (current.total_usd ?? 0) + (cost ?? 0) }
        : {}),
      as_of: Math.max(current.as_of, row.created_at),
    }
  }, initial)
}

function convexUsageByProfile(
  rows: readonly Doc<"llm_usage_events">[],
  agents: readonly unknown[],
  profileByRun: ReadonlyMap<string, unknown>,
  initial: readonly { profile: string; total_tokens: number; total_usd?: number }[],
) {
  return Array.from(
    rows.reduce((totals, row) => {
      ...
      return totals
    }, new Map(initial.map((spend) => [spend.profile, spend]))).values(),
  ).sort((left, right) => left.profile.localeCompare(right.profile))
}

function convexUsageProfile(agents: readonly unknown[], input: unknown) {
  const resolved = ResolvedExecutionProfileSchema.safeParse(input)
  if (!resolved.success) return "Default"
  return agents
    .flatMap((input) => {
      const profile = AgentProfileSchema.safeParse(input)
      return profile.success ? [profile.data] : []
    })
    .find((profile) =>
      profile.generation.harness === resolved.data.harness &&
      profile.generation.agent === resolved.data.agent &&
      profile.generation.model.providerId === resolved.data.model.providerId &&
      profile.generation.model.modelId === resolved.data.model.modelId &&
      profile.generation.effort === resolved.data.effort &&
      JSON.stringify(profile.generation.tools) === JSON.stringify(resolved.data.tools) &&
      JSON.stringify(profile.generation.connectionIds) === JSON.stringify(resolved.data.connectionIds)
    )?.name ?? "Default"
}
...
/** Oldest undrained tenants form the bounded recovery work-set. */
export const listDirtyTenants = serviceQuery({
  args: { limit: v.number(), before: v.number() },
  handler: async (ctx, args) =>
    (await ctx.db.query("workgraph_dirty_events")
      .withIndex("by_dirty", (query: any) =>
        query.eq("drained_at", undefined).lte("dirty_at", args.before))
      .take(Math.max(1, Math.min(500, args.limit))))
      .map((row) => ({
        organizationId: String(row.organization_id),
        ownerUserId: String(row.owner_user_id),
        dirtyAt: row.dirty_at,
        dirtyToken: row.dirty_token,
      })),
})

export const markTenantDrained = serviceMutation({
  args: {
    organization_id: v.id("orgs"),
    owner_user_id: v.id("users"),
    dirty_token: v.string(),
    drained_at: v.number(),
    prune_before: v.optional(v.number()),
    prune_limit: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    const rows = await ctx.db.query("workgraph_dirty_events")
      .withIndex("by_token", (query: any) =>
        query
          .eq("organization_id", args.organization_id)
          .eq("owner_user_id", args.owner_user_id)
          .eq("dirty_token", args.dirty_token))
      .collect()
    if (rows.length === 0) return false
    await Promise.all(rows.map((row) => ctx.db.patch(row._id, { drained_at: args.drained_at })))
    if (args.prune_before !== undefined) {
      const expired = await ctx.db.query("workgraph_dirty_events")
        .withIndex("by_drained", (query: any) =>
          query.gt("drained_at", 0).lte("drained_at", args.prune_before))
        .take(Math.max(1, Math.min(500, args.prune_limit ?? 500)))
...
export const listStaleTenants = serviceQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // The dirty set is the primary sweep source. This outbox-derived backstop
    // recovers durable effects whose fast-lane nudge was missed. Scan more rows
    // than the tenant result cap so one noisy tenant cannot consume the entire
    // distinct-tenant dispatch window.
...
export const claimMasterTurn = serviceMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // never starts while a Run is live — the wake defers and re-fires.
    if (!reserved) {
...
export const claimLaunches = serviceMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const budgetGates = new Map<string, Promise<boolean>>()
    return await Promise.all(
      rows.map(async (row) => {
        ...
        if (
          ...
          !["admitted", "placing", "parked"].includes(run.state)
        ) {
        ...
        const stream = await ctx.db
          ...
          .unique()
        const parent = stream?.parent_stream_id
          ? await ctx.db
              .query("workgraph_streams")
              .withIndex("by_tenant_id", (query: any) =>
                query
        ...
        const budgetKey = `${row.organization_id}:${row.owner_user_id}:${run.stream_id}`
        const budgetGate =
          budgetGates.get(budgetKey) ??
          foldAndGateStreamBudget(ctx, run, args.now)
        budgetGates.set(budgetKey, budgetGate)
        if (await budgetGate) {
          await ctx.db.patch(row._id, {
            status: "pending",
            ...
            last_error: "Stream budget exhausted",
            updated_at: args.now,
          })
          return null
        }
        ...
        if (run.state === "admitted" || run.state === "parked") {
          const resuming = run.state === "parked"
          const placing = {
            ...run,
            state: "placing",
            row_version: runRowVersion + (renewal.recovered && !resuming ? 2 : 1),
            updated_at: args.now,
          }
          await ctx.db.patch(run._id, {
            state: placing.state,
            child_workspace_id:
              resolvedPlacement(run.resolved_execution?.environment?.placement) === "shared" ? undefined : run.id,
            row_version: placing.row_version,
            updated_at: placing.updated_at,
          })
          if (resuming) await syncAttentionRecord(ctx, "workgraph_runs", placing)
        }
        ...
        return {
          ...
          ...(stream?.parent_stream_id
            ? {
                ...
                parentSource: {
                  repoUrl:
                    parent?.base_repository ??
                    parent?.execution_defaults?.environment?.repositoryUrl ??
                    parent?.execution_defaults?.repository?.remoteUrl,
                  branch:
                    parent?.base_revision ??
                    parent?.execution_defaults?.repository?.baseRevision ??
                    "HEAD",
                },
              }
            : {}),
  ...
  handler: async (ctx, args) => {
    const run = await ctx.db
      ...
      .filter((query) =>
        query.and(
          query.eq(query.field("organization_id"), args.organization_id),
          query.eq(query.field("owner_user_id"), args.owner_user_id),
...
export const completeControlEffect = serviceMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      (payload.finalize === "cancel" || payload.finalize === "restart") &&
      controlPayload.sessionId !== args.observed_session_id
    )
      return { settled: false }
    if ((payload.finalize === "cancel" || payload.finalize === "restart") && payload.runId) {
      if (
        !(await finalizeRunCancellation(ctx, args.organization_id, args.owner_user_id, payload.runId, args.now))
      )
        return { settled: false }
      if (payload.finalize === "restart") {
        await finalizeRunRestart(
          ctx,
          args.organization_id,
          args.owner_user_id,
          payload.runId,
          payload.reason ?? "Owner requested restart",
          args.now,
        )
      }
    }
    ...
    if (payload.finalize === "delete" && row.stream_id) {
      const complete = await deleteStreamGraphPage(
        ctx,
        args.organization_id,
        args.owner_user_id,
        row.stream_id,
      ...
      if (!complete) {
        ...
        return { settled: true, retryAfterMs: 0 }
      }
  ...
  if ((payload.finalize === "cancel" || payload.finalize === "restart") && payload.runId) {
    ...
    if (run) {
      const parked = {
        ...
        cancellation: { ...(run.cancellation as object | undefined), state: "attention" as const, reason, at: now },
        row_version: run.row_version + 1,
        updated_at: now,
      }
...
async function finalizeRunRestart(
  ctx: RuntimeMutationCtx,
  organization: Id<"orgs">,
  owner: Id<"users">,
  runId: string,
  ...
  const run = await ctx.db
    .query("workgraph_runs")
    .withIndex("by_tenant_id", (query) =>
      query.eq("organization_id", organization).eq("owner_user_id", owner).eq("id", runId),
    )
    .unique()
  if (!run || run.state !== "cancelled") return
  await ctx.db.patch(run._id, { parked_reason: `Restarted: ${reason}` })
  const item = await ctx.db
    .query("workgraph_work_items")
    .withIndex("by_tenant_id", (query) =>
      query.eq("organization_id", organization).eq("owner_user_id", owner).eq("id", run.work_item_id),
    )
    .unique()
  if (item?.state === "active") {
    const pendingItem = { ...item, state: "pending", row_version: item.row_version + 1, updated_at: now }
    await ctx.db.patch(item._id, {
      state: pendingItem.state,
      row_version: pendingItem.row_version,
      updated_at: pendingItem.updated_at,
    })
    await syncAttentionRecord(ctx, "workgraph_work_items", pendingItem)
  }
  const stream = await ctx.db
    .query("workgraph_streams")
    .withIndex("by_tenant_id", (query) =>
      query.eq("organization_id", organization).eq("owner_user_id", owner).eq("id", run.stream_id),
    )
    .unique()
...
export const WORKGRAPH_STREAM_DELETION_PAGE_SIZE = 100

export async function deleteStreamGraphPage(
  ctx: RuntimeMutationCtx,
  organization: Id<"orgs">,
  owner: Id<"users">,
  streamId: string,
  activeOutboxId?: string,
) {
  const tables = [
  ...
  for (const table of tables) {
    const page = await ctx.db
      ...
      .filter((query: any) => query.and(
        query.eq(query.field("organization_id"), organization),
        ...(table === "workgraph_outbox" && activeOutboxId
          ? [query.neq(query.field("id"), activeOutboxId)]
          : []),
      ))
      .take(WORKGRAPH_STREAM_DELETION_PAGE_SIZE)
...
export const recordResult = serviceMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const run = await ctx.db
      .query("workgraph_runs")
      .withIndex("by_tenant_id", (query) =>
        query.eq("organization_id", args.organization_id),
      )
      .filter((query) =>
        query.and(
          query.eq(query.field("organization_id"), args.organization_id),
    ...
    await foldAndGateStreamBudget(ctx, run, args.now)
    return { settled: true }
  },
})
...
export const markParked = serviceMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const renewal = await renewWorkGraphRunLease(ctx, {
      ...
      now: args.now,
      durationMs: PARKED_LEASE_TTL_MS,
      preserveGeneration: true,
    })
    if (!renewal || renewal.leaseEpoch !== args.lease_epoch) return { settled: false }
    return parkRunForResume(ctx, run, outbox, args.reason, args.now)
  },
})

export const parkRunning = serviceMutation({
  args: {
    organization_id: v.id("orgs"),
    owner_user_id: v.id("users"),
    run_id: v.string(),
  ...
  handler: async (ctx, args) => {
    const run = await ctx.db
      ...
      .unique()
    if (!run || run.state !== "running" || run.session_id !== args.session_id || run.generation !== args.lease_epoch)
      return { settled: false }
    const renewal = await renewWorkGraphRunLease(ctx, {
      organizationId: args.organization_id,
      ownerUserId: args.owner_user_id,
    ...
    const outbox = await ctx.db
      ...
      .unique()
    if (!outbox) return { settled: false }
    return parkRunForResume(ctx, run, outbox, args.reason, args.now)
  },
})

async function parkRunForResume(
  ctx: RuntimeMutationCtx,
  run: Doc<"workgraph_runs">,
  outbox: Doc<"workgraph_outbox">,
  reason: string,
  now: number,
) {
  const resumeAttempts = (run.resume_attempts ?? 0) + 1
  if (isRunResumeCapExceeded(resumeAttempts)) {
    ...
    const item = await ctx.db
      .query("workgraph_work_items")
      .withIndex("by_tenant_id", (query) =>
        query.eq("organization_id", run.organization_id),
      )
      .filter((query) =>
        query.and(
          query.eq(query.field("owner_user_id"), run.owner_user_id),
          query.eq(query.field("id"), run.work_item_id),
        ),
      )
      .unique()
    if (item) {
      const failedItem = { ...item, state: "failed", row_version: item.row_version + 1, updated_at: now }
      await ctx.db.patch(item._id, { state: "failed", row_version: failedItem.row_version, updated_at: now })
      await syncAttentionRecord(ctx, "workgraph_work_items", failedItem)
    }
    await releaseRunLeases(ctx, run.organization_id!, run.owner_user_id, run)
    await ctx.db.patch(outbox._id, { status: "failed", last_error: reason, claim_expires_at: undefined, updated_at: now })
    return { settled: true, state: "failed" as const }
  }
  ...
  await ctx.db.patch(run._id, {
    ...
    resume_attempts: parked.resume_attempts,
    row_version: parked.row_version,
    updated_at: parked.updated_at,
  })
  await syncAttentionRecord(ctx, "workgraph_runs", parked)
  await ctx.db.patch(outbox._id, {
    status: "pending",
    available_at: now + retryAfterMs,
    claimed_by: undefined,
    claim_expires_at: undefined,
    last_error: reason,
    updated_at: now,
  })
  return { settled: false, state: "parked" as const, retryAfterMs }
}
...
export async function renewWorkGraphRunLease(
  ...
  const lease = await ctx.db
    ...
    .unique()
  const sharedPlacement = resolvedPlacement(run.resolved_execution?.environment?.placement) === "shared"
  const streamLease = sharedPlacement
    ? await ctx.db
        .query("workgraph_leases")
        .withIndex("by_tenant_resource", (query: any) =>
  ...
  const advancesGeneration = recovered && run.state !== "parked" && !input.preserveGeneration
  const leaseEpoch = advancesGeneration ? lease.epoch + 1 : lease.epoch
