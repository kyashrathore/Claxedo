...
export const revoke = authedMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const grants = await ctx.db
      ...
      .collect()
    const matching = grants.filter((item) => {
    ...
    const active = matching.filter((item) => !item.revoked_at)
    if (active.length === 0) return { revoked: false }
    const revokedAt = Date.now()
    for (const grant of active) await ctx.db.patch(grant._id, { revoked_at: revokedAt })
    const canonical = active[0]!
    const userIds = canonical.granted_to_user_id
      ? [canonical.granted_to_user_id]
      : canonical.granted_to_org_id
        ? await orgUserIds(ctx, canonical.granted_to_org_id)
        : []
    return {
      revoked: true,
