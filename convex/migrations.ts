...
import { ensureDefaultTeam, ensureDefaultTeamMembership, ensureDefaultTeamProjectGrant } from "./teams"
...
// Default-team mirroring used to be re-run from the interactive org-list path.
// Each migration below owns one source row so large organizations remain
// inside Convex transaction limits. New writes maintain the same projections
// incrementally; this ledger chain repairs only historical rows.
export async function backfillDefaultTeamForOrg(ctx: GenericMutationCtx<DataModel>, org: Doc<"orgs">) {
  if (org.deleted_at || org.kind === "personal" || !org.owner_user_id) return
  await ensureDefaultTeam(ctx, { org, creatorUserId: org.owner_user_id, now: Date.now() })
}

export async function backfillDefaultTeamMembership(
  ctx: GenericMutationCtx<DataModel>,
  membership: Doc<"org_memberships">,
) {
  await ensureDefaultTeamMembership(ctx, {
    orgId: membership.org_id,
    userId: membership.user_id,
    role: membership.role,
    creatorUserId: membership.user_id,
    now: Date.now(),
  })
}

export async function backfillDefaultTeamProjectGrant(ctx: GenericMutationCtx<DataModel>, project: Doc<"projects">) {
  if (project.deleted_at || !project.org_id || !project.owner_user_id) return
...
async function defaultTeamForLegacyShare(
  ctx: GenericMutationCtx<DataModel>,
  orgId: Doc<"orgs">["_id"] | undefined,
  creatorUserId: Doc<"users">["_id"],
) {
  if (!orgId) return
  const org = await ctx.db.get(orgId)
  if (!org || org.deleted_at || org.kind === "personal") return
  return await ensureDefaultTeam(ctx, { org, creatorUserId, now: Date.now() })
}

export async function backfillDefaultTeamWorkspaceShare(
  ctx: GenericMutationCtx<DataModel>,
  share: Doc<"workspace_share_grants">,
) {
  if (share.revoked_at || !share.granted_to_org_id) return
  const team = await defaultTeamForLegacyShare(ctx, share.granted_to_org_id, share.created_by_user_id)
  if (!team) return
  const now = Date.now()
  const duplicates = await ctx.db
    .query("workspace_share_grants")
    .withIndex("by_workspace_team", (q) => q.eq("workspace_id", share.workspace_id).eq("granted_to_team_id", team._id))
    .collect()
  const keeper = duplicates.find((candidate) => !candidate.revoked_at) ?? duplicates[0]
  if (!keeper) {
    await ctx.db.patch(share._id, { granted_to_org_id: undefined, granted_to_team_id: team._id })
    return
  }
  if (keeper.revoked_at) {
  ...
  for (const duplicate of duplicates) {
    if (duplicate._id !== keeper._id && !duplicate.revoked_at) await ctx.db.patch(duplicate._id, { revoked_at: now })
  }
  await ctx.db.patch(share._id, { revoked_at: now })
}

export async function backfillDefaultTeamSessionShare(
  ctx: GenericMutationCtx<DataModel>,
  share: Doc<"session_share_grants">,
) {
  if (share.revoked_at || !share.granted_to_org_id) return
  const team = await defaultTeamForLegacyShare(ctx, share.granted_to_org_id, share.created_by_user_id)
  if (!team) return
  ...
  const duplicates = await ctx.db
    ...
    .withIndex("by_session_team", (q) => q.eq("session_id", share.session_id).eq("granted_to_team_id", team._id))
    .collect()
  const keeper = duplicates.find((candidate) => !candidate.revoked_at) ?? duplicates[0]
  if (!keeper) {
    await ctx.db.patch(share._id, { granted_to_org_id: undefined, granted_to_team_id: team._id })
...
export const backfillDefaultTeams = migrations.define({
  table: "orgs",
  migrateOne: backfillDefaultTeamForOrg,
})

export const backfillDefaultTeamMemberships = migrations.define({
  table: "org_memberships",
  migrateOne: backfillDefaultTeamMembership,
})

export const backfillDefaultTeamProjectGrants = migrations.define({
  table: "projects",
  migrateOne: backfillDefaultTeamProjectGrant,
})

export const backfillDefaultTeamWorkspaceShares = migrations.define({
  table: "workspace_share_grants",
  migrateOne: backfillDefaultTeamWorkspaceShare,
})

export const backfillDefaultTeamSessionShares = migrations.define({
  table: "session_share_grants",
  migrateOne: backfillDefaultTeamSessionShare,
})
  ...
  migrateOne: async (ctx, membership) => {
    ...
    const duplicate = await ctx.db
      ...
      .withIndex("by_project_user", (query) =>
        query.eq("project_id", project.project_id).eq("user_id", membership.user_id),
      )
      .unique()
...
export async function migrateSessionTenantIdentity(ctx: any, session: any) {
  ...
  const storedProject = session.project_id ? await projectFromStoredIdentity(ctx, session.project_id) : undefined
...
export async function migrateWorkspaceTenantIdentity(ctx: any, workspace: any) {
  const storedProject = workspace.project_id ? await projectFromStoredIdentity(ctx, workspace.project_id) : undefined
...
async function uniqueUserOrganization(ctx: any, userId: unknown) {
  ...
  const memberships = await ctx.db
    ...
    .collect()
  const organizations = (await Promise.all(memberships.map((membership: any) => ctx.db.get(membership.org_id)))).filter(
    (organization: any) => organization && !organization.deleted_at,
  )
...
export const scopeWorkGraphRunConnectionBindingsByOrganization = workGraphTenancyMigration(
...
export const scopeWorkGraphDurableReceiptsByOrganization = workGraphTenancyMigration(
...
export const scopeWorkGraphDeletionBarriersByOrganization = workGraphTenancyMigration(
...
export async function migrateWorkGraphTenancyRow(ctx: any, table: WorkGraphTenantTable, row: any) {
  ...
  const existing = await ctx.db
    .query("workgraph_tenancy_migration_quarantine")
