...
function sourceViewTarget(value: unknown) {
  ...
  const placement = environment.placement ?? "shared"
  if (placement !== "shared" && placement !== "worktree" && placement !== "sandbox") {
    throw new Error("Source View target placement is unsupported")
  }
  if (kind === "local_worktree") {
    return {
      environment: { kind, placement, directory: requiredString(environment.directory, "Source View target project directory") },
      repository: {
  ...
  return {
    environment: { kind, placement, repositoryUrl },
    repository: {
