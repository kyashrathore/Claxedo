...
/**
 * Ceiling on rows any one billing sweep tick reads (W5 index pass).
 *
 * Each sweep below now ranges an index down to just the orgs that can possibly
 * match (has a Polar customer / is flagged / holds a subscription), which is
 * bounded by paying customers rather than by signups. This is the second bound:
 * a hard constant, so no sweep can grow into Convex's per-transaction read cap
 * and start throwing on a cron path where nobody is watching.
 *
 * Both sweeps are LEVEL-TRIGGERED and re-run on their cron, and both remove
 * rows from their own range as they succeed (flagging clears on the next
 * `applyPolarState`; a cancelled subscription drops out). So a saturated tick
 * means the work continues next tick, not that an org is skipped forever.
 */
const BILLING_SWEEP_SCAN_LIMIT = 1_000
 ...
 * Convex never talks to Polar.
 *
 * INDEXED (W5). "The orgs table is small at launch" was true and is the wrong
 * bar: this is a 6-hourly cron, so the day the table outgrows Convex's
 * per-transaction read cap, billing reconciliation starts throwing where no
 * user is watching — and a silently dead staleness sweep is precisely the
 * failure this function exists to detect in Polar.
 *
 * The index is `by_polar_customer_id`, ranged above `undefined`: only an org
 * with a Polar customer can be stale, so that range is the exact candidate set
 * and every free org — the overwhelming majority — never enters the read-set.
 * The staleness cutoff stays a JS filter, because Convex needs `.eq` on a
 * leading index field before ranging a later one and there is no single
 * customer id to pin here. Bounding the read to PAYING orgs is the win; they
 * are bounded by the subscription count, not by signups.
 */
export const flagStaleBillingSync = cronMutation({
  ...
  handler: async (ctx, args) => {
    ...
    let flagged = 0
    const billed = await ctx.db.query("orgs")
      .withIndex("by_polar_customer_id", (q: any) => q.gt("polar_customer_id", undefined))
      .take(BILLING_SWEEP_SCAN_LIMIT)
    for (const org of billed) {
 ...
 *
 * W5: `by_billing_reconcile_flagged_at` ranged above `undefined` is exactly
 * "flagged" — the flag is set by the sweep above and CLEARED by
 * `applyPolarState`, so this set is self-draining and small by construction.
 * Previously it read every org in the deployment to find them.
 */
export const listReconcileFlagged = serviceQuery({
  args: {},
  handler: async (ctx) => {
    const orgs = await ctx.db.query("orgs")
      .withIndex("by_billing_reconcile_flagged_at", (q: any) => q.gt("billing_reconcile_flagged_at", undefined))
      .take(BILLING_SWEEP_SCAN_LIMIT)
    return orgs
      .filter((org: any) => org.polar_customer_id && !org.deleted_at)
      .map((org: any) => ({
        org_id: String(org._id),
...
export const listDeletedWithSubscription = serviceQuery({
  args: {},
  handler: async (ctx) => {
    // W5: keyed on the SUBSCRIPTION, not on `deleted_at`. Both fields are
    // optional, so either could index this, but orgs holding a live Polar
    // subscription are bounded by paying customers whereas deleted orgs
    // accumulate forever — the subscription is the smaller side, and it is also
    // the field whose presence makes this the "bills forever" path.
    const orgs = await ctx.db.query("orgs")
      .withIndex("by_polar_subscription_id", (q: any) => q.gt("polar_subscription_id", undefined))
      .take(BILLING_SWEEP_SCAN_LIMIT)
    return orgs
      .filter((org: any) => org.deleted_at !== undefined && org.polar_customer_id)
      .map((org: any) => ({
        org_id: String(org._id),
        polar_customer_id: org.polar_customer_id as string,
