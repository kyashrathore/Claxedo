...
const stateFile = process.env.CLAXEDO_CBX_CI_STATE_FILE
  ? resolve(root, process.env.CLAXEDO_CBX_CI_STATE_FILE)
  : join(root, ".crabbox", "ci", "last-run.json")
...
const focusedJobs = ["focus-agent-sdk-runtime-windows"] as const
...
function usage(): never {
  ...
  script/cbx-ci.ts dry-run [pr-linux|pr-native|pr|job ...]
  script/cbx-ci.ts run [--concurrency N] [--id LEASE] [pr-linux|pr-native|pr|job ...]
...
Set CLAXEDO_CBX_CI_STATE_FILE to isolate the result ledger for parallel orchestrators.`)
  process.exit(2)
}
