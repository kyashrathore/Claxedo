...
- Prefer direct edits to `.opencode/processes.jsonc` for process and port config.
- Use the `process-config` skill as a companion when editing `.opencode/processes.jsonc`.
...
- edit `.opencode/processes.jsonc` directly
- keep names short and stable
- use `{{port:name}}` references in env values when needed
...
4. Edit `.opencode/processes.jsonc` directly for process topology changes.
