...
| Runtime config apply | `/api/wr/config` applies a `RuntimeSnapshot` containing harnesses, model, auth, MCP, workspace harness, commands, and Agent Extension desired state. When harness id, access, or connection changes, the host replaces the active adapter. |
