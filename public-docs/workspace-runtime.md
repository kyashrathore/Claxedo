...
| Runtime config apply | `/api/wr/config` applies the host's current runtime snapshot. Generic connections arrive through the strict v3 connection schema and are selected with `{ kind: "connection", connectionId }`; there is no v2 connection importer or string-encoded connection identity. A snapshot may also carry `harnessLaunch`: opaque per-harness launch options a containing product projects (Claxedo's Agent Plugins module contributes plugin roots this way). See [Agent Connections](./acp-connections.md). |
