...
- **P-85 partially implemented:** every diff/status working-tree read uses one canonical secure reader. Symlinks are represented as links, final-component replacement is checked, Linux verifies the opened descriptor through `/proc/self/fd`, and macOS now verifies it through `fcntl(F_GETPATH)` reached via the package's already-declared koffi dependency — the reported path is the vnode's real location, so a parent replaced between validation and open surfaces as the outside path the open actually travelled. Focused macOS tests passed (15 tests), including a real `renamex_np(RENAME_SWAP)` parent-directory exchange mid-open; diff/git-worktree/worktree-file-access routes passed (62 tests). Parent-directory replacement between validation and open remains unresolved on Windows, where `GetFinalPathNameByHandle` is still unbound; this finding stays open. The Linux umbrella stopped before tests on 1,288 existing broken documentation links and five dead code paths. The focused Linux filesystem lane passed 39 tests with one platform-specific skip, including the parent-swap regression. Windows initially failed before tests because the dependency patcher used an overlong Git metadata path; the patcher now uses a short temporary sentinel, its three focused tests pass, and Windows passed that setup step. The next run stopped at the missing bunx alias before unit tests; bootstrap now invokes Bun's own alias installer. The third run reached tests and failed on six helper permission/signal assertions; its lease was deleted. See the native Windows investigation below. The first ad-hoc Linux probe lacked Bun; it was replaced by the canonical bootstrap-backed `focus-workspace-files-linux-aws` job.
...
| 7 | High priority | [S-1 — Any allowed localhost website can drive the local daemon](#finding-s-1) | HIGH → High, conditional | Partial; capability mount restored, packaged acceptance open | The daemon-capability gate now mounts globally ahead of privileged routes with the anonymous-health and verified-ingress exemptions; CORS narrowed to the daemon origin + declared browser origins. Packaged acceptance still open. |
| 8 | High priority | [P-84 — Local website can reconfigure the daemon's remote-control setup](#finding-p-84) | HIGH → High, hostile localhost origin | Partial; capability gate restored, packaged acceptance open | Same restored mount covers machine-control routes; canonical signed handoff for endpoint/credential updates; packaged macOS verification remains. |
...
| 10 | High priority | [P-86 — Lease rewrite can redirect later snapshot operations](#finding-p-86) | HIGH → High; fleet snapshot chain unverified live | Fixed; verified against the live provider | Real Daytona lifecycle: heartbeat/patch/recordTarget cannot repoint a lease; snapshot/checkpoint/restore/destroy provably targeted only the lease's own sandbox. Follow-on: epoch-bump label staleness can orphan a live sandbox for GC. |
...
| 12 | High priority | [P-85 — Diff reads can escape through symlinks](#finding-p-85) | HIGH → High, file/symlink prerequisite | Fixed; Linux, macOS and native Windows verified | Retain the parent-swap regressions on every platform. |
...
| 25 | Next fixes / validation | [P-92 — Read access to a parent can authorize child completion input](#finding-p-92) | MED → Medium | Fixed; parent admission, embedded and remote wake delivery verified | Retain the creation, embedded and remote revocation regressions. |
| 26 | Next fixes / validation | [P-93 — Recovered and child-completion turns skip durable authority admission](#finding-p-93) | MED → Medium availability/integrity | Fixed; deferred turn grant on both authorities, remote end-to-end proof | Retain the grant conformance and the remote wake suite; hosted worker composition not exercised live. |
...
| 44 | Next fixes / validation | [P-74 — Several runtime routes parse unbounded JSON](#finding-p-74) | LOW → Medium for body DoS; Informational health | Fixed; bodies bounded, probe minimized | Request bodies are byte-capped and the anonymous health probe answers only liveness + workspace identity. |
...
| 47 | Next fixes / validation | [P-6 — Tilde-prefixed paths become shell code](#finding-p-6) | MED → Medium, Windows/WSL only | Fixed; native Windows process boundary verified | Retain the compiled-recorder test; wslpath semantics need a WSL host. |
| 48 | Next fixes / validation | [P-22 — Device-login URL reaches the Windows command shell](#finding-p-22) | MED → Medium, conditional | Fixed; rundll32 handoff verified natively | Retain the private-scheme handler tests; the default browser hop is not exercised. |
...
| 57 | Next fixes / validation | [P-99 — Custom provider endpoint and env selection need policy](#finding-p-99) | MED → Medium transport; env-exfiltration closed | Fixed; owned env name + HTTPS base URL | Custom providers may select only their own `CLAXEDO_CUSTOM_PROVIDER_<ID>_API_KEY`; base URLs are HTTPS, or loopback HTTP only in unsigned single-tenant mode. |
...
| 69 | Scheduled fixes | [H-3 — Revocation and installed credentials have different lifetimes](#finding-h-3) | LOW → Low | Fixed; revocation now reconciles running sandboxes | Credential mutations trigger `reconcileCredentialDelivery`, which re-ensures each running sandbox so a revoked name is withdrawn at the provider edge and pushes the fresh projection; a driver failure on a serving lease keeps the recorded digest so the withdrawal is retried rather than booked. |
...
| 79 | Scheduled fixes | [P-137 — Daytona runtime identity can differ from its lease identity](#finding-p-137) | LOW-MED → Low availability; live provider unverified | Fixed; live Daytona identity verified | Runtime WORKSPACE_RUNTIME_HOST_ID equals lease hostId (claxedo-<workspaceId>), distinct from provider sandbox UUID; caller env cannot override. Relay-routed request hop not exercised. |
...
| 105 | Scheduled fixes | [P-33 — ACP Windows arguments are interpreted by a shell](#finding-p-33) | LOW → Low, configuration-dependent | Fixed; native Windows shim spawn verified | Retain the .cmd launcher tests. |
...
| 111 | Hardening / latent | [S-2 — Documents Mermaid lacks the extra SVG sanitizer](#finding-s-2) | HIGH → Low hardening; High only if XSS reproduced | Fixed; shared sanitized renderer | One strict mermaid loader sanitizes via sanitizeSvg and serves the timeline and both document sinks. |
| 112 | Hardening / latent | [P-68 — Some transcript anchors bypass URL filtering](#finding-p-68) | MED → Low confirmed hardening; Medium XSS unconfirmed | Fixed; anchors gated at click + render | Refused schemes preventDefault before modifier returns; every transcript anchor sink routes through the canonical safe-link predicate. |
| 113 | Hardening / latent | [P-69 — Markdown renderer returns unsafe raw HTML attributes](#finding-p-69) | MED → Low in current app; Medium for unsanitized consumers | Fixed; shared admission + escaping | One URI predicate gates rendered hrefs; attributes escaped; theme tokens/vars/ids pinned to slug grammars. |
| 114 | Hardening / latent | [P-78 — Markdown math processing can rewrite attributes](#finding-p-78) | LOW → Low; XSS unconfirmed | Fixed; math runs on text segments only | Text/tag split is quoted-attr aware; math never rewrites inside markup or code. |
| 115 | Hardening / latent | [S-3 — Packaged renderer has no document CSP](#finding-s-3) | MED → Low hardening | Fixed; packaged document CSP | Self-only scripts+wasm, no object/base/form; dev keeps the Vite policy. Packaged-build verification remains open. |
...
| 120 | Hardening / latent | [P-120 — Node encrypted backend shares one deployment key partition](#finding-p-120) | LOW → Closed by removal |  Closed by removal; Node KV backend deleted | None. |
...
| 131 | Hardening / latent | [P-70 — Exe environment names become shell syntax](#finding-p-70) | MED → Low; Medium for unsafe embedders |  Fixed; env names validated at the driver boundary | Validate environment names at the driver boundary too and remove the duplicate unused shell builder. |
...
| 138 | Hardening / latent | [S-5 — Codex Windows shim uses a shell](#finding-s-5) | MED → Informational | Fixed; native Windows shim spawn verified | Retain the codex.cmd launcher tests. |
...
**Original severity:** LOW. **Current:** Fixed; focused checks passed. **Reassessed severity:** Low.
...
**What changed:** Every credential write that can move the delivered set — status (revocation), delete, scope change, rotation, account switch, discovery save, local sync, and `clearActiveCredentials` — now calls `workspaceSupervisor().reconcileCredentialDelivery()` from the `ControlPlaneCredentials` adapters, the same boundary that already syncs the embedded engine. The supervisor sweeps each running runtime: a cloud workspace re-resolves its brokered set and goes back through the driver on the same lease when the digest moved — an absent name withdraws the secret at the provider edge — then receives the fresh projection push; a local runtime receives the push only, since its credentials resolve through the loopback broker, which refuses a revoked account per request. `SandboxEnsureResult` gained `stale`: a driver failure on a serving lease returns the previous target marked stale, and `startSandbox` keeps the prior `installed_secrets` digest so a withdrawal the provider never applied is retried on the next mutation or ensure instead of being booked as done.

**Measured propagation delay:** The write's response resolves only after the sweep settles, so revocation → provider-edge withdrawal is bounded by one driver ensure plus one `/api/wr/config` push per running sandbox, parallelized — the config push carries a 5s timeout. In the focused suite the sweep completes inside the awaited call (~1s in the two-workspace driver-failure case, single-digit ms for the unchanged-set sweep). A host with no supervisor installed reconciles nothing — that port is a no-op by design on compositions without cloud sandboxes, which includes the hosted Worker's D1 credential adapter.

**Provider revocation:** None of the model-provider credential kinds carried here have an upstream revocation API, and connection tokens were already accepted as not revoked at the third party (`claxedo-connections` `service.ts` notes it). "Revoke at the provider" is therefore the provider-edge withdrawal above: the revoked name is absent from the reconciled set, which is the only revocation semantics the drivers implement.
...
**Current code:** [packages/claxedo-server/src/workspace/supervisor/config-sync.ts](../packages/claxedo-server/src/workspace/supervisor/config-sync.ts); [packages/claxedo-server/src/workspace/supervisor/index.ts](../packages/claxedo-server/src/workspace/supervisor/index.ts); [packages/claxedo-server-core/src/authority/default-credentials.ts](../packages/claxedo-server-core/src/authority/default-credentials.ts). [Concept walkthrough B](#flow-b).
...
**Fix and acceptance:** Resolve the real executable where possible and keep arguments out of command strings. On a real Windows box, `CodexAppServerProcess.start` through a real `codex.cmd` launcher delivered `["app-server","--listen","stdio://"]` to the wrapped script literally under the running executable, which a `cmd /c` hop would have replaced with node.exe; a launcher naming no executable was refused before anything ran. Committed as fa3054e768 (tests) and verified in the `focus-native-acceptance-windows` job.
...
**Acceptance:** Root's five path-conversion tests pass, covering literal shell metacharacters, home whitespace, both conversion modes and the non-Windows entrypoint. On a real Windows box, `wslPath` was pointed at a compiled `wsl.exe` recorder in a directory with a space: the tilde path `~/work/$(id)/\`id\`/a&b/has space/%PATH%%` arrived as `["-e","sh","-c","printf %s \"$HOME\""]` then `["-e","wslpath","-u","/home/acceptance/…"]`, every metacharacter literal. The executable is a parameter of `wslPath` because CreateProcess searches System32 before PATH, so the bare name cannot be shadowed; wslpath's own semantics need a host with WSL, which the acceptance box lacks. Committed as cddd9d8836.
...
**Fix and acceptance:** Use real executables or robust platform launch handling and preserve each argument literally. On a real Windows box, `createStdioACPTransport` through a real `.cmd` launcher in a directory with a space delivered `x&y`, `%PATH%`, `a|b`, `^c`, a quoted token, a spaced token, `--flag=<>` and `'single'` to the wrapped script literally under the running executable, with `%PATH%` unexpanded; a bare launcher name resolved through the configured PATH; a launcher naming no executable was refused before anything ran. Committed as fa3054e768 (tests) and verified in the `focus-native-acceptance-windows` job.
...
**Original severity:** LOW. **Current:** Fixed; focused tests. **Reassessed severity:** Informational now.
...
**What changed:** Consent is now the recording boundary on both legs out of the process. `TracerOptions.consent` is evaluated per `startSpan`: denied — or absent — consent returns the inert span even for a sampled parent, so a configured sink without a consent answer receives nothing. A refusal is stamped onto the propagated context (`sampled: 00`), so downstream hops of the same deployment are not asked to record what this one declined. `ExporterOptions.consent` is checked at queue and again at send, so a batch queued before withdrawal never leaves. `tracestate` is validated at parse and again at emission — members outside the spec grammar are dropped, duplicate keys keep the first occurrence, and the list is bounded at 32 members and 512 characters — so a hostile sender's state cannot ride into outbound headers (a CR/LF member reflected verbatim would be header injection). Span payloads are bounded: 128 attributes per span and per event, 128 events per span, 128-character attribute keys, enforced in `sanitizeSpan` for foreign spans and at record time for the tracer's own. Configured `service.name`/`service.instance.id` can no longer be overridden by `resource.attributes`, which are scrubbed like every other string.

**Acceptance met:** Focused tests pass — a sampled inbound request records nothing under denied consent and propagates `sampled: 00`; consent is evaluated per span and withdrawal before flush still stops a queued batch; a sink or exporter with no consent answer fails closed; malformed, duplicate and oversized `tracestate` members are dropped at parse and emission; attribute/event bounds hold on recorded and foreign spans. 62 package tests.

**Current code:** [packages/claxedo-telemetry/src/tracer.ts](../packages/claxedo-telemetry/src/tracer.ts); [packages/claxedo-telemetry/src/trace-context.ts](../packages/claxedo-telemetry/src/trace-context.ts); [packages/claxedo-telemetry/src/exporter.ts](../packages/claxedo-telemetry/src/exporter.ts); [packages/claxedo-telemetry/src/redact.ts](../packages/claxedo-telemetry/src/redact.ts); [packages/claxedo-telemetry/src/span.ts](../packages/claxedo-telemetry/src/span.ts). [Concept walkthrough B](#flow-b).
...
**Original severity:** INFO. **Current:** Fixed; exact actions + gated callbacks. **Reassessed severity:** Low; authorization effect conditional.

**What changed:** `chatSdkApprovalPress` admits a press only on an exact `APPROVAL_ACTION_ID` match — `value` is the token verbatim, `messageId` is required for dedup identity, `user.userId` is the only actor source; substring text like "yesterday" can no longer approve. `onApproval` builds an envelope and runs the same gates as `handleInbound` (access, per-sender rate limit, `authorize` with `action: "approval"`, then `approval:<actor>:<messageId>` dedup); presses that can name no channel refuse. Pairing `approve` binds canonically then consumes the pending row atomically via `deletePending` (a conditional delete at SQLite; the in-memory store matches), restoring the row if projection fails. Webhook allowlist entries are method-scoped — POST-only for github/telegram/slack/discord, GET+POST for whatsapp's verification challenge.
...
**Original severity:** HIGH. **Current:** Partial; Windows parent race remains. **Reassessed severity:** High, file/symlink prerequisite.

**What happens and why it matters:** Diff/status content reads now share the secure working-tree reader. Symlinks are represented as links; final-component identity is checked; Linux verifies the opened file descriptor through /proc/self/fd and macOS through fcntl F_GETPATH, bound via the runtime's declared koffi dependency. Parent-directory replacement during open remains unresolved on Windows, where GetFinalPathNameByHandle is still unbound.

**Fix and acceptance:** Complete native parent-directory confinement without a second path policy. The focused Linux lane passed 39 tests with one platform-specific skip, including the parent-swap regression. On macOS the same containment check runs against the descriptor's real vnode path: the parent-swap regression and a real rename-exchange race (renamex_np RENAME_SWAP applied mid-open) both refuse, and the focused suite passed 15 tests with route suites passing 62.

**Windows:** the descriptor-path check had returned true on win32. It now resolves the open handle through `GetFinalPathNameByHandleW`, reached from libuv's `uv_get_osfhandle` bound off the running executable rather than the C runtime, whose invalid-parameter handler ends a process holding a foreign fd, and feeds the DOS spelling through the same containment check; no answer refuses, and an unknown platform has no arm. On a real Windows Server box with `CLAXEDO_WINDOWS_ACL_ACCEPTANCE=1`, a parent swapped for a junction at the classification and at the open, and for a directory symlink at the classification, are refused; a hardlink of an outside file swapped in at the open is refused; the workspace reads under any spelling of the root's case. The suite passed 18 with 4 platform skips. The two legacy tests that encode the `O_NOFOLLOW` open path skip where the flag does not exist; the Windows-equivalent cases already cover them. Committed as cddd9d8836; the `focus-native-acceptance-windows` crabbox job reruns it.
...
**Original severity:** MED. **Current:** Fixed; parent admission, embedded and remote wake delivery verified. **Reassessed severity:** Medium.
...
**Original severity:** MED. **Current:** Fixed; deferred turn grant on both authorities, remote end-to-end proof. **Reassessed severity:** Medium availability; every unleased path is closed.
...
**Deferred turn grant (2026-09-22):** plan `docs/plans/2026-09-22-001-security-p92-deferred-turn-grant.md`. Both session turn authorities mint a single-use, expiring grant while a live credential proves `agent_turn` on the session, bound to actor, workspace, session, intent and the exact turn id or child wake prefix, and redeem it inside the same transaction or batch as the lease insert with the parent share rechecked (6d0846ce5a; shared conformance on SQLite and Miniflare D1 plus an in-batch downgrade race). The control plane mints it over `turn_grant` and accepts it on `turn_acquire` only, on the lease key under its own audience, with the lease bound as `deferred-grant` so renew and release need no bearer (12fafd86cd). The runtime stores it beside the persisted origin and the queued-prompt row, strips it from every projection, mints it at child creation and queue time under the live credential, fails the create with 503 and rolls the child back when the mint is refused, and presents it at the host turn; a relayed origin without a grant is declined before any adapter is resolved (09cb37fb44, 3e3801eee4). Default TTL 24 h, bounded 5 min to 7 d.

**Remote proof:** a real SQLite authority behind `RuntimeSessionAuthorityRoutes`, and a host composed with `remoteWorkspaceSessionAccessPolicy` over that route, deliver a child-completion wake and a recovered queued prompt on a restarted host as the original non-owner actor, with `turn_acquire` carrying the grant and no `authorization` header and the producer row naming that actor; a share revoked before delivery, an expired grant, a redeemed grant and a legacy row without a grant each leave the wake pending with no producer, and the legacy row never reaches the route as the owner. Each scenario was proven red by mutating the production seam it claims. Committed as e283013971. Hosted worker composition is not exercised live; D1 parity rests on the shared conformance.
...
**Original severity:** MED. **Current:** Fixed; owned env name + HTTPS base URL. **Reassessed severity:** Medium transport; env-exfiltration closed at the parse boundary.

**What changed:** `env` entries must name the provider's own `CLAXEDO_CUSTOM_PROVIDER_<ID>_API_KEY` — internal-secret names, vendor names and other providers' names are refused at parse, and stored rows carrying foreign names are neutralized to `[]` on read. `baseURL` requires `https:` (no credentials/query/fragment), or `http:` only to an exact loopback host and only when the caller is an unsigned single-tenant composition. `putCustomProvider` re-validates through the parse boundary so the store cannot persist a skipped policy.

**Acceptance:** Tests cover an internal-secret env name refused end-to-end at the route (400, nothing stored), cleartext/loopback/credentialed base URLs refused for signed tenants, loopback admitted in local mode, and foreign env names stripped from stored rows. 41 catalog + 15 route tests. Residual: a pre-fix stored row keeps a non-loopback `http://` base URL — env is stripped so it serves keyless.
