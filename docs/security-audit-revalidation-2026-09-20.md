...
| 12 | High priority | [P-85 — Diff reads can escape through symlinks](#finding-p-85) | HIGH → High, file/symlink prerequisite | Partial; Windows race remains | Finish parent-directory replacement protection on Windows and native verification. |
...
| 138 | Hardening / latent | [S-5 — Codex Windows shim uses a shell](#finding-s-5) | MED → Informational | Partial; source fixed, Windows acceptance open | Same resolution applies to the Codex app-server launch; unresolvable shims fail closed. Native Windows run remains. |
...
**Fix and acceptance:** Complete native parent-directory confinement without a second path policy. The focused Linux lane passed 39 tests with one platform-specific skip, including the parent-swap regression. On macOS the same containment check now runs against the descriptor's real vnode path: the parent-swap regression and a real rename-exchange race (renamex_np RENAME_SWAP applied mid-open) both refuse, and the focused suite passed 15 tests with route suites passing 62. Windows validation is ongoing; packaged macOS is blocked by missing Crabbox host configuration.
