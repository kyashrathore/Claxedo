...
| 7 | High priority | [S-1 — Any allowed localhost website can drive the local daemon](#finding-s-1) | HIGH → High, conditional | Fixed; capability mount restored | The daemon-capability gate now mounts globally ahead of privileged routes with the anonymous-health and verified-ingress exemptions; CORS narrowed to the daemon origin + declared browser origins. Packaged acceptance still open. |
