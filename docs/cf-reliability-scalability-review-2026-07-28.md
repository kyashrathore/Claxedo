...
|---|---|---|
| **Control-plane Worker** (`packages/claxedo-server`, `src/deployments/hosted-workerd/better-auth-d1-locked-worker.cf.ts`, composing `src/deployments/hosted-shared/hosted-core-app.ts`'s `createHostedCoreApp`) | A Cloudflare Worker: serverless code that runs in many short-lived **isolates** all over the world. There is no long-lived process and no shared memory between isolates. | Nothing durable. Every request re-derives what it needs. |
