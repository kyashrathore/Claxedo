---
title: "Sync Architecture (Current Implementation)"
status: active
type: current-state
date: 2026-04-22
---
...
- [docs/sync-architecture-target.md](/Users/yashvardhansingh/.codex/worktrees/b0da/opencode/docs/sync-architecture-target.md) — adopted target architecture
...
- source of truth: `.claxedo/processes.jsonc` in each workspace/project directory
...
| Process definitions | Workspace file `.claxedo/processes.jsonc` | `ProcessManager.configs(...)` / process routes | Not centrally synchronized in current implementation. |
...
- explicit metadata, timeline, and projection ports around the current SQLite-backed implementation
- canonical metadata tables separated cleanly from replay/timeline durability
- durable runtime/timeline append contract for hosted replay-worthy events
- projection/read layer as the primary hosted read model
...
- relay/bootstrap-aligned hosted attachment path replacing reliance on merged proxy-first state
...
1. Introduce explicit metadata, timeline, and projection interfaces around the current SQLite-backed paths.
2. Keep SQLite as the current in-app implementation while those interfaces settle.
3. Expand central durability coverage for shared metadata, replay-worthy timeline data, and PTY lifecycle metadata.
4. Replace merged-source hosted reads with projection-backed reads over time.
5. Treat the current proxy-first hosted path as transitional compatibility, not as the long-term sync center.
