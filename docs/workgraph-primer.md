...
**Package shape** (`packages/workgraph/src/`): `contracts/` — Zod schemas that *are* the API (38 commands, all reads, all ids); `domain/` — pure functions only: the state machines, the launchability oracle, completion contracts, landing integrity; `application/` — services (webhook intake, admission, execution placement, session intake); `adapters/` — the two store implementations (SQLite + the Convex functions at repo-root `convex/workgraph*.ts`); `connectors/` — GitHub/Linear/Jira; `http/` — the router; `hosted.ts` — the Cloudflare-hosted wiring.
