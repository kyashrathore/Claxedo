...
|---|---|---|
| P1: Markdown paste drops unsupported content | **Partially closed** | [markdown-paste.ts:28](packages/claxedo-app/src/features/documents/editor/markdown-paste.ts:28) rejects ordinary HTML comments, which the default paste path preserves as literal text. Frontmatter escapes that check but is subsequently parsed, still losing comments. |
| P1: External description replacements bypass fidelity gate | **Partially closed** | [tasks-prose-editor.tsx:29](packages/claxedo-app/src/app/integrations/tasks/tasks-prose-editor.tsx:29) re-detects ordinary replacements. However, `emitted` survives external replacements, allowing an earlier textarea value to inherit a later value’s rich admission. |
| P2: Inline paste splits the paragraph | **Closed** | [markdown-paste.ts:58](packages/claxedo-app/src/features/documents/editor/markdown-paste.ts:58) fits inline content into the selection. The production handler now produces `hello nice world`; block formatting has a separate regression below. |
| P2: Create a preset opens nothing | **Closed** | [tasks-view.tsx:109](packages/claxedo-app/src/features/tasks/ui/tasks-view.tsx:109) opens the draft and navigates to Presets, where the existing editor renders it. The added test checks editor appearance, not merely callback invocation. |
| P2: Deleted sessions open; their slots cannot restart | **Closed** | [start-task.ts:80](packages/claxedo-app/src/features/tasks/data/start-task.ts:80) derives the next attempt from detail, and [start-task.ts:112](packages/claxedo-app/src/features/tasks/data/start-task.ts:112) navigates only to a live session. The new primary-only selection causes a distinct regression below. |
| P2: Subtask progress excludes completed children | **Closed** | [tasks-view.tsx:61](packages/claxedo-app/src/features/tasks/ui/tasks-view.tsx:61) reads authoritative `children` counts. All three stores count unarchived children independently of the displayed page and status filter. |

**New findings, ordered by severity**

1. **P1 — Pasted frontmatter bypasses representability checking.**  
   [markdown-paste.ts:28](packages/claxedo-app/src/features/documents/editor/markdown-paste.ts:28), [markdown-paste.ts:50](packages/claxedo-app/src/features/documents/editor/markdown-paste.ts:50)
...
2. **P1 — An earlier emitted value can inherit an unrelated replacement’s admission.**  
   [tasks-prose-editor.tsx:25](packages/claxedo-app/src/app/integrations/tasks/tasks-prose-editor.tsx:25)
...
3. **P2 — Maximally opening every pasted block strips its structure.**  
   [markdown-paste.ts:58](packages/claxedo-app/src/features/documents/editor/markdown-paste.ts:58)
   ...
   **Fix:** open a lone ordinary paragraph for inline insertion, while preserving explicit heading, quote, list and other block boundaries. Extend [markdown-paste.vitest.tsx:81](packages/claxedo-app/src/features/documents/editor/markdown-paste.vitest.tsx:81): its empty-document fixture cannot expose this regression.

4. **P2 — Row Open no longer opens a task with only a secondary-slot session.**  
   [start-task.ts:107](packages/claxedo-app/src/features/tasks/data/start-task.ts:107)
...
5. **P2 — The restart regression test can pass when no restart happens.**  
   packages/claxedo-app/src/features/tasks/ui/row-start-actions.vitest.tsx:155
...
- The new [conformance case:535](packages/claxedo-tasks/src/conformance/store.ts:535) proves page-independent totals, completed-child inclusion, scope isolation and zero counts. Its page-folding mutant is caught. Coverage still lacks archived-*done* children and UI assertions displaying nonzero progress.
