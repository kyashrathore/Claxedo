...
| T21 | none | none | NOT RUN | Skill and plugin selection is S6; this slice reports `cloudSelectedCapabilities: false`, so no selected closure is ever built. |
| T22 | `packages/claxedo-server/src/tasks/hosted-composition.test.ts` placement rows | none | NOT RUN (credential scope GATING) | Proven only that the placement is offered exactly when the deployment has a sandbox driver: a driverless hosted composition answers `placements: ["local"]` and refuses a saved cloud preset, and preview on it blocks with `placement_unsupported`; a driver-configured one answers `placements: ["local", "cloud"]`. Nothing about credential scope, gateway enforcement or revocation was exercised — that is S6, and until it lands a cloud preset still cannot be saved on either composition (`cloudSelectedCapabilities: false`), so cloud Start is not reachable by a user through the real routes. |
