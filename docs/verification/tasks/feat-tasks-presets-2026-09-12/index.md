...
## S4 live: the Marketplace switch decides a session's Tasks tools (2026-09-14)

On the running local stack (server 2594 at `8fe26e7c41`, data dir `stack2`), the built-in `claxedo` row served eight groups with `tasks` off at revision 0. Two fresh Claude-harness sessions were created through `POST /session` and asked, without calling any tool, to list their MCP tool names.

| Step | Request | Result |
|---|---|---|
| Tasks off | fresh session `396943b0…` | `mcp__claxedo__*` for attention, documents, processes, sessions, subagents; no `task_*` |
| Enable | `POST /api/claxedo/plugins/activation {pluginInstanceId: "claxedo:tasks", choice: true, expectedRevision: 0}` | `{"revision":1,"reconciliation":{"state":"applied"}}`; catalog now `tasks: true` |
| Tasks on | fresh session `94db1aeb…` | the same list plus `task_create`, `task_get`, `task_list`, `task_start` |

No server restart between the two sessions: the mount reads the consent per connection. The in-process proof of the same flow is `packages/claxedo-local-server/src/app/first-party-mcp.live.test.ts` ("the Marketplace switch is what puts the Tasks tools on the next connection"). The Tasks group was left enabled on the dev stack.
