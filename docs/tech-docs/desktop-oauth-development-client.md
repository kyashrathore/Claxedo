...
3. Compared the token's `iss` to the operator's local `CLERK_JWT_ISSUER` value;
   `packages/claxedo-server/.env.example` owns the repository-visible configuration contract — **match**.
