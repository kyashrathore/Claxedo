...
| `controlPlane.events` | `app/integrations/claxedo-events.tsx` | `GET /api/cp/events` | stream | safe | The control plane's notice stream (provision steps, worktree readiness, document doorbells, share grants, inventory changes) — never a session's frames, which are the workspace runtime's `/api/wr/events` on the RAT data plane. Resumes via declared `Last-Event-ID` header param. |
