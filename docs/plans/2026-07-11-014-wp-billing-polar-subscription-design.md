...
Siblings: `2026-07-11-012-feat-cloud-subscription-launch-plan.md` (launch plan; this ADR is the design record behind its WS-B), doc 013 (Better Auth migration design — the auth stack this billing design rides on)
Scope of this document: architecture decisions and their reasoning. NOT an edit map, NOT a stage plan — doc 012 §3/§4 owns sequencing.
...
- **Self-host stays free and Polar-free.** Any design that leaks a Polar dependency into the self-host default path is wrong by construction (doc 012 invariant I-1).
...
- **A plan concept.** Nothing distinguishes a free org from a paying org. Needed: plan + subscription-status + seats fields on `orgs` (doc 012 B1 names them).
...
- **Margin nibbles, acknowledged not engineered around:** non-US cards carry an extra 1.5% pass-through on every plan; the 5%+$0.50 tier takes $9/mo to ≈$8.05 net. Both are priced into doc 012's unit economics; neither changes architecture.
...
Everything above is decided; one dependency is *asserted by documentation but unverified in practice*: Polar's mid-cycle seat-quantity behavior. S2 (doc 012 §2) runs against a Polar sandbox org with the real $9/mo and $89/yr per-seat products and must answer, precisely:
