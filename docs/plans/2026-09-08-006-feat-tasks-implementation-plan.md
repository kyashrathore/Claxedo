...
Persist personal presets, project-owned tasks, TaskSessionLinks unique per slot attempt and plain command receipts. There is no Run, plan, activity or bot table. Add finite commands, expected revisions, payload bounds and atomic parent/link guards. Preset references are provenance; a session must survive preset archival without loading private preset content through task APIs. Migrations use host lifecycle and are staged only for enabled products.

Run the same conformance suite against SQLite and Miniflare D1, including independent concurrent requests, failed-predicate rollback, restart, unauthorized IDs/cursors and revocation. Verify at most one link per slot attempt, that a new attempt is refused while the session owner reports the current attempt's session live, and no private preset disclosure through shared tasks. Acceptance: T02–T07, T14–T15, T17, T24.
...
Task UI retains manual statuses and Circle-style list/board/detail. Start requires a preset and selects a configuration; when no presets exist the dialog creates one inline without discarding the draft, and there is no hidden default. Live linked slots open ordinary sessions. A slot whose session the owner reports archived, deleted or unavailable offers Start again and, when the previous transcript is readable, Continue from previous session. Optional handoff text is explicit and is not an activity comment or plan document. Use mounted Vitest for Solid interactions and existing browser tiers, not duplicate test frameworks. Acceptance: T02–T07, T13, T17–T18.

### S4 — Canonical session-start configuration and local linking

Trace and extend current registration/create/storage contracts for a generic resolved start configuration and durable source-origin association. Keep host task/preset resolution in proposed `tasks/session-bridge.ts`; runtime receives resolved values, not Tasks service access. Include instruction content, model group, selected slot, attempt and source/config hash as needed by the canonical owner. No secret values or live preset dereference on resume.

Qualify unique `(scope, taskId, slot, attempt)` origin reservation before side effects. Two clients with the same resolved settings return one session; competing settings conflict. Attempt current+1 is accepted only when the session owner reports the current attempt's session archived, deleted or unavailable; liveness is read from that owner, never stored by Tasks. Continue from previous session reuses the existing handoff transaction under its own caps and records `continuedFrom` on the link; a deleted or unreadable transcript starts from task text. Same client request after uncertainty recovers the canonical receipt. Create/link and first-message are distinct failure points: persist link before handoff, distinguish initial submission from replay, never blindly resend. Review the existing child-create/first-prompt replay path as well; its request ID alone must not be assumed to deduplicate prompt submission.

For Local, use the selected actual host/workspace and existing skills/plugins without config mutation. Resolve real harness/model/effort, carried as the runtime's `variant`, and wire instructions through each supported adapter, retaining them across resume. Open later must not create/send or reapply an edited preset. Task edits/archive/access loss during start cannot attach a session to a different target. Created resources remain under canonical host recovery/cleanup.
...
Extend current workspace creation/sandbox preparation to reserve and recover a dedicated workspace for each root origin, with the user's authorized project source/ref. Different tasks/slots/attempts cannot share mutable runtime/home/plugin state; retries of one attempt reuse their own workspace. Children of a root can share that root's capability set.
...
| Slot restarted while its session is still live | Attempt current+1 is gated by owner-reported liveness; the live session is returned, never replaced; Tasks stores no liveness flag |
| Continue silently copies a transcript | Continue is an explicit choice; the existing handoff transaction renders the prior session under its caps; deleted or unreadable transcript starts from task text |
...
- **G1 — S2 store/auth owner:** atomic task/parent/slot-attempt/preset guards in SQLite and D1, bounded data and personal-preset/project-task isolation.
- **G2 — S4 session owner:** durable origin/config association, canonical instruction/group persistence, two-client creation, first-message replay across restart, attempt gating by owner-reported liveness and continue-from-previous handoff. Existing ID parameters do not close this gate.
