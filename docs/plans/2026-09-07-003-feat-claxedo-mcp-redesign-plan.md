...
- [ ] A child session never appears in the sidebar, `sessions_list`, or any root listing; archiving the parent cancels its active children and deleting the parent deletes them. *Progress:* met. Three list routes honour `roots`: `GET /session` and `GET /experimental/session` in `session-core.ts`, which the sidebar reaches through `platform/sync/session-load.ts`, and the desktop's flat inventory `GET /api/claxedo/session`, which the rail reaches through `inventory-source.ts`; `sessions_list` asks the second for `roots: true`. `session-children.routes.test.ts` pins both runtime routes and `first-party-mcp-subagents.live.test.ts` pins all three plus the tool against a real local server, a real workspace and a child created through `create_subagent`. `cascadeToChildren` runs on archive and on delete.
