...
the inventory does not have. Read-only is a property of the credential — an
OAuth token whose granted scopes are only `claxedo:read` — and it hides every
tool whose operation is in `WRITE_OPERATIONS` and refuses it if called anyway.
...
- [ ] Every operation in `SESSION_CORE_ROUTE_ACCESS` has a tool, and the guard test that proves it lives in `packages/claxedo-mcp`. *Progress:* met. `tools/inventory.ts` builds `RUNTIME_OPERATIONS` from the runtime's exported table; `inventory.test.ts` fails on an operation with no tool, on a tool naming an operation the table lacks, and pins each deliberate exclusion with its reason.
- [ ] Read-only mode is computed from `WRITE_OPERATIONS`; no tool has a hand-written read-only branch. *Progress:* met. `runtimeToolAccess` takes `write` from `sessionAccessRequiresWrite`. Read-only is a property of the credential — for an OAuth token, scopes ⊆ {read} — and `tools/list` and the handler both read it through `assertToolAccess`. The mount hook nothing computed is gone, and `CLAXEDO_MCP_READ_ONLY` is read nowhere: it belonged to the stdio server that ran inside the harness child, which no longer exists.
- [ ] One client answers on local, hosted and self-hosted; no tool constructs a URL. *Progress:* met. `client/index.ts` plus `client/relay-connection.ts` resolve a target to a base URL and headers for all three; the plan's `createClaxedoServerClient` move was not needed because the runtime's own `createWorkspaceRuntimeClient` is already Node-safe and is what the client wraps. Four call sites pass a runtime or control-plane *path* rather than a typed member — session create (`?nativeHarness=`), worktree create, cloud workspace create, subagent create — because those routes have no typed member; none builds an origin.
- [ ] The loopback mount rejects anything but an injected runtime credential; installed apps use the hosted URL with MCP OAuth consent or `~/.claxedo/credentials.json`; the desktop writes the shared credential file. *Progress:* met. Loopback: `server.test.ts` pins the 401, the non-loopback `Origin`/`Host` 403 and the absent CORS header. OAuth is reachable on both remote mounts as of this branch's last slice. The shared file is `@claxedo/helpers/claxedo-credentials`, written by `claxedo login` and, opt-in, by `claxedo-desktop/src/main/account/cli-credential-file.ts`.
- [ ] `attention_list` reflects truthful capabilities and pending lists (Cursor never pending; aborted ACP turns cleared). *Progress:* met in code as `sessions_board`, which reads `permission.list`, `question.list` and `session.status` live per workspace and reports a machine it cannot reach as offline rather than as empty. The two harness fixes it depends on are on this branch (`29f4044518`, `9db3689de0`), and the workspace list it spans answers one envelope on every node, signed or not, pinned in `local-product-contract.test.ts` against the real self-hosted app. Unverified: no live Cursor or ACP session was driven from this machine.
- [ ] `wait_for_attention` returns within the host's tool timeout on Codex CLI and Claude Code and raises an elicitation there. *Progress:* partly met. Bounded at 50 000 ms — under Codex CLI's 60 s default — and a host's own `timeoutMs` is clamped rather than rejected; a pending permission is offered through a form elicitation when the client declared the capability, and the answer is applied. Unverified: no Codex CLI or Claude Code host drove it.
- [ ] `create_subagent` creates a host-owned child on the requested harness with no parent history copied; the call renders as the subagent card on every rail; `wait` is bounded; completion wakes an idle parent and queues behind a busy one, and a cancelled wake is re-offered by recovery; the permission ceiling holds; `clientRequestId` retries are idempotent; children cannot recurse. *Progress:* met. `tools/subagents.ts` with `MAX_WAIT_MS` 50 000; the card aliases `create_subagent` and `mcp__claxedo__create_subagent` (`subagent-presentation.ts`, tested). The wake queue lives in `workspace-runtime/src/routes/session-children.ts` and `session-children.test.ts` pins the idle wake, the busy hold, the recovery re-offer and the queue order. Ceiling, idempotency and recursion refusal are pinned in `subagents.test.ts`.
- [ ] A child session never appears in the sidebar, `sessions_list`, or any root listing; archiving the parent cancels its active children and deleting the parent deletes them. *Progress:* met. Three list routes honour `roots`: `GET /session` and `GET /experimental/session` in `session-core.ts`, which the sidebar reaches through `platform/sync/session-load.ts`, and the desktop's flat inventory `GET /api/claxedo/session`, which the rail reaches through `inventory-source.ts`; `sessions_list` asks the second for `roots: true`. The fourth, `GET /api/claxedo/session-list`, is the rail's paginated navigation source and takes no `roots` flag: it has no reader that wants children, so `buildSessionListResponse` drops a parented row and `listSessionNavigationMetas` excludes one in SQL, where the `limit + 1` page window would otherwise spend a root's slot on a child and retire the cursor early. `session-children.routes.test.ts` pins both runtime routes, `meta-routes.test.ts` pins the navigation list's paged and grouped reads, and `first-party-mcp-subagents.live.test.ts` pins all four plus the tool against a real local server, a real workspace and a child created through `create_subagent`. `cascadeToChildren` runs on archive and on delete. Open: a workspace whose runtime declares `managed-private` registers every created session with the control-plane authority, whose `sessionJson` / `publicSession` rows carry no parent, so a child there would list as a root. No child can exist on such a runtime today — `create_subagent` sends no reservation operation id and `POST /session` answers `session_reservation_required` — and closing it means teaching session registration to carry a parent for a `create`, which its authority currently forbids.
- [ ] Every session Claxedo launches carries the first-party MCP entry with the runtime credential and its session id; the endpoint rejects a request without a valid credential and accepts the session id from a valid one. *Progress:* met on this machine. `claxedo-local-server/src/app/first-party-mcp.live.test.ts` boots the real local server, creates a session on the embedded `opencode` harness, takes the entry that session's runtime injects, and drives an MCP client over it: the URL carries `?session=<the created id>`, the tool list is the runtime audience, and the same URL without the bearer is 401. Unverified: the same path inside a cloud VM.
- [ ] No `ui://` resource and no `_meta.ui` on any tool; every tool's text content is complete on Claude Code, Codex CLI and Claude desktop alike. *Progress:* met for the first half: no `ui://` anywhere in the package and `_meta` is never populated with a UI hint. The second half is a claim about three hosts and is unverified from this machine.
- [ ] A per-user `seen_at` per session exists on the control plane and the local store; "finished but not seen" is computed server-side and written by pane focus and by opening a card. *Progress:* not met. Nothing exists; the only `seen_at` in the repo is machine heartbeat. Owned by plan 004's lanes S1–S4.
- [ ] A child's pending permission or question raises an attention badge on the parent's card and is answerable through the child's own permission id. *Progress:* not met for the badge — the subagent presentation carries no attention field. The second half holds: `question_reply` answers a child's question through the child's own request id, and refuses any other session's.
- [ ] The directory event stream carries no message-level events for a `parentID` session whose pane is not subscribed. *Progress:* not met. No subscription-gated suppression exists on the directory stream.
- [ ] Central `spawn_session`, `summarize_logs`, the transcript fallback, `_meta` approval flags, the stale dist file and the source-text contract test are gone. *Progress:* met. None of `spawn_session`, `summarize_logs`, `get_logs`, `browser-tools.ts`, `desktop-request.ts`, `message-text.ts`, `tool-policy.ts`, `cloud-workspace-tools.ts` or `server.contract.test.ts` exists anywhere under `packages/`, and `git ls-files packages/claxedo-mcp` tracks no `dist`.
- [ ] `bun run test:architecture-ratchets` green; the shared-client extraction re-measures each product closure with no headroom and names the owner. *Progress:* met. The ratchets pass; the self-hosted box's ceiling moved 137 → 138 for `src/mcp/oauth-credential.ts`, measured with no headroom and named in both the policy and the closure test. No shared-client extraction was needed: the client is built on the runtime's own Node-safe client, so no module moved between products.
- [ ] Verified through real entrypoints: the hosted URL added to Claude Code, Codex CLI, Cursor and Claude Desktop by the desktop's one-click config write, with one consent tap; the same URL from Claude mobile against staging; the injected loopback entry from a Claxedo-launched session on the laptop and in a cloud VM. *Progress:* partly met. The laptop half is automated (see the first-party MCP line above). The rest cannot be verified from this machine: no staging deploy, no phone, and no live Claude Code, Codex CLI, Cursor or Claude Desktop driving the hosted URL, and no cloud VM.
- [ ] No stdio binary, no npm publish of `@claxedo/mcp`, no `CLAXEDO_DESKTOP_TOKEN` read anywhere in the MCP; `claxedo-mcp documents` relocated or removed. *Progress:* met. The package is `private: true` with no `bin` and is absent from the publisher's list; `CLAXEDO_DESKTOP_TOKEN` and `CLAXEDO_DESKTOP_URL` appear nowhere in the repo, and the desktop browser HTTP bridge is gone. Documents moved twice: `claxedo documents` in `packages/cli`, and `documents_list` / `documents_open` as MCP tools.
- [ ] A runtime credential cannot call `permission_reply`, `question_reject`, `workspace_lifecycle`, `workspace_restore` or `session_delete`; the handler denies, not just `tools/list`; a test proves the self-approval path in S1 is closed. *Progress:* met by construction: `toolListed` is implemented as `assertToolAccess`, so hiding and denying are one rule, and the handler re-runs it on every call. The S1 path is the one where a second entry point exists — `wait_for_attention`'s elicitation reaches the permission reply without the tool — and `attention.test.ts` calls `replyToPermission` directly with a runtime credential and asserts the `audience` denial. The other four are pinned as absent from the runtime credential's list and refused when called anyway.
- [ ] Cross-machine writes from a runtime credential are denied unless the account setting is on; the lane-E grant is read-wide and write-own-workspace. *Progress:* met. `assertWritableTarget` is the single gate; `sessions.test.ts` pins a refused `session_send` to another workspace beside a `session_transcript` read of the same target that succeeds, and cloud placement refused until `crossMachineWrites` is on.
- [ ] OAuth consent offers `claxedo:read | act | approve | admin`; a token without `approve` cannot reply to permissions; the client list with revoke exists in account settings. *Progress:* met. Both mounts now resolve a consented token through `resolveOAuthMcpCredential`; `first-party-mcp.test.ts` drives the mounted route and asserts a `claxedo:read` token is shown no write at all, a `claxedo:approve` token answers a permission, and a `read`+`act` token is never shown `permission_reply`. The revoke list is `connected-apps-section.tsx` over Better Auth's consents.
- [ ] Every session created through a runtime credential is capped at the caller's permission mode. *Progress:* met. `permissionCeiling` sends the caller's own level and falls to `ask` for an unranked one; pinned in `sessions.test.ts`, and the child path is refused by the runtime's own `permission_ceiling_exceeded`.
- [ ] The loopback mount rejects non-loopback `Origin`/`Host`, sends no CORS headers, and the credential is never accepted from the URL. *Progress:* met. All three pinned in `server.test.ts` against `endpoint/loopback.ts`.
- [ ] Every MCP write is audited with actor, client or runtime id, and session id. *Progress:* met. `mcpAuditRecord` carries all three. The two question tools were recording a write with no session — a question is addressed by request id — so the registry now lets a tool name its session from inside the handler and writes the line when the call settles; `attention.test.ts` pins it for `question_reply` and `question_reject`, and pins the empty session id for a question that was not pending.
- [ ] `tools/list` under a runtime credential returns only the inside-session set; under a user credential only the outside-in set; a test pins both lists. *Progress:* met. `inventory.test.ts` pins the 18-tool inside-session list, the 25-tool outside-in list and the read-only list; the live local-server test pins the same inside-session list over a real connection.
- [ ] No `browser_*` tool, no desktop HTTP bridge, no `CLAXEDO_DESKTOP_*` env anywhere; the in-app pane keeps viewing and annotation and nothing agent-facing. *Progress:* met. No `browser_*` MCP tool, no `browser/http-bridge.ts` or its token module, and no `CLAXEDO_DESKTOP_*` name in any source file.
