...
- [ ] A child session never appears in the sidebar, `sessions_list`, or any root listing; archiving the parent cancels its active children and deleting the parent deletes them. *Progress:* met. The root listing filters `parentID` in `session-core.ts` and `sessions_list` asks for `roots: true`; `cascadeToChildren` runs on archive and on delete, pinned by `session-children.routes.test.ts`.
