...
**Not done:** (1) Codex's app-server is never reaped when idle, a real U8-F4 gap but a behaviour change needing a measured teardown cost rather than a migration; (2) `deployments/local/server.ts` still holds process-lifecycle responsibility that should become host policy only. Parent-loss cleanup is NOT outstanding: it is already handled a layer up by `watchDesktopParent` in the desktop server child and by the runtime server's `signals` option, so installing it per-adapter would create a second shutdown path racing the host's.
...
**The remaining work is larger than the listing change, and the failing tests say why.** "lists experimental sessions through the sandbox" creates sessions through `POST /session` and then lists them. Those sessions pass the store-only list only if session CREATION writes them to the runtime store — and today it does not. They live in the adapter, and the store learns about them only through `bindDiscoveredSession` during the fan-out. That is the actual shape of this unit: the store has to become the owner of session inventory at MUTATION time, not just at read time, which is what U8-F7's "runtime-owned mutations publish canonical local metadata events" means.

**Next step:** make session creation, update, title, status, and delete write through the Workspace Runtime store and publish one canonical event each; then move the server and app consumers onto that inventory plus an explicit selected-harness refresh; then land the two-line store-only listing with them in one slice.
