...
**LAUNCH POSTURE (decided 2026-07-12): launch FREE.** Costs are almost entirely fixed and low (~$35–100/mo floor; ~$1.50/mo per *heavy* active user, near-zero for light — BYO compute + BYO AI keys means users carry the VMs and inference). Free-tier = "everyone entitled": the billing spine ships built-but-dormant behind `requireEntitlement` + default-off flags, so switching free→paid later is a config flip (create Polar products, flip the gate), NOT new engineering. **Revisit-pricing triggers:** heavy-user count into the many-hundreds (variable cost approaching/exceeding the fixed floor), OR crossing the Convex/Clerk free tiers. The per-seat price below is the RESERVED paid design, held for when a trigger fires.

Reserved paid pricing (fixed; owner memory `claxedo-cloud-pricing`): $9/mo · $89/yr per SEAT (annual = "2 months free"; decided 2026-07-12), flat for individuals (org of 1) and teams; BYO compute (user-hosted VM via relay) + BYO AI keys; Polar as Merchant of Record; free tier = self-host-equivalent; 14-day Polar-native trial. Fee-adjusted contribution: monthly seat ≈ $6.55/mo ($9 − 5%+$0.50 Polar fee − ~$1.50 infra); **annual seat ≈ $5.50/mo effective** ($89/yr nets ~$84.05 after fees = ~$7.00/mo, − ~$1.50 infra). $5K/mo profit is therefore **~780 seats all-monthly, ~830–930 seats at a realistic annual mix** (F13 review correction — the earlier single "~780" figure was monthly-only and predated the $89/yr decision). Clerk cost at that scale ≈ $25/mo (personal orgs are Convex-native, never Clerk MAOs; first 100 MAOs free).
