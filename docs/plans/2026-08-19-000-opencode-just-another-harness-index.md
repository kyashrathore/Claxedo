...
|---|-----------|------|--------|
| W1 | Open the harness surface: any operator-configured stdio ACP agent becomes a selectable harness; first-party ACP duplicates removed | [2026-07-22-001-refactor-scriptable-acp-connections-plan.md](./2026-07-22-001-refactor-scriptable-acp-connections-plan.md) (pre-existing, `status: active`) | Units 1–4 landed 2026-08-19 (open identity, trusted-config registry + mutation API, runtime enforcement, discovery-driven app picker). Units 5–6 remain: 5 (independent interop pin — blocked on choosing/pinning a non-first-party ACP implementation and its test credentials), 6 (first-party ACP artifact removal — gated on the Native Readiness Gate). |
...
The boot-trigger audit found and gated one additional eager engine consumer
beyond the plan's candidates: the MCP config fan-out
(`packages/claxedo-local-server/src/opencode/mcp-sync.ts`) posted MCP entries
into the engine on every MCP mutation regardless of harness; it now arms an
engine boot re-sync when the engine is cold, mirroring the auth bridge. The
plan's app-level cold-boot test was realized at the `createSelfHostedApp`
composition level (`deployments/self-hosted-node/cold-engine.test.ts`) plus
per-trigger gate suites (auth bridge, MCP sync, capability catalog, engine
boot hooks); a full `startControlPlaneStack` boot assertion remains unwritten
— no existing test boots the full stack (port binding, data-dir ownership,
long-lived subsystems), and building that harness is a test-infrastructure
project of its own. The eager-trigger audit of `startOwnedControlPlaneStack`
was done by exhaustive read of its `opencodeRequest` consumers instead; each
remaining consumer is lazy or opencode-surface-gated.
