...
- **R31. Multiplayer is the target data model:** the migration lands directly on application-owned `org → project → workspace → session` identity, immutable workspace tenant/project assignment, private sessions, active participants, and current workspace authority. Session access requires workspace authority **and** creator/participant/org-admin status; files remain workspace-scoped. No D1/Better Auth intermediate is certified with the old single-user session model.
...
- **R34. The readiness branch is specification input, not merge evidence:** `codex/single-tenant-multiplayer-ready` and this migration branch currently share the exact latest `origin/dev` base, `834307041e`; a rebase is therefore a no-op. Its current implementation chain ends at `2adbe6ca4c`. The 15 P1 and 3 P2 review findings recorded in its dossier were closed by `fc2c5fc51a`, `593dd1f94f`, and `2adbe6ca4c`, but the final branch changes 267 files, overlaps this migration on 75 paths, assumes provider-subject relay claims that the migration has already removed, and registers runtime sessions without the D1 reservation transition this migration requires. Do not merge or cherry-pick that chain. Semantically port its final behavior and executable evidence in ordered, reviewable slices, preserving the migration branch's canonical `principal_kind`/actor claims, provider-neutral sharing contract, D1 reservation state machine, and Pi/Codex producer fixes. The Pi and Codex ACP real-provider failures must be fixed through canonical producers and the complete real-harness lane rerun before either hosted profile is certified. The branch is reviewed input, never merge-by-assertion evidence.
- **R35. One tenant-safe model, two organization policies:** all schemas, tokens, queries, events, caches, and object keys carry canonical org/project/workspace/session scope in every deployment. Claxedo-hosted permits many customer orgs and multiplayer inside each. A user-deployed instance bootstraps exactly one org owned by the deploying administrator and supports multiplayer only inside that org; it hides/rejects additional-org creation, org switching, and cross-org transfer without replacing tenant checks with constants or unscoped queries.
...
| Private session | Session authority store | Workspace/org/project, creator actor, active participants/grants/revocations, lifecycle generation, and visibility state. Creator/admin participant management still requires current workspace authority. |
...
58. **The multiplayer branch is explicitly not release-ready despite closing its review findings.** `.branch_status/README.md` records all 15 P1 and 3 P2 findings closed in code, with affected package suites passing, but also records two unresolved real-provider browser failures: Pi receives canonical model state while the picker hydrates empty, and Codex ACP reaches `Working` without the external child reaching `Completed` inside the acceptance window. The migration may integrate its reviewed contracts and closure commits, but neither profile may certify until both failures are fixed at their authoritative producers and the complete real-harness browser lane passes on the integrated tree.
...
71. **Core and service lifecycle revisions currently diverge after the first health probe.** Core's `recordProbe` advances the installation revision, while each service's local D1 lifecycle remains at its previous revision and rejects the next probe as stale. Freeze one coordinated revision protocol: every probe/state operation advances both ledgers under the same immutable operation identity, exact replay is idempotent, and a crash between halves resumes the missing half rather than minting another revision. Reinstalling retained resources must adopt the existing local revision explicitly or destroy/recreate them; it may not silently reset append-only history.
72. **The normal Cloudflare deploy still targets the legacy combined Worker.** A clean factory and empty catalog do not change physical ownership while `wrangler.toml`, `deploy-control-plane.yml`, and `deploy-hosted.ts` still publish `hosted-workerd/worker.ts` with WorkGraph/Documents R2/DO/cron bindings. Certified deployment tooling must use a closed artifact mapping, fresh non-legacy Worker names, real default-exporting static auth/authority/product roots, and generated configs. The old Worker/name/migration tags remain immutable recovery state until a recorded custom-domain cutover and rollback window close.
73. **Optional-service workflows stop before installation.** Dark deployment alone does not register the disabled catalog row, add the exact core binding, probe through it, enable both ledgers, drain, revoke, or unbind. One environment-scoped coordinator with a shared core/service deployment lock, persisted CAS operation identity, exact descriptor/build/binding provenance, and crash-resume tests owns the complete sequence; independent service and core workflows may not race it.
...
- [ ] **Unit 1: Prove foundations and freeze the contracts/retirement plan**
  ...
  **Work:**
  ...
  - Integrate the reviewed multiplayer contracts from `codex/single-tenant-multiplayer-ready@2adbe6ca4c` onto the current migration base: one actor model (human client stays human; autonomous agent is separate), canonical repo key, project/workspace/session equality, private-session participant rules, idempotent create/fork registration, durable turn lease, producer-owned attribution, one live/replay policy, renewable stream grant, bounded delivery/reconnect, and exact 401/403/503 semantics. Preserve the fixing commits and executable disposition for all 18 recorded findings. Then fix the remaining Pi hydration and Codex ACP terminal-lifecycle failures at their canonical producers and rerun the complete real-harness browser lane; do not replace either result with UI synthesis or timeout completion.
...
- [ ] **Unit 4: Switch shared code to explicit adapter composition and migrate all clients**
  ...
  **Work:**
  ...
  - Integrate multiplayer semantically in this fixed order, committing each slice only after its focused cross-adapter tests pass: (1) freeze one neutral private-session authority port including reservation/registration/reconciliation, participant grants, read/write/runtime authorization, display-author resolution, and explicit RHT parent-token identity; (2) implement it in D1, Convex, and SQLite; (3) add the control-plane runtime session oracle using the current actor claims and D1 reservation operation ID; (4) enforce the private-session policy and create/fork compensation in the runtime; (5) cover every adjacent PTY/process/V2/checkpoint/tool/agent/event route through an executable inventory; (6) replace process-local prompt admission with a durable store lease and canonical `409 session_turn_in_progress`; (7) add bounded renewable event/PTY authorization and per-principal replay retention; (8) persist internal producer actor IDs and resolve display-safe authors on read, plus loser-only optimistic rollback; (9) gate the retained Convex rollout on ordered tenant backfills and the migration ledger after the optional-service carve-out; and (10) certify the real two-user flows. Apply the hardening behavior from `fc2c5fc51a` and `593dd1f94f` in the same slices as the feature behavior from `4abe60113a`; include the durable-admission and agent-hook closures from `76c7134950` and `2adbe6ca4c`. Never recreate the readiness branch's vulnerable intermediate states.
...
- `codex/single-tenant-multiplayer-ready@2adbe6ca4c` (reviewed implementation chain: `4abe60113a`, `fc2c5fc51a`, `593dd1f94f`, `76c7134950`, `2adbe6ca4c`; dossier closure: `8d63e6c839`)
