...
> **Post-campaign burndown (2026-07-20)**: the 583dc949c1 validation run's 21 residuals resolved to two campaign regressions (a wave-3 merge reverted the mock house-model fix a52c18a333 — 13 tests; and the campaign's own e2e seams were DEV-gated so the prebuilt CI bundle stripped them — 4 tests, now gated DEV||VITE_CLAXEDO_E2E), two contention items (processes b19 green after fixes; settings-auth b25 asserts a transient "Redirecting…" state that is racy by construction — needs an owner call on the assertion), and the two known-opens (behavior 32, workgraph-real audit completion). RULE LEARNED: every e2e-facing hook must gate on DEV || VITE_CLAXEDO_E2E, and shared-helper edits get a conflict check at integration.
...
- **Decision**: A (owner: agreed with recommendation)
- **Executed (2026-07-27, e2e fix campaign)**: two DISTINCT failure modes turned out to hide under this entry.
  - *Post-setup mode (any real-Session test, intermittent)*: the spawned engine ran from source with a per-test-fresh `XDG_CACHE_HOME`, so it had no models.dev cache and no baked snapshot and fetched `https://models.dev` on every boot. The V2 catalog commits atomically only after the whole plugin-boot batch (`core/src/state.ts` `State.batch`), that batch blocks on the models.dev populate (same flock as the startup refresh), and the boot is a fire-and-forget fork nothing in the drain path waits for — so while the fetch was slow/rate-limited the catalog stayed EMPTY and the first prompt's drain died with `ModelUnavailableError: workgraph-e2e/workgraph-model` (`core/src/session/runner/model.ts:199`), `providerRequests: 0`, Session never answering. Reproduced at branch base on a full-lane run (hit the `:717` sibling on that occasion; which test it lands on is timing luck). Fixed hermetically: `OPENCODE_DISABLE_MODELS_FETCH=true` in both engine spawn sites (`real-workgraph-harness.ts`, `workgraph-connection-browser-use-server.ts`) — populate returns `{}` instantly and the catalog holds exactly the config-defined fake provider.
  - *CI `beforeEach`-stall mode*: distinct — `/global/health` is a static handler with no models.dev dependency; the stall matched the untimed health probe hanging on a socket accepted before the route layer wired, which the probe-timeout rework already in `waitForOpenCode` (2s `AbortSignal` per probe, 120s budget) addresses. Needs a CI run to confirm.
...
| --- | --- | --- | --- | --- |
| live-claxedo-mcp-tools `:547` | process tool add/update/remove hit the wrong path and 404 — behavior 5 | **RESOLVED — fixed 2026-07-20 in `e26b79b98c`; `test.fixme` deleted 2026-07-25 (it was stale).** Was a REAL BUG: `process-handler.ts:275/299/311` POST/PUT/DELETE'd to bare `/process` instead of `/api/wr/process`, and the package's own unit test asserted the wrong path, hiding it. Both fixed: `process-handler.ts:276/300-301/313-314` now route through the `PROCESS_PATH` constant, matching the real routes at `packages/workspace-runtime/src/routes/process.ts:111` (POST), `:120` (PUT), `:133` (DELETE) — verified by reading the server route table, not by a live run | RESOLVED — no app fix outstanding. Regression coverage in the owning package: `packages/claxedo-mcp/src/process-handler.test.ts:290,367,400` now assert `/api/wr/process` (suite green, 120/120) | approved |
| live-claxedo-mcp-tools `:739` | summarize_logs never surfaces a raw JSON-parse crash — behavior 10b | **RESOLVED — both halves fixed; `test.fixme` deleted 2026-07-25 (it was stale).** (a) parse-before-ok: real at the history-reset commit `00a533c2fb`, fixed incidentally in `52d52c2143` — `server.ts:74-79` now checks `if (!res.ok)` before parsing and routes the error body through `parseHttpErrorBody()`, which wraps `JSON.parse` in try/catch. The fixme's `server.ts:66-67` citation was already stale when written, which is why the `e26b79b98c` author logged "did not reproduce". (b) dead fallback: real and correctly diagnosed — `GET /session/:id/message/:messageId` genuinely does not exist on the workspace-runtime/claxedo-server Hono surface (only on the vendored opencode engine); fixed in `e26b79b98c`, now `resolveResponseText()` in `message-text.ts` re-fetches the real list route `GET /session/:id/message` (`session-core.ts:628`) and matches by id | RESOLVED — no app fix outstanding. Regression coverage: `packages/claxedo-mcp/src/message-text.test.ts` ("resolveResponseText", 8 tests). Residual gap (not a bug): the res.ok-before-parse half has no direct unit test because `httpRequest()` is unexported and `server.ts` connects a stdio transport on import | approved |
